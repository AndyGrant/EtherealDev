...
void initTexelEntries(TexelEntry *tes, Thread *thread) {
    ...
    for (i = 0; i < NPOSITIONS; i++) {
        ...
        // Clear out all of the hash and history tables
...
void initCoefficients(int coeffs[NTERMS]) {
    ...
    if (TuneKnightBehindPawn            ) INIT_COEFF_0(KnightBehindPawn)            ;
    ...
    if (TuneBishopBehindPawn            ) INIT_COEFF_0(BishopBehindPawn)            ;
...
void initCurrentParameters(double cparams[NTERMS][PHASE_NB]) {
    ...
    if (TuneKnightBehindPawn            ) INIT_PARAM_0(KnightBehindPawn)            ;
    ...
    if (TuneBishopBehindPawn            ) INIT_PARAM_0(BishopBehindPawn)            ;
...
void printParameters(double params[NTERMS][PHASE_NB], double cparams[NTERMS][PHASE_NB]) {
    ...
    if (TuneKnightBehindPawn            ) PRINT_PARAM_0(KnightBehindPawn)           ;
    ...
    if (TuneBishopBehindPawn            ) PRINT_PARAM_0(BishopBehindPawn)           ;
