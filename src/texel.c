...
double linearEvaluation(TexelEntry *te, TexelVector params) {
    ...
    te->dynamicCoeffs[WHITE][MG] = (wmg < 0) ? 1.0 / 16.0 : 0;
    te->dynamicCoeffs[BLACK][MG] = (bmg < 0) ? 1.0 / 16.0 : 0;
    ...
    wmg = (wmg < 0) ? wmg / 16.0 : 0; weg = (weg < 0) ? weg / 20.0 : 0;

    // Perform the KS transformation found in evaluate() for BLACK
    bmg = (bmg < 0) ? bmg / 16.0 : 0; beg = (beg < 0) ? beg / 20.0 : 0;
