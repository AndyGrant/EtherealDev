...
extern const int PassedPawnCanAdvance[8];
extern const int PassedPawnSafeAdvance[8];
extern const int PassedPawnConnected[8];
...
void runTexelTuning(Thread *thread) {
    ...
    TexelEntry *tes;
    int i, j, iteration = -1;
    double K, thisError, bestError = 1e6, baseRate = 5.0;
...
void initCoefficients(int coeffs[NTERMS]) {
    ...
    if (TunePassedPawnRank              ) INIT_COEFF_1(PassedPawnRank, 8)           ;
    if (TunePassedPawnFile              ) INIT_COEFF_1(PassedPawnFile, 4)           ;
    if (TunePassedPawnCanAdvance        ) INIT_COEFF_1(PassedPawnCanAdvance, 8)     ;
    if (TunePassedPawnSafeAdvance       ) INIT_COEFF_1(PassedPawnSafeAdvance, 8)    ;
    if (TunePassedPawnConnected         ) INIT_COEFF_1(PassedPawnConnected, 8)      ;
...
void initCurrentParameters(double cparams[NTERMS][PHASE_NB]) {
    ...
    if (TunePassedPawnRank              ) INIT_PARAM_1(PassedPawnRank, 8)           ;
    if (TunePassedPawnFile              ) INIT_PARAM_1(PassedPawnFile, 4)           ;
    if (TunePassedPawnCanAdvance        ) INIT_PARAM_1(PassedPawnCanAdvance, 8)     ;
    if (TunePassedPawnSafeAdvance       ) INIT_PARAM_1(PassedPawnSafeAdvance, 8)    ;
    if (TunePassedPawnConnected         ) INIT_PARAM_1(PassedPawnConnected, 8)      ;
...
void printParameters(double params[NTERMS][PHASE_NB], double cparams[NTERMS][PHASE_NB]) {
    ...
    if (TunePassedPawnRank              ) PRINT_PARAM_1(PassedPawnRank, 8)          ;
    if (TunePassedPawnFile              ) PRINT_PARAM_1(PassedPawnFile, 4)          ;
    if (TunePassedPawnCanAdvance        ) PRINT_PARAM_1(PassedPawnCanAdvance, 8)    ;
    if (TunePassedPawnSafeAdvance       ) PRINT_PARAM_1(PassedPawnSafeAdvance, 8)   ;
    if (TunePassedPawnConnected         ) PRINT_PARAM_1(PassedPawnConnected, 8)     ;
