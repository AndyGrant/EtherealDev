...
const int PawnValue   = S(  85, 146);
const int KnightValue = S( 432, 484);
const int BishopValue = S( 451, 522);
const int RookValue   = S( 638, 820);
const int QueenValue  = S(1307,1693);
...
const int PawnPSQT[SQUARE_NB] = {
    ...
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S( -14,  10), S(  -5,   2), S(   1,   6), S(   6,   2),
    S(   3,  12), S( -10,   4), S( -10,   2), S( -18,  10),
    S( -22,   7), S( -17,   6), S(   0,  -6), S(  13, -15),
    S(   9, -10), S(  -4,  -6), S( -14,   4), S( -27,  14),
    S( -15,  17), S( -23,  18), S(   9, -12), S(  11, -25),
    S(   5, -24), S(   5, -12), S( -21,  15), S( -18,  20),
    S( -15,  19), S( -19,  11), S( -17,  -9), S(   5, -33),
    S(  -1, -26), S( -18, -11), S( -24,  10), S( -19,  23),
    S( -22,  54), S( -12,  48), S(   0,  34), S(  19,  -9),
    S(  37,  -5), S( -11,  33), S(  -9,  47), S( -26,  55),
    S( -40, -61), S( -73,  -1), S( -21, -16), S(  71, -20),
    S(  43, -13), S(  54, -16), S( -44,   8), S( -70, -47),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT[SQUARE_NB] = {
    S( -30, -35), S(  -5, -22), S( -19, -21), S( -14,   1),
    S(  -9,   0), S( -21, -18), S(  -7, -18), S( -41, -26),
    S(   2,  -1), S(  -9,   5), S(  -5, -19), S(   1,  -1),
    S(   2,   1), S(  -8, -15), S(  -7,  -1), S(  -5,   4),
    S(   9, -20), S(  10,  -4), S(   9,   2), S(  12,  20),
    S(  13,  20), S(   6,   2), S(  10,  -3), S(   4, -17),
    S(  19,  23), S(  19,  33), S(  25,  43), S(  29,  54),
    S(  27,  56), S(  26,  44), S(  21,  31), S(  16,  29),
    S(  16,  33), S(  25,  32), S(  40,  54), S(  33,  74),
    S(  29,  71), S(  41,  54), S(  25,  36), S(  17,  32),
    S( -22,  28), S(  -3,  40), S(  28,  60), S(  24,  63),
    S(  30,  58), S(  32,  59), S(   1,  36), S( -16,  28),
    S(  15,  -2), S( -11,  21), S(  30,  -2), S(  40,  25),
    S(  44,  24), S(  45,  -8), S( -12,  19), S(   5,  -1),
    S(-173,  -5), S( -97,  13), S(-124,  43), S( -39,  18),
    S( -18,  20), S(-109,  50), S(-124,  26), S(-169, -17),
};

const int BishopPSQT[SQUARE_NB] = {
    S(   5, -20), S(   2,   2), S(  -1,   6), S(   2,   5),
    S(   2,   9), S(  -6,  -1), S(   0,   2), S(   4, -24),
    S(  27, -16), S(   2, -31), S(  16,  -1), S(   8,   8),
    S(   9,   9), S(  13,  -3), S(  10, -31), S(  26, -29),
    S(  10,   3), S(  23,   9), S(  -5,  -2), S(  19,  20),
    S(  18,  21), S(  -5,  -5), S(  20,   4), S(  15,   9),
    S(   0,  13), S(  11,  18), S(  17,  34), S(  21,  34),
    S(  25,  36), S(  13,  32), S(  16,  19), S(   0,  16),
    S( -20,  36), S(  14,  32), S(   1,  40), S(  22,  48),
    S(  12,  49), S(   7,  41), S(  13,  35), S( -14,  40),
    S( -12,  34), S( -11,  47), S(  -7,  25), S(   2,  42),
    S(   9,  41), S( -21,  37), S(  -5,  49), S(  -9,  38),
    S( -60,  38), S( -50,  22), S( -13,  31), S( -36,  36),
    S( -33,  36), S( -13,  33), S( -69,  21), S( -55,  37),
    S( -68,  18), S( -70,  36), S(-132,  50), S(-112,  56),
    S(-117,  54), S(-101,  44), S( -38,  24), S( -77,  17),
};

const int RookPSQT[SQUARE_NB] = {
    S( -27,   0), S( -21,   5), S( -15,   5), S(  -6,  -4),
    S(  -6,  -3), S( -10,   5), S( -13,  -2), S( -22, -14),
    S( -71,   5), S( -25, -13), S( -18,  -8), S( -11, -12),
    S(  -9, -14), S( -15, -17), S( -13, -20), S( -77,   1),
    S( -39,   1), S( -17,  15), S( -25,   8), S( -14,   1),
    S( -11,   3), S( -25,   7), S(  -3,  10), S( -37,  -3),
    S( -32,  21), S( -21,  36), S( -21,  37), S(  -5,  27),
    S(  -8,  27), S( -18,  34), S( -11,  32), S( -29,  22),
    S( -21,  45), S(   5,  35), S(  17,  39), S(  36,  31),
    S(  34,  32), S(  13,  33), S(  21,  25), S( -13,  42),
    S( -34,  61), S(  18,  42), S(  -1,  58), S(  34,  37),
    S(  32,  35), S(   6,  51), S(  39,  27), S( -25,  58),
    S( -20,  43), S( -27,  52), S(  -3,  42), S(  11,  45),
    S(  11,  42), S(  -1,  36), S( -23,  50), S( -11,  39),
    S(  27,  64), S(  19,  71), S(  -7,  82), S(   3,  75),
    S(   4,  77), S(  -4,  76), S(  34,  64), S(  37,  61),
};

const int QueenPSQT[SQUARE_NB] = {
    S(  21, -35), S(   3, -26), S(   9, -35), S(  17, -16),
    S(  18, -18), S(  13, -46), S(   8, -28), S(  23, -46),
    S(   6, -15), S(  16, -23), S(  22, -42), S(  13,   3),
    S(  18,   0), S(  23, -50), S(  19, -30), S(   4, -19),
    S(   6,  -1), S(  22,   8), S(   5,  37), S(   1,  36),
    S(   2,  36), S(   5,  39), S(  25,  10), S(  14, -15),
    S(   9,  17), S(  12,  48), S(  -6,  61), S( -19, 113),
    S( -18, 111), S(  -4,  62), S(  19,  51), S(   8,  38),
    S( -10,  43), S(  -9,  81), S( -20,  69), S( -33, 126),
    S( -33, 134), S( -24,  88), S(  -8, 102), S( -10,  74),
    S( -29,  57), S( -23,  50), S( -35,  71), S( -19,  74),
    S( -18,  77), S( -20,  71), S( -18,  73), S( -28,  84),
    S( -17,  63), S( -75, 114), S( -21,  69), S( -56, 122),
    S( -57, 127), S( -15,  67), S( -72, 125), S( -11,  79),
    S(   7,  46), S(  16,  53), S(  -2,  84), S(   1,  84),
    S(  -4,  95), S(  11,  73), S(  17,  87), S(  22,  62),
};

const int KingPSQT[SQUARE_NB] = {
    S(  90, -77), S(  68, -45), S(   2,  -3), S( -11, -26),
    S( -12, -29), S( -12,   2), S(  58, -50), S(  81, -85),
    S(  34,  11), S( -30,   4), S( -45,  22), S( -96,  34),
    S( -70,  29), S( -70,  34), S( -30,   1), S(  28,  -1),
    S( -58, -14), S( -20, -15), S(  32,   8), S(  -1,  40),
    S(  18,  35), S(  30,  13), S( -14, -16), S( -40, -10),
    S( -59, -46), S( 121, -46), S(  87,  11), S( -19,  48),
    S(  17,  43), S(  89,  11), S( 128, -36), S( -52, -43),
    S( -20, -12), S( 130, -52), S(  66,   6), S( -35,  12),
    S( -15,  12), S(  50,  11), S( 104, -41), S( -54, -12),
    S(  10, -46), S( 173, -41), S( 162, -22), S(   6, -39),
    S( -25, -34), S( 132, -37), S( 130, -38), S( -30, -50),
    S(  -5,-129), S(  18, -31), S(  16, -38), S(  16, -65),
    S( -47, -59), S(  50, -48), S(  16, -33), S( -15,-139),
    S( -43,-193), S(  46,-125), S( -54, -98), S(  -6, -50),
    S( -73, -75), S( -35, -88), S(  73,-118), S( -98,-148),
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -13, -17), S( -17,  19), S( -19,  30),
    S( -23,  63), S(  19,  64), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -13,  22), S(  -7,  28), S(   1,  55),
    S(  23, 119), S(  50,  80), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated[FILE_NB] = {
    S( -14, -10), S(  -1, -15), S(   0, -16), S(   3, -18),
    S(   6, -18), S(   3, -15), S(  -5, -14), S(  -4, -18),
};

const int PawnStacked[2][FILE_NB] = {
   {S(  10, -31), S(  -1, -30), S(  -1, -23), S(  -2, -20),
    S(   2, -20), S(   5, -26), S(   4, -32), S(   8, -31)},
   {S(   4, -17), S(   1, -17), S(  -6, -10), S(  -7, -10),
    S(  -5,  -9), S(  -1, -10), S(   0, -13), S(   1, -19)},
};

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   0,  -8), S(   7,  -7), S(   6, -19),
    S(  -3, -31), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S(  -9, -33), S(  -5, -30), S(   3, -31),
...
const int PawnConnected32[32] = {
    ...
    S(   8,  14), S(  21,  18), S(  32,  24), S(  25,  19),
    S(  49,  35), S(  39,  64), S(  61,  74), S(  66,  88),
    S( 107,  28), S( 216,  35), S( 206,  74), S( 218,  65),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightInSiberia[4] = {
    S(  -9,  -7), S( -12, -22), S( -28, -21), S( -48, -23),
};

const int KnightMobility[9] = {
    S(-104,-148), S( -44,-117), S( -21, -36), S(  -6,   5),
    S(   8,  18), S(  13,  38), S(  21,  42), S(  32,  42),
    S(  46,  22),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  21,  90);
...
const int BishopMobility[14] = {
    S( -96,-190), S( -43,-123), S( -12, -51), S(  -1, -11),
    S(  10,   5), S(  18,  25), S(  21,  40), S(  23,  44),
    S(  23,  54), S(  31,  54), S(  31,  54), S(  56,  38),
    S(  60,  53), S(  88,   9),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  10,   9), S(  35,   8) };

const int RookOnSeventh = S(   0,  42);

const int RookMobility[15] = {
    S(-127,-176), S( -57,-129), S( -25, -84), S( -11, -27),
    S( -10,   4), S( -12,  29), S( -10,  45), S(  -4,  50),
    S(   5,  56), S(  10,  59), S(  12,  68), S(  20,  72),
    S(  21,  77), S(  40,  63), S( 101,  17),
};
...
const int QueenMobility[28] = {
    S(-142,-294), S(-283,-430), S(-128,-244), S( -45,-246),
    S( -18,-178), S(  -7, -86), S(   1, -31), S(   3,   3),
    S(  10,  12), S(  11,  36), S(  17,  43), S(  18,  61),
    S(  22,  52), S(  25,  64), S(  24,  65), S(  23,  71),
    S(  26,  69), S(  17,  71), S(  14,  70), S(  19,  55),
    S(  26,  38), S(  40,  14), S(  35,  -5), S(  29, -22),
    S(   8, -33), S(   0, -62), S( -29, -20), S(  -5, -32),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -39,  -1), S( -19,   3), S(  -1,   7), S(  11,   8),
    S(  21,   8), S(  32,   0), S(  39, -14), S(   9, -17),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  36,  45), S(  21,  30), S(  11,  15), S( -10, -21),
    S(  -6, -61), S(   0, -75), S(  -7, -82), S(  -1, -73),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -4,  -8), S(  17, -34), S(  26,  -4), S(  25,   7),
    S(   5,   0), S( -10,   1), S( -23, -31), S( -62,  26)},
   {S(  14,  -9), S(   3, -18), S(  -5,  -4), S(   4,  -7),
    S( -11,   4), S( -50,  71), S(  78,  98), S( -22,   2)},
   {S(  41,  -5), S(   5,  -7), S( -35,   5), S( -17, -16),
    S(  -8,  -9), S( -25,  10), S(  26,  71), S( -18,   0)},
   {S(  17,  13), S(  25, -17), S(   0, -13), S(  10, -21),
    S(  22, -33), S( -47,   7), S(-130,  48), S(  -5,  -4)},
   {S( -11,  15), S(   2,  -3), S( -45,   7), S( -28,  10),
    S( -27,   0), S( -43,  -4), S(  50, -30), S( -14,   3)},
   {S(  53, -14), S(  15, -12), S( -25,   7), S( -11, -20),
    S(  10, -34), S(  36, -22), S(  49, -44), S( -23,   2)},
   {S(  43, -18), S(   3, -24), S( -32,   0), S( -24,  -7),
    S( -32,   3), S( -17,  28), S(  11,  46), S( -16,   2)},
   {S(  10, -21), S(   4, -25), S(  10,   2), S(   1,  17),
    S( -10,  25), S( -10,  47), S(-176,  84), S( -19,  16)}},
  {{S(   0,   0), S( -17, -41), S(   8, -31), S( -53,  15),
    S( -39,   4), S(  -5,  48), S(-176, -12), S( -61,  21)},
   {S(   0,   0), S(  17, -20), S(   9, -13), S(  -9,  -9),
    S(   2, -31), S(  25,  72), S(-196,  11), S( -35,  12)},
   {S(   0,   0), S(  20, -11), S(   2, -13), S(  10, -30),
    S(  32, -11), S( -90,  57), S( -93, -76), S(  -9,   1)},
   {S(   0,   0), S(   0,   4), S(  -5,  -6), S( -35,  11),
    S( -52,  17), S(-104,  31), S( -42, -57), S( -34,  -5)},
   {S(   0,   0), S(  12,  -3), S(  17, -15), S(  17, -16),
    S(  -6, -14), S( -35,   4), S(-108, -44), S( -18,  -3)},
   {S(   0,   0), S(  -7,  -4), S( -22,   2), S( -15,  -7),
    S(  25, -21), S( -25,   8), S(  42,  44), S( -16,  -2)},
   {S(   0,   0), S(  34, -22), S(  19, -14), S(  -8,  -5),
    S( -29,  15), S(   0,  16), S( -44, -39), S( -29,  17)},
   {S(   0,   0), S(  16, -58), S(  16, -33), S( -20,  -7),
    S( -32,  26), S(  -8,  24), S(-227, -44), S( -31,   3)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -14,  41), S( 145,  -2), S( -13,  27), S(  -5,   1),
    S( -11,  -3), S(  -8,  -6), S( -19,   9), S( -27,  -3)},
   {S( -19,  63), S(  63,  18), S( -11,  22), S(   8,  11),
    S(   3,   9), S(   6,  -2), S(  -5,   1), S( -16,   9)},
   {S(  -3,  53), S(   8,  33), S( -18,  20), S( -12,  10),
    S(   0,   5), S(   8,   3), S(   9,  -7), S(   7,   9)},
   {S(  -6,  27), S(  11,  25), S( -31,  11), S( -22,   1),
    S( -14,   4), S(  14, -10), S(   4,  -5), S( -20,   8)}},
  {{S(   0,   0), S( -20, -22), S( -17,  -3), S(  30, -26),
    S(  12,  -8), S(  16, -27), S(  -5,   4), S(   6,  29)},
   {S(   0,   0), S( -13, -46), S(  -2, -19), S(  54, -19),
    S(  17,  -7), S(  22, -31), S( -10, -20), S( -42,   8)},
   {S(   0,   0), S( -36, -73), S( -15, -15), S(  11,  -8),
    S(   6,  -2), S(  -1, -19), S(  -3, -22), S(  -4,   2)},
   {S(   0,   0), S(   2, -32), S( -28, -20), S( -20,   3),
    S(  -9,  -7), S(   9, -38), S(  66, -27), S(   7,  25)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -35,  -9), S( -37,  22), S( -57,  26),
    S(  12,  20), S(  92,  -4), S( 165,  41), S(   0,   0)},
   {S(   0,   0), S( -23,   9), S( -34,  38), S( -51,  42),
    S(   2,  55), S( 108,  58), S( 188, 103), S(   0,   0)}},
  {{S(   0,   0), S( -23,  27), S( -40,  32), S( -55,  53),
    S(  11,  66), S( 100,  78), S( 258, 126), S(   0,   0)},
   {S(   0,   0), S( -24,  21), S( -33,  31), S( -50,  58),
    S(  12,  90), S(  86, 177), S( 113, 316), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(  -3,   1), S(   1,  -4), S(   6, -14),
    S(   5, -19), S(  -9, -18), S( -11,  -6), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   5,   0), S(   6,   1), S(   8,  12),
    S(   0,  25), S(   2,  37), S(  19,  36), S(   0,   0),
};

const int PassedSafePromotionPath = S( -51,  59);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -11, -39);
const int ThreatMinorAttackedByPawn  = S( -55, -86);
...
const int ThreatMinorAttackedByKing  = S( -42, -22);
const int ThreatRookAttackedByKing   = S( -29, -20);
const int ThreatQueenAttackedByOne   = S( -50,  -8);
const int ThreatOverloadedPieces     = S(  -7, -17);
const int ThreatByPawnPush           = S(  15,  34);
...
const int ClosednessKnightAdjustment[9] = {
    S(  -3,  13), S(  -5,  35), S(  -6,  42), S(  -3,  42),
    S(   0,  49), S(   2,  42), S(   4,  39), S(  -8,  58),
    S(  -5,  37),
};

const int ClosednessRookAdjustment[9] = {
    S(  51,  48), S(  -2,  91), S(   7,  69), S(  -2,  58),
    S(  -4,  53), S(   1,  34), S(  -3,  23), S( -15,  25),
    S( -30,   2),
};
