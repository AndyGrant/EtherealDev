...
const int PawnValue   = S(  93, 134);
const int KnightValue = S( 424, 430);
const int BishopValue = S( 437, 448);
const int RookValue   = S( 617, 717);
const int QueenValue  = S(1295,1353);
...
const int PawnPSQT32[32] = {
    ...
    S( -14,   5), S(   0,  -1), S(  -9,   3), S(  -6,  -1),
    S( -21,   4), S( -14,   2), S( -12,  -7), S(  -5, -15),
    S( -15,  11), S(  -9,   8), S(   2, -11), S(   6, -26),
    S( -10,  15), S(  -7,   7), S( -13, -10), S( -10, -24),
    S( -15,  38), S(   0,  31), S(  -2,  19), S(  16,  -6),
    S( -19, -60), S( -56, -20), S(   0, -41), S(  46, -51),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -34, -31), S(   0, -34), S( -11, -31), S(  -3, -17),
    S(   4, -17), S(  -2, -14), S(   3, -32), S(   8, -16),
    S(  12, -33), S(  14, -19), S(  13, -14), S(  18,   0),
    S(  20,   3), S(  21,   9), S(  28,  20), S(  29,  28),
    S(  17,  11), S(  27,  11), S(  41,  27), S(  31,  42),
    S( -17,  12), S(   3,  13), S(  28,  31), S(  25,  33),
    S(  13, -13), S(  -2,  -4), S(  34, -21), S(  39,   3),
    S(-177, -25), S( -84,  -2), S(-110,  22), S( -32,  -1),
};

const int BishopPSQT32[32] = {
    S(  10, -35), S(   5, -16), S(   1, -16), S(   6, -11),
    S(  25, -35), S(  11, -45), S(  18, -20), S(  12, -10),
    S(  17, -12), S(  24, -11), S(   2, -21), S(  21,  -1),
    S(   6,  -8), S(  18,  -4), S(  18,   9), S(  23,  11),
    S( -10,  15), S(  17,  11), S(   7,  15), S(  16,  23),
    S(   0,   9), S(  -2,  21), S(  -5,   5), S(  11,  16),
    S( -45,  14), S( -46,   3), S(  -5,  11), S( -25,  12),
    S( -59,  -5), S( -45,   7), S( -87,  16), S( -89,  30),
};

const int RookPSQT32[32] = {
    S( -15, -29), S(  -9, -22), S(  -4, -20), S(   1, -27),
    S( -66, -18), S( -12, -35), S( -11, -33), S(  -3, -33),
    S( -28, -22), S(  -7, -12), S( -16, -18), S(  -3, -24),
    S( -22,  -2), S(  -9,   8), S( -11,   6), S(   0,  -2),
    S(  -6,  15), S(  15,   7), S(  21,   7), S(  39,   0),
    S( -16,  25), S(  30,   7), S(   9,  20), S(  37,   5),
    S(  -3,  10), S(  -8,  16), S(  12,   6), S(  25,   5),
    S(  37,  22), S(  29,  28), S(   3,  37), S(  14,  33),
};

const int QueenPSQT32[32] = {
    S(  24, -65), S(   9, -48), S(  14, -62), S(  20, -43),
    S(   9, -41), S(  20, -55), S(  25, -73), S(  18, -28),
    S(  11, -30), S(  24, -23), S(   7,   4), S(   6,  -3),
    S(  11,  -8), S(  17,  15), S(   0,  20), S( -14,  65),
    S(  -5,  14), S(  -6,  49), S( -14,  24), S( -24,  78),
    S( -24,  29), S( -17,  18), S( -21,  24), S(  -9,  28),
    S(  -5,  26), S( -59,  68), S(  -7,  13), S( -40,  55),
    S(  -1,  20), S(  22,  12), S(  11,  15), S(   2,  30),
};

const int KingPSQT32[32] = {
    S(  52, -79), S(  43, -52), S( -10, -15), S( -19, -34),
    S(  24, -18), S( -19, -15), S( -44,   7), S( -72,  12),
    S(  -8, -25), S(   3, -25), S(   9,   0), S( -16,  18),
    S(  -3, -33), S(  90, -35), S(  48,   7), S(  -5,  29),
    S(  10,  -9), S( 109, -26), S(  57,  19), S(  10,  24),
    S(  50, -23), S( 135, -11), S( 105,   9), S(  43,   5),
    S(   7, -57), S(  49,  -6), S(  32,   7), S(   9,  -3),
...
const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -27, -10), S( -10,   9), S( -20,  29),
    S(  -7,  56), S(  26,  60), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -16,  18), S(  -5,  22), S(   1,  51),
    S(  27,  94), S(  39,  63), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -5, -13);

const int PawnStacked[2] = { S(  -1, -22), S(  -4,  -9) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   4,  -4), S(  10,  -7), S(  10, -16),
    S(   7, -18), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -14, -25), S(  -6, -23), S(   2, -26),
    S(  34, -32), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -4,  -6), S(  10,  -5), S(   2,   0), S(   8,  10),
    S(  11,   1), S(  23,  -5), S(  25,   3), S(  26,  11),
    S(   7,  -2), S(  18,   0), S(  10,   4), S(  17,  13),
    S(   6,  13), S(  18,  17), S(  28,  21), S(  32,  15),
    S(  47,  25), S(  44,  48), S(  63,  61), S(  73,  68),
...
const int KnightInSiberia[4] = {
    S(  -8,  -5), S( -10, -17), S( -21, -14), S( -31,  -9),
};

const int KnightMobility[9] = {
    S( -76,-106), S( -39,-107), S( -19, -45), S(  -7, -10),
    S(   6,  -1), S(   9,  16), S(  16,  18), S(  26,  17),
    S(  40,  -1),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  19,  73);
...
const int BishopLongDiagonal = S(  22,  18);

const int BishopMobility[14] = {
    S( -79,-158), S( -39,-114), S( -13, -60), S(  -4, -26),
    S(   6, -13), S(  13,   3), S(  15,  15), S(  16,  18),
    S(  15,  27), S(  24,  26), S(  24,  25), S(  46,  11),
    S(  49,  22), S(  73, -18),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(   6,   8), S(  29,   6) };

const int RookOnSeventh = S(  -4,  36);

const int RookMobility[15] = {
    S(-153,-115), S( -54,-111), S( -23, -74), S( -10, -29),
    S(  -9,  -3), S( -13,  18), S( -11,  31), S(  -5,  34),
    S(   3,  38), S(   7,  40), S(   9,  48), S(  17,  51),
    S(  17,  56), S(  34,  43), S(  92,   2),
};
...
const int QueenMobility[28] = {
    S( -63,-263), S(-214,-388), S( -80,-203), S( -32,-196),
    S( -14,-147), S(  -6, -82), S(   1, -44), S(   1, -14),
    S(   6,  -4), S(   8,  14), S(  14,  18), S(  16,  35),
    S(  19,  25), S(  22,  34), S(  20,  37), S(  17,  44),
    S(  18,  42), S(  11,  44), S(   8,  41), S(  11,  30),
...
const int KingDefenders[12] = {
    S( -27,  -6), S( -14,   2), S(   1,   6), S(  11,   8),
    S(  19,   8), S(  30,   0), S(  32,  -4), S(  12,  -1),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  48,  43), S(  28,  32), S(  11,  15), S( -27, -24),
    S( -30, -60), S( -22, -70), S( -17, -69), S( -11, -64),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -8,   1), S(  15, -25), S(  17,  -6), S(  12,   6),
    S(   7,   1), S(  -5,  -1), S(  -2, -34), S( -43,  23)},
   {S(  17,  -7), S(  14, -15), S(  -1,  -2), S(  -5,   2),
    S( -23,  15), S( -72,  66), S(  91,  80), S( -29,   4)},
   {S(  37,  -2), S(  12,  -6), S( -25,   5), S( -11, -10),
    S( -13,  -3), S( -11,   0), S(   0,  68), S( -15,   1)},
   {S(  13,  10), S(  19, -15), S(   0, -14), S(   6, -19),
    S(  20, -37), S( -61,   3), S(-135,  51), S(  -3,  -3)},
   {S( -10,  14), S(   9,  -4), S( -27,   2), S( -12,   3),
    S( -10,  -5), S( -45,  -6), S(  32, -17), S(  -8,   2)},
   {S(  44, -12), S(  14, -10), S( -21,   3), S(  -9, -24),
    S(   5, -33), S(  18, -21), S(  40, -32), S( -24,   2)},
   {S(  35, -15), S(   4, -21), S( -27,  -1), S( -20,  -9),
    S( -33,  -4), S( -41,  29), S(   1,  44), S( -15,   3)},
   {S(   1, -15), S(   6, -23), S(   8,   1), S(  -1,  11),
    S( -13,  17), S( -11,  39), S(-189,  90), S( -18,  18)}},
  {{S(   0,   0), S( -14, -29), S(   7, -22), S( -39,  16),
    S( -24,   2), S(   3,  42), S(-167,  -7), S( -36,  16)},
   {S(   0,   0), S(  18, -19), S(   7, -11), S( -17,   0),
    S(   0, -20), S(  27,  65), S(-184,  -3), S( -34,  11)},
   {S(   0,   0), S(  19,  -9), S(   2, -12), S(   2, -21),
    S(  17,  -4), S( -87,  47), S( -84, -74), S( -17,   1)},
   {S(   0,   0), S(   0,   2), S(  -6,  -5), S( -19,   6),
    S( -20,   4), S(-100,  30), S(   7, -40), S( -19,  -1)},
   {S(   0,   0), S(   8,   0), S(  11, -11), S(   8, -13),
    S(  12, -31), S( -57,  14), S(-101, -60), S(  -6,   1)},
   {S(   0,   0), S(  -1,  -8), S( -20,   0), S( -15,  -8),
    S(  21, -24), S( -37,   3), S(  55,  39), S( -14,  -3)},
   {S(   0,   0), S(  27, -18), S(  17, -14), S( -10, -11),
    S( -28,  14), S(  -8,  19), S( -57, -49), S( -33,  16)},
   {S(   0,   0), S(  13, -46), S(  15, -31), S( -18,  -8),
    S( -25,  19), S( -10,  20), S(-228, -56), S( -23,   8)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -14,  29), S( 112,  -9), S( -20,  20), S( -11,   1),
    S( -15,   0), S(  -8,  -2), S( -15,   8), S( -26,   0)},
   {S(  -9,  53), S(  56,  14), S( -15,  16), S(  -4,   9),
    S(  -4,   8), S(   4,  -2), S(  -2,   2), S( -17,  10)},
   {S(   3,  40), S(   6,  25), S( -19,  15), S( -12,   8),
    S(   0,   5), S(   8,   2), S(   8,  -8), S(   4,   7)},
   {S(  -7,  22), S(   4,  19), S( -26,   7), S( -15,  -3),
    S(  -9,   1), S(  12, -10), S(   3,  -8), S( -10,   7)}},
  {{S(   0,   0), S( -18, -16), S( -12,  -4), S(  21, -18),
    S(   6,  -3), S(   5, -18), S(   0,   3), S(  18,  30)},
   {S(   0,   0), S( -17, -37), S(  -1, -11), S(  42, -13),
    S(   4,  -3), S(  19, -27), S(  -2,  -6), S( -19,   0)},
   {S(   0,   0), S( -28, -52), S( -17, -11), S(  13,  -9),
    S(   9,  -2), S(  -4, -16), S( -11, -13), S( -11,   6)},
   {S(   0,   0), S(  -2, -19), S( -16, -16), S( -11,   2),
    S(   2,  -9), S(   1, -24), S(  71, -13), S(  15,  20)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -36,   1), S( -45,  26), S( -69,  29),
    S(  11,  11), S(  80, -18), S( 155,  43), S(   0,   0)},
   {S(   0,   0), S( -29,  13), S( -44,  42), S( -60,  42),
    S(   2,  44), S( 109,  29), S( 195,  93), S(   0,   0)}},
  {{S(   0,   0), S( -26,  27), S( -48,  36), S( -66,  51),
    S(   9,  51), S( 104,  48), S( 271, 106), S(   0,   0)},
   {S(   0,   0), S( -27,  19), S( -43,  34), S( -62,  56),
    S(   9,  72), S( 110, 127), S( 158, 267), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(  -2,   3), S(   2,  -3), S(   8, -12),
    S(   5, -17), S(  -9, -16), S( -17,  -6), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   4,  -2), S(   5,  -1), S(   7,   9),
    S(   0,  23), S(   4,  34), S(  24,  35), S(   0,   0),
};
...
const int ThreatMinorAttackedByMajor = S( -28, -50);
const int ThreatRookAttackedByLesser = S( -46, -24);
...
const int ClosednessKnightAdjustment[9] = {
    S( -10,  -5), S(  -8,  11), S( -10,  17), S(  -6,  16),
    S(  -4,  23), S(  -3,  15), S(   0,  10), S( -12,  27),
    S(  -8,  11),
};

const int ClosednessRookAdjustment[9] = {
    S(  45,   9), S(   0,  37), S(   2,  20), S(  -5,   8),
