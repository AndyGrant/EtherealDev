...
const int KnightValue = S( 458, 398);
const int BishopValue = S( 474, 420);
const int RookValue = S( 649, 719);
const int QueenValue = S(1310,1345);
const int KingValue = S(   0,   0);

const int PieceValues[8][PHASE_NB] = {
    { 104, 130}, { 458, 398}, { 474, 420}, { 649, 719},
    {1310,1345}, {   0,   0}, {   0,   0}, {   0,   0},
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][8] = {
   {S(   0,   0), S(  -4,   0), S(  -9,   5), S( -10,  27), S(   2,  45), S(  25,  18), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S(  -4,   6), S(  -5,  19), S(   7,  50), S(  24,  66), S(  14,  10), S(   0,   0), S(   0,   0)},
};
const int PawnIsolated = S(  -8, -11);
const int PawnStacked = S( -14, -26);
const int PawnBackwards[2] = {
    S(   6,  -4), S(  -8, -18),
};
const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -1,  -9), S(  10,   2), S(   1,   1), S(   4,  17),
    S(  14,   1), S(  26,   0), S(  18,   8), S(  22,  17),
    S(   6,   0), S(  19,   4), S(  10,   7), S(  16,  16),
    S(   6,  10), S(  21,  15), S(  23,  22), S(  38,  19),
    S(  24,  52), S(  24,  63), S(  64,  61), S(  50,  73),
    S( 104, -14), S( 197,  14), S( 227,  20), S( 248,  73),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightMobility[9] = {
    S( -89, -85), S( -36, -93), S( -20, -40), S(  -7, -15),
    S(   3, -11), S(   9,   2), S(  18,   3), S(  29,  -2),
    S(  48, -35),
};
const int BishopPair = S(  30,  70);
...
const int BishopMobility[14] = {
    S( -60,-128), S( -43, -68), S( -13, -47), S(  -1, -22),
    S(  10, -12), S(  18,   0), S(  21,   7), S(  23,   8),
    S(  23,  13), S(  30,   7), S(  33,   4), S(  45, -12),
    S(  45,  -1), S(  40, -33),
};
...
const int RookMobility[15] = {
    S(-147,-107), S( -69,-119), S( -18, -69), S(  -9, -27),
    S(  -9,  -6), S(  -9,   9), S(  -7,  22), S(  -1,  27),
    S(   5,  33), S(  10,  37), S(  12,  45), S(  19,  49),
    S(  21,  53), S(  28,  48), S(  21,  41),
};
const int QueenMobility[28] = {
    S( -61,-263), S(-216,-390), S( -48,-205), S( -38,-190),
    S( -18,-132), S( -24, -69), S( -14, -89), S( -14, -75),
    S( -10, -60), S(  -7, -50), S(  -4, -28), S(  -1, -25),
    S(   0, -14), S(   2,  -6), S(   1,  -1), S(   0,   3),
    S(   2,  15), S(  -1,  13), S(   8,  19), S(   0,  19),
    S(  -1,  16), S(  17,  20), S(   4,  -1), S(  31,   3),
...
const int KingDefenders[12] = {
    S( -29,   3), S( -15,   1), S(  -2,   0), S(   9,  -1),
    S(  21,  -5), S(  33,   2), S(  32,  11), S(  23,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};
const int KingShelter[2][8][8] = {
  {{S( -17,  14), S(   5, -11), S(  15,   0), S(  22,   1), S(   7,   6), S(  30,   3), S(   0, -32), S( -31,   4)},
   {S(   4,   5), S(  16,  -8), S(  10, -10), S(  -2, -12), S( -26,   0), S( -66,  78), S( 100,  93), S( -29,   2)},
   {S(  13,  12), S(   7,  -1), S( -20,   0), S(  -7,  -2), S( -22,  -2), S(  13,  -8), S(   3,  65), S( -16,   2)},
   {S(   8,  30), S(  12,   0), S(  -3,  -7), S(  17, -16), S(  17, -35), S( -22, -32), S(-147,  41), S(   0,   0)},
   {S( -18,  19), S(  -2,   2), S( -31,   2), S( -11,   4), S( -24, -16), S( -40, -18), S(  38,  -8), S( -11,   0)},
   {S(  24,   1), S(  19,  -5), S( -20,   0), S(  -5, -21), S(   7, -31), S(  17, -48), S(  49, -33), S( -18,   0)},
   {S(  19,   1), S(   0, -11), S( -28,  -6), S( -17, -13), S( -23, -20), S( -42,   9), S(   6,  46), S( -21,  10)},
   {S( -19,  -3), S(   0, -10), S(   4,   0), S(   2,   3), S( -11,  13), S(   4,  33), S(-200,  79), S( -19,  15)}},
  {{S(   0,   0), S(   3, -16), S(   5, -14), S( -45,  19), S(   0,  -9), S(   8,  50), S(-170, -14), S( -50,  12)},
   {S(   0,   0), S(  15,  -7), S(   6,  -7), S(  -5,  -8), S(  10, -32), S(  51, 102), S(-187, -10), S( -44,   6)},
   {S(   0,   0), S(  25,   0), S(   0,  -6), S(  19, -26), S(  14,  -7), S(-113,  45), S( -89, -83), S( -22,  -1)},
   {S(   0,   0), S(  -5,   9), S( -10,  12), S( -13,   3), S( -23,   0), S(-116,  10), S(   5, -49), S( -27,   0)},
   {S(   0,   0), S(   5,   5), S(   6,  -6), S(  20,  -4), S(  10, -23), S( -44,   8), S(-108, -72), S(  -2,  -4)},
   {S(   0,   0), S(   8,   0), S( -17,  -1), S( -12, -14), S(  30, -37), S( -44,   5), S(  58,  46), S( -28,   1)},
   {S(   0,   0), S(  13,  -2), S(  -2,  -1), S( -20,  -5), S( -21, -12), S(  22,  11), S( -58, -63), S( -43,  16)},
   {S(   0,   0), S(   6, -28), S(   9, -16), S( -21,   0), S( -26,  -2), S(   7, -16), S(-239, -73), S( -41,  17)}},
};
const int KingStorm[2][4][8] = {
  {{S(   2,   7), S(  34,  21), S(  -5,  11), S( -13,   5), S(  -3,   0), S(  -3,  -2), S( -11,   4), S(  -8,  -2)},
   {S(   5,  25), S(  12,  19), S(  -6,   7), S(  -2,   2), S(   0,   0), S(   6,  -5), S(   0,   3), S( -11,   0)},
   {S(   3,  17), S(   3,  20), S( -11,   6), S( -16,   3), S(   3,  -3), S(   2,  -3), S(   0,  -1), S(   0,  -3)},
   {S(   5,  15), S(   6,  19), S(  -5,  -4), S( -17,  -4), S(  -8,  -4), S(   4, -10), S(  -1,  -3), S( -14,  -1)}},
  {{S(   0,   0), S(   0,   0), S(   5, -12), S(  21, -14), S(   7,   0), S(  -1,  -6), S(  -6,   0), S(   0,   1)},
   {S(   0,   0), S(   0,   0), S(   0, -20), S(  28,  -9), S(   0,   1), S(  -3,   0), S(  -1,   0), S(  -2,   0)},
   {S(   0,   0), S(  -1,  -1), S(  -9, -21), S(  -2,  -4), S(  -6,   4), S( -18,  -3), S(  -4,  -1), S(   0,   0)},
   {S(   0,   0), S(   0,   0), S(  -7, -20), S(  -5,  -3), S(  -9,  -5), S(   8, -10), S(  15,   1), S(   1,   1)}},
};
...
const int PassedPawn[2][2][8] = {
  {{S(   0,   0), S( -27, -24), S( -22,   2), S( -12,   0), S(  15,   0), S(  54,   1), S( 141,  32), S(   0,   0)},
   {S(   0,   0), S(  -5,  -7), S( -23,  10), S( -15,  23), S(   2,  41), S(  65,  66), S( 190, 134), S(   0,   0)}},
  {{S(   0,   0), S( -13,   3), S( -18,   0), S( -10,  18), S(  24,  37), S(  76,  74), S( 238, 158), S(   0,   0)},
   {S(   0,   0), S( -21,  -5), S( -16,   0), S( -16,  34), S(  -2,  94), S(  43, 218), S( 124, 379), S(   0,   0)}},
};
const int PassedFriendlyDistance[8] = {
    S(   0,   0), S(   1,   2), S(   2,   0), S(   2,  -7),
    S(   0, -13), S(   0, -16), S(   2, -16), S(   0,   0),
};
const int PassedEnemyDistance[8] = {
    S(   0,   0), S(   0,   2), S(   0,   3), S(   0,   9),
    S(   5,  18), S(   5,  25), S(   5,  30), S(   0,   0),
};
const int PassedSafePromotionPath = S(  -1,  25);
const int ThreatWeakPawn = S( -15, -29);
const int ThreatMinorAttackedByPawn = S( -63, -52);
const int ThreatMinorAttackedByMinor = S( -30, -17);
const int ThreatMinorAttackedByMajor = S( -39, -41);
const int ThreatRookAttackedByLesser = S( -53, -23);
const int ThreatQueenAttackedByOne = S( -83,   2);
const int ThreatOverloadedPieces = S(  -8, -23);
const int ThreatByPawnPush = S(  19,  15);
