...
const int PawnValue   = S( 108, 144);
const int KnightValue = S( 416, 411);
const int BishopValue = S( 440, 427);
const int RookValue   = S( 625, 717);
const int QueenValue  = S(1308,1347);
const int KingValue   = S(   0,   0);

const int PieceValues[8][PHASE_NB] = {
    { 108, 144}, { 416, 411}, { 440, 427}, { 625, 717},
    {1308,1347}, {   0,   0}, {   0,   0}, {   0,   0},
};

const int PawnCandidatePasser[2][8] = {
   {S(   0,   0), S(  -4,   0), S( -10,   5), S( -13,  26), S(  -2,  45), S(  27,  20), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S(  -5,   6), S(  -7,  19), S(   5,  51), S(  21,  69), S(  15,  11), S(   0,   0), S(   0,   0)},
};
const int PawnIsolated = S(  -9, -10);
const int PawnStacked = S( -23, -24);
const int PawnBackwards[2] = {
    S(   8,  -4), S(  -7, -18),
};
const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -4,  -1), S(  11,   1), S(   3,   5), S(   5,  17),
    S(  15,   3), S(  32,   0), S(  21,  12), S(  28,  18),
    S(   9,   0), S(  22,   6), S(  12,  10), S(  16,  21),
    S(  10,  11), S(  23,  14), S(  31,  22), S(  34,  16),
    S(  29,  53), S(  27,  65), S(  64,  61), S(  53,  73),
    S( 104, -13), S( 196,  15), S( 227,  20), S( 248,  73),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightMobility[9] = {
    S( -89, -85), S( -40, -95), S( -27, -40), S( -12, -20),
    S(  -2,  -9), S(   5,   1), S(  14,   5), S(  24,   0),
    S(  37, -19),
};
const int BishopPair = S(  14,  78);
...
const int BishopMobility[14] = {
    S( -62,-128), S( -33, -71), S( -11, -51), S(  -1, -27),
    S(   7, -16), S(  14,  -1), S(  16,   7), S(  15,  13),
    S(  16,  15), S(  19,  17), S(  23,   6), S(  42,  -6),
    S(  46,  -1), S(  40, -30),
};
...
const int RookMobility[15] = {
    S(-147,-107), S( -60,-119), S( -19, -70), S( -10, -26),
    S( -10,  -5), S( -11,  10), S( -10,  25), S(  -4,  27),
    S(   2,  32), S(   7,  35), S(   7,  44), S(  13,  47),
    S(  17,  49), S(  25,  46), S(  22,  48),
};
const int QueenMobility[28] = {
    S( -61,-263), S(-216,-389), S( -48,-204), S( -35,-189),
    S( -22,-132), S( -16, -70), S(  -9, -89), S(  -9, -75),
    S(  -5, -60), S(  -4, -48), S(  -2, -28), S(   0, -23),
    S(   1, -14), S(   4,  -6), S(   1,   0), S(   0,   5),
    S(  -3,  13), S(  -9,  11), S(  -2,  16), S(  -5,  20),
    S(  -2,  17), S(  13,  18), S(   4,  -1), S(  31,   3),
    S(  33,  12), S(  55,  -6), S( -50, -16), S(   0,   0),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -27,  -1), S(  -9,   3), S(   0,   0), S(   8,   0),
    S(  17,   0), S(  28,   0), S(  33,  11), S(  23,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};
const int KingShelter[2][8][8] = {
  {{S( -17,  14), S(  10, -11), S(  11,  -2), S(  19,   1), S(   7,   6), S(  30,   3), S(  -1, -33), S( -34,   9)},
   {S(   3,   4), S(  22,  -8), S(   7,  -9), S(  -2, -11), S( -26,   0), S( -66,  78), S( 100,  93), S( -29,   4)},
   {S(  14,  11), S(  10,  -1), S( -22,   1), S(  -8,  -3), S( -22,  -3), S(  13,  -9), S(   4,  65), S( -16,   4)},
   {S(   6,  26), S(  15,   0), S(   0,  -6), S(  12, -17), S(  23, -35), S( -21, -32), S(-147,  41), S(  -1,   0)},
   {S( -18,  17), S(   0,   1), S( -30,   3), S( -14,   3), S( -24, -16), S( -41, -19), S(  38,  -8), S(  -9,   1)},
   {S(  31,  -1), S(  23, -11), S( -19,   5), S(  -9, -23), S(   8, -33), S(  16, -48), S(  49, -34), S( -25,   3)},
   {S(  17,   0), S(  -6,  -9), S( -27,  -2), S( -19, -13), S( -24, -20), S( -42,   9), S(   6,  47), S( -14,   5)},
   {S( -17,  -2), S(   4, -13), S(   5,   2), S(  -1,   4), S( -11,  10), S(   3,  32), S(-201,  79), S( -20,  16)}},
  {{S(   0,   0), S(   0, -15), S(   4, -15), S( -46,  18), S(   0,  -9), S(   9,  51), S(-170, -14), S( -49,  13)},
   {S(   0,   0), S(  17, -10), S(   1,  -5), S(  -7,  -8), S(  10, -32), S(  51, 102), S(-187, -10), S( -44,   9)},
   {S(   0,   0), S(  34,  -1), S(   1,  -7), S(  13, -27), S(  14,  -7), S(-113,  45), S( -89, -84), S( -22,   3)},
   {S(   0,   0), S(  -5,   9), S(  -8,  10), S( -11,   3), S( -23,   0), S(-116,  10), S(   5, -50), S( -25,   0)},
   {S(   0,   0), S(   8,   4), S(   9,  -5), S(  13,  -7), S(  13, -24), S( -44,   7), S(-109, -73), S(  -4,  -4)},
   {S(   0,   0), S(   3,   0), S( -15,   0), S( -13, -13), S(  29, -37), S( -45,   4), S(  59,  46), S( -28,   0)},
   {S(   0,   0), S(  14,  -3), S(   2,  -4), S( -18,  -4), S( -26, -12), S(  20,  11), S( -58, -64), S( -43,  19)},
   {S(   0,   0), S(   5, -25), S(  10, -17), S( -23,   0), S( -26,  -2), S(   7, -16), S(-239, -73), S( -36,  17)}},
};
const int KingStorm[2][4][8] = {
  {{S(   2,   8), S(  36,  22), S(  -6,  11), S( -14,   7), S(  -5,   1), S(  -1,  -2), S( -11,   5), S( -15,  -1)},
   {S(   6,  30), S(  14,  19), S(  -4,   8), S(  -3,   2), S(  -3,   1), S(   8,  -7), S(   2,   1), S( -16,   1)},
   {S(   3,  20), S(   3,  21), S( -10,   6), S( -13,   0), S(   2,  -4), S(   2,  -3), S(   2,  -6), S(  -5,   0)},
   {S(   5,  16), S(   6,  18), S(  -5,  -3), S( -13,  -7), S( -11,  -4), S(  10, -12), S(   0,  -3), S( -16,   0)}},
  {{S(   0,   0), S(   0,   0), S(   3, -15), S(  20, -16), S(   7,   0), S(   0,  -7), S(  -6,   0), S(   0,   1)},
   {S(   0,   0), S(   0,   0), S(   0, -21), S(  26,  -9), S(  -1,   1), S(  -2,   0), S(  -2,   0), S(  -2,   0)},
   {S(   0,   0), S(  -1,  -1), S(  -9, -21), S(  -2,  -4), S(  -5,   5), S( -20,  -4), S(  -4,  -1), S(   0,   0)},
   {S(   0,   0), S(   0,   0), S(  -8, -22), S(  -6,  -4), S(  -5,  -6), S(   6, -12), S(  18,   1), S(   1,   1)}},
};
...
const int PassedPawn[2][2][8] = {
  {{S(   0,   0), S( -26, -24), S( -23,   2), S( -14,   0), S(  18,   4), S(  57,   5), S( 140,  34), S(   0,   0)},
   {S(   0,   0), S(  -7,  -9), S( -24,  10), S( -18,  19), S(   1,  42), S(  66,  70), S( 190, 136), S(   0,   0)}},
  {{S(   0,   0), S( -14,   1), S( -19,   0), S( -16,  20), S(  18,  39), S(  74,  83), S( 239, 161), S(   0,   0)},
   {S(   0,   0), S( -21,  -3), S( -17,   1), S( -15,  33), S(  -1,  83), S(  40, 202), S( 123, 372), S(   0,   0)}},
};
const int PassedFriendlyDistance[8] = {
    S(   0,   0), S(   2,   2), S(   6,  -1), S(   9,  -9),
    S(   0, -15), S(  -4, -20), S(  -6, -19), S(   0,   0),
};
const int PassedEnemyDistance[8] = {
    S(   0,   0), S(  -1,   0), S(  -3,   1), S(  -4,  11),
    S(   3,  22), S(   8,  29), S(  12,  35), S(   0,   0),
};
const int PassedSafePromotionPath = S(  -2,  24);
const int ThreatWeakPawn = S( -14, -37);
const int ThreatMinorAttackedByPawn = S( -56, -50);
const int ThreatMinorAttackedByMinor = S( -27, -31);
const int ThreatMinorAttackedByMajor = S( -31, -40);
const int ThreatRookAttackedByLesser = S( -50, -25);
const int ThreatQueenAttackedByOne = S( -57,   1);
const int ThreatOverloadedPieces = S(  -6, -16);
const int ThreatByPawnPush = S(  16,  18);
