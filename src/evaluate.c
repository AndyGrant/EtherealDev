...
const int KingShelter[2][FILE_NB/2][RANK_NB] = {
  {{S( -10,  -1), S(   1,  -9), S(   9,  -3), S(   3,   5),
    S(  -5,  10), S(   2,  15), S(  -4,   1), S( -12,   4)},
   {S(  14,  -5), S(   8,  -8), S(  -5,   0), S( -13,   2),
    S( -20,  10), S(  -6,   3), S(   1,   2), S(  -5,   1)},
   {S(  25,  -3), S(  16,  -5), S( -13,   8), S( -10,   0),
    S(   9,  -8), S(  10,   1), S(   1,   0), S( -11,   3)},
   {S( -10,  10), S(   9,  -1), S(   0,   0), S(   8,  -5),
    S(  16, -12), S(   1,  -3), S(  -2,  -4), S(   5,  -1)}},
  {{S(   0,   0), S(   2, -16), S(  11, -13), S(  -6,   2),
    S(  -4,   9), S(  -3,   5), S(   0,   0), S( -10,   6)},
   {S(   0,   0), S(  20, -10), S(   9,  -5), S( -10,   5),
    S( -17,  16), S(   1,  10), S(  -4,   0), S( -15,   6)},
   {S(   0,   0), S(   8,  -1), S( -14,   7), S(  -5,  -1),
    S(   7,   3), S(  -1,   2), S(   1,  -1), S( -11,   4)},
   {S(   0,   0), S(   6,   1), S(   8,  -4), S(  10,  -9),
    S(   8, -12), S(   8,  -2), S(   2,   0), S(   0,  -1)}},
};
