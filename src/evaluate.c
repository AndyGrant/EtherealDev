...
const int PawnValue = S( 108, 132);

const int KnightValue = S( 457, 409);

const int BishopValue = S( 478, 425);

const int RookValue = S( 671, 712);

const int QueenValue = S(1247,1360);

const int KingValue = S(   0,   0);

const int PieceValues[8][PHASE_NB] = {
    { 108, 132}, { 457, 409}, { 478, 425}, { 671, 712},
    {1247,1360}, {   0,   0}, {   0,   0}, {   0,   0},
};

const int PawnCandidatePasser[2][8] = {
   {S(   0,   0), S( -26, -11), S( -13,  10), S( -12,  28), S(   4,  62), S(  46,  67), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -13,  15), S(  -6,  21), S(   4,  45), S(  16,  86), S(  33,  52), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -9,  -9);
...
const int PawnConnected32[32] = {
    ...
    S(  -1,  -8), S(  11,   0), S(   4,   0), S(   4,  18),
    S(  15,   0), S(  33,  -2), S(  22,   9), S(  26,  18),
    S(  10,   0), S(  23,   4), S(  11,  11), S(  15,  22),
    S(  16,   7), S(  24,  14), S(  30,  20), S(  34,  21),
    S(  57,  27), S(  53,  47), S(  68,  54), S(  82,  59),
    S( 110,  -2), S( 202,  11), S( 227,  28), S( 240,  51),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightMobility[9] = {
    S( -82,-101), S( -33, -99), S( -18, -44), S(  -4, -19),
    S(   7,  -7), S(  13,   8), S(  23,  10), S(  33,  10),
    S(  44,  -3),
};
...
const int BishopBehindPawn = S(   3,  20);

const int BishopMobility[14] = {
    S( -64,-146), S( -28, -96), S(  -8, -57), S(   1, -29),
    S(  11, -16), S(  18,   0), S(  21,   8), S(  21,  13),
    S(  21,  18), S(  24,  18), S(  22,  19), S(  38,   4),
    S(  40,  12), S(  64, -26),
};
...
const int RookOnSeventh = S(  -1,  32);

const int RookMobility[15] = {
    S(-149,-112), S( -54,-117), S( -14, -65), S(  -6, -21),
    S(  -6,   0), S(  -7,  14), S(  -6,  25), S(   0,  28),
    S(   6,  31), S(   9,  36), S(  11,  43), S(  15,  48),
    S(  14,  53), S(  22,  46), S(  71,  15),
};

const int QueenMobility[28] = {
    S( -61,-263), S(-212,-388), S( -60,-203), S( -27,-192),
    S( -15,-141), S( -11, -90), S(  -5, -64), S(  -6, -37),
    S(  -3, -26), S(  -2,  -4), S(   0,   7), S(   1,  21),
    S(   3,  22), S(   4,  33), S(   2,  38), S(   1,  39),
    S(   0,  42), S(  -3,  42), S(  -1,  36), S(  -5,  30),
    S(   0,   7), S(  17, -10), S(  17, -39), S(  20, -56),
    S(   4, -70), S(  21, -96), S( -57, -39), S( -27, -53),
};

const int KingDefenders[12] = {
    S( -18,  -3), S(  -9,   0), S(  -1,   2), S(   7,   4),
    S(  16,   4), S(  26,   2), S(  32,   0), S(  15,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingShelter[2][8][8] = {
  {{S( -13,   5), S(  16, -23), S(  18,  -8), S(   9,   2), S(   4,   3), S(   8,   2), S(  -2, -32), S( -49,  19)},
   {S(  16,  -6), S(  22, -18), S(  -1,  -6), S( -17,   1), S( -30,  13), S( -69,  70), S(  94,  85), S( -29,   3)},
   {S(  31,   0), S(  16, -10), S( -26,   5), S( -11, -10), S( -22,  -4), S(  -7,  -2), S(   0,  66), S( -15,   0)},
   {S(   3,  13), S(  20,  -8), S(   3, -11), S(  15, -23), S(  23, -35), S( -50,   1), S(-138,  51), S(   3,  -4)},
   {S( -12,   8), S(   0,  -2), S( -26,   0), S( -18,   1), S( -20,  -9), S( -38,  -1), S(  35, -15), S(  -6,  -1)},
   {S(  43, -13), S(  25, -18), S( -21,   0), S( -12, -20), S(   4, -25), S(  17, -25), S(  43, -30), S( -26,   2)},
   {S(  24, -10), S(   1, -18), S( -23,  -4), S( -19, -12), S( -29, -10), S( -34,  30), S(   0,  44), S( -13,   3)},
   {S( -10,  -8), S(   7, -18), S(   8,  -2), S(  -2,   6), S( -12,  13), S(  -9,  37), S(-191,  87), S( -19,  16)}},
  {{S(   0,   0), S(  -9, -23), S(   0, -17), S( -43,  17), S( -18,  -1), S(   4,  44), S(-168, -10), S( -46,  12)},
   {S(   0,   0), S(  25, -19), S(   6,  -8), S( -19,  -4), S(   0, -16), S(  29,  72), S(-185,  -5), S( -42,   7)},
   {S(   0,   0), S(  34, -10), S(  -1,  -6), S(   8, -18), S(  13,  -8), S( -92,  48), S( -86, -76), S( -22,  -2)},
   {S(   0,   0), S(  -1,   8), S(  -1,   1), S( -16,   0), S( -26,   0), S(-102,  29), S(   7, -43), S( -22,  -4)},
   {S(   0,   0), S(  11,   2), S(  11,  -5), S(  14, -12), S(  13, -24), S( -54,  14), S(-106, -64), S(  -1,  -6)},
   {S(   0,   0), S(   8,  -6), S( -20,   0), S( -27,  -9), S(  19, -27), S( -40,   3), S(  57,  40), S( -20,  -3)},
   {S(   0,   0), S(  24, -13), S(  10, -12), S( -10, -10), S( -28,   5), S(  -8,  14), S( -57, -52), S( -35,  15)},
   {S(   0,   0), S(  14, -38), S(  19, -27), S( -19,  -5), S( -18,  15), S(  -1,  15), S(-230, -58), S( -26,   8)}},
};

const int KingStorm[2][4][8] = {
  {{S(   2,  21), S( 113, -11), S( -28,  24), S( -22,  10), S( -13,   3), S(  -8,  -2), S( -19,   7), S( -21,  -2)},
   {S(   2,  44), S(  56,   9), S( -18,  24), S(  -5,  12), S(  -3,   5), S(   5,  -3), S(   0,   0), S( -13,   0)},
   {S(  12,  35), S(  20,  18), S( -23,  20), S( -13,  10), S(   3,   1), S(   6,   0), S(   8,  -7), S(   1,  -1)},
   {S(   1,  24), S(  20,  19), S( -11,   8), S( -15,   3), S( -13,   1), S(   6, -10), S(   0,  -7), S( -14,   2)}},
  {{S(   0,   0), S( -11, -13), S( -15,   0), S(  20, -16), S(  11,  -6), S(   0, -17), S(  -8,   0), S(  16,  27)},
   {S(   0,   0), S( -13, -27), S(  -8,  -9), S(  32, -12), S(  -1,  -1), S(  10, -22), S( -12,  -8), S( -15,   2)},
   {S(   0,   0), S( -23, -41), S( -31,  -8), S(   9, -10), S(   2,  -2), S( -10, -15), S( -14, -13), S( -10,   5)},
   {S(   0,   0), S(  -1, -14), S( -18, -18), S( -11,  -3), S(  -4,  -6), S(   6, -29), S(  66,  -6), S(  12,  18)}},
};
...
const int PassedPawn[2][2][8] = {
  {{S(   0,   0), S( -39,   0), S( -52,  22), S( -82,  26), S(  -6,  16), S(  65,   0), S( 152,  58), S(   0,   0)},
   {S(   0,   0), S( -26,   1), S( -46,  22), S( -71,  27), S( -13,  29), S(  84,  28), S( 184,  95), S(   0,   0)}},
  {{S(   0,   0), S( -26,  10), S( -47,  17), S( -73,  32), S(  -2,  34), S(  90,  40), S( 258, 127), S(   0,   0)},
   {S(   0,   0), S( -28,   6), S( -40,  14), S( -66,  37), S(  -4,  60), S(  74, 144), S( 154, 302), S(   0,   0)}},
};

const int PassedFriendlyDistance[8] = {
    S(   0,   0), S(   0,   0), S(   2,  -4), S(   7, -11),
    S(   5, -16), S(  -6, -16), S( -10, -12), S(   0,   0),
};

const int PassedEnemyDistance[8] = {
    S(   0,   0), S(   3,   0), S(   5,   0), S(   9,   9),
    S(   2,  24), S(   8,  34), S(  20,  37), S(   0,   0),
};

const int PassedSafePromotionPath = S( -32,  36);
...
const int ThreatRookAttackedByLesser = S( -58, -13);

const int ThreatQueenAttackedByOne = S( -49, -19);

const int ThreatOverloadedPieces = S(  -8, -17);
