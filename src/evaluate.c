...
const int KSAttackWeight[6] = {
    S(   0,   0), S( -13, -15), S(  -4,  -6), S(  -5,  -8),
    S(  -2,  -6), S(   0,   0),
};

const int KSAttackValue = S( -27, -34);

const int KSWeakSquares = S( -30, -31);

const int KSFriendlyPawns = S(  20,  22);

const int KSNoEnemyQueens = S( 276, 276);

const int KSSafeQueenCheck = S( -90, -91);

const int KSSafeRookCheck = S( -94, -93);

const int KSSafeBishopCheck = S( -51, -51);

const int KSSafeKnightCheck = S(-122,-122);

const int KSAdjustment = S(  19,  19);
