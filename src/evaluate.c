...
const int PawnValue = S( 116, 130);

const int KnightValue = S( 440, 394);

const int BishopValue = S( 465, 399);

const int RookValue = S( 674, 678);

const int QueenValue = S(1287,1361);

const int KingValue = S(   0,   0);

const int PawnPSQT32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S( -19,   7), S(  -3,   7), S( -12,   5), S( -14,   0),
    S( -27,   0), S( -21,   6), S( -15,  -4), S( -13, -13),
    S( -17,   2), S(  -8,   9), S(   6, -11), S(   6, -26),
    S(  -4,   3), S(  -4,   6), S(   0,  -7), S(   8, -19),
    S(   0,  15), S(   0,  24), S(   7,   9), S(  31, -12),
    S( -19, -53), S( -66, -17), S(   2, -26), S(  39, -42),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -49, -26), S( -14, -37), S( -16, -31), S(  -3, -21),
    S(  -3, -23), S(   2, -13), S(  -5, -31), S(   3, -22),
    S(   6, -27), S(  16, -27), S(  15, -20), S(  22,  -5),
    S(  23,   5), S(  24,   7), S(  25,  14), S(  25,  19),
    S(  26,  17), S(  37,  12), S(  43,  22), S(  35,  29),
    S( -12,  17), S(  10,  13), S(  33,  26), S(  32,  29),
    S(   9, -10), S(  -5,   3), S(  34, -19), S(  44,   0),
    S(-169, -17), S( -81,  -1), S(-110,  20), S( -29,   2),
};

const int BishopPSQT32[32] = {
    S(   5, -26), S(  14, -19), S( -13, -12), S(   4, -17),
    S(  29, -32), S(   4, -39), S(  22, -22), S(   6, -12),
    S(  16, -13), S(  27, -15), S(   1, -19), S(  18,  -6),
    S(  18,  -7), S(  18,  -4), S(  23,   3), S(  21,   4),
    S( -10,   8), S(  19,   6), S(  11,  14), S(  15,  13),
    S(   3,   8), S(   0,  15), S(   2,   3), S(  19,  13),
    S( -44,  13), S( -53,   3), S(  -4,   8), S( -20,  13),
    S( -50,  -4), S( -47,   8), S( -86,  19), S( -89,  27),
};

const int RookPSQT32[32] = {
    S( -17, -33), S( -12, -25), S(  -3, -30), S(   4, -29),
    S( -46, -18), S( -15, -32), S(  -9, -35), S(  -5, -35),
    S( -34, -21), S(  -8, -12), S( -16, -15), S(   0, -22),
    S( -18,  -1), S(  -5,   7), S(  -3,   8), S(   5,  -2),
    S(   1,  10), S(  19,  10), S(  27,   7), S(  40,   2),
    S(  -8,  20), S(  28,   7), S(   5,  17), S(  37,   4),
    S(   3,   7), S( -14,  15), S(   9,   7), S(  24,   8),
    S(  36,  20), S(  27,  24), S(   7,  26), S(  19,  26),
};

const int QueenPSQT32[32] = {
    S(  24, -53), S(   5, -40), S(   9, -50), S(  13, -43),
    S(  16, -39), S(  26, -58), S(  30, -71), S(  17, -27),
    S(  12, -22), S(  27, -20), S(   7,   2), S(   9,  -1),
    S(  14,  -3), S(  16,  17), S(  -2,  18), S( -16,  62),
    S(  -6,  18), S(  -6,  41), S( -14,  22), S( -28,  71),
    S( -21,  28), S( -14,  19), S( -23,  23), S( -13,  26),
    S(  -4,  27), S( -56,  65), S(  -9,  15), S( -40,  53),
    S(  -5,  18), S(  18,   8), S(   9,   9), S(  -5,  21),
};

const int KingPSQT32[32] = {
    S(  36, -80), S(  37, -55), S( -16, -16), S( -36, -17),
    S(  29, -32), S(  -1, -27), S( -30,   2), S( -48,   9),
    S(   7, -30), S(  16, -32), S(  17,  -3), S(  -9,  12),
    S(   4, -32), S(  84, -35), S(  41,  -3), S(  -8,  12),
    S(   6, -17), S(  97, -30), S(  46,   5), S(   0,   8),
    S(  47, -20), S( 122, -14), S(  95,   8), S(  39,   8),
    S(   6, -42), S(  48,   0), S(  34,  17), S(  11,  10),
    S(   9, -98), S(  76, -49), S( -18,  -8), S( -17,  -6),
};

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -27, -10), S( -12,   9), S( -14,  28),
    S(  -2,  59), S(  45,  66), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -16,  17), S(  -7,  19), S(   5,  42),
    S(  21,  83), S(  33,  55), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S( -11, -12);

const int PawnStacked[2] = { S(  -8, -21), S( -13,  -8) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   0,  -1), S(   5,  -4), S(  14, -11),
    S(  11, -10), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -18, -30), S( -11, -25), S(  14, -24),
    S(  14, -15), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(   0,   1), S(   6,   7), S(   3,   1), S(   3,  16),
    S(  22,   1), S(  30,  -4), S(  24,  10), S(  27,  18),
    S(   9,  -2), S(  19,  -1), S(  13,   8), S(  19,  15),
    S(  15,   7), S(  21,  14), S(  35,  19), S(  31,  16),
    S(  57,  24), S(  54,  50), S(  70,  54), S(  83,  58),
    S( 112,   1), S( 204,  10), S( 228,  31), S( 240,  51),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightBehindPawn = S(  10,  17);

const int KnightInSiberia[4] = {
    S( -11,   2), S( -12,  -8), S( -26,  -7), S( -18,  -2),
};

const int KnightMobility[9] = {
    S( -74,-104), S( -34, -96), S( -22, -42), S( -12, -20),
    S(   5, -12), S(  11,   9), S(  23,   8), S(  31,  13),
    S(  42,  -9),
};

const int BishopPair = S(  32,  75);
...
const int BishopBehindPawn = S(   9,  23);

const int BishopLongDiagonal = S(  26,  10);

const int BishopMobility[14] = {
    S( -66,-147), S( -31, -94), S( -17, -54), S(  -8, -29),
    S(   5, -16), S(  14,  -4), S(  24,   5), S(  23,  10),
    S(  26,  17), S(  27,  16), S(  28,  18), S(  41,  -2),
    S(  42,  17), S(  67, -27),
};

const int RookFile[2] = { S(  21,   3), S(  45,  -7) };

const int RookOnSeventh = S(   6,  37);

const int RookMobility[15] = {
    S(-148,-113), S( -53,-113), S( -22, -62), S( -10, -21),
    S(  -4,  -2), S(  -7,  15), S(  -7,  30), S(   1,  31),
    S(  11,  36), S(  14,  38), S(  18,  43), S(  25,  45),
    S(  21,  45), S(  33,  34), S(  79, -18),
};

const int QueenMobility[28] = {
    S( -61,-263), S(-210,-387), S( -58,-201), S( -17,-192),
    S(  -5,-139), S(  -1, -84), S(   1, -52), S(   2, -23),
    S(   7, -16), S(  10,   5), S(  12,  12), S(  14,  26),
    S(  18,  24), S(  17,  32), S(  18,  38), S(  13,  40),
    S(  11,  43), S(   3,  44), S(   5,  40), S(   0,  36),
    S(   7,  14), S(  21, -10), S(  27, -35), S(  29, -53),
    S(  12, -68), S(  24, -95), S( -56, -39), S( -31, -61),
};

const int KingDefenders[12] = {
    S( -22,   7), S( -14,  -4), S(   0,   0), S(   9,   4),
    S(  18,   4), S(  28,   3), S(  31,  -2), S(  13,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  32,  29), S(  14,  23), S(   4,  12), S( -15, -20),
    S( -17, -47), S( -15, -62), S( -14, -65), S( -10, -68),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -11,   4), S(  17, -24), S(  20,  -9), S(   9,   2),
    S(   5,   3), S(   1,   2), S(  -3, -33), S( -48,  21)},
   {S(  20,  -5), S(  22, -14), S(  -2,  -6), S( -16,   2),
    S( -30,  14), S( -70,  67), S(  92,  82), S( -27,   5)},
   {S(  35,  -4), S(  17,  -6), S( -28,   6), S( -12,  -7),
    S( -21,  -4), S( -11,   1), S(   0,  66), S( -14,  -1)},
   {S(   5,  13), S(  19,  -9), S(  -3, -12), S(  13, -21),
    S(  25, -35), S( -58,   5), S(-136,  52), S(   4, -11)},
   {S( -11,   9), S(   8,  -3), S( -28,  -1), S( -20,   2),
    S( -20,  -6), S( -41,  -1), S(  33, -17), S(  -3,  -6)},
   {S(  51, -17), S(  25, -15), S( -22,   0), S( -19, -22),
    S(   6, -25), S(  18, -21), S(  41, -30), S( -30,   0)},
   {S(  29, -15), S(   3, -17), S( -24,  -4), S( -20,  -9),
    S( -30,  -7), S( -36,  32), S(   0,  44), S( -13,   1)},
   {S(  -9, -12), S(   4, -17), S(  11,  -4), S(   4,  10),
    S( -12,  15), S(  -8,  37), S(-190,  87), S( -18,  14)}},
  {{S(   0,   0), S( -10, -22), S(   4, -18), S( -41,  16),
    S( -22,   2), S(   3,  42), S(-167,  -8), S( -45,  12)},
   {S(   0,   0), S(  24, -20), S(   6,  -9), S( -18,  -1),
    S(  -2, -13), S(  26,  65), S(-184,  -3), S( -38,   7)},
   {S(   0,   0), S(  32, -11), S(  -4,  -8), S(   7, -17),
    S(  14,  -8), S( -87,  47), S( -84, -73), S( -19,  -2)},
   {S(   0,   0), S(  -2,   9), S(  -3,   1), S( -18,   1),
    S( -27,   0), S( -99,  31), S(   7, -41), S( -19,  -5)},
   {S(   0,   0), S(   9,   0), S(  10,  -6), S(  11, -11),
    S(  13, -27), S( -57,  15), S(-104, -61), S(  -2,  -4)},
   {S(   0,   0), S(   9,  -9), S( -17,  -3), S( -27,  -8),
    S(  16, -25), S( -38,   3), S(  55,  38), S( -17,  -6)},
   {S(   0,   0), S(  32, -15), S(  12, -15), S(  -5,  -8),
    S( -29,   7), S(  -9,  15), S( -56, -49), S( -37,  12)},
   {S(   0,   0), S(  16, -41), S(  12, -31), S( -17,  -5),
    S( -16,  18), S(  -5,  20), S(-228, -55), S( -22,  10)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S(  -4,  27), S( 116,  -6), S( -24,  22), S( -20,   7),
    S( -12,   0), S( -11,  -4), S( -16,   8), S( -19,   5)},
   {S(  -3,  49), S(  58,  12), S( -20,  21), S(  -2,  11),
    S(  -2,   4), S(   4,  -6), S(   1,   0), S(  -7,   9)},
   {S(   9,  42), S(  17,  24), S( -23,  20), S(  -9,  10),
    S(   0,  -1), S(   7,  -5), S(   2,  -7), S(  11,  -2)},
   {S(  -1,  28), S(  16,  21), S( -17,  11), S( -13,   2),
    S( -17,  -3), S(   5, -13), S(  -2,  -8), S(  -9,   0)}},
  {{S(   0,   0), S( -15, -16), S( -17,  -1), S(  19, -17),
    S(  11,  -7), S(   0, -18), S(  -2,  -1), S(  17,  29)},
   {S(   0,   0), S( -16, -34), S(  -6, -12), S(  35, -11),
    S(  -1,  -2), S(  13, -24), S(  -7, -10), S( -17,   2)},
   {S(   0,   0), S( -28, -49), S( -27,  -6), S(  11, -12),
    S(   1,  -4), S(  -7, -14), S( -12, -15), S( -11,   6)},
   {S(   0,   0), S(  -2, -18), S( -15, -16), S( -15,  -7),
    S(  -8,  -8), S(   4, -26), S(  72, -11), S(  13,  20)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -37,   4), S( -56,  21), S( -79,  29),
    S(  -5,  14), S(  70,  -2), S( 158,  60), S(   0,   0)},
   {S(   0,   0), S( -28,   9), S( -50,  24), S( -72,  32),
    S( -11,  33), S(  90,  29), S( 183, 101), S(   0,   0)}},
  {{S(   0,   0), S( -26,  15), S( -50,  21), S( -75,  36),
    S(  -3,  36), S(  92,  43), S( 265, 114), S(   0,   0)},
   {S(   0,   0), S( -28,  13), S( -45,  18), S( -66,  38),
    S(   3,  57), S(  91, 110), S( 160, 269), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   4,  -1), S(   6,  -5), S(   6, -11),
    S(  -2, -14), S(  -3, -16), S(  -4, -13), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   0,   1), S(   0,   3), S(   9,  11),
    S(  13,  21), S(  15,  28), S(  34,  26), S(   0,   0),
};

const int PassedSafePromotionPath = S( -27,  31);

const int ThreatWeakPawn = S( -10, -39);

const int ThreatMinorAttackedByPawn = S( -53, -53);

const int ThreatMinorAttackedByMinor = S( -37, -33);

const int ThreatMinorAttackedByMajor = S( -21, -45);

const int ThreatRookAttackedByLesser = S( -52, -21);

const int ThreatMinorAttackedByKing = S( -17, -20);

const int ThreatRookAttackedByKing = S( -15, -23);

const int ThreatQueenAttackedByOne = S( -38, -29);

const int ThreatOverloadedPieces = S(  -5, -12);

const int ThreatByPawnPush = S(  18,  19);

const int SpaceRestrictPiece = S(  -4,   0);

const int SpaceRestrictEmpty = S(  -7,   1);

const int SpaceCenterControl = S(   3,   3);

const int ClosednessKnightAdjustment[9] = {
    S( -12, -15), S( -10,   2), S(  -9,   9), S(  -5,  10),
    S(  -3,  15), S(   4,  16), S(   3,  12), S(  -8,  27),
    S(  -7,  16),
};

const int ClosednessRookAdjustment[9] = {
    S(  59, -26), S(   9,  27), S(   3,  15), S(  -2,   7),
    S(  -5,   5), S(  -9,  -8), S( -12, -10), S( -21, -15),
    S( -25, -16),
};
