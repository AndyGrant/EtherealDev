...
const int KnightValue = S( 458, 386);
const int BishopValue = S( 470, 411);
const int RookValue   = S( 642, 708);
const int QueenValue  = S(1303,1332);
const int KingValue   = S(   0,   0);

const int PieceValues[8][PHASE_NB] = {
    { 100, 121}, { 458, 386}, { 470, 411}, { 642, 708},
    {1303,1332}, {   0,   0}, {   0,   0}, {   0,   0},
};
...
const int PawnStacked = S(  -9, -32);

const int PawnBackwards[2] = { S(   7,  -3), S( -10, -11) };

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(   0, -16), S(   6,   1), S(   3,  -4), S(   5,  19),
    S(   8,   0), S(  22,   0), S(  15,   8), S(  17,  20),
    S(   6,   0), S(  20,   3), S(  13,   7), S(  16,  17),
    S(   7,  10), S(  20,  20), S(  19,  23), S(  35,  23),
    S(  22,  53), S(  20,  64), S(  56,  63), S(  47,  73),
    S(  88, -33), S( 209,  12), S( 215,   4), S( 287,  65),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightRammedPawns = S(   2,   7);

const int KnightOutpost[2] = { S(  20, -34), S(  36,   3) };

const int KnightMobility[9] = {
    S( -88, -94), S( -37, -91), S( -22, -42),
    S(  -7, -15), S(   2, -14), S(   7,   0),
    S(  17,  -2), S(  31,  -3), S(  48, -33),
};
...
const int BishopRammedPawns = S( -10,  -7);

const int BishopOutpost[2] = { S(  18, -16), S(  48,  -8) };

const int BishopMobility[14] = {
    S( -64,-131), S( -47, -69), S( -18, -46), S(  -5, -21),
    S(   5, -10), S(  17,   0), S(  22,   6), S(  27,   4),
    S(  29,   9), S(  34,   2), S(  36,   5), S(  44, -18),
    S(  47,  -4), S(  52, -43),
};
...
const int RookMobility[15] = {
    S(-142,-123), S( -72,-119), S( -15, -68), S(  -8, -25),
    S(  -7,  -3), S(  -7,  13), S(  -7,  24), S(  -3,  31),
    S(   2,  33), S(   6,  36), S(  11,  41), S(  17,  47),
    S(  20,  49), S(  28,  45), S(  21,  47),
};
...
const int QueenMobility[28] = {
    S( -60,-260), S(-351,-486), S( -53,-217), S( -41,-206),
    S( -14,-132), S( -26, -71), S( -13, -90), S( -17, -79),
    S( -12, -61), S( -10, -53), S(  -7, -28), S(  -5, -27),
    S(  -6, -14), S(   0,  -9), S(   0,  -4), S(  -1,   4),
    S(   4,  18), S(   0,  16), S(   9,  24), S(  -3,  23),
    S(   1,  21), S(  18,  24), S(   3,   1), S(  27,   8),
    S(  27,  10), S(  61,  -3), S( -58, -19), S( -11,  -7),
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -26, -23), S( -18,   7), S( -13,  -2), S(  19,   0), S(  51,  -4), S( 138,  35), S(   0,   0)},
   {S(   0,   0), S(  -1,   3), S( -16,  23), S(  -7,  37), S(   7,  48), S(  62,  66), S( 186, 133), S(   0,   0)}},
  {{S(   0,   0), S(  -7,  15), S( -11,  10), S(  -4,  27), S(  29,  34), S(  68,  66), S( 219, 152), S(   0,   0)},
   {S(   0,   0), S( -11,   9), S( -13,  17), S( -14,  53), S(   0, 112), S(  51, 214), S( 141, 376), S(   0,   0)}},
};
