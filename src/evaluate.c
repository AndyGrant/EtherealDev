...
const int PawnValue = S( 107, 134);

const int KnightValue = S( 457, 409);

const int BishopValue = S( 477, 423);

const int RookValue = S( 670, 712);

const int QueenValue = S(1245,1358);

const int KingValue = S(   0,   0);

const int PieceValues[8][PHASE_NB] = {
    { 107, 134}, { 457, 409}, { 477, 423}, { 670, 712},
    {1245,1358}, {   0,   0}, {   0,   0}, {   0,   0},
};

const int PawnCandidatePasser[2][8] = {
   {S(   0,   0), S( -26, -12), S( -13,  10), S( -13,  28), S(   4,  63), S(  46,  68), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -14,  14), S(  -7,  21), S(   4,  46), S(  16,  87), S(  33,  52), S(   0,   0), S(   0,   0)},
};
...
const int PawnConnected32[32] = {
    ...
    S(  -1,  -8), S(  11,   1), S(   4,   0), S(   3,  18),
    S(  14,   0), S(  33,  -1), S(  22,   9), S(  25,  18),
    S(  10,   0), S(  22,   4), S(  10,  11), S(  15,  23),
    S(  16,   7), S(  24,  14), S(  30,  20), S(  34,  21),
    S(  57,  26), S(  53,  46), S(  68,  54), S(  81,  59),
    S( 110,  -2), S( 202,  10), S( 227,  28), S( 240,  51),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightBehindPawn = S(   4,  23);

const int KnightMobility[9] = {
    S( -82,-101), S( -34,-100), S( -18, -44), S(  -4, -20),
    S(   7,  -8), S(  13,   8), S(  23,  11), S(  33,  11),
    S(  44,  -5),
};
...
const int BishopMobility[14] = {
    S( -64,-146), S( -28, -97), S(  -8, -58), S(   1, -30),
    S(  11, -16), S(  18,   0), S(  21,   8), S(  21,  13),
    S(  21,  19), S(  24,  18), S(  22,  19), S(  38,   3),
    S(  40,  12), S(  64, -27),
};
...
const int RookOnSeventh = S(   0,  33);

const int RookMobility[15] = {
    S(-150,-112), S( -54,-117), S( -14, -66), S(  -6, -22),
    S(  -6,  -1), S(  -7,  13), S(  -6,  24), S(   0,  28),
    S(   6,  32), S(   9,  37), S(  12,  43), S(  15,  49),
    S(  15,  54), S(  22,  47), S(  71,  13),
};

const int QueenMobility[28] = {
    S( -61,-263), S(-212,-388), S( -60,-203), S( -27,-193),
    S( -15,-142), S( -11, -91), S(  -5, -64), S(  -6, -38),
    S(  -3, -27), S(  -2,  -5), S(   0,   7), S(   1,  21),
    S(   3,  22), S(   4,  33), S(   2,  38), S(   1,  39),
    S(   0,  42), S(  -4,  41), S(  -2,  35), S(  -6,  29),
    S(   0,   6), S(  16, -11), S(  17, -40), S(  19, -57),
    S(   4, -71), S(  20, -96), S( -58, -39), S( -27, -53),
};

const int KingDefenders[12] = {
    S( -17,  -2), S( -10,   0), S(  -1,   1), S(   7,   4),
    S(  16,   5), S(  26,   2), S(  32,   0), S(  15,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingShelter[2][8][8] = {
  {{S( -13,   5), S(  16, -24), S(  19,  -8), S(   9,   2), S(   4,   3), S(   8,   2), S(  -2, -32), S( -49,  19)},
   {S(  17,  -6), S(  22, -18), S(  -1,  -6), S( -18,   2), S( -30,  13), S( -69,  70), S(  94,  85), S( -29,   3)},
   {S(  32,   0), S(  16,  -9), S( -26,   5), S( -11, -10), S( -23,  -4), S(  -7,  -2), S(   0,  66), S( -15,   0)},
   {S(   4,  14), S(  19,  -8), S(   2, -11), S(  14, -24), S(  23, -35), S( -50,   1), S(-138,  51), S(   3,  -5)},
   {S( -12,   9), S(   0,  -2), S( -26,   0), S( -18,   0), S( -20, -10), S( -38,   0), S(  35, -15), S(  -6,  -2)},
   {S(  44, -13), S(  25, -18), S( -22,   0), S( -12, -20), S(   4, -25), S(  18, -24), S(  43, -29), S( -26,   1)},
   {S(  25,  -9), S(   1, -18), S( -23,  -5), S( -19, -13), S( -29, -10), S( -34,  31), S(   0,  44), S( -13,   4)},
   {S( -10,  -8), S(   6, -18), S(   8,  -2), S(  -2,   6), S( -12,  13), S(  -9,  37), S(-190,  87), S( -19,  17)}},
  {{S(   0,   0), S(  -9, -23), S(   0, -17), S( -43,  17), S( -18,  -1), S(   4,  44), S(-168, -10), S( -45,  13)},
   {S(   0,   0), S(  25, -19), S(   6,  -8), S( -20,  -4), S(   1, -16), S(  29,  72), S(-185,  -5), S( -42,   7)},
   {S(   0,   0), S(  34, -10), S(  -1,  -6), S(   8, -19), S(  13,  -8), S( -92,  49), S( -86, -76), S( -23,  -3)},
   {S(   0,   0), S(  -1,   9), S(  -1,   1), S( -16,  -1), S( -26,   0), S(-102,  29), S(   7, -43), S( -22,  -5)},
   {S(   0,   0), S(  10,   2), S(  11,  -5), S(  14, -12), S(  13, -24), S( -54,  15), S(-106, -64), S(  -1,  -6)},
   {S(   0,   0), S(   8,  -7), S( -20,   0), S( -27, -10), S(  19, -27), S( -40,   2), S(  57,  40), S( -19,  -3)},
   {S(   0,   0), S(  24, -12), S(  10, -12), S( -10, -10), S( -28,   5), S(  -8,  14), S( -56, -52), S( -36,  14)},
   {S(   0,   0), S(  14, -38), S(  19, -27), S( -19,  -5), S( -18,  15), S(  -1,  15), S(-230, -58), S( -25,  10)}},
};

const int KingStorm[2][4][8] = {
  {{S(   2,  21), S( 113, -12), S( -28,  24), S( -22,  10), S( -13,   3), S(  -8,  -2), S( -19,   7), S( -22,  -2)},
   {S(   1,  44), S(  56,   9), S( -18,  24), S(  -4,  13), S(  -3,   5), S(   5,  -3), S(   0,   0), S( -13,   0)},
   {S(  12,  35), S(  20,  18), S( -22,  20), S( -12,  10), S(   3,   1), S(   7,   0), S(   8,  -7), S(   0,  -1)},
   {S(   1,  25), S(  21,  19), S( -11,   7), S( -14,   3), S( -13,   0), S(   6, -11), S(   0,  -7), S( -15,   1)}},
  {{S(   0,   0), S( -11, -13), S( -16,  -1), S(  20, -16), S(  11,  -6), S(   0, -17), S(  -8,   0), S(  16,  27)},
   {S(   0,   0), S( -13, -27), S(  -9, -10), S(  32, -12), S(  -1,  -1), S(  10, -22), S( -12,  -8), S( -15,   3)},
   {S(   0,   0), S( -23, -41), S( -31,  -8), S(   9, -10), S(   1,  -3), S( -10, -15), S( -13, -13), S( -10,   6)},
   {S(   0,   0), S(  -1, -14), S( -18, -18), S( -11,  -4), S(  -4,  -6), S(   6, -30), S(  66,  -6), S(  12,  18)}},
};
...
const int PassedPawn[2][2][8] = {
  {{S(   0,   0), S( -38,   0), S( -52,  23), S( -82,  26), S(  -6,  16), S(  66,   1), S( 153,  59), S(   0,   0)},
   {S(   0,   0), S( -26,   1), S( -46,  23), S( -71,  27), S( -13,  29), S(  83,  28), S( 184,  95), S(   0,   0)}},
  {{S(   0,   0), S( -27,  10), S( -47,  17), S( -73,  32), S(  -1,  34), S(  90,  39), S( 258, 127), S(   0,   0)},
   {S(   0,   0), S( -28,   6), S( -40,  14), S( -66,  37), S(  -5,  60), S(  73, 144), S( 154, 301), S(   0,   0)}},
};

const int PassedFriendlyDistance[8] = {
    S(   0,   0), S(   0,   0), S(   2,  -3), S(   7, -11),
    S(   5, -16), S(  -6, -15), S( -10, -11), S(   0,   0),
};

const int PassedEnemyDistance[8] = {
    S(   0,   0), S(   3,   0), S(   5,   1), S(   9,  10),
    S(   2,  25), S(   7,  33), S(  20,  36), S(   0,   0),
};

const int PassedSafePromotionPath = S( -32,  37);

const int ThreatWeakPawn = S( -12, -27);

const int ThreatMinorAttackedByPawn = S( -57, -50);

const int ThreatMinorAttackedByMinor = S( -28, -36);

const int ThreatMinorAttackedByMajor = S( -26, -42);

const int ThreatRookAttackedByLesser = S( -58, -14);

const int ThreatQueenAttackedByOne = S( -50, -19);
