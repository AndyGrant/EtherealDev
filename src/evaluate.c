...
const int KSAttackWeight[6] = {
    S(   0,   0), S( -14, -17), S(  -8,  -7), S(  -6, -10),
    S(  -1,  -8), S(   0,   0),
};

const int KSAttackValue = S( -18, -45);

const int KSWeakSquares = S( -15, -36);

const int KSFriendlyPawns = S(  21,  20);

const int KSNoEnemyQueens = S( 276, 276);

const int KSSafeQueenCheck = S( -67, -93);

const int KSSafeRookCheck = S( -91, -94);

const int KSSafeBishopCheck = S( -50, -51);

const int KSSafeKnightCheck = S(-120,-123);

const int KSAdjustment = S(  24,  18);
