...
const int KnightValue = S( 458, 411);
const int BishopValue = S( 476, 427);
const int RookValue   = S( 669, 715);
const int QueenValue  = S(1300,1343);
const int KingValue   = S(   0,   0);

const int PieceValues[8][PHASE_NB] = {
    { 107, 135}, { 458, 411}, { 476, 427}, { 669, 715},
    {1300,1343}, {   0,   0}, {   0,   0}, {   0,   0},
};

const int PawnCandidatePasser[2][8] = {
   {S(   0,   0), S(  -9,  -5), S( -10,   4), S( -12,  27), S(   6,  54), S(  47,  39), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S(  -9,   8), S(  -6,  20), S(   5,  46), S(  22,  78), S(  18,  15), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -8, -10);

const int PawnStacked = S( -19, -28);
...
const int PawnConnected32[32] = {
    ...
    S(  -1,  -8), S(  11,   1), S(   4,   0), S(   5,  19),
    S(  15,   0), S(  34,  -2), S(  22,   9), S(  26,  17),
    S(  10,   0), S(  22,   4), S(  10,  11), S(  16,  22),
    S(  15,   8), S(  23,  15), S(  28,  22), S(  33,  21),
    S(  34,  45), S(  32,  60), S(  67,  61), S(  61,  73),
    S( 104, -13), S( 197,  14), S( 227,  20), S( 248,  72),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightMobility[9] = {
    S( -89, -86), S( -37, -94), S( -21, -41), S(  -7, -19),
    S(   4, -11), S(  11,   2), S(  20,   5), S(  30,   3),
    S(  42, -15),
};

const int BishopPair = S(  25,  75);
...
const int BishopMobility[14] = {
    S( -64,-130), S( -31, -79), S(  -8, -56), S(   1, -30),
    S(  11, -16), S(  19,  -1), S(  21,   8), S(  22,  12),
    S(  22,  17), S(  25,  16), S(  24,  15), S(  42,  -1),
    S(  46,   2), S(  43, -29),
};
...
const int RookMobility[15] = {
    S(-147,-107), S( -56,-118), S( -14, -69), S(  -6, -25),
    S(  -7,  -3), S(  -7,  13), S(  -6,  24), S(   0,  28),
    S(   6,  33), S(   9,  37), S(  11,  44), S(  14,  50),
    S(  14,  55), S(  22,  49), S(  46,  24),
};

const int QueenMobility[28] = {
    S( -61,-263), S(-216,-389), S( -49,-205), S( -32,-190),
    S( -17,-133), S( -13, -73), S(  -6, -89), S(  -6, -72),
    S(  -2, -58), S(  -1, -40), S(   0, -24), S(   2, -11),
    S(   3,  -7), S(   5,   4), S(   2,  11), S(   1,  14),
    S(   0,  21), S(  -5,  21), S(  -4,  19), S(  -9,  17),
    S(  -8,   8), S(   4,   3), S(  -1, -13), S(  19, -15),
    S(  25,  -2), S(  49, -18), S( -51, -18), S(  -1,  -3),
};

const int KingDefenders[12] = {
    S( -21,   0), S( -12,   0), S(  -2,   1), S(   7,   1),
    S(  17,   0), S(  28,  -2), S(  34,  10), S(  23,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingShelter[2][8][8] = {
  {{S( -18,  10), S(  12, -17), S(  16,  -3), S(  13,   0), S(   6,   5), S(  29,   3), S(  -1, -32), S( -42,  19)},
   {S(   7,   1), S(  19, -13), S(   0,  -6), S(  -6,  -8), S( -26,   1), S( -66,  78), S( 100,  93), S( -27,   6)},
   {S(  21,   8), S(  11,  -3), S( -27,   5), S( -13,  -8), S( -23,  -3), S(  12,  -8), S(   3,  65), S( -17,   3)},
   {S(   7,  22), S(  16,  -1), S(   0,  -5), S(  11, -19), S(  19, -34), S( -21, -27), S(-146,  42), S(   0,   0)},
   {S( -13,  15), S(  -1,   1), S( -30,   4), S( -18,   1), S( -23, -15), S( -39, -15), S(  38,  -8), S(  -7,   0)},
   {S(  39,  -8), S(  22, -13), S( -23,   2), S( -12, -20), S(   6, -28), S(  19, -43), S(  49, -33), S( -27,   5)},
   {S(  22,  -3), S(   0, -12), S( -26,   0), S( -18, -12), S( -24, -19), S( -41,  11), S(   6,  46), S( -12,   8)},
   {S( -15,  -4), S(   4, -15), S(   6,   1), S(  -3,   6), S( -11,  10), S(   0,  33), S(-200,  79), S( -20,  19)}},
  {{S(   0,   0), S(  -8, -19), S(   2, -14), S( -45,  19), S(  -1,  -9), S(   8,  50), S(-170, -14), S( -46,  18)},
   {S(   0,   0), S(  18, -14), S(   0,  -6), S( -13,  -9), S(   9, -31), S(  49, 100), S(-187, -10), S( -46,  11)},
   {S(   0,   0), S(  34,  -4), S(   0,  -4), S(  13, -25), S(  14,  -7), S(-112,  45), S( -89, -83), S( -21,   0)},
   {S(   0,   0), S(  -1,  13), S(  -7,   8), S( -16,   1), S( -24,   0), S(-115,  11), S(   5, -49), S( -22,  -2)},
   {S(   0,   0), S(   9,   5), S(   9,  -3), S(  12, -10), S(  11, -24), S( -43,   9), S(-108, -72), S(  -2,  -5)},
   {S(   0,   0), S(   5,  -2), S( -20,   2), S( -18, -14), S(  29, -36), S( -44,   5), S(  58,  46), S( -20,   0)},
   {S(   0,   0), S(  17,  -6), S(   4,  -5), S( -16,  -5), S( -27,  -7), S(  16,  10), S( -58, -63), S( -42,  20)},
   {S(   0,   0), S(   6, -28), S(  12, -18), S( -22,  -1), S( -23,   2), S(   6, -14), S(-239, -72), S( -26,  15)}},
};

const int KingStorm[2][4][8] = {
  {{S(   4,  16), S(  47,  12), S( -10,  18), S( -16,   8), S(  -8,   0), S(  -3,  -4), S( -14,   6), S( -17,  -5)},
   {S(   7,  35), S(  21,  17), S(  -6,  16), S(  -2,   8), S(  -3,   1), S(   5,  -7), S(  -1,  -2), S( -14,  -3)},
   {S(   6,  28), S(   5,  18), S( -14,  12), S( -15,   5), S(   0,  -2), S(   3,  -4), S(   4, -10), S(  -2,  -5)},
   {S(   6,  19), S(   8,  20), S(  -5,   0), S( -12,  -1), S( -12,  -2), S(   8, -15), S(   1,  -6), S( -13,  -1)}},
  {{S(   0,   0), S(   0,   0), S(  -6,  -7), S(  24, -17), S(  13,  -4), S(   0,  -9), S(  -8,   0), S(   1,   3)},
   {S(   0,   0), S(   0,  -2), S(  -2, -17), S(  30, -13), S(  -4,   0), S(  -1,  -1), S(  -6,  -1), S(  -3,   0)},
   {S(   0,   0), S(  -2,  -4), S( -19, -19), S(   1,  -9), S(  -6,   0), S( -21,  -5), S(  -4,  -1), S(   0,   0)},
   {S(   0,   0), S(   0,  -1), S( -12, -24), S(  -9,  -7), S(  -3,  -8), S(   6, -17), S(  23,   1), S(   2,   3)}},
};
...
const int PassedPawn[2][2][8] = {
  {{S(   0,   0), S( -25, -19), S( -24,   6), S( -30,   6), S(  11,  12), S(  59,  16), S( 147,  51), S(   0,   0)},
   {S(   0,   0), S( -11,  -6), S( -23,  14), S( -24,  18), S(  -1,  34), S(  63,  58), S( 186, 122), S(   0,   0)}},
  {{S(   0,   0), S( -16,   3), S( -23,   8), S( -26,  21), S(  14,  33), S(  77,  64), S( 241, 156), S(   0,   0)},
   {S(   0,   0), S( -19,   0), S( -19,   5), S( -20,  26), S(   3,  65), S(  41, 181), S( 121, 351), S(   0,   0)}},
};

const int PassedFriendlyDistance[8] = {
    S(   0,   0), S(   0,   0), S(   2,  -3), S(   6, -10),
    S(   0, -15), S(  -6, -17), S( -10, -14), S(   0,   0),
};

const int PassedEnemyDistance[8] = {
    S(   0,   0), S(   1,   0), S(   0,   3), S(   0,  12),
    S(   4,  23), S(  11,  30), S(  27,  29), S(   0,   0),
};

const int PassedSafePromotionPath = S( -20,  33);

const int ThreatWeakPawn = S( -13, -27);
const int ThreatMinorAttackedByPawn = S( -58, -49);
const int ThreatMinorAttackedByMinor = S( -28, -35);
const int ThreatMinorAttackedByMajor = S( -26, -43);
const int ThreatRookAttackedByLesser = S( -56, -17);
const int ThreatQueenAttackedByOne = S( -55,  -3);
const int ThreatOverloadedPieces = S(  -9, -20);
const int ThreatByPawnPush = S(  17,  20);
