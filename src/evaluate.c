...
void initializeEvalInfo(EvalInfo* ei, Board* board, PawnKingTable* pktable){
    ...
    if (File(wKingSq) == 0) ei->kingAreas[WHITE] >>= 1;
    if (File(wKingSq) == 7) ei->kingAreas[WHITE] <<= 1;
    
    ei->kingAreas[BLACK] = KingMap[bKingSq] | (1ull << bKingSq) | (KingMap[bKingSq] >> 8);
    if (File(bKingSq) == 0) ei->kingAreas[BLACK] >>= 1;
    if (File(bKingSq) == 7) ei->kingAreas[BLACK] <<= 1;
