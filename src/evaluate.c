...
const int PawnValue   = S(  98, 124);
const int KnightValue = S( 456, 406);
const int BishopValue = S( 475, 423);
const int RookValue   = S( 657, 685);
const int QueenValue  = S(1299,1343);
...
const int PawnPSQT32[32] = {
    ...
    S( -16,   6), S(   2,   1), S(  -6,   4), S(  -4,  -1),
    S( -24,   5), S( -13,   5), S( -11,  -5), S(  -2, -12),
    S( -18,  12), S( -12,  11), S(   4,  -8), S(   6, -22),
    S( -14,  15), S(  -9,   8), S( -14,  -6), S( -10, -20),
    S(  -8,  29), S(  -5,  25), S(   0,  13), S(  26, -13),
    S( -21, -61), S( -59, -30), S(   2, -44), S(  45, -59),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -48, -31), S(  -1, -29), S( -10, -29), S(   3, -14),
    S(   2, -19), S(   2, -11), S(   4, -27), S(  11, -16),
    S(  14, -27), S(  18, -21), S(  15, -16), S(  23,  -4),
    S(  18,   2), S(  25,   6), S(  32,  17), S(  36,  24),
    S(  24,  13), S(  30,   9), S(  42,  27), S(  34,  38),
    S( -18,  11), S(   1,  12), S(  31,  29), S(  31,  27),
    S(  12, -12), S(  -5,  -2), S(  30, -22), S(  40,  -1),
    S(-178, -25), S( -85,  -4), S(-111,  18), S( -33,  -4),
};

const int BishopPSQT32[32] = {
    S(  10, -26), S(  13, -18), S(  -1, -14), S(  11, -11),
    S(  27, -36), S(  13, -37), S(  20, -21), S(  13, -10),
    S(  16, -12), S(  27, -15), S(   4, -17), S(  22,  -4),
    S(  12,  -7), S(  21,  -3), S(  18,   5), S(  27,   6),
    S( -10,  14), S(  20,   6), S(   5,  14), S(  16,  17),
    S(   3,   7), S(  -5,  17), S(  -1,   6), S(  13,  14),
    S( -44,  15), S( -51,   5), S(  -7,   8), S( -23,  13),
    S( -58,  -7), S( -45,   6), S( -84,  15), S( -90,  27),
};

const int RookPSQT32[32] = {
    S( -13, -28), S( -13, -20), S(  -2, -21), S(   4, -25),
    S( -49, -14), S(  -9, -33), S(  -7, -30), S(   4, -35),
    S( -28, -15), S(  -6, -12), S( -14, -16), S(   2, -25),
    S( -19,  -3), S( -11,   7), S(  -6,   3), S(   8,  -3),
    S(  -6,   9), S(   9,   8), S(  21,   4), S(  41,   0),
    S( -17,  18), S(  28,   4), S(   5,  19), S(  38,   2),
    S(   0,   4), S( -12,  12), S(  12,   5), S(  28,   6),
    S(  35,  21), S(  27,  23), S(   1,  32), S(   9,  28),
};

const int QueenPSQT32[32] = {
    S(  33, -66), S(   7, -45), S(  16, -59), S(  23, -43),
    S(  14, -42), S(  29, -58), S(  28, -71), S(  18, -26),
    S(  10, -30), S(  24, -19), S(   7,   7), S(   7,   2),
    S(  12,  -9), S(  17,  12), S(   0,  24), S( -13,  65),
    S(  -4,  14), S(  -8,  47), S( -13,  22), S( -28,  75),
    S( -25,  32), S( -17,  19), S( -23,  25), S( -11,  27),
    S(  -2,  27), S( -64,  67), S(  -3,  12), S( -38,  52),
    S( -12,  18), S(  20,   9), S(   9,  10), S(  -5,  21),
};

const int KingPSQT32[32] = {
    S(  50, -77), S(  40, -51), S( -18, -10), S( -29, -24),
    S(  27, -20), S( -17, -10), S( -45,  12), S( -64,  14),
    S(  -5, -20), S(   4, -18), S(  13,   3), S( -17,  19),
    S(   1, -30), S(  95, -32), S(  46,   9), S(  -4,  30),
    S(  10, -13), S( 108, -30), S(  56,  14), S(  12,  21),
    S(  50, -27), S( 134, -22), S( 105,   3), S(  43,   0),
    S(   7, -58), S(  48, -10), S(  32,   4), S(   9,  -4),
...
const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -27, -10), S( -10,   9), S( -17,  32),
    S(   3,  60), S(  38,  65), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -15,  19), S(  -4,  22), S(   3,  47),
    S(  21,  87), S(  38,  62), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -4, -13);

const int PawnStacked[2] = { S(  -2, -20), S(  -5,  -9) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   3,  -7), S(  10,  -6), S(   8, -11),
    S(   6, -12), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -14, -27), S(  -5, -25), S(   2, -26),
    S(  29, -30), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -6,  -7), S(  10,   1), S(   0,   1), S(   8,  12),
    S(  12,   1), S(  21,  -4), S(  22,   3), S(  24,  10),
    S(   7,  -1), S(  23,   0), S(   8,   7), S(  15,  14),
    S(   9,  10), S(  20,  15), S(  24,  20), S(  28,  18),
    S(  55,  18), S(  41,  44), S(  61,  54), S(  74,  60),
...
const int KnightInSiberia[4] = {
    S(  -9,  -2), S( -13,  -7), S( -26,  -5), S( -26,  -6),
};

const int KnightMobility[9] = {
    S( -78,-106), S( -31,-102), S( -14, -42), S(  -2, -16),
    S(   9,  -7), S(  11,  11), S(  19,  13), S(  29,  12),
    S(  43,  -5),
};
...
const int BishopMobility[14] = {
    S( -69,-149), S( -34, -99), S( -10, -55), S(  -2, -28),
    S(   9, -18), S(  16,  -3), S(  19,   7), S(  20,  13),
...
const int RookFile[2] = { S(   8,   4), S(  31,   4) };

const int RookOnSeventh = S(  -4,  31);

const int RookMobility[15] = {
    S(-159,-116), S( -58,-115), S( -15, -66), S(  -8, -23),
    S(  -9,  -1), S( -11,  14), S(  -9,  26), S(  -2,  29),
    S(   5,  34), S(   8,  37), S(  11,  45), S(  18,  48),
    S(  20,  52), S(  37,  40), S( 106,  -9),
};
...
const int QueenMobility[28] = {
    S( -61,-263), S(-213,-388), S( -60,-200), S( -23,-191),
    S(  -6,-144), S(  -4, -80), S(   0, -44), S(  -1, -12),
    S(   5, -11), S(   7,  12), S(  13,  14), S(  15,  31),
    S(  18,  23), S(  20,  33), S(  17,  38), S(  14,  44),
    S(  16,  42), S(   7,  44), S(   9,  41), S(   8,  31),
...
const int KingDefenders[12] = {
    S( -27,  -1), S(  -6,   0), S(   3,   5), S(  10,   7),
    S(  18,   6), S(  28,   1), S(  27,  -4), S(  12,  -1),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  40,  30), S(  31,  20), S(  10,   7), S( -24, -16),
    S( -28, -47), S( -21, -62), S( -17, -66), S( -11, -64),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -9,   2), S(  14, -26), S(  24,  -9), S(  18,   6),
    S(  14,   3), S(  -1,  -1), S(  -3, -34), S( -51,  20)},
   {S(  16,  -7), S(  20, -18), S(   4,  -5), S( -15,   4),
    S( -30,  16), S( -73,  67), S(  91,  81), S( -18,   0)},
   {S(  36,  -4), S(   9,  -8), S( -30,   7), S( -14,  -8),
    S( -17,  -5), S( -12,   1), S(   0,  68), S( -12,  -1)},
   {S(   2,  13), S(  26, -14), S(   5, -13), S(  15, -21),
    S(  30, -38), S( -58,   3), S(-135,  51), S(   7,  -7)},
   {S( -17,  13), S(   4,  -1), S( -32,   4), S( -20,   8),
    S( -21,  -6), S( -45,  -5), S(  32, -18), S(  -9,   2)},
   {S(  48, -17), S(  22, -15), S( -22,   5), S(  -8, -21),
    S(   5, -26), S(  19, -21), S(  40, -32), S( -22,   2)},
   {S(  32, -16), S(  -1, -17), S( -29,  -1), S( -23,  -7),
    S( -31,  -5), S( -41,  30), S(   1,  44), S(  -8,   0)},
   {S( -10, -14), S(   4, -20), S(   9,   3), S(  -2,  12),
    S( -10,  17), S( -10,  36), S(-190,  89), S( -17,  16)}},
  {{S(   0,   0), S( -13, -26), S(   7, -23), S( -39,  20),
    S( -23,   1), S(   3,  42), S(-167,  -7), S( -42,  10)},
   {S(   0,   0), S(  23, -22), S(   8,  -6), S( -16,   0),
    S(   3, -18), S(  26,  65), S(-184,  -3), S( -30,   5)},
   {S(   0,   0), S(  22, -12), S(  -1, -11), S(   6, -20),
    S(  21,  -3), S( -87,  48), S( -84, -74), S( -18,  -2)},
   {S(   0,   0), S( -10,   9), S(  -7,  -2), S( -25,   5),
    S( -27,   4), S(-100,  30), S(   7, -40), S( -22,  -4)},
   {S(   0,   0), S(  15,   0), S(  12,  -7), S(  15, -13),
    S(  18, -31), S( -57,  14), S(-101, -60), S(  -2,  -3)},
   {S(   0,   0), S(  -1,  -6), S( -26,   3), S( -23,  -9),
    S(  23, -23), S( -37,   3), S(  55,  39), S( -16,  -4)},
   {S(   0,   0), S(  24, -17), S(  13, -15), S( -14,  -8),
    S( -29,  14), S(  -8,  19), S( -57, -50), S( -31,  14)},
   {S(   0,   0), S(  12, -44), S(  22, -32), S( -19,  -7),
    S( -20,  19), S(  -8,  21), S(-228, -56), S( -24,   3)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -13,  33), S( 112,  -6), S( -23,  24), S( -12,   2),
    S( -14,  -1), S(  -6,  -7), S( -14,   4), S( -28,   2)},
   {S(  -8,  53), S(  51,  15), S( -20,  20), S(  -4,   7),
    S(  -1,   4), S(   6,  -5), S(  -4,   4), S( -13,   5)},
   {S(   3,  44), S(   4,  28), S( -22,  19), S( -10,   5),
    S(   1,   2), S(   8,   0), S(  12,  -7), S(   4,   4)},
   {S(  -7,  28), S(   4,  25), S( -23,   8), S( -15,  -1),
    S( -10,   0), S(  11, -13), S(   2,  -8), S( -13,   7)}},
  {{S(   0,   0), S( -18, -16), S( -16,  -1), S(  23, -20),
    S(  11,  -7), S(   7, -21), S(   5,   1), S(  16,  29)},
   {S(   0,   0), S( -17, -37), S(   3, -11), S(  46, -13),
    S(   3,  -4), S(  17, -25), S(  -6,  -5), S( -19,   0)},
   {S(   0,   0), S( -28, -52), S( -23,  -7), S(  18, -12),
    S(   7,  -2), S(  -9, -13), S( -10, -12), S( -11,   6)},
   {S(   0,   0), S(  -2, -19), S( -20, -17), S( -13,  -2),
    S(  -1,  -8), S(   0, -23), S(  73, -12), S(  15,  20)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -37,   1), S( -48,  26), S( -73,  31),
    S(   7,  10), S(  81, -16), S( 158,  41), S(   0,   0)},
   {S(   0,   0), S( -28,  13), S( -46,  37), S( -66,  38),
    S(   1,  36), S( 106,  23), S( 189,  88), S(   0,   0)}},
  {{S(   0,   0), S( -23,  27), S( -48,  34), S( -66,  45),
    S(  10,  42), S( 103,  33), S( 271,  95), S(   0,   0)},
   {S(   0,   0), S( -29,  22), S( -43,  32), S( -62,  52),
    S(  11,  62), S( 104, 117), S( 157, 265), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   1,   2), S(   4,  -4), S(   7, -12),
    S(   4, -15), S( -12,  -9), S( -17,   0), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   3,  -1), S(   4,   0), S(   7,   9),
    S(   1,  21), S(   7,  28), S(  28,  23), S(   0,   0),
};
...
const int ThreatMinorAttackedByPawn  = S( -52, -57);
...
const int ClosednessKnightAdjustment[9] = {
    S(  -9,  -8), S(  -6,   8), S(  -7,  16), S(  -1,  16),
    S(  -1,  19), S(   2,  14), S(   6,   5), S(  -7,  24),
    S(  -7,   9),
};

const int ClosednessRookAdjustment[9] = {
    S(  67, -13), S(   4,  34), S(   3,  18), S(  -2,   1),
    S( -11,   6), S( -11, -12), S( -12, -12), S( -23, -16),
    S( -35, -21),
};
...
const int ComplexityPawnEndgame = S(   0,  58);
const int ComplexityAdjustment  = S(   0,-155);
