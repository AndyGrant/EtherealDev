...
const int PawnPSQT32[32] = {
    ...
    S( -15,   7), S(   0,   2), S( -10,   5), S(  -6,   0),
    S( -22,   5), S( -15,   5), S( -11,  -6), S(  -3, -12),
    S( -17,  12), S( -12,  11), S(   5, -11), S(   6, -24),
    S( -11,  16), S(  -7,   9), S( -12,  -6), S(  -8, -21),
    S(  -8,  33), S(  -5,  27), S(   1,  17), S(  25, -10),
    S( -20, -60), S( -59, -27), S(   2, -41), S(  45, -56),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -48, -31), S(  -2, -29), S( -11, -29), S(   1, -16),
    S(   1, -19), S(   1, -13), S(   3, -30), S(  10, -17),
    S(  13, -29), S(  18, -21), S(  14, -17), S(  23,  -3),
    S(  21,   3), S(  24,   6), S(  32,  18), S(  31,  24),
    S(  25,  13), S(  30,   8), S(  43,  27), S(  33,  38),
    S( -17,  12), S(   2,  14), S(  31,  30), S(  30,  29),
    S(  13, -13), S(  -4,  -1), S(  32, -21), S(  41,   2),
    S(-178, -25), S( -85,  -4), S(-111,  18), S( -33,  -3),
};

const int BishopPSQT32[32] = {
    S(  10, -26), S(  13, -17), S(  -2, -15), S(   8, -12),
    S(  27, -36), S(  14, -38), S(  22, -22), S(  11, -12),
    S(  18, -12), S(  26, -15), S(   2, -17), S(  21,  -4),
    S(  11,  -8), S(  20,  -3), S(  16,   6), S(  25,   6),
    S( -10,  14), S(  19,   5), S(   6,  14), S(  16,  18),
    S(   3,   8), S(  -5,  17), S(  -1,   6), S(  13,  14),
    S( -44,  15), S( -49,   5), S(  -7,   8), S( -23,  13),
    S( -58,  -6), S( -45,   6), S( -84,  16), S( -90,  27),
};

const int RookPSQT32[32] = {
    S( -12, -29), S( -11, -22), S(  -4, -21), S(   2, -28),
    S( -51, -14), S( -10, -34), S(  -9, -32), S(   0, -36),
    S( -28, -16), S(  -6, -13), S( -17, -16), S(  -1, -26),
    S( -19,  -2), S( -10,   8), S(  -8,   3), S(   5,  -5),
    S(  -5,  12), S(  12,   8), S(  22,   6), S(  39,  -1),
    S( -15,  22), S(  30,   6), S(   7,  21), S(  38,   3),
    S(   2,   7), S( -10,  15), S(  12,   6), S(  27,   6),
    S(  35,  22), S(  28,  25), S(   1,  34), S(   9,  29),
};

const int QueenPSQT32[32] = {
    S(  31, -66), S(   6, -45), S(  12, -60), S(  22, -45),
    S(  14, -42), S(  27, -58), S(  28, -70), S(  18, -28),
    S(  12, -30), S(  25, -20), S(   6,   5), S(   5,   0),
    S(  13,  -8), S(  18,  12), S(  -1,  24), S( -14,  64),
    S(  -3,  15), S(  -6,  47), S( -12,  23), S( -28,  76),
    S( -24,  31), S( -16,  20), S( -22,  26), S( -11,  27),
    S(  -2,  26), S( -62,  68), S(  -4,  12), S( -37,  53),
    S(  -9,  19), S(  20,   9), S(  10,  11), S(  -4,  23),
};

const int KingPSQT32[32] = {
    S(  49, -77), S(  40, -49), S( -18, -12), S( -28, -26),
    S(  26, -21), S( -18, -13), S( -46,  10), S( -67,  12),
    S(  -5, -23), S(   3, -22), S(  11,   0), S( -18,  18),
    S(   0, -29), S(  94, -33), S(  46,   8), S(  -4,  30),
    S(  10, -12), S( 108, -28), S(  56,  16), S(  12,  22),
    S(  50, -25), S( 134, -18), S( 105,   7), S(  43,   3),
    S(   7, -58), S(  48,  -7), S(  32,   6), S(   9,  -3),
...
const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -27, -10), S( -10,   9), S( -18,  32),
    S(  -2,  59), S(  38,  64), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -15,  19), S(  -4,  23), S(   2,  48),
    S(  21,  88), S(  38,  62), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -6, -13);

const int PawnStacked[2] = { S(  -3, -20), S(  -6,  -9) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   4,  -3), S(   9,  -7), S(  10, -13),
    S(   7, -13), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -14, -26), S(  -6, -24), S(   4, -25),
    S(  30, -28), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -6,  -8), S(  10,  -1), S(   0,   1), S(   8,  12),
    S(  12,   1), S(  24,  -4), S(  23,   4), S(  26,  12),
    S(   8,  -1), S(  24,  -1), S(   9,   7), S(  16,  15),
    S(  10,  11), S(  21,  16), S(  28,  21), S(  30,  18),
    S(  54,  20), S(  41,  46), S(  62,  57), S(  75,  63),
...
const int KnightInSiberia[4] = {
    S(  -9,  -2), S( -11,  -9), S( -21,  -6), S( -26,  -6),
};

const int KnightMobility[9] = {
    S( -78,-106), S( -33,-103), S( -15, -44), S(  -2, -16),
    S(   8,  -6), S(  11,  10), S(  18,  13), S(  28,  12),
    S(  41,  -4),
};
...
const int BishopRammedPawns = S(  -6, -15);
...
const int BishopMobility[14] = {
    S( -69,-149), S( -31,-100), S( -10, -56), S(  -2, -29),
    S(   9, -18), S(  15,  -3), S(  17,   7), S(  18,  13),
    S(  17,  19), S(  25,  19), S(  24,  21), S(  45,   7),
    S(  47,  20), S(  75, -17),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  10,   7), S(  29,   6) };

const int RookOnSeventh = S(  -2,  35);

const int RookMobility[15] = {
    S(-159,-116), S( -58,-115), S( -16, -65), S(  -7, -22),
    S(  -7,  -1), S( -11,  16), S( -10,  29), S(  -4,  32),
    S(   4,  34), S(   8,  37), S(  10,  46), S(  17,  48),
    S(  18,  51), S(  35,  39), S( 104,  -8),
};
...
const int QueenMobility[28] = {
    ...
    S(  -6,-144), S(  -3, -80), S(   2, -44), S(   1, -12),
    S(   5, -11), S(   6,  11), S(  11,  14), S(  13,  31),
    S(  17,  24), S(  18,  34), S(  16,  38), S(  14,  44),
    S(  15,  41), S(   8,  45), S(   8,  41), S(   8,  31),
...
const int KingDefenders[12] = {
    S( -28,  -3), S( -11,   0), S(   1,   6), S(  11,   8),
    S(  19,   8), S(  28,   1), S(  30,  -4), S(  12,  -1),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  42,  31), S(  28,  23), S(  10,   9), S( -24, -18),
    S( -28, -50), S( -21, -63), S( -17, -67), S( -11, -64),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -9,   2), S(  14, -25), S(  22,  -9), S(  18,   6),
    S(  14,   3), S(  -1,  -1), S(  -3, -34), S( -54,  21)},
   {S(  16,  -6), S(  21, -17), S(   2,  -5), S( -15,   4),
    S( -30,  16), S( -73,  67), S(  91,  81), S( -21,   0)},
   {S(  35,  -3), S(   9,  -7), S( -30,   7), S( -14,  -8),
    S( -17,  -5), S( -12,   1), S(   0,  68), S( -14,   0)},
   {S(   2,  13), S(  27, -13), S(   8, -14), S(  16, -21),
    S(  27, -39), S( -59,   3), S(-135,  51), S(   5,  -7)},
   {S( -16,  14), S(   3,  -2), S( -31,   4), S( -20,   8),
    S( -21,  -6), S( -45,  -5), S(  32, -18), S(  -9,   0)},
   {S(  47, -15), S(  21, -12), S( -20,   3), S(  -8, -21),
    S(   6, -27), S(  20, -21), S(  40, -32), S( -24,   0)},
   {S(  33, -14), S(   0, -17), S( -29,  -3), S( -23,  -9),
    S( -31,  -5), S( -41,  30), S(   1,  44), S( -11,   0)},
   {S(  -7, -13), S(   4, -20), S(   9,   2), S(  -3,  11),
    S( -12,  17), S( -10,  37), S(-190,  89), S( -20,  16)}},
  {{S(   0,   0), S( -13, -26), S(   7, -23), S( -39,  20),
    S( -23,   1), S(   3,  42), S(-167,  -7), S( -43,  11)},
   {S(   0,   0), S(  22, -21), S(   9,  -7), S( -16,   0),
    S(   3, -18), S(  26,  65), S(-184,  -3), S( -32,   6)},
   {S(   0,   0), S(  23, -12), S(  -1, -10), S(   5, -20),
    S(  21,  -3), S( -87,  48), S( -84, -74), S( -20,  -2)},
   {S(   0,   0), S( -10,   9), S(  -6,  -2), S( -25,   5),
    S( -27,   4), S(-100,  30), S(   7, -40), S( -22,  -4)},
   {S(   0,   0), S(  14,   0), S(  14,  -8), S(  15, -13),
    S(  17, -31), S( -57,  14), S(-101, -60), S(  -1,  -3)},
   {S(   0,   0), S(   1,  -7), S( -26,   1), S( -22,  -8),
    S(  23, -23), S( -37,   3), S(  55,  39), S( -18,  -5)},
   {S(   0,   0), S(  26, -15), S(  13, -14), S( -11,  -9),
    S( -29,  12), S(  -8,  19), S( -57, -50), S( -34,  13)},
   {S(   0,   0), S(  14, -43), S(  20, -33), S( -19,  -7),
    S( -20,  19), S(  -8,  21), S(-228, -56), S( -24,   3)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -13,  33), S( 112,  -7), S( -24,  22), S( -12,   2),
    S( -14,   0), S(  -7,  -4), S( -15,   7), S( -26,   2)},
   {S(  -8,  53), S(  51,  14), S( -20,  19), S(  -4,   7),
    S(  -2,   4), S(   5,  -4), S(  -2,   2), S( -12,   7)},
   {S(   3,  43), S(   4,  27), S( -23,  17), S( -11,   6),
    S(   3,   2), S(   9,   0), S(  10, -11), S(   5,   6)},
   {S(  -7,  27), S(   4,  23), S( -23,   7), S( -14,  -2),
    S(  -9,   0), S(  11, -12), S(   2,  -9), S( -13,   8)}},
  {{S(   0,   0), S( -18, -16), S( -15,  -2), S(  24, -21),
    S(  10,  -7), S(   6, -20), S(   5,   2), S(  16,  29)},
   {S(   0,   0), S( -17, -37), S(   2, -13), S(  44, -13),
    S(   3,  -3), S(  17, -25), S(  -6,  -5), S( -19,   0)},
   {S(   0,   0), S( -28, -52), S( -24, -10), S(  16, -12),
    S(   7,  -2), S(  -8, -13), S(  -9, -12), S( -11,   6)},
   {S(   0,   0), S(  -2, -19), S( -20, -18), S( -12,  -2),
    S(  -1,  -8), S(   0, -24), S(  73, -12), S(  15,  20)}},
};

/* Safety Evaluation Terms */

const int SafetyKnightWeight    = S(  46,  41);
const int SafetyBishopWeight    = S(  22,  35);
const int SafetyRookWeight      = S(  34,   8);
const int SafetyQueenWeight     = S(  28,   6);

const int SafetyAttackValue     = S(  43,  32);
const int SafetyWeakSquares     = S(  40,  39);
const int SafetyNoEnemyQueens   = S(-238,-259);
const int SafetySafeQueenCheck  = S(  91,  83);
const int SafetySafeRookCheck   = S(  88,  98);
const int SafetySafeBishopCheck = S(  58,  59);
const int SafetySafeKnightCheck = S( 110, 117);
const int SafetyAdjustment      = S( -76, -27);

/* Passed Pawn Evaluation Terms */

const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -36,   1), S( -48,  26), S( -72,  31),
    S(   9,  11), S(  82, -15), S( 158,  42), S(   0,   0)},
   {S(   0,   0), S( -28,  13), S( -45,  38), S( -65,  41),
    S(   1,  39), S( 106,  25), S( 190,  89), S(   0,   0)}},
  {{S(   0,   0), S( -24,  26), S( -48,  34), S( -67,  48),
    S(  11,  44), S( 104,  38), S( 272,  99), S(   0,   0)},
   {S(   0,   0), S( -28,  22), S( -42,  33), S( -62,  52),
    S(  12,  63), S( 104, 117), S( 157, 267), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(  -2,   2), S(   3,  -3), S(   7, -11),
    S(   6, -15), S( -10, -11), S( -16,  -3), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   4,  -2), S(   4,   0), S(   8,  10),
    S(   1,  24), S(   7,  33), S(  27,  31), S(   0,   0),
};

const int PassedSafePromotionPath = S( -22,  44);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -12, -32);
const int ThreatMinorAttackedByPawn  = S( -52, -58);
const int ThreatMinorAttackedByMinor = S( -22, -39);
const int ThreatMinorAttackedByMajor = S( -27, -47);
const int ThreatRookAttackedByLesser = S( -49, -23);
const int ThreatMinorAttackedByKing  = S( -17, -19);
const int ThreatRookAttackedByKing   = S( -18, -18);
const int ThreatQueenAttackedByOne   = S( -45, -24);
...
const int SpaceCenterControl = S(   3,  -4);

/* Closedness Evaluation Terms */

const int ClosednessKnightAdjustment[9] = {
    S( -10,  -6), S(  -6,   8), S(  -8,  15), S(  -4,  15),
    S(  -2,  19), S(   0,  14), S(   4,   5), S(  -8,  24),
    S(  -8,   9),
};

const int ClosednessRookAdjustment[9] = {
    S(  64,  -8), S(   4,  34), S(   3,  18), S(  -1,   3),
