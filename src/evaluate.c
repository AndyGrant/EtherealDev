...
const int KingSafety[100] = { // Taken from CPW / Stockfish
     ...
     495, 507, 520, 533, 546, 559, 572, 585, 598, 612,
     626, 640, 650, 650, 650, 650, 650, 650, 650, 650,
...
void evaluateKings(EvalInfo* ei, Board* board, int colour){
    ...
    if (ei->attackerCounts[!colour] >= 2){
        ...
        if (ei->attackCounts[!colour] >= 4)
            attackCounts *= 1.10;
        
        else if (ei->attackCounts[!colour] >= 3)
            attackCounts *= 1.05;
        
        if (ei->attackCounts[!colour] - 2 >= ei->attackCounts[colour])
            attackCounts *= 1.10;
