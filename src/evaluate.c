...
const int KnightBonusForPawns[PHASE_NB] = {   3,   1};
...
const int BishopPenaltyForPawns[PHASE_NB] = {  -2,  -1};
...
void evaluateKnights(EvalInfo* ei, Board* board, int colour){
    ...
    if (tempKnights){
        count = popcount(tempKnights) * (popcount(myPawns) - 4);
...
void evaluateBishops(EvalInfo* ei, Board* board, int colour){
    ...
    // Apply a bonus when there for number of pawns on the board
    if (tempBishops){
        count = popcount(tempBishops) * (popcount(myPawns) - 4);
