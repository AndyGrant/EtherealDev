...
const int KnightMobility[9] = {
    S( -39, -91), S( -18, -93), S(  -8, -36), S( -12, -18),
    S(   0,  -7), S(   4,   5), S(  11,   6), S(  23,   9),
    S(  35,  -2),
};
...
const int BishopMobility[14] = {
    S( -10,-130), S( -10, -85), S( -10, -50), S(  -6, -29),
    S(   0, -19), S(   7,  -5), S(   9,   4), S(  13,   9),
    S(  19,  17), S(  20,  17), S(  23,  19), S(  38,   8),
    S(  40,  13), S(  65, -19),
};
...
const int RookMobility[15] = {
    S(-139,-104), S( -45,-111), S( -18, -58), S(  -7, -19),
    S(  -9,   0), S(  -7,  14), S(  -2,  23), S(   1,  26),
    S(   8,  28), S(  10,  33), S(  14,  41), S(  18,  45),
    S(  18,  49), S(  24,  44), S(  69,  16),
};

/* Queen Evaluation Terms */

const int QueenMobility[28] = {
    S( -59,-262), S(-202,-385), S( -53,-200), S( -19,-190),
    S(  -9,-139), S(  -4, -88), S(  -3, -60), S(  -2, -34),
    S(   0, -23), S(  -1,  -1), S(   0,   9), S(   0,  21),
    S(   1,  23), S(   2,  32), S(   1,  37), S(   2,  40),
    S(   1,  45), S(  -1,  44), S(   0,  38), S(  -3,  32),
    S(   3,  11), S(  18,  -7), S(  19, -36), S(  21, -54),
    S(   5, -69), S(  20, -94), S( -56, -39), S( -27, -54),
};
...
int evaluateBishops(EvalInfo *ei, Board *board, int colour) {
    ...
    while (tempBishops) {
        ...
        count = popcount(ei->mobilityAreas[US] & attacks)
              - popcount(ei->pawnAttacks[THEM] & attacks);
        eval += BishopMobility[MAX(0, count)];
        if (TRACE) T.BishopMobility[MAX(0, count)][US]++;
...
int evaluateRooks(EvalInfo *ei, Board *board, int colour) {
    ...
    while (tempRooks) {
        ...
        count = popcount(ei->mobilityAreas[US] & attacks)
              - popcount(ei->pawnAttacks[THEM] & attacks);
        eval += RookMobility[MAX(0, count)];
        if (TRACE) T.RookMobility[MAX(0, count)][US]++;
...
int evaluateQueens(EvalInfo *ei, Board *board, int colour) {
    ...
    while (tempQueens) {
        ...
        count = popcount(ei->mobilityAreas[US] & attacks)
              - popcount(ei->pawnAttacks[THEM] & attacks);
        eval += QueenMobility[MAX(0, count)];
        if (TRACE) T.QueenMobility[MAX(0, count)][US]++;
