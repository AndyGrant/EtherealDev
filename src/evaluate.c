...
const int PawnValue   = S(  85, 148);
const int KnightValue = S( 430, 483);
const int BishopValue = S( 447, 521);
const int RookValue   = S( 636, 820);
const int QueenValue  = S(1317,1664);
...
const int PawnPSQT[SQUARE_NB] = {
    ...
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S( -13,   9), S(  -4,   1), S(   1,   5), S(   6,   2),
    S(   3,  11), S( -10,   4), S(  -9,   3), S( -16,   9),
    S( -21,   6), S( -17,   6), S(   0,  -6), S(  13, -15),
    S(   9, -10), S(  -4,  -5), S( -15,   6), S( -25,  13),
    S( -14,  17), S( -22,  18), S(   9, -12), S(  11, -25),
    S(   4, -23), S(   5, -11), S( -20,  16), S( -17,  20),
    S( -14,  19), S( -18,  11), S( -16,  -9), S(   5, -32),
    S(  -1, -25), S( -18, -10), S( -23,  12), S( -17,  22),
    S( -21,  52), S(  -9,  46), S(   1,  32), S(  19,  -9),
    S(  37,  -5), S(  -9,  32), S(  -6,  47), S( -23,  53),
    S( -35, -68), S( -68,  -7), S( -17, -21), S(  68, -22),
    S(  42, -16), S(  43, -14), S( -45,   6), S( -65, -52),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT[SQUARE_NB] = {
    S( -31, -38), S(  -6, -24), S( -20, -22), S( -15,   0),
    S( -10,   0), S( -21, -19), S(  -7, -19), S( -41, -30),
    S(   1,  -3), S( -10,   4), S(  -6, -19), S(   0,  -1),
    S(   1,   1), S(  -8, -16), S(  -7,  -2), S(  -5,   3),
    S(   8, -21), S(   9,  -5), S(   8,   2), S(  11,  20),
    S(  12,  20), S(   5,   2), S(   9,  -4), S(   4, -19),
    S(  16,  22), S(  17,  31), S(  23,  42), S(  27,  52),
    S(  23,  55), S(  23,  42), S(  17,  29), S(  12,  27),
    S(  12,  31), S(  23,  30), S(  38,  52), S(  29,  72),
    S(  25,  69), S(  38,  53), S(  21,  34), S(  13,  29),
    S( -25,  25), S(  -5,  38), S(  26,  58), S(  21,  61),
    S(  25,  57), S(  28,  57), S(  -3,  33), S( -22,  26),
    S(  13,  -3), S( -12,  19), S(  28,  -3), S(  36,  25),
    S(  40,  24), S(  40,  -7), S( -13,  17), S(   1,  -3),
    S(-168,  -5), S( -92,  12), S(-118,  41), S( -39,  17),
    S( -19,  19), S(-106,  48), S(-120,  24), S(-165, -17),
};

const int BishopPSQT[SQUARE_NB] = {
    S(   6, -20), S(   2,   2), S(  -1,   7), S(   2,   6),
    S(   3,  10), S(  -6,   0), S(   1,   2), S(   5, -25),
    S(  27, -15), S(   3, -31), S(  16,  -1), S(   9,   9),
    S(   9,   9), S(  13,  -2), S(  10, -31), S(  26, -29),
    S(  10,   4), S(  23,  10), S(  -4,  -2), S(  19,  20),
    S(  17,  21), S(  -5,  -5), S(  20,   4), S(  15,   9),
    S(   0,  13), S(  11,  18), S(  17,  34), S(  21,  34),
    S(  24,  36), S(  12,  31), S(  16,  18), S(  -1,  15),
    S( -19,  35), S(  14,  32), S(   1,  39), S(  21,  47),
    S(  12,  48), S(   6,  40), S(  13,  34), S( -16,  39),
    S( -12,  32), S( -10,  46), S(  -5,  24), S(   2,  41),
    S(   8,  40), S( -21,  35), S(  -6,  47), S( -10,  36),
    S( -60,  39), S( -49,  23), S( -13,  32), S( -35,  37),
    S( -34,  37), S( -14,  34), S( -69,  22), S( -57,  37),
    S( -66,  19), S( -66,  37), S(-124,  50), S(-108,  57),
    S(-113,  54), S( -98,  44), S( -33,  23), S( -75,  17),
};

const int RookPSQT[SQUARE_NB] = {
    S( -26,   1), S( -21,   5), S( -14,   6), S(  -5,  -3),
    S(  -5,  -3), S( -10,   5), S( -13,  -1), S( -22, -14),
    S( -71,   5), S( -24, -12), S( -17,  -8), S( -10, -12),
    S(  -8, -14), S( -15, -17), S( -14, -20), S( -78,   2),
    S( -39,   1), S( -16,  15), S( -25,   8), S( -13,   1),
    S( -11,   2), S( -25,   6), S(  -3,   9), S( -38,  -2),
    S( -31,  21), S( -21,  36), S( -21,  37), S(  -5,  26),
    S(  -7,  27), S( -18,  33), S( -12,  32), S( -29,  22),
    S( -21,  46), S(   5,  36), S(  17,  39), S(  37,  30),
    S(  34,  32), S(  12,  33), S(  19,  27), S( -13,  43),
    S( -33,  62), S(  18,  42), S(  -1,  58), S(  34,  37),
    S(  30,  36), S(   4,  53), S(  35,  30), S( -25,  59),
    S( -18,  43), S( -25,  51), S(  -1,  41), S(  14,  43),
    S(  14,  41), S(  -2,  36), S( -24,  51), S( -11,  40),
    S(  31,  63), S(  23,  70), S(  -3,  80), S(   8,  73),
    S(   9,  75), S(   0,  74), S(  35,  64), S(  38,  61),
};

const int QueenPSQT[SQUARE_NB] = {
    S(  21, -34), S(   4, -26), S(  10, -34), S(  18, -15),
    S(  19, -17), S(  14, -47), S(   9, -28), S(  23, -44),
    S(   7, -15), S(  16, -22), S(  23, -42), S(  14,   3),
    S(  18,   1), S(  23, -50), S(  19, -29), S(   3, -18),
    S(   6,   0), S(  22,   8), S(   5,  37), S(   1,  35),
    S(   2,  35), S(   5,  38), S(  24,   9), S(  13, -16),
    S(  10,  18), S(  12,  47), S(  -6,  61), S( -19, 112),
    S( -18, 109), S(  -5,  59), S(  18,  49), S(   7,  34),
    S(  -9,  44), S(  -8,  81), S( -19,  68), S( -33, 124),
    S( -33, 131), S( -24,  82), S(  -9,  98), S( -12,  69),
    S( -29,  57), S( -23,  51), S( -33,  69), S( -19,  72),
    S( -18,  73), S( -20,  65), S( -20,  67), S( -30,  78),
    S( -16,  62), S( -73, 113), S( -19,  67), S( -53, 117),
    S( -55, 123), S( -15,  61), S( -70, 118), S( -11,  75),
    S(   7,  44), S(  19,  49), S(  -1,  80), S(   2,  79),
    S(  -4,  90), S(  12,  66), S(  18,  81), S(  21,  57),
};

const int KingPSQT[SQUARE_NB] = {
    S(  88, -78), S(  67, -48), S(   3,  -5), S( -11, -28),
    S( -11, -29), S( -10,   0), S(  57, -51), S(  80, -85),
    S(  35,   9), S( -28,   1), S( -42,  19), S( -91,  32),
    S( -65,  28), S( -67,  32), S( -27,   0), S(  29,  -2),
    S( -48, -17), S( -18, -17), S(  32,   8), S(   3,  39),
    S(  22,  35), S(  33,  12), S( -12, -16), S( -36, -11),
    S( -50, -47), S( 104, -41), S(  77,  14), S( -19,  49),
    S(  17,  45), S(  84,  13), S( 122, -33), S( -56, -41),
    S(  -8, -11), S(  99, -44), S(  44,  12), S( -47,  16),
    S( -25,  17), S(  38,  16), S(  86, -35), S( -50, -11),
    S(  23, -42), S( 127, -29), S( 111,  -9), S( -21, -30),
    S( -39, -28), S(  80, -24), S( 103, -29), S( -18, -49),
    S(  -1,-106), S(  27, -24), S(   6, -26), S(  -4, -49),
    S( -58, -46), S(  13, -35), S(  21, -28), S( -16,-126),
    S( -17,-156), S(  48, -98), S( -23, -77), S( -19, -34),
    S( -52, -58), S( -42, -66), S(  53, -96), S( -59,-135),
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -12, -19), S( -17,  18), S( -19,  30),
    S( -22,  63), S(  21,  63), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -12,  21), S(  -7,  28), S(   1,  56),
    S(  23, 119), S(  50,  80), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated[FILE_NB] = {
    S( -14,  -9), S(  -1, -15), S(   0, -16), S(   3, -18),
    S(   6, -19), S(   3, -15), S(  -4, -15), S(  -5, -17),
};

const int PawnStacked[2][FILE_NB] = {
   {S(  10, -31), S(  -2, -30), S(   0, -23), S(   0, -21),
    S(   3, -21), S(   5, -27), S(   4, -33), S(   8, -32)},
   {S(   4, -17), S(   0, -17), S(  -6, -10), S(  -7, -10),
    S(  -5,  -9), S(  -2, -10), S(   0, -14), S(   0, -18)},
};

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   0,  -7), S(   7,  -7), S(   6, -19),
    S(  -3, -30), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S(  -9, -32), S(  -5, -30), S(   3, -30),
    S(  30, -40), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -1, -11), S(  13,  -4), S(   0,  -1), S(   6,   8),
    S(  14,   0), S(  20,  -5), S(  19,   3), S(  17,   9),
    S(   7,  -1), S(  20,   1), S(   6,   4), S(  14,  11),
    S(   8,  15), S(  21,  18), S(  32,  23), S(  26,  18),
    S(  47,  37), S(  37,  65), S(  60,  74), S(  65,  87),
    S( 108,  33), S( 213,  42), S( 215,  69), S( 232,  61),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightInSiberia[4] = {
    S(  -8,  -5), S( -11, -20), S( -26, -20), S( -46, -19),
};

const int KnightMobility[9] = {
    S(-105,-140), S( -45,-116), S( -22, -36), S(  -7,   5),
    S(   7,  17), S(  12,  37), S(  20,  42), S(  31,  41),
    S(  45,  20),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  22,  91);

const int BishopRammedPawns = S(  -8, -18);
...
const int BishopBehindPawn = S(   4,  25);

const int BishopLongDiagonal = S(  27,  21);

const int BishopMobility[14] = {
    S(-100,-188), S( -45,-125), S( -14, -53), S(  -3, -13),
    S(   8,   4), S(  15,  24), S(  18,  39), S(  20,  43),
    S(  20,  53), S(  28,  53), S(  27,  53), S(  53,  36),
    S(  56,  51), S(  84,   4),
};
...
const int RookFile[2] = { S(  10,   9), S(  34,   9) };

const int RookOnSeventh = S(  -1,  44);

const int RookMobility[15] = {
    S(-128,-150), S( -56,-129), S( -25, -86), S( -11, -28),
    S( -10,   3), S( -12,  29), S( -10,  44), S(  -4,  49),
    S(   5,  55), S(   9,  58), S(  12,  68), S(  20,  71),
    S(  20,  76), S(  39,  62), S(  99,  15),
};
...
const int QueenMobility[28] = {
    S(-113,-273), S(-255,-402), S(-129,-229), S( -46,-238),
    S( -20,-174), S(  -8, -86), S(   0, -34), S(   2,   0),
    S(   9,   9), S(  10,  33), S(  16,  39), S(  17,  58),
    S(  21,  49), S(  24,  60), S(  23,  62), S(  21,  68),
    S(  24,  66), S(  16,  69), S(  13,  67), S(  18,  52),
    S(  25,  34), S(  39,  12), S(  35, -10), S(  29, -27),
    S(  10, -42), S(   7, -77), S( -41, -29), S( -22, -49),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -38,  -4), S( -18,   2), S(  -1,   6), S(  12,   8),
    S(  21,   8), S(  33,   0), S(  39, -14), S(  10,  -5),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  36,  47), S(  21,  32), S(  14,  15), S(  -9, -24),
    S(  -6, -65), S(  -4, -78), S( -15, -84), S( -12, -75),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -4,  -6), S(  16, -33), S(  26,  -4), S(  24,   8),
    S(   4,   1), S( -11,   4), S( -17, -33), S( -58,  25)},
   {S(  13,  -7), S(   2, -16), S(  -6,  -3), S(   4,  -5),
    S( -10,   7), S( -52,  72), S(  81,  84), S( -19,   2)},
   {S(  41,  -4), S(   5,  -7), S( -35,   5), S( -18, -15),
    S(  -9,  -6), S( -26,  12), S(  12,  73), S( -15,   0)},
   {S(  19,  12), S(  25, -18), S(  -1, -14), S(   9, -21),
    S(  22, -34), S( -47,   9), S(-139,  49), S(  -4,  -5)},
   {S( -10,  16), S(   1,  -3), S( -46,   7), S( -29,  11),
    S( -25,  -1), S( -36,  -5), S(  40, -25), S( -11,   4)},
   {S(  54, -15), S(  15, -13), S( -25,   6), S( -11, -21),
    S(  11, -35), S(  35, -21), S(  48, -39), S( -21,   1)},
   {S(  42, -17), S(   1, -24), S( -33,   0), S( -25,  -7),
    S( -32,   3), S( -19,  30), S(   4,  49), S( -14,   2)},
   {S(  11, -21), S(   4, -25), S(   9,   3), S(   1,  17),
    S( -10,  25), S(  -9,  46), S(-184,  82), S( -15,  16)}},
  {{S(   0,   0), S( -16, -40), S(   8, -30), S( -51,  14),
    S( -37,   5), S(  -8,  50), S(-168,  -3), S( -58,  20)},
   {S(   0,   0), S(  17, -19), S(   9, -12), S( -11,  -7),
    S(  -1, -26), S(  26,  73), S(-186,   4), S( -31,  12)},
   {S(   0,   0), S(  19,  -9), S(   1, -11), S(   8, -27),
    S(  29,  -6), S( -91,  57), S( -88, -74), S(  -7,   2)},
   {S(   0,   0), S(  -1,   3), S(  -6,  -6), S( -36,  11),
    S( -48,  14), S( -98,  33), S(  -8, -46), S( -33,  -5)},
   {S(   0,   0), S(  12,  -2), S(  17, -15), S(  17, -15),
    S(  -5, -14), S( -35,   5), S(-101, -52), S( -17,  -1)},
   {S(   0,   0), S(  -8,  -4), S( -23,   3), S( -16,  -5),
    S(  25, -22), S( -27,  10), S(  52,  39), S( -12,  -2)},
   {S(   0,   0), S(  33, -22), S(  18, -15), S( -10,  -5),
    S( -30,  14), S(  -6,  22), S( -50, -39), S( -27,  18)},
   {S(   0,   0), S(  16, -58), S(  17, -33), S( -19,  -7),
    S( -32,  25), S( -10,  24), S(-225, -49), S( -28,   4)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S(  -5,  39), S( 146,  -3), S( -12,  27), S(  -5,   0),
    S( -12,  -3), S(  -9,  -7), S( -20,   8), S( -29,  -3)},
   {S( -14,  62), S(  66,  18), S(  -7,  21), S(   9,  11),
    S(   3,   9), S(   5,  -2), S(  -6,   2), S( -16,   9)},
   {S(   3,  51), S(  15,  32), S( -16,  20), S( -13,  10),
    S(  -1,   6), S(   7,   3), S(   7,  -7), S(   7,   9)},
   {S(   0,  26), S(  15,  25), S( -30,  10), S( -22,   1),
    S( -15,   4), S(  13, -10), S(   2,  -5), S( -20,   9)}},
  {{S(   0,   0), S( -18, -16), S( -18,  -4), S(  28, -26),
    S(  10,  -7), S(  15, -25), S(  -5,   9), S(   8,  30)},
   {S(   0,   0), S( -15, -42), S(  -1, -19), S(  54, -19),
    S(  16,  -6), S(  20, -29), S( -11, -17), S( -34,   6)},
   {S(   0,   0), S( -34, -62), S( -15, -15), S(   9,  -7),
    S(   6,  -2), S(  -2, -18), S(  -3, -20), S(  -3,   2)},
   {S(   0,   0), S(  -1, -26), S( -28, -20), S( -21,   4),
    S( -10,  -6), S(   8, -36), S(  67, -29), S(  11,  25)}},
};

/* Safety Evaluation Terms */

const int SafetyKnightWeight    = S(  58,  44);
const int SafetyBishopWeight    = S(  31,  38);
const int SafetyRookWeight      = S(  36,   9);
const int SafetyQueenWeight     = S(  36,  10);

const int SafetyAttackValue     = S(  45,  44);
const int SafetyWeakSquares     = S(  42,  45);
const int SafetyNoEnemyQueens   = S(-241,-260);
const int SafetySafeQueenCheck  = S(  89,  92);
const int SafetySafeRookCheck   = S(  86,  99);
const int SafetySafeBishopCheck = S(  61,  63);
const int SafetySafeKnightCheck = S( 109, 115);
const int SafetyAdjustment      = S( -74, -21);

const int SafetyShelter[2][RANK_NB] = {
   {S(   2,   8), S(  -4,  21), S(  -3,   9), S(   2,   9),
    S(   7,   3), S(   5,   0), S(   2,   0), S(   4, -17)},
   {S(   0,   0), S(  -3,  16), S(  -3,  13), S(   4,   7),
    S(   1,   2), S(  -1,   0), S(  -2,   0), S(   3, -14)},
};

const int SafetyStorm[2][RANK_NB] = {
   {S(  -1,  -1), S(  -7,   5), S(   1,   6), S(   3,   2),
    S(   3,  12), S(  -3,  26), S(  -4,  17), S(   4, -19)},
   {S(   0,   0), S(   1,   0), S(  -3,   5), S(   0,   0),
    S(   2,   8), S(   0,   2), S(   7,   0), S(   2,   0)},
};

/* Passed Pawn Evaluation Terms */

const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -38,  -6), S( -42,  24), S( -61,  27),
    S(   9,  18), S(  96,  -8), S( 161,  39), S(   0,   0)},
   {S(   0,   0), S( -27,  11), S( -39,  41), S( -55,  44),
    S(  -1,  55), S( 113,  54), S( 191,  96), S(   0,   0)}},
  {{S(   0,   0), S( -27,  29), S( -45,  35), S( -59,  54),
    S(   9,  65), S( 106,  74), S( 259, 120), S(   0,   0)},
   {S(   0,   0), S( -28,  23), S( -39,  34), S( -54,  60),
    S(   9,  90), S(  93, 175), S( 126, 309), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(  -3,   2), S(   1,  -4), S(   5, -14),
    S(   5, -19), S( -10, -17), S( -10,  -5), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   5,   0), S(   7,   1), S(   9,  12),
    S(   0,  26), S(   2,  38), S(  17,  38), S(   0,   0),
};

const int PassedSafePromotionPath = S( -50,  61);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -11, -40);
const int ThreatMinorAttackedByPawn  = S( -55, -85);
const int ThreatMinorAttackedByMinor = S( -25, -46);
const int ThreatMinorAttackedByMajor = S( -31, -57);
...
const int ThreatQueenAttackedByOne   = S( -51,  -7);
const int ThreatOverloadedPieces     = S(  -7, -18);
...
const int ClosednessKnightAdjustment[9] = {
    S(  -5,  14), S(  -5,  33), S(  -7,  41), S(  -4,  41),
    S(  -1,  48), S(   1,  40), S(   3,  36), S(  -9,  54),
    S(  -6,  32),
};

const int ClosednessRookAdjustment[9] = {
    S(  46,  53), S(  -2,  88), S(   6,  66), S(  -2,  53),
    S(  -4,  47), S(   0,  28), S(  -3,  15), S( -15,  14),
    S( -32,  -9),
};
...
const int ComplexityPawnFlanks  = S(   0,  99);
const int ComplexityPawnEndgame = S(   0,  86);
const int ComplexityAdjustment  = S(   0,-186);
