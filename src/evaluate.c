...
const int SafetyBishopWeight    = S(  23,  36);
const int SafetyRookWeight      = S(  35,   7);
const int SafetyQueenWeight     = S(  30,   7);

const int SafetyAttackValue     = S(  44,  32);
const int SafetyWeakSquares     = S(  40,  39);
const int SafetyNoEnemyQueens   = S(-240,-260);
const int SafetySafeQueenCheck  = S(  91,  82);
const int SafetySafeRookCheck   = S(  88,  98);
const int SafetySafeBishopCheck = S(  58,  59);
const int SafetySafeKnightCheck = S( 110, 116);
const int SafetyAdjustment      = S( -78, -26);
...
int evaluateKings(EvalInfo *ei, Board *board, int colour) {
    ...
    if (ei->kingAttackersCount[US] > 1 - popcount(enemyQueens)) {
        ...
        eval += MakeScore(-mg * MAX(0, mg) / 720, -eg / 20);
