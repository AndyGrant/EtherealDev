...
const int PawnPSQT32[32] = {
    ...
    S( -16,   9), S(  -1,   3), S( -12,   7), S(  -6,  -2),
    S( -24,   5), S( -17,   4), S( -13,  -4), S(  -5, -10),
    S( -19,  12), S( -13,  11), S(   4, -10), S(   8, -21),
    S( -11,  15), S(  -6,  10), S( -10,  -4), S(  -5, -20),
    S(  -8,  29), S(  -4,  25), S(   1,  15), S(  28, -12),
    S( -21, -60), S( -60, -29), S(   2, -43), S(  45, -58),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -48, -31), S(   2, -28), S( -12, -29), S(   1, -16),
...
const int RookPSQT32[32] = {
    S(  -9, -29), S( -12, -19), S(  -2, -21), S(   2, -28),
    S( -52, -14), S( -10, -33), S(  -8, -31), S(   3, -36),
...
const int QueenPSQT32[32] = {
    ...
    S(  14, -42), S(  27, -58), S(  29, -71), S(  19, -26),
...
const int KnightMobility[9] = {
    ...
    S(   9,  -6), S(  12,  10), S(  18,  13), S(  27,  12),
    S(  41,  -3),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  19,  67);
...
const int BishopBehindPawn = S(   5,  18);

const int BishopLongDiagonal = S(  17,  11);

const int BishopMobility[14] = {
    S( -69,-149), S( -32, -99), S(  -7, -54), S(   1, -29),
    S(   9, -17), S(  14,  -4), S(  15,   7), S(  19,  12),
    S(  20,  18), S(  24,  19), S(  24,  20), S(  46,   8),
    S(  47,  19), S(  75, -17),
};
...
const int RookMobility[15] = {
    S(-159,-116), S( -57,-115), S( -10, -65), S(  -7, -21),
    S(  -9,   0), S( -11,  15), S( -10,  27), S(  -4,  30),
    S(   3,  33), S(   8,  37), S(  12,  44), S(  18,  47),
    S(  20,  51), S(  37,  40), S( 106,  -7),
};
...
const int KingDefenders[12] = {
    S( -27,  -3), S(  -7,  -1), S(   2,   6), S(  11,   8),
    S(  18,   6), S(  26,   1), S(  29,  -4), S(  12,  -1),
...
const int KingShelter[2][FILE_NB][RANK_NB] = {
  ...
  {{S(   0,   0), S( -13, -26), S(   7, -23), S( -39,  20),
   ...
   {S(   0,   0), S(  24, -14), S(  11, -14), S( -11,  -8),
    S( -28,  14), S(  -8,  19), S( -57, -50), S( -33,  10)},
   {S(   0,   0), S(  12, -41), S(  20, -32), S( -19,  -7),
    S( -20,  19), S(  -8,  21), S(-228, -56), S( -25,   2)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -13,  32), S( 112,  -6), S( -23,  24), S( -14,   3),
    S( -13,   0), S(  -7,  -5), S( -16,   4), S( -24,   2)},
   {S(  -8,  53), S(  51,  14), S( -20,  21), S(  -4,   7),
    S(  -2,   3), S(   4,  -5), S(  -3,   3), S( -11,   7)},
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -36,   1), S( -48,  26), S( -73,  31),
   ...
   {S(   0,   0), S( -28,  13), S( -46,  36), S( -66,  38),
    S(   0,  36), S( 106,  24), S( 189,  88), S(   0,   0)}},
  {{S(   0,   0), S( -24,  26), S( -48,  33), S( -68,  44),
    S(   9,  42), S( 102,  35), S( 271,  98), S(   0,   0)},
   {S(   0,   0), S( -29,  22), S( -43,  31), S( -62,  50),
    S(  11,  59), S( 104, 116), S( 157, 265), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   0,   2), S(   3,  -2), S(   5,  -9),
    S(   3, -10), S( -11,  -7), S( -17,   3), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   2,   0), S(   3,   1), S(   8,  12),
    S(   1,  26), S(   7,  33), S(  28,  30), S(   0,   0),
};
...
const int ThreatWeakPawn             = S(  -7, -28);
const int ThreatMinorAttackedByPawn  = S( -48, -56);
const int ThreatMinorAttackedByMinor = S( -20, -37);
...
const int ComplexityPawnFlanks  = S(   0,  70);
