...
int evaluateBoard(Thread *thread, Board *board) {
    ...
    EvalInfo ei;
    int phase, factor, eval, pkeval, pksafetyw, pksafetyb, hashed;
    ...
    if (!TRACE && ei.pkentry == NULL) {
        pksafetyw = ei.pksafety[WHITE];
        pksafetyb = ei.pksafety[BLACK];
        storeCachedPawnKingEval(thread, board, ei.passedPawns, pkeval, pksafetyw, pksafetyb);
    }
...
int evaluatePieces(EvalInfo *ei, Board *board) {
    ...
    eval += evaluateKingsPawns(ei, board, WHITE)   - evaluateKingsPawns(ei, board, BLACK);
