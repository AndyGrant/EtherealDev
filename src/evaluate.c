...
const int KingShelter[2][FILE_NB/2][RANK_NB] = {
  {{S(   0, -14), S(   6, -23), S(  10,   0), S(   0,  10),
    S( -12,  15), S(  -3,  27), S(  -4,   0), S( -18,  15)},
   {S(  38, -10), S(  10, -15), S( -19,   2), S( -18,  -3),
    S( -27,   2), S(   0,  12), S(   3,   8), S( -12,   7)},
   {S(  46,  -1), S(  17,   1), S( -21,  14), S( -11, -11),
    S(   2, -20), S(  13,  -2), S(   4,   4), S( -25,  13)},
   {S(  -2,  17), S(  26,  -6), S(   7,  -6), S(  14,  -6),
    S(  24, -23), S( -30,  -7), S(  -2,   0), S(   6,   1)}},
  {{S(   0,   0), S(  11, -31), S(  16, -18), S( -21,   5),
    S(  -8,  14), S(   3,  10), S(  -1,  -1), S( -25,  20)},
   {S(   0,   0), S(  28, -10), S(  17,  -7), S(  -9,  -2),
    S( -23,  15), S(   4,  16), S(  -6,  -3), S( -31,  22)},
   {S(   0,   0), S(  18,  -3), S( -10,   4), S(   8, -11),
    S(   6,   1), S(  -1,   3), S(   1,   2), S( -23,   7)},
   {S(   0,   0), S(  13,   3), S(  16,  -9), S(  15,  -9),
    S(  14, -14), S( -10,   0), S(  -1,  -1), S(  -3,  -1)}},
};
...
int evaluateKings(EvalInfo *ei, Board *board, int colour) {
    ...
    for (int file = MAX(0, fileOf(kingSq) - 1); file <= MIN(FILE_NB - 1, fileOf(kingSq) + 1); file++) {
        ...
        ei->pkeval[US] += KingShelter[file == fileOf(kingSq)][mirrorFile(file)][ourDist];
        if (TRACE) T.KingShelter[file == fileOf(kingSq)][mirrorFile(file)][ourDist][US]++;
