...
const int KnightValue[PHASE_NB] = { 325, 309};

const int BishopValue[PHASE_NB] = { 339, 328};

const int RookValue[PHASE_NB]   = { 505, 538};

const int QueenValue[PHASE_NB]  = { 998, 998};

const int KingValue[PHASE_NB]   = { 108, 108};

const int PawnIsolated[PHASE_NB] = {  -8, -10};

const int PawnStacked[PHASE_NB] = { -23, -24};

const int PawnBackwards[2][PHASE_NB] = { {   2,   6}, { -14, -15} };
...
const int PawnConnected32[32][PHASE_NB] = {
    ...
    {   3,   0}, {   9,   1}, {  10,   1}, {  14,   7},
    {  16,   1}, {  15,   9}, {  23,   7}, {  23,   5},
    {  23, -18}, {  37,  -5}, {  63,  19}, {  76,  26},
    { 151, -73}, { 215, -62}, { 121, -78}, { 276, -16},
    {   0,   0}, {   0,   0}, {   0,   0}, {   0,   0},
};

const int KnightAttackedByPawn[PHASE_NB] = { -38, -18};

const int KnightOutpost[2][PHASE_NB] = { {  12,  -9}, {  29,  16} };
...
const int BishopWings[PHASE_NB] = {   1,  -7};

const int BishopPair[PHASE_NB] = {  26,  33};

const int BishopAttackedByPawn[PHASE_NB] = { -43, -27};

const int BishopOutpost[2][PHASE_NB] = { {  13,  -2}, {  26,   4} };

const int BishopMobility[14][PHASE_NB] = {
    { -75, -50}, { -35, -40}, { -14, -26}, {  -9, -16},
    {   0,  -9}, {   5,  -1}, {   8,   4}, {  10,   2},
    {  13,   6}, {  16,   2}, {  16,   4}, {   7,  -9},
    {  26,   2}, { -79, -57},
};

const int RookFile[2][PHASE_NB] = { {  16,  -2}, {  23,   7} };

const int RookOnSeventh[PHASE_NB] = {  13,  21};

const int RookMobility[15][PHASE_NB] = {
    {-153,-103}, { -45, -71}, { -14, -40}, {  -6, -24},
    {  -6, -16}, {  -7,  -9}, {  -6,  -7}, {  -1,  -3},
    {   4,   1}, {  11,   7}, {  15,   7}, {  20,  10},
    {  19,  10}, {  18,   1}, {   7,  -6},
};

const int QueenChecked[PHASE_NB] = { -34,  -7};

const int QueenCheckedByPawn[PHASE_NB] = { -45, -12};

const int QueenMobility[28][PHASE_NB] = {
    {-109,-136}, { -99, -98}, { -62, -59}, { -28, -58},
    { -17, -55}, { -13, -31}, {  -6, -28}, {  -7, -17},
    {  -4, -14}, {  -2,  -9}, {   0,  -8}, {   3,  -2},
    {   4,   0}, {   7,   3}, {   7,   6}, {   9,   8},
    {   9,  10}, {  10,  10}, {  12,  11}, {  10,   8},
    {  12,   6}, {  15,   5}, {  13,   7}, {  14,   4},
    {  17,   5}, {  13,   3}, {  15,   6}, {  -5,  -6},
};
...
const int KingDefenders[12][PHASE_NB] = {
    { -21,  10}, { -17,  -6}, {  -4,  -3}, {   2,   1},
    {  10,   4}, {  18,  11}, {  19,   6}, {  23,  22},
    {   0,   0}, {   0,   0}, {   0,   0}, {   0,   0},
};

const int KingShelter[2][FILE_NB][RANK_NB][PHASE_NB] = {
  {{{  -6,  -1}, {  12, -11}, {  19,  -5}, {   8,  -9}, {   8,  -3}, { -10,  -3}, {  41,  -2}, { -20,   5}},
   {{   0,   5}, {  20,  -1}, {  24,  -2}, {   0,  -4}, {  -2,  -9}, { -36,  33}, { -89,   2}, { -17,   0}},
   {{   0,   4}, {   5,  -3}, { -15,   0}, {  -6,  -7}, { -12,  -6}, { -10, -20}, {   2,  15}, { -10,   1}},
   {{  -9,  12}, {   4,  -2}, {  -9,  -4}, {   9,  -2}, {  11, -12}, { -38, -12}, { -11,  24}, {  -2,   2}},
   {{ -13,   7}, { -11,   4}, { -18,   1}, { -12,  -7}, { -15, -18}, { -44, -22}, { -14, -18}, { -11,   3}},
   {{   5,   5}, {  15,   0}, { -12,  -3}, { -12, -12}, {   7, -17}, {  -3, -13}, { -26,   9}, { -11,   1}},
   {{   3,   1}, {   6,  -5}, { -10,  -3}, { -16,  -6}, { -25, -14}, { -39,   8}, {  53,  26}, { -12,   4}},
   {{ -12,  -4}, {   2,  -7}, {   7,   0}, {   3,   0}, { -15,  -1}, { -11,  25}, { -21,  48}, { -10,   5}}},
  {{{   0,   0}, {  -3, -12}, {   8,  -7}, { -11,   4}, {   5,  38}, { -11,  55}, {  32,  15}, { -24,   1}},
   {{   0,   0}, {  19,  -2}, {   9,  -6}, { -17,   0}, {   0,  -5}, { -32,   2}, { -60,  42}, { -25,   1}},
   {{   0,   0}, {  24,   2}, {   2,  -3}, {  18,  -8}, {  18,   0}, { -42, -15}, { -61,  -7}, {  -8,   0}},
   {{   0,   0}, { -12,   3}, { -17,  16}, { -19,  -4}, { -26,  -6}, { -28, -11}, { -39, -13}, { -18,   0}},
   {{   0,   0}, {  -1,   6}, {   2,   0}, {   4,  -8}, {   6, -17}, { -35, -18}, { -56,  38}, {  -6,   1}},
   {{   0,   0}, {   0,   5}, { -12,   1}, { -13,  -4}, {  -5, -14}, { -64,   0}, { -41,  79}, { -15,   3}},
   {{   0,   0}, {  13,  -2}, {   1,  -3}, { -14,  -7}, { -23, -14}, { -15,  24}, { -12, -30}, { -28,   2}},
   {{   0,   0}, {  10, -10}, {  14,  -6}, { -15,  -5}, {  -8,  17}, {  -2,   7}, { -66,   0}, { -19,   0}}},
};

const int PassedPawn[2][2][RANK_NB][PHASE_NB] = {
  {{{   0,   0}, { -34, -18}, { -23, -19}, { -17, -25}, {  21, -24}, {  21, -32}, {  12, -22}, {   0,   0}},
   {{   0,   0}, { -10,  -6}, { -11,  -6}, {  -6,  -1}, {  22,  -6}, {  40,  -5}, {  53,   0}, {   0,   0}}},
  {{{   0,   0}, { -12,  -8}, { -16, -14}, {  -3, -10}, {  22,  -2}, {  47,   0}, {  86, -18}, {   0,   0}},
   {{   0,   0}, {   0,  -4}, {  -4,  -4}, {   4,  14}, {  32,  41}, {  87,  75}, { 141, 100}, {   0,   0}}},
};
...
void evaluateKings(EvalInfo* ei, Board* board, int colour){
    ...
    if (ei->attackerCounts[!colour] >= 2){
        ...
        ei->midgame[colour] -= (int)(1.56 * KingSafety[attackCounts]);
    }
