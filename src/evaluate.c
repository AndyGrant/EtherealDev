...
const int PawnValue   = S(  96, 129);
const int KnightValue = S( 444, 414);
const int BishopValue = S( 464, 433);
const int RookValue   = S( 643, 697);
const int QueenValue  = S(1300,1346);
...
const int PawnPSQT32[32] = {
    ...
    S( -14,   6), S(   1,   1), S(  -8,   3), S(  -7,   1),
    S( -22,   5), S( -14,   3), S( -10,  -7), S(  -3, -13),
    S( -16,  11), S( -12,   9), S(   5, -12), S(   6, -26),
    S( -12,  15), S(  -8,   8), S( -15,  -7), S(  -9, -23),
    S(  -9,  35), S(  -7,  29), S(  -2,  17), S(  22,  -9),
    S( -21, -61), S( -57, -26), S(   2, -41), S(  45, -56),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -48, -31), S(  -2, -30), S( -12, -28), S(   2, -15),
    S(   2, -18), S(   1, -13), S(   4, -30), S(   9, -16),
    S(  13, -30), S(  18, -20), S(  14, -16), S(  22,  -2),
    S(  20,   3), S(  25,   7), S(  30,  18), S(  31,  26),
    S(  25,  13), S(  30,   8), S(  42,  27), S(  33,  40),
    S( -18,  11), S(   0,  13), S(  30,  30), S(  29,  29),
    S(  13, -13), S(  -4,  -2), S(  32, -21), S(  40,   2),
    S(-178, -25), S( -85,  -4), S(-111,  19), S( -33,  -2),
};

const int BishopPSQT32[32] = {
    S(  12, -27), S(  12, -16), S(  -3, -14), S(   7, -11),
    S(  26, -37), S(  13, -38), S(  20, -22), S(  11, -11),
    S(  17, -13), S(  25, -14), S(   3, -18), S(  22,  -3),
    S(  11,  -8), S(  19,  -4), S(  17,   6), S(  25,   8),
    S( -10,  14), S(  19,   7), S(   6,  14), S(  16,  19),
    S(   2,   8), S(  -6,  18), S(  -1,   6), S(  11,  14),
    S( -45,  15), S( -46,   5), S(  -7,   8), S( -23,  13),
    S( -57,  -5), S( -45,   6), S( -84,  17), S( -90,  28),
};

const int RookPSQT32[32] = {
    S( -13, -29), S( -12, -22), S(  -4, -21), S(   3, -27),
    S( -51, -15), S(  -9, -33), S(  -9, -32), S(  -1, -35),
    S( -27, -17), S(  -5, -13), S( -16, -16), S(  -1, -24),
    S( -18,  -3), S( -11,   7), S(  -8,   4), S(   4,  -3),
    S(  -6,  11), S(  11,   7), S(  21,   6), S(  39,   0),
    S( -17,  22), S(  29,   5), S(   8,  21), S(  38,   3),
    S(   0,   7), S( -11,  14), S(  12,   5), S(  26,   6),
    S(  35,  22), S(  28,  26), S(   1,  35), S(   9,  30),
};

const int QueenPSQT32[32] = {
    S(  31, -66), S(   6, -45), S(  13, -59), S(  21, -44),
    S(  14, -42), S(  27, -57), S(  28, -70), S(  18, -28),
    S(  10, -30), S(  25, -20), S(   8,   5), S(   6,   0),
    S(  12,  -8), S(  18,  11), S(   0,  23), S( -14,  65),
    S(  -4,  15), S(  -6,  46), S( -12,  24), S( -28,  77),
    S( -25,  30), S( -17,  19), S( -23,  25), S( -11,  27),
    S(  -4,  25), S( -62,  67), S(  -5,  12), S( -36,  54),
    S(  -9,  20), S(  21,   9), S(  11,  12), S(  -2,  24),
};

const int KingPSQT32[32] = {
    S(  51, -78), S(  43, -51), S( -16, -12), S( -26, -29),
    S(  27, -19), S( -17, -14), S( -45,   9), S( -69,  12),
    S(  -7, -24), S(   1, -23), S(  11,   0), S( -17,  18),
    S(  -1, -31), S(  93, -34), S(  46,   7), S(  -5,  29),
    S(  10, -12), S( 108, -27), S(  56,  17), S(  12,  23),
    S(  50, -25), S( 135, -16), S( 105,   8), S(  43,   3),
...
const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -27, -10), S( -11,   9), S( -19,  32),
    S(  -2,  58), S(  36,  63), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -17,  18), S(  -4,  22), S(   2,  48),
    S(  23,  90), S(  38,  62), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -6, -12);

const int PawnStacked[2] = { S(  -2, -22), S(  -5, -10) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   2,  -4), S(  10,  -6), S(   9, -13),
    S(   5, -14), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -15, -26), S(  -6, -25), S(   3, -25),
    S(  30, -28), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -5,  -8), S(  11,  -2), S(   1,  -1), S(   9,  12),
    S(  12,   0), S(  24,  -5), S(  23,   3), S(  26,  10),
    S(   8,  -2), S(  23,  -1), S(   9,   6), S(  17,  15),
    S(   8,  10), S(  20,  15), S(  28,  19), S(  31,  17),
    S(  52,  19), S(  40,  46), S(  62,  56), S(  75,  64),
    S( 110,  -9), S( 202,  -4), S( 226,  25), S( 235,  39),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightInSiberia[4] = {
    S(  -8,  -3), S( -10, -12), S( -21,  -9), S( -27,  -7),
};

const int KnightMobility[9] = {
    S( -78,-106), S( -35,-104), S( -16, -43), S(  -4, -14),
    S(   8,  -5), S(  11,  12), S(  19,  15), S(  28,  14),
    S(  42,  -4),
};
...
const int BishopLongDiagonal = S(  20,  13);

const int BishopMobility[14] = {
    S( -69,-149), S( -32,-101), S( -11, -57), S(  -4, -28),
    S(   7, -17), S(  15,  -2), S(  17,   9), S(  18,  14),
    S(  17,  21), S(  24,  22), S(  23,  22), S(  46,   7),
    S(  48,  21), S(  75, -17),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(   9,   6), S(  30,   6) };

const int RookOnSeventh = S(  -4,  34);

const int RookMobility[15] = {
    S(-159,-116), S( -61,-116), S( -17, -67), S(  -8, -23),
    S(  -8,  -2), S( -11,  16), S( -10,  28), S(  -3,  31),
    S(   4,  35), S(   8,  38), S(  11,  45), S(  17,  49),
    S(  19,  53), S(  36,  41), S( 104,  -5),
};
...
const int QueenMobility[28] = {
    ...
    S(  -8,-144), S(  -4, -80), S(   1, -44), S(   0, -12),
    S(   6, -11), S(   7,  11), S(  12,  15), S(  13,  32),
    S(  17,  23), S(  19,  34), S(  17,  38), S(  14,  43),
    S(  16,  41), S(   7,  43), S(   8,  41), S(   7,  31),
...
const int KingDefenders[12] = {
    S( -28,  -4), S( -10,   0), S(   1,   5), S(  10,   8),
    S(  18,   8), S(  30,   1), S(  31,  -4), S(  12,  -1),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  41,  35), S(  30,  24), S(  10,  11), S( -25, -20),
    S( -29, -52), S( -21, -65), S( -17, -68), S( -11, -64),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -9,   2), S(  15, -26), S(  22,  -8), S(  19,   6),
    S(  14,   3), S(  -1,  -1), S(  -3, -34), S( -54,  21)},
   {S(  16,  -6), S(  23, -18), S(   2,  -5), S( -15,   4),
    S( -30,  16), S( -73,  67), S(  91,  81), S( -21,   1)},
   {S(  35,  -2), S(   9,  -7), S( -30,   6), S( -14,  -8),
    S( -17,  -5), S( -12,   1), S(   0,  68), S( -14,   0)},
   {S(   2,  13), S(  27, -14), S(   7, -15), S(  16, -22),
    S(  26, -39), S( -60,   3), S(-135,  51), S(   5,  -5)},
   {S( -17,  13), S(   3,  -2), S( -33,   4), S( -20,   8),
    S( -20,  -6), S( -45,  -5), S(  32, -18), S(  -8,   1)},
   {S(  48, -15), S(  20, -13), S( -21,   3), S(  -8, -23),
    S(   7, -29), S(  21, -21), S(  40, -32), S( -23,   1)},
   {S(  34, -15), S(   1, -18), S( -29,  -2), S( -22,  -9),
    S( -30,  -4), S( -41,  30), S(   1,  44), S( -11,   2)},
   {S(  -5, -14), S(   5, -21), S(   8,   1), S(  -2,  11),
    S( -12,  17), S( -10,  38), S(-190,  89), S( -19,  17)}},
  {{S(   0,   0), S( -13, -26), S(   6, -23), S( -39,  20),
    S( -23,   1), S(   3,  42), S(-167,  -7), S( -43,  13)},
   {S(   0,   0), S(  22, -21), S(  10,  -8), S( -16,   0),
    S(   3, -18), S(  26,  65), S(-184,  -3), S( -32,   7)},
   {S(   0,   0), S(  23, -12), S(  -1, -11), S(   6, -21),
    S(  21,  -3), S( -87,  48), S( -84, -74), S( -19,  -1)},
   {S(   0,   0), S( -10,   8), S(  -6,  -2), S( -25,   5),
    S( -27,   4), S(-100,  30), S(   7, -40), S( -22,  -3)},
   {S(   0,   0), S(  13,   1), S(  13,  -9), S(  16, -13),
    S(  17, -31), S( -57,  14), S(-101, -60), S(  -2,  -2)},
   {S(   0,   0), S(  -1,  -7), S( -25,   2), S( -21,  -8),
    S(  23, -23), S( -37,   3), S(  55,  39), S( -16,  -4)},
   {S(   0,   0), S(  26, -16), S(  15, -15), S( -10,  -9),
    S( -30,  12), S(  -8,  19), S( -57, -50), S( -33,  16)},
   {S(   0,   0), S(  14, -45), S(  21, -33), S( -19,  -8),
    S( -21,  19), S(  -8,  21), S(-228, -56), S( -22,   5)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -13,  33), S( 112,  -7), S( -23,  22), S( -12,   1),
    S( -14,   0), S(  -7,  -3), S( -16,   7), S( -27,   0)},
   {S(  -8,  53), S(  51,  13), S( -19,  19), S(  -2,   8),
    S(  -1,   6), S(   5,  -3), S(  -1,   2), S( -14,   7)},
   {S(   2,  42), S(   4,  27), S( -21,  17), S( -11,   7),
    S(   2,   3), S(   9,   1), S(  11,  -8), S(   3,   6)},
   {S(  -7,  25), S(   4,  22), S( -23,   7), S( -15,   0),
    S( -10,   2), S(  11, -11), S(   3,  -8), S( -14,   7)}},
  {{S(   0,   0), S( -18, -16), S( -15,  -2), S(  24, -21),
    S(  12,  -5), S(   6, -19), S(   6,   3), S(  16,  29)},
   {S(   0,   0), S( -17, -37), S(   2, -13), S(  45, -13),
    S(   3,  -2), S(  17, -25), S(  -6,  -5), S( -19,   0)},
   {S(   0,   0), S( -28, -52), S( -24,  -9), S(  15, -12),
    S(   8,  -2), S(  -7, -13), S(  -8, -12), S( -11,   6)},
   {S(   0,   0), S(  -2, -19), S( -20, -18), S( -13,  -2),
    S(  -2,  -8), S(   1, -24), S(  72, -12), S(  15,  20)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -37,   0), S( -47,  26), S( -70,  30),
    S(  10,  11), S(  82, -16), S( 158,  42), S(   0,   0)},
   {S(   0,   0), S( -28,  14), S( -44,  39), S( -65,  41),
    S(   0,  41), S( 106,  26), S( 191,  88), S(   0,   0)}},
  {{S(   0,   0), S( -24,  26), S( -47,  34), S( -67,  49),
    S(  10,  46), S( 104,  41), S( 271, 100), S(   0,   0)},
   {S(   0,   0), S( -29,  22), S( -43,  33), S( -63,  54),
    S(  11,  66), S( 104, 121), S( 157, 268), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(  -1,   2), S(   3,  -3), S(   7, -12),
    S(   5, -16), S( -11, -13), S( -16,  -4), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   4,  -2), S(   5,  -1), S(   8,   8),
    S(   1,  22), S(   6,  32), S(  25,  31), S(   0,   0),
};

const int PassedSafePromotionPath = S( -26,  45);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -11, -31);
const int ThreatMinorAttackedByPawn  = S( -53, -60);
...
const int ThreatMinorAttackedByKing  = S( -19, -19);
const int ThreatRookAttackedByKing   = S( -18, -17);
const int ThreatQueenAttackedByOne   = S( -46, -23);
...
const int ClosednessKnightAdjustment[9] = {
    S( -10,  -4), S(  -5,  10), S(  -8,  17), S(  -3,  15),
    S(  -2,  20), S(   1,  14), S(   3,   6), S(  -9,  24),
    S(  -8,   9),
};

const int ClosednessRookAdjustment[9] = {
    S(  60,  -3), S(   4,  34), S(   3,  18), S(  -3,   3),
    S( -13,   5), S( -10, -12), S( -12, -12), S( -23, -16),
    S( -35, -21),
};
