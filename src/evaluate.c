...
const int KingShelter[2][8] = {
   {S(  -7,   2), S(   7,   1), S( -11,   6), S(  -2,  -1),
    S(  -3,   4), S(   3,  21), S(  -6,  37), S(   0,   0)},
   {S(  -9, -11), S(   4,  -7), S(   0,  -8), S(   3, -12),
    S( -11,  -2), S(  11,  10), S(   0,  31), S(   0,   0)},
};
...
int evaluateKings(EvalInfo *ei, Board *board, int colour) {
    ...
    for (int file = MAX(0, kingFile - 1); file <= MIN(FILE_NB - 1, kingFile + 1); file++) {
        ...
        int distance = filePawns ? relativeRankOf(US, (backmost(US, filePawns))) : 0;

        pkeval += KingShelter[file == kingFile][distance];
        if (TRACE) T.KingShelter[file == kingFile][distance][US]++;
    }
