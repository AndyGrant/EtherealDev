...
const int KnightValue = S( 465, 400);
const int BishopValue = S( 486, 419);
const int RookValue   = S( 669, 678);
const int QueenValue  = S(1315,1320);
...
const int PawnPSQT32[32] = {
    ...
    S( -21,   8), S(   4,   2), S(  -6,   3), S(  -2,  -3),
    S( -27,   6), S( -12,   5), S( -11,  -3), S(  -1, -13),
    S( -20,  13), S( -14,  13), S(   5,  -7), S(   5, -20),
    S( -12,  14), S(  -6,  10), S( -11,  -4), S(  -6, -17),
    S( -10,  31), S(  -5,  27), S(   0,  18), S(  27, -10),
    S( -23, -67), S( -57, -37), S(   3, -52), S(  46, -67),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -48, -33), S(  -5, -22), S( -11, -30), S(   3, -14),
    S(   1, -19), S(   2, -11), S(   5, -26), S(  12, -17),
    S(  13, -27), S(  21, -21), S(  15, -15), S(  26,  -3),
    S(  20,   0), S(  26,   6), S(  32,  19), S(  37,  24),
    S(  27,  11), S(  33,   8), S(  43,  29), S(  37,  39),
    S( -20,   8), S(  -1,  14), S(  29,  32), S(  33,  28),
    S(  16, -12), S(  -4,  -4), S(  27, -23), S(  40,  -1),
    S(-182, -29), S( -88,  -6), S(-111,  14), S( -33,  -6),
};

const int BishopPSQT32[32] = {
    S(   9, -28), S(  12, -18), S(  -6,  -9), S(  14, -12),
    S(  32, -38), S(  11, -35), S(  22, -21), S(  15, -12),
    S(  16, -13), S(  29, -16), S(   6, -18), S(  26,  -6),
    S(  15,  -7), S(  22,  -3), S(  17,   4), S(  29,   4),
    S(  -9,  15), S(  23,   6), S(   3,  16), S(  17,  17),
    S(   3,   7), S(  -7,  18), S(  -2,   9), S(  10,  17),
    S( -43,  14), S( -49,   5), S(  -7,   9), S( -25,  15),
    S( -62,  -9), S( -44,   2), S( -84,  12), S( -90,  26),
};

const int RookPSQT32[32] = {
    S( -14, -27), S( -16, -17), S(  -4, -21), S(   5, -27),
    S( -53, -13), S(  -8, -35), S(  -7, -32), S(   6, -37),
    S( -29, -14), S(  -4, -11), S( -14, -17), S(   4, -26),
    S( -19,  -3), S( -14,   8), S(  -6,   2), S(   9,  -3),
    S(  -6,  10), S(   9,   7), S(  22,   4), S(  45,  -1),
    S( -17,  21), S(  31,   4), S(   9,  19), S(  42,   2),
    S(   0,   5), S( -10,  11), S(  14,   2), S(  33,   4),
    S(  34,  22), S(  28,  22), S(  -2,  33), S(   6,  28),
};

const int QueenPSQT32[32] = {
    S(  35, -73), S(   8, -48), S(  18, -64), S(  22, -43),
    S(  13, -45), S(  27, -57), S(  29, -71), S(  18, -27),
    S(  11, -36), S(  25, -20), S(   6,   9), S(   6,   3),
    S(  12, -12), S(  18,   9), S(  -2,  27), S( -14,  67),
    S(  -2,  13), S(  -8,  50), S( -12,  23), S( -27,  75),
    S( -25,  34), S( -18,  20), S( -22,  26), S(  -9,  26),
    S(   2,  26), S( -67,  69), S(   1,   9), S( -36,  50),
    S( -12,  17), S(  22,   8), S(   9,   9), S(  -6,  20),
};

const int KingPSQT32[32] = {
    S(  51, -73), S(  39, -48), S( -18,  -7), S( -32, -18),
    S(  27, -13), S( -20,  -5), S( -48,  18), S( -70,  20),
    S( -10, -13), S(  -1, -12), S(  11,   7), S( -23,  24),
    S(   2, -27), S( 103, -32), S(  49,  11), S(  -5,  31),
    S(  13, -11), S( 114, -31), S(  61,  14), S(  16,  20),
    S(  50, -33), S( 139, -24), S( 110,   1), S(  45,  -3),
    S(   6, -70), S(  48, -13), S(  32,  -1), S(   8,  -9),
    S(   2,-134), S(  72, -70), S( -25, -30), S( -21, -24),
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -27,  -9), S( -11,  10), S( -18,  36),
    S(   3,  62), S(  36,  67), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -16,  20), S(  -5,  21), S(   3,  49),
    S(  18,  91), S(  41,  66), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -4, -13);

const int PawnStacked[2] = { S(  -4, -19), S(  -7,  -8) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   3,  -6), S(  10,  -6), S(   9, -14),
    S(   7, -15), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -10, -27), S(  -5, -23), S(   3, -28),
    S(  41, -35), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -3,  -5), S(  12,   0), S(   1,   2), S(   6,  10),
    S(  14,   0), S(  26,  -6), S(  21,   2), S(  23,  10),
    S(   8,  -2), S(  25,  -1), S(   9,   6), S(  17,  11),
    S(   8,  10), S(  21,  14), S(  24,  21), S(  29,  18),
    S(  58,  17), S(  41,  42), S(  60,  54), S(  72,  61),
    S( 109, -15), S( 202, -12), S( 226,  22), S( 233,  33),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightInSiberia[4] = {
    S(  -9,  -1), S( -12,  -7), S( -25,  -3), S( -29,  -9),
};

const int KnightMobility[9] = {
    S( -80,-106), S( -30,-106), S( -14, -43), S(  -3, -15),
    S(   9,  -6), S(  13,  10), S(  21,  13), S(  32,  11),
    S(  46,  -6),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  20,  69);
...
const int BishopMobility[14] = {
    S( -71,-150), S( -35,-102), S( -12, -53), S(  -2, -27),
    S(   9, -16), S(  17,  -3), S(  21,   7), S(  21,  12),
    S(  20,  18), S(  26,  20), S(  26,  18), S(  51,   6),
    S(  49,  19), S(  76, -20),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  10,   2), S(  32,   2) };

const int RookOnSeventh = S(  -3,  31);

const int RookMobility[15] = {
    S(-163,-117), S( -63,-117), S( -20, -70), S( -11, -22),
    S( -11,   0), S( -11,  14), S( -10,  28), S(  -3,  30),
    S(   5,  35), S(   8,  39), S(  12,  45), S(  20,  49),
    S(  22,  54), S(  42,  39), S( 128, -18),
};

/* Queen Evaluation Terms */

const int QueenMobility[28] = {
    S( -61,-263), S(-214,-388), S( -61,-200), S( -27,-191),
    S(  -9,-146), S(  -7, -78), S(  -2, -40), S(  -2,  -3),
    S(   6,  -8), S(   7,  15), S(  14,  14), S(  15,  35),
    S(  19,  21), S(  22,  33), S(  20,  37), S(  16,  46),
    S(  21,  39), S(   9,  43), S(  13,  38), S(  13,  26),
    S(  23,   2), S(  34, -21), S(  31, -53), S(  28, -68),
    S(   6, -83), S(  18,-111), S( -55, -36), S( -33, -64),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -25,   4), S(  -5,   0), S(   3,   5), S(   9,   7),
    S(  17,   4), S(  27,  -1), S(  27,  -5), S(  11,  -1),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  47,  32), S(  37,  20), S(  13,   7), S( -31, -16),
    S( -34, -51), S( -25, -65), S( -19, -66), S( -11, -60),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -8,   2), S(  14, -27), S(  29,  -9), S(  19,   7),
    S(  17,   3), S(  -2,  -2), S(  -3, -34), S( -53,  22)},
   {S(  17,  -7), S(  23, -20), S(   5,  -6), S( -17,   3),
    S( -30,  16), S( -75,  66), S(  91,  81), S( -15,   1)},
   {S(  37,  -3), S(   8,  -7), S( -32,   7), S( -15,  -8),
    S( -16,  -6), S( -14,   1), S(   0,  69), S( -14,   1)},
   {S(   1,  13), S(  27, -15), S(   6, -12), S(  15, -21),
    S(  35, -40), S( -60,   3), S(-135,  51), S(   6,  -5)},
   {S( -20,  14), S(   5,  -1), S( -35,   5), S( -21,  10),
    S( -23,  -5), S( -48,  -8), S(  32, -18), S(  -9,   4)},
   {S(  51, -18), S(  20, -12), S( -20,   5), S( -10, -21),
    S(   8, -27), S(  20, -19), S(  40, -33), S( -23,   4)},
   {S(  33, -18), S(  -1, -17), S( -27,   0), S( -25,  -6),
    S( -31,  -5), S( -43,  30), S(   2,  45), S(  -9,   0)},
   {S(  -8, -14), S(   5, -19), S(   9,   4), S(   0,  12),
    S( -11,  17), S( -12,  36), S(-191,  89), S( -20,  16)}},
  {{S(   0,   0), S( -14, -29), S(   9, -27), S( -39,  20),
    S( -24,   0), S(   3,  42), S(-167,  -7), S( -41,  14)},
   {S(   0,   0), S(  27, -23), S(   9,  -6), S( -18,   1),
    S(   4, -20), S(  25,  64), S(-184,  -3), S( -26,   6)},
   {S(   0,   0), S(  23, -13), S(   1, -13), S(   4, -24),
    S(  24,   0), S( -86,  49), S( -84, -74), S( -20,   2)},
   {S(   0,   0), S( -13,   8), S(  -9,  -4), S( -29,   5),
    S( -27,   6), S(-101,  29), S(   7, -40), S( -24,   0)},
   {S(   0,   0), S(  15,   1), S(  13,  -7), S(  16, -15),
    S(  19, -34), S( -56,  14), S(-100, -60), S(  -3,   1)},
   {S(   0,   0), S(  -2,  -6), S( -30,   3), S( -19, -11),
    S(  26, -22), S( -37,   3), S(  55,  39), S( -17,  -1)},
   {S(   0,   0), S(  24, -18), S(  13, -17), S( -12,  -8),
    S( -29,  18), S(  -6,  21), S( -58, -50), S( -32,  14)},
   {S(   0,   0), S(  13, -45), S(  24, -35), S( -19,  -9),
    S( -21,  20), S( -10,  22), S(-228, -57), S( -28,   7)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -17,  34), S( 111,  -7), S( -22,  24), S( -11,   0),
    S( -12,  -1), S(  -6,  -7), S( -14,   1), S( -26,   4)},
   {S( -11,  55), S(  49,  13), S( -19,  19), S(  -1,   7),
    S(  -1,   3), S(   7,  -6), S(  -2,   3), S( -12,   5)},
   {S(   0,  45), S(  -4,  30), S( -21,  19), S(  -9,   5),
    S(   4,   0), S(   7,   0), S(  14,  -5), S(   6,   4)},
   {S( -10,  30), S(  -3,  27), S( -27,   9), S( -14,  -2),
    S( -12,   1), S(  10, -13), S(   4, -11), S( -12,   6)}},
  {{S(   0,   0), S( -19, -16), S( -15,  -1), S(  24, -20),
    S(  10,  -6), S(   9, -23), S(   6,   3), S(  15,  28)},
   {S(   0,   0), S( -17, -37), S(   6, -11), S(  47, -12),
    S(   6,  -5), S(  18, -24), S(  -6,  -3), S( -20,  -1)},
   {S(   0,   0), S( -28, -53), S( -22,  -6), S(  20, -13),
    S(   9,  -1), S(  -8, -12), S( -11, -11), S( -11,   6)},
   {S(   0,   0), S(  -2, -19), S( -21, -14), S( -16,   0),
    S(  -3,  -6), S(   2, -24), S(  74, -13), S(  16,  21)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -37,  -3), S( -45,  26), S( -68,  30),
    S(  13,   8), S(  87, -21), S( 156,  33), S(   0,   0)},
   {S(   0,   0), S( -30,  14), S( -44,  41), S( -63,  42),
    S(   5,  39), S( 114,  20), S( 191,  83), S(   0,   0)}},
  {{S(   0,   0), S( -26,  31), S( -48,  39), S( -68,  47),
    S(  15,  43), S( 108,  31), S( 277,  87), S(   0,   0)},
   {S(   0,   0), S( -31,  26), S( -43,  39), S( -62,  56),
    S(  15,  64), S( 109, 118), S( 154, 256), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   1,   2), S(   5,  -4), S(   6, -11),
    S(   3, -14), S( -11,  -9), S( -18,   4), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   3,  -1), S(   4,  -1), S(   9,   8),
    S(   1,  20), S(   6,  27), S(  29,  22), S(   0,   0),
};

const int PassedSafePromotionPath = S( -13,  40);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -11, -29);
const int ThreatMinorAttackedByPawn  = S( -54, -58);
const int ThreatMinorAttackedByMinor = S( -23, -37);
const int ThreatMinorAttackedByMajor = S( -31, -46);
const int ThreatRookAttackedByLesser = S( -49, -23);
const int ThreatMinorAttackedByKing  = S( -16, -16);
const int ThreatRookAttackedByKing   = S( -18, -14);
const int ThreatQueenAttackedByOne   = S( -40, -20);
...
const int ClosednessKnightAdjustment[9] = {
    S(  -6,  -9), S(  -3,   8), S(  -6,  17), S(  -3,  15),
    S(  -1,  19), S(   2,  11), S(   7,   1), S(  -8,  26),
    S(  -5,   9),
};

const int ClosednessRookAdjustment[9] = {
    S(  84, -25), S(   0,  41), S(   2,  21), S(  -2,  -3),
    S( -12,   7), S(  -8, -14), S( -10, -12), S( -21, -16),
    S( -38, -23),
};
