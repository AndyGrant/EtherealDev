...
const int PawnValue[PHASE_NB]   = {  78,  79};

const int KnightValue[PHASE_NB] = { 298, 266};

const int BishopValue[PHASE_NB] = { 298, 267};

const int RookValue[PHASE_NB]   = { 400, 438};

const int QueenValue[PHASE_NB]  = { 800, 839};

const int KingValue[PHASE_NB]   = { 100, 100};

const int PawnIsolated[PHASE_NB] = {   1,  -5};

const int PawnStacked[PHASE_NB] = { -15, -20};

const int PawnBackwards[2][PHASE_NB] = { {   2,  -2}, {  -9,  -8} };
...
const int PawnConnected32[32][PHASE_NB] = {
    ...
    {   7,   2}, {  14,   2}, {  12,   8}, {  14,  17},
    {   8,   1}, {  17,   4}, {  12,   6}, {  16,  14},
    {  14,   5}, {  19,  13}, {  22,  15}, {  29,  17},
    {  20,   7}, {  25,  14}, {  55,  32}, {  51,  40},
    { 118, -58}, { 121, -41}, {  96, -42}, { 207,   2},
    {   0,   0}, {   0,   0}, {   0,   0}, {   0,   0},
};

const int KnightAttackedByPawn[PHASE_NB] = { -44, -18};

const int KnightOutpost[2][PHASE_NB] = { {  16, -20}, {  29,   2} };
...
const int BishopWings[PHASE_NB] = {   0,  -6};

const int BishopPair[PHASE_NB] = {  25,  41};

const int BishopAttackedByPawn[PHASE_NB] = { -39, -33};

const int BishopOutpost[2][PHASE_NB] = { {  16,  -9}, {  34,  -5} };

const int BishopMobility[14][PHASE_NB] = {
    { -58, -65}, { -28, -42}, { -12, -26}, {  -4, -14},
    {   5,  -6}, {  11,   0}, {  15,   6}, {  18,   3},
    {  20,   8}, {  24,   2}, {  24,   5}, {  28, -10},
    {  27,   3}, {  -8, -42},
};

const int RookFile[2][PHASE_NB] = { {   9,   0}, {  27,  -4} };

const int RookOnSeventh[PHASE_NB] = {   6,   5};

const int RookMobility[15][PHASE_NB] = {
    {-108,-106}, { -34, -72}, { -10, -42}, {  -4, -19},
    {  -5,  -5}, {  -6,   3}, {  -6,  11}, {  -3,  16},
    {   0,  22}, {   7,  25}, {  10,  27}, {  12,  32},
    {  12,  34}, {  12,  28}, {   0,  22},
};

const int QueenChecked[PHASE_NB] = { -41,   0};

const int QueenCheckedByPawn[PHASE_NB] = { -40,  -8};

const int QueenMobility[28][PHASE_NB] = {
    {-143, -59}, { -58,-352}, { -48,-109}, { -28,-114},
    { -19,-113}, { -17, -54}, { -12, -52}, { -13, -48},
    { -11, -47}, {  -8, -38}, {  -6, -30}, {  -3, -20},
    {  -2, -13}, {  -1,  -5}, {   0,  -2}, {   1,   3},
    {   0,  10}, {   1,  11}, {   0,  15}, {   0,  15},
    {  -2,  18}, {   8,  15}, {   7,  13}, {  17,   5},
    {  15,   8}, {  40,  -3}, {   9,   9}, {  -3,   1},
...
const int KingDefenders[12][PHASE_NB] = {
    { -19,   7}, { -14,  -3}, {   0,   0}, {   6,   1},
    {  15,   0}, {  22,   4}, {  28, -13}, {  55,  16},
    {   8,   4}, {   8,   4}, {   8,   4}, {   8,   4},
};

const int KingShelter[2][FILE_NB][RANK_NB][PHASE_NB] = {
  {{{  -9,   1}, {  11,  -7}, {  19,  -2}, {  11,  -6}, {  11,  -3}, { -16,  -2}, {  -5,  17}, { -17,  10}},
   {{   0,   3}, {  13,  -4}, {  21,  -7}, {  -5,  -9}, {   1,  -8}, { -75,  52}, {-112,  45}, { -18,   2}},
   {{   8,   5}, {   6,   0}, { -12,   2}, {  -6,  -5}, {  -2, -10}, {  40, -14}, {  -5, -15}, {  -7,   0}},
   {{   2,  15}, {   8,   0}, {  -4,  -5}, {  12,  -5}, {  12, -11}, { -18, -19}, {  -5,  15}, {   0,  -1}},
   {{  -5,  11}, {  -2,   2}, { -15,   1}, { -10,  -6}, { -13, -15}, { -15, -26}, {  46, -18}, { -11,   0}},
   {{  12,   1}, {  14,   0}, {  -6,  -1}, {  -7, -12}, {   5, -22}, {  -4,  -9}, { -10,  -2}, { -13,   0}},
   {{   6,   0}, {   1,  -7}, { -13,  -4}, { -14, -10}, { -21, -14}, { -36,  10}, {  48,   5}, { -20,   7}},
   {{  -9,  -4}, {   1,  -3}, {   3,   1}, {   0,   2}, { -16,   0}, {  -6,  28}, {   0,  45}, { -18,  10}}},
  {{{   0,   0}, {  -9,  -8}, {   6, -10}, { -32,  13}, { -15,  30}, { -17,  61}, {   3,   6}, { -34,  10}},
   {{   0,   0}, {  17,   0}, {  13,  -4}, {  -9,   1}, {   4, -13}, { -90,  15}, {-301,  39}, { -24,   5}},
   {{   0,   0}, {  21,   0}, {   2,  -4}, {  20, -15}, {  20,  -9}, { -89,  -5}, { -79, -75}, { -12,   0}},
   {{   0,   0}, {   0,   1}, {  -4,   9}, { -15,  -6}, { -16,  -6}, {   0,   2}, { -38,   4}, { -18,  -5}},
   {{   0,   0}, {   1,   4}, {   2,  -1}, {   8,  -9}, {   7, -14}, { -17, -17}, { -55,  33}, {  -3,  -4}},
   {{   0,   0}, {   1,   2}, {  -7,  -1}, {  -7, -13}, {   0, -22}, { -67,  -7}, { -37,  80}, { -17,   0}},
   {{   0,   0}, {   8,   0}, {   0,   0}, {  -6,  -7}, { -15,  -8}, {  -5,  15}, {  16, -70}, { -29,  10}},
   {{   0,   0}, {   4, -12}, {   6,  -8}, { -30,   2}, { -22,  12}, {   2, -11}, { -68,  -3}, { -32,  10}}},
};

const int PassedPawn[2][2][RANK_NB][PHASE_NB] = {
  {{{   0,   0}, { -25,  -8}, { -23,  -1}, { -15,  -5}, {  25,  -8}, {  55, -14}, { 114,  14}, {   0,   0}},
   {{   0,   0}, {  -1,   2}, {  -6,  13}, {  -9,  20}, {  13,  20}, {  64,  27}, { 137,  67}, {   0,   0}}},
  {{{   0,   0}, {  -1,   6}, {  -7,   3}, {  -4,  11}, {  24,  16}, {  76,  29}, { 153,  63}, {   0,   0}},
   {{   0,   0}, {   3,   9}, {  -2,  13}, { -10,  37}, {   0,  73}, {  32, 141}, {  88, 219}, {   0,   0}}},
};
...
void evaluateKings(EvalInfo* ei, Board* board, int colour){
    ...
    if (ei->attackerCounts[!colour] >= 2){
        ...
        ei->endgame[colour] += (int)(-0.0465 * X * X - 0.4223 * X + 0.6851);
    }
