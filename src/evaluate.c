...
const int PawnValue   = S( 104, 121);
const int KnightValue = S( 462, 407);
const int BishopValue = S( 482, 428);
const int RookValue   = S( 664, 686);
const int QueenValue  = S(1306,1340);
...
const int PawnPSQT32[32] = {
    ...
    S( -21,   7), S(   4,   2), S(  -6,   2), S(  -2,  -3),
    S( -27,   6), S( -12,   5), S( -11,  -4), S(  -1, -14),
    S( -20,  13), S( -14,  13), S(   5,  -7), S(   6, -20),
    S( -12,  15), S(  -6,  10), S( -11,  -5), S(  -6, -18),
    S( -10,  31), S(  -5,  26), S(   0,  16), S(  28, -11),
    S( -22, -65), S( -58, -34), S(   2, -50), S(  45, -65),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -48, -33), S(  -5, -23), S( -11, -30), S(   2, -14),
    S(   1, -19), S(   2, -11), S(   5, -25), S(  12, -16),
    S(  13, -27), S(  21, -21), S(  15, -15), S(  26,  -3),
    S(  20,   0), S(  26,   6), S(  32,  19), S(  37,  24),
    S(  27,  12), S(  32,   8), S(  42,  29), S(  36,  39),
    S( -20,   9), S(   0,  13), S(  30,  31), S(  33,  28),
    S(  15, -12), S(  -4,  -3), S(  27, -23), S(  40,   0),
    S(-180, -28), S( -86,  -5), S(-111,  16), S( -33,  -5),
};

const int BishopPSQT32[32] = {
    S(   9, -27), S(  11, -18), S(  -6,  -9), S(  14, -12),
    S(  31, -37), S(  11, -34), S(  22, -21), S(  15, -11),
    S(  16, -13), S(  28, -15), S(   5, -17), S(  26,  -6),
    S(  15,  -7), S(  21,  -3), S(  17,   4), S(  28,   4),
    S( -10,  15), S(  23,   6), S(   3,  15), S(  17,  16),
    S(   3,   7), S(  -7,  18), S(  -2,   8), S(  11,  16),
    S( -43,  15), S( -49,   6), S(  -8,  10), S( -24,  15),
    S( -61,  -9), S( -44,   4), S( -84,  14), S( -90,  28),
};

const int RookPSQT32[32] = {
    S( -14, -28), S( -16, -18), S(  -4, -21), S(   5, -27),
    S( -53, -14), S(  -7, -35), S(  -7, -32), S(   6, -37),
    S( -29, -15), S(  -4, -11), S( -14, -17), S(   4, -26),
    S( -18,  -3), S( -14,   8), S(  -6,   2), S(   9,  -3),
    S(  -6,  11), S(   8,   8), S(  21,   6), S(  44,   0),
    S( -18,  22), S(  30,   5), S(   7,  21), S(  40,   3),
    S(  -1,   5), S( -11,  12), S(  13,   2), S(  31,   5),
    S(  36,  21), S(  28,  22), S(  -1,  33), S(   7,  28),
};

const int QueenPSQT32[32] = {
    S(  34, -69), S(   8, -46), S(  18, -62), S(  22, -42),
    S(  13, -44), S(  27, -57), S(  28, -69), S(  18, -26),
    S(  11, -33), S(  25, -20), S(   6,   8), S(   6,   2),
    S(  11, -10), S(  17,  10), S(  -2,  26), S( -14,  66),
    S(  -3,  14), S(  -8,  49), S( -12,  22), S( -27,  75),
    S( -25,  34), S( -18,  20), S( -22,  26), S( -10,  26),
    S(   1,  28), S( -67,  68), S(   0,  11), S( -37,  51),
    S( -12,  19), S(  21,   9), S(   9,  10), S(  -5,  20),
};

const int KingPSQT32[32] = {
    S(  52, -76), S(  41, -50), S( -17,  -9), S( -30, -20),
    S(  28, -15), S( -18,  -7), S( -47,  17), S( -69,  19),
    S(  -8, -15), S(   1, -14), S(  11,   6), S( -21,  23),
    S(   1, -28), S( 100, -32), S(  48,  11), S(  -4,  30),
    S(  12, -11), S( 111, -31), S(  59,  15), S(  14,  21),
    S(  50, -32), S( 137, -23), S( 107,   2), S(  44,  -3),
    S(   6, -67), S(  48, -11), S(  32,   1), S(   8,  -8),
    S(   3,-130), S(  72, -68), S( -24, -28), S( -21, -23),
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -27, -10), S( -11,   9), S( -18,  34),
    S(   4,  60), S(  37,  66), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -15,  19), S(  -5,  21), S(   3,  48),
    S(  19,  89), S(  39,  64), S(   0,   0), S(   0,   0)},
};
...
const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   3,  -6), S(  10,  -5), S(   9, -14),
    S(   7, -14), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -10, -26), S(  -5, -22), S(   3, -27),
    S(  37, -31), S(   0,   0), S(   0,   0), S(   0,   0)},
};
...
const int PawnConnected32[32] = {
    ...
    S(   7,  10), S(  21,  14), S(  24,  21), S(  29,  18),
    S(  57,  18), S(  41,  43), S(  61,  55), S(  73,  61),
    S( 110, -12), S( 202,  -9), S( 226,  23), S( 234,  35),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightInSiberia[4] = {
    S(  -9,  -1), S( -12,  -7), S( -25,  -3), S( -27,  -7),
};

const int KnightMobility[9] = {
    S( -79,-106), S( -31,-104), S( -14, -42), S(  -3, -14),
    S(   9,  -5), S(  13,  10), S(  20,  14), S(  31,  12),
    S(  45,  -6),
};
...
const int BishopMobility[14] = {
    S( -70,-150), S( -35,-101), S( -12, -52), S(  -2, -26),
    S(   9, -16), S(  17,  -3), S(  20,   7), S(  21,  13),
    S(  20,  18), S(  26,  21), S(  26,  20), S(  50,   7),
    S(  48,  20), S(  75, -20),
};
...
const int RookMobility[15] = {
    S(-161,-116), S( -62,-116), S( -19, -68), S( -10, -22),
    S( -11,   0), S( -11,  14), S( -10,  28), S(  -3,  30),
    S(   5,  35), S(   8,  39), S(  12,  45), S(  19,  50),
    S(  21,  54), S(  41,  39), S( 122, -17),
};

/* Queen Evaluation Terms */

const int QueenMobility[28] = {
    S( -61,-263), S(-214,-388), S( -60,-200), S( -27,-191),
    S(  -9,-145), S(  -7, -79), S(  -2, -41), S(  -2,  -6),
    S(   6,  -9), S(   7,  13), S(  14,  13), S(  15,  34),
    S(  19,  20), S(  22,  33), S(  19,  37), S(  15,  46),
    S(  20,  41), S(   8,  45), S(  12,  41), S(  11,  30),
    S(  20,   6), S(  32, -18), S(  30, -50), S(  29, -66),
    S(   7, -80), S(  18,-110), S( -55, -36), S( -32, -63),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -24,   3), S(  -5,   0), S(   2,   5), S(   9,   7),
    S(  17,   5), S(  27,  -1), S(  27,  -5), S(  12,  -1),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  44,  35), S(  33,  23), S(  12,   9), S( -27, -17),
    S( -31, -53), S( -23, -66), S( -18, -67), S( -11, -62),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -9,   1), S(  14, -27), S(  28,  -9), S(  19,   7),
    S(  16,   3), S(  -1,  -2), S(  -3, -34), S( -53,  21)},
   {S(  16,  -7), S(  23, -20), S(   5,  -6), S( -16,   3),
    S( -30,  16), S( -74,  67), S(  91,  81), S( -16,   1)},
   {S(  37,  -3), S(   8,  -8), S( -32,   6), S( -15,  -8),
    S( -17,  -5), S( -13,   1), S(   0,  68), S( -14,   1)},
   {S(   1,  13), S(  27, -15), S(   6, -13), S(  15, -21),
    S(  35, -39), S( -59,   3), S(-135,  51), S(   6,  -5)},
   {S( -20,  13), S(   5,  -1), S( -35,   5), S( -22,  10),
    S( -23,  -6), S( -46,  -7), S(  32, -18), S(  -9,   4)},
   {S(  50, -18), S(  20, -12), S( -20,   4), S( -10, -21),
    S(   8, -27), S(  19, -20), S(  40, -33), S( -23,   3)},
   {S(  32, -18), S(  -1, -17), S( -27,  -1), S( -24,  -6),
    S( -31,  -4), S( -42,  30), S(   1,  44), S(  -9,   0)},
   {S(  -8, -14), S(   5, -19), S(   9,   3), S(   0,  12),
    S( -11,  16), S( -12,  36), S(-190,  89), S( -21,  16)}},
  {{S(   0,   0), S( -14, -28), S(   9, -25), S( -39,  20),
    S( -23,   1), S(   3,  42), S(-167,  -7), S( -41,  13)},
   {S(   0,   0), S(  26, -23), S(   8,  -6), S( -17,   1),
    S(   4, -19), S(  26,  65), S(-184,  -3), S( -26,   6)},
   {S(   0,   0), S(  23, -13), S(   2, -13), S(   4, -23),
    S(  23,  -1), S( -87,  48), S( -84, -74), S( -19,   1)},
   {S(   0,   0), S( -13,   7), S(  -9,  -4), S( -28,   5),
    S( -27,   5), S(-100,  30), S(   7, -40), S( -24,   0)},
   {S(   0,   0), S(  16,   1), S(  13,  -7), S(  16, -15),
    S(  19, -33), S( -57,  14), S(-100, -60), S(  -3,   1)},
   {S(   0,   0), S(  -1,  -6), S( -29,   3), S( -20, -10),
    S(  25, -22), S( -37,   3), S(  55,  39), S( -17,  -2)},
   {S(   0,   0), S(  24, -18), S(  13, -17), S( -12,  -8),
    S( -29,  17), S(  -7,  20), S( -57, -50), S( -33,  14)},
   {S(   0,   0), S(  13, -45), S(  24, -36), S( -19,  -9),
    S( -20,  19), S(  -9,  21), S(-228, -56), S( -27,   6)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -15,  33), S( 111,  -5), S( -22,  24), S( -11,  -1),
    S( -12,  -1), S(  -6,  -7), S( -14,   1), S( -26,   4)},
   {S(  -9,  55), S(  50,  14), S( -19,  19), S(  -2,   7),
    S(  -1,   3), S(   7,  -6), S(  -3,   4), S( -12,   6)},
   {S(   2,  45), S(   0,  30), S( -22,  18), S( -10,   5),
    S(   3,   0), S(   7,   0), S(  13,  -5), S(   5,   5)},
   {S(  -8,  29), S(   0,  26), S( -26,   8), S( -14,  -2),
    S( -12,   0), S(  10, -13), S(   4, -11), S( -13,   6)}},
  {{S(   0,   0), S( -18, -16), S( -15,  -3), S(  24, -21),
    S(  10,  -6), S(   8, -22), S(   5,   2), S(  16,  29)},
   {S(   0,   0), S( -17, -37), S(   6, -12), S(  47, -12),
    S(   5,  -4), S(  18, -24), S(  -6,  -4), S( -19,   0)},
   {S(   0,   0), S( -28, -53), S( -23,  -6), S(  19, -13),
    S(   8,  -2), S(  -8, -13), S( -11, -11), S( -11,   6)},
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -37,  -2), S( -45,  27), S( -68,  31),
    S(  11,   8), S(  83, -19), S( 156,  36), S(   0,   0)},
   {S(   0,   0), S( -29,  14), S( -44,  41), S( -64,  43),
    S(   3,  40), S( 111,  20), S( 190,  83), S(   0,   0)}},
  {{S(   0,   0), S( -25,  30), S( -48,  40), S( -68,  50),
    S(  13,  44), S( 105,  32), S( 274,  89), S(   0,   0)},
   {S(   0,   0), S( -30,  26), S( -43,  39), S( -63,  57),
    S(  13,  65), S( 106, 120), S( 156, 261), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   1,   3), S(   5,  -4), S(   7, -12),
    S(   4, -15), S( -11,  -9), S( -17,   3), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   3,  -1), S(   3,  -1), S(   9,   8),
    S(   0,  20), S(   5,  28), S(  29,  22), S(   0,   0),
};

const int PassedSafePromotionPath = S( -15,  41);
...
const int ThreatRookAttackedByLesser = S( -48, -25);
const int ThreatMinorAttackedByKing  = S( -16, -17);
const int ThreatRookAttackedByKing   = S( -18, -16);
const int ThreatQueenAttackedByOne   = S( -40, -25);
...
const int ClosednessKnightAdjustment[9] = {
    S(  -7,  -6), S(  -3,   9), S(  -6,  18), S(  -3,  16),
    S(  -1,  20), S(   2,  12), S(   7,   2), S(  -8,  25),
    S(  -5,   9),
};

const int ClosednessRookAdjustment[9] = {
    S(  76, -16), S(   3,  40), S(   2,  20), S(  -3,  -2),
    S( -12,   6), S(  -9, -13), S( -11, -12), S( -22, -16),
    S( -37, -22),
};
...
const int ComplexityPawnFlanks  = S(   0,  87);
const int ComplexityPawnEndgame = S(   0,  64);
const int ComplexityAdjustment  = S(   0,-167);
