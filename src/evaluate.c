...
const int PawnValue[PHASE_NB]   = {  93, 120};

const int KnightValue[PHASE_NB] = { 370, 354};

const int BishopValue[PHASE_NB] = { 378, 360};

const int RookValue[PHASE_NB]   = { 516, 579};

const int QueenValue[PHASE_NB]  = {1062,1055};

const int KingValue[PHASE_NB]   = {   0,   0};

const int PawnIsolated[PHASE_NB] = {  -5, -14};

const int PawnStacked[PHASE_NB] = { -21, -26};

const int PawnBackwards[2][PHASE_NB] = { {   1,  10}, {  -7, -17} };
...
const int PawnConnected32[32][PHASE_NB] = {
    ...
    {   3,  -4}, {   8,  -2}, {   8,  -2}, {  11,   5},
    {  14,  -3}, {  14,   3}, {  22,   3}, {  19,   3},
    {  28, -20}, {  40, -10}, {  56,  14}, {  69,  22},
    { 199, -69}, { 246, -57}, { 140, -83}, { 263, -30},
    {   0,   0}, {   0,   0}, {   0,   0}, {   0,   0},
};

const int KnightAttackedByPawn[PHASE_NB] = { -39, -22};

const int KnightOutpost[2][PHASE_NB] = { {  13, -19}, {  25,   9} };
...
const int BishopWings[PHASE_NB] = {  -2, -12};

const int BishopPair[PHASE_NB] = {  18,  31};

const int BishopAttackedByPawn[PHASE_NB] = { -40, -30};

const int BishopOutpost[2][PHASE_NB] = { {  13,  -6}, {  26,   1} };

const int BishopMobility[14][PHASE_NB] = {
    { -62, -59}, { -31, -44}, { -16, -30}, {  -9, -19},
    {  -1, -13}, {   3,  -5}, {   5,   0}, {   7,  -2},
    {   9,   1}, {  11,  -4}, {  11,   0}, {   2, -18},
    {  13,  -4}, {-106, -72},
};

const int RookFile[2][PHASE_NB] = { {  15,  -4}, {  22,   0} };

const int RookOnSeventh[PHASE_NB] = {  12,  22};

const int RookMobility[15][PHASE_NB] = {
    {-128,-104}, { -42, -76}, { -17, -50}, {  -9, -32},
    {  -8, -21}, {  -8, -13}, {  -8, -10}, {  -3,  -6},
    {   1,  -2}, {   6,   2}, {  10,   2}, {  14,   4},
    {  14,   4}, {  15,  -4}, {   6, -17},
};

const int QueenChecked[PHASE_NB] = { -36, -13};

const int QueenCheckedByPawn[PHASE_NB] = { -39, -16};

const int QueenMobility[28][PHASE_NB] = {
    { -77,-126}, { -86, -82}, { -47, -44}, { -24, -56},
    { -14, -59}, { -10, -36}, {  -5, -30}, {  -6, -19},
    {  -4, -16}, {  -2, -10}, {   0, -10}, {   1,  -3},
    {   3,  -1}, {   5,   1}, {   5,   4}, {   7,   5},
    {   7,   7}, {   7,   6}, {   8,   7}, {   8,   1},
    {  10,   0}, {  11,  -2}, {  10,  -2}, {   7,  -8},
...
const int KingShelter[2][FILE_NB][RANK_NB][PHASE_NB] = {
  {{{  -3,  -1}, {  13, -13}, {  19,  -6}, {   8, -11}, {  10,  -5}, { -10,  -1}, {  31, -17}, { -13,   7}},
   {{  -1,   4}, {  16,  -2}, {  18,  -3}, {  -5,  -5}, {  -7,  -9}, { -46,  29}, { -83, -20}, { -18,   1}},
   {{   3,   4}, {   6,  -3}, { -17,   2}, {  -8,  -8}, { -17,  -6}, {  -6, -23}, { -32,   4}, { -10,   2}},
   {{  -8,  16}, {   1,   0}, {  -9,  -3}, {   6,  -6}, {  10, -20}, { -34,  -9}, { -68,  35}, {  -6,   2}},
   {{ -17,  10}, {  -8,   5}, { -18,   2}, {  -9,  -7}, { -14, -18}, { -37, -23}, { -37, -13}, { -11,   4}},
   {{  10,   3}, {  15,  -2}, { -10,  -2}, { -10, -13}, {   4, -21}, {  -8, -16}, { -23,  -3}, { -12,   2}},
   {{   2,   0}, {   7,  -7}, { -11,  -3}, { -16,  -9}, { -28, -17}, { -56,  13}, {  -9,  17}, { -18,   5}},
   {{  -8,  -5}, {   5,  -7}, {   9,  -2}, {   4,  -1}, { -15,  -2}, { -14,  28}, { -38,  53}, {  -3,   6}}},
  {{{   0,   0}, {  -7, -13}, {   2,  -9}, { -23,   3}, { -36,  34}, { -35,  70}, {  95,  20}, { -33,   3}},
   {{   0,   0}, {  22,  -5}, {  12,  -7}, { -16,   0}, {   2,  -6}, { -48,  18}, {-204,  55}, { -24,   3}},
   {{   0,   0}, {  21,   1}, {   0,  -1}, {  12, -11}, {  13,  -2}, {-102, -13}, {-166,   7}, {  -9,   1}},
   {{   0,   0}, {  -9,   5}, { -19,  16}, { -15,  -5}, { -29,  -8}, { -34, -11}, { -33,  -4}, { -23,   0}},
   {{   0,   0}, {  -5,   7}, {  -1,   0}, {   5, -10}, {   6, -23}, { -47, -16}, {-174,  51}, {  -8,   2}},
   {{   0,   0}, {   4,   4}, {  -9,   2}, { -11,  -3}, {  -2, -17}, { -60,   3}, { -72,  85}, { -17,   4}},
   {{   0,   0}, {  15,  -4}, {   3,  -4}, { -12,  -8}, { -21, -14}, { -22,  18}, { -37, -43}, { -24,   5}},
   {{   0,   0}, {   6, -16}, {   9, -12}, { -24,  -9}, { -22,  14}, { -18,   8}, { -69,  24}, { -29,   0}}},
};

const int PassedPawn[2][2][RANK_NB][PHASE_NB] = {
  {{{   0,   0}, { -22, -17}, { -17, -22}, { -10, -30}, {  27, -31}, {  36, -34}, {  30, -19}, {   0,   0}},
   {{   0,   0}, {  -6, -13}, {  -6, -13}, {  -1,  -8}, {  27, -12}, {  52,  -6}, {  76,   3}, {   0,   0}}},
  {{{   0,   0}, {  -3,  -9}, {  -8, -18}, {   4, -13}, {  27,  -5}, {  60,   0}, { 113, -13}, {   0,   0}},
   {{   0,   0}, {   4,  -8}, {  -1,  -8}, {   4,   9}, {  28,  39}, {  87,  81}, { 158, 113}, {   0,   0}}},
};
