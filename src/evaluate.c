...
const int PawnValue   = S(  90, 135);
const int KnightValue = S( 417, 442);
const int BishopValue = S( 422, 459);
const int RookValue   = S( 598, 734);
const int QueenValue  = S(1170,1459);
...
const int PawnPSQT[SQUARE_NB] = {
    ...
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S( -12,   7), S(  -1,   8), S(  -9,   3), S(  -4,  -1),
    S(  -5,   3), S(  -7,   6), S(   6,   5), S( -11,   7),
    S( -19,   6), S( -14,  10), S( -10,  -6), S(  -5, -14),
    S(   0, -13), S( -12,  -3), S(  -7,   8), S( -21,   6),
    S( -13,  12), S(  -9,  15), S(   2, -12), S(   7, -25),
    S(   9, -26), S(   3,  -7), S(  -7,  15), S( -16,  13),
    S(  -7,  18), S(  -7,  14), S( -13,  -8), S(  -8, -25),
    S(  -6, -24), S( -12,  -7), S(  -8,  16), S( -10,  16),
    S( -11,  39), S(  -4,  38), S(  -5,  20), S(  13,  -7),
    S(  15,  -5), S(  -6,  24), S(  -7,  41), S( -17,  39),
    S(  -1, -54), S( -30, -11), S(  11, -27), S(  50, -41),
    S(  43, -34), S(  10, -23), S( -44,  -1), S( -13, -43),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT[SQUARE_NB] = {
    S( -29, -32), S(  -7, -23), S( -15, -21), S( -13,  -3),
    S(  -8,  -3), S( -19, -18), S(  -6, -19), S( -35, -28),
    S(   0,  -3), S(  -7,   0), S(  -2, -19), S(   1,  -3),
    S(   2,  -2), S(  -6, -17), S(  -7,  -4), S(  -5,   0),
    S(   7, -19), S(   8,  -6), S(   9,   1), S(  10,  14),
    S(  10,  14), S(   6,   1), S(   7,  -5), S(   4, -19),
    S(  14,  18), S(  13,  27), S(  22,  34), S(  22,  44),
    S(  21,  46), S(  19,  36), S(  12,  25), S(  11,  21),
    S(   9,  26), S(  19,  27), S(  31,  44), S(  25,  61),
    S(  21,  60), S(  32,  43), S(  18,  29), S(   7,  25),
    S( -24,  23), S(  -7,  32), S(  19,  50), S(  14,  53),
    S(  19,  48), S(  19,  49), S( -15,  32), S( -25,  23),
    S(  11,  -3), S(  -9,  13), S(  26,  -6), S(  31,  17),
    S(  29,  19), S(  33,  -7), S( -15,  12), S(   0,  -5),
    S(-155, -12), S( -85,   6), S(-110,  35), S( -38,  12),
    S( -26,  12), S(-100,  37), S(-113,  17), S(-158, -24),
};

const int BishopPSQT[SQUARE_NB] = {
    S(   5, -21), S(   0,  -4), S(  -5,   2), S(   0,   0),
    S(   1,   5), S(  -6,  -2), S(  -1,  -1), S(   5, -24),
    S(  21, -17), S(   4, -29), S(  13,  -5), S(   7,   4),
    S(   7,   5), S(  12,  -7), S(   8, -29), S(  21, -24),
    S(   9,   0), S(  18,   5), S(  -3,  -5), S(  16,  13),
    S(  15,  14), S(  -3,  -7), S(  18,   2), S(  14,   4),
    S(  -1,   9), S(   9,  13), S(  14,  26), S(  14,  27),
    S(  17,  29), S(  10,  25), S(  13,  13), S(   0,  10),
    S( -16,  29), S(  11,  26), S(  -1,  32), S(  12,  39),
    S(   6,  40), S(   1,  32), S(   9,  28), S( -16,  31),
    S(  -8,  25), S( -13,  41), S(  -6,  20), S(  -1,  34),
    S(   4,  32), S( -17,  27), S( -13,  40), S( -10,  28),
    S( -52,  30), S( -42,  16), S( -10,  23), S( -30,  28),
    S( -32,  28), S( -11,  25), S( -56,  15), S( -55,  30),
    S( -57,  12), S( -58,  27), S(-108,  37), S( -99,  46),
    S(-103,  44), S( -85,  34), S( -32,  16), S( -67,  12),
};

const int RookPSQT[SQUARE_NB] = {
    S( -26,  -4), S( -18,   2), S( -13,   2), S(  -7,  -5),
    S(  -7,  -5), S( -11,   2), S( -14,  -1), S( -20, -12),
    S( -67,   3), S( -24, -10), S( -19,  -8), S( -12, -11),
    S( -11, -12), S( -18, -12), S( -18, -13), S( -76,   5),
    S( -38,   1), S( -18,  11), S( -25,   6), S( -14,   0),
    S( -13,   1), S( -27,   7), S( -12,  10), S( -38,   0),
    S( -30,  18), S( -20,  31), S( -21,  30), S(  -8,  22),
    S(  -9,  22), S( -21,  31), S( -16,  30), S( -31,  21),
    S( -16,  38), S(   3,  32), S(  11,  32), S(  29,  25),
    S(  27,  26), S(   7,  31), S(   9,  29), S( -14,  38),
    S( -27,  49), S(  16,  32), S(  -2,  45), S(  25,  31),
    S(  26,  29), S(  -3,  46), S(  22,  29), S( -27,  50),
    S( -15,  32), S( -20,  39), S(  -3,  31), S(  11,  30),
    S(   9,  30), S(  -2,  30), S( -22,  42), S( -13,  33),
    S(  26,  45), S(  15,  53), S( -10,  63), S(  -2,  58),
    S(  -2,  60), S(  -9,  62), S(  21,  53), S(  27,  48),
};

const int QueenPSQT[SQUARE_NB] = {
    S(  18, -29), S(   4, -23), S(   9, -31), S(  16, -15),
    S(  16, -14), S(  12, -39), S(   8, -23), S(  21, -37),
    S(   5,  -8), S(  13, -19), S(  19, -38), S(  12,   3),
    S(  15,   1), S(  21, -49), S(  16, -28), S(   2, -14),
    S(   4,   3), S(  17,  11), S(   2,  34), S(  -1,  31),
    S(   0,  32), S(   2,  33), S(  19,   7), S(   9, -13),
    S(   7,  19), S(  11,  44), S(  -6,  55), S( -21, 100),
    S( -19,  96), S(  -9,  52), S(  14,  40), S(   3,  25),
    S(  -8,  41), S(  -9,  73), S( -20,  59), S( -31, 110),
    S( -35, 114), S( -25,  63), S( -14,  82), S( -16,  56),
    S( -26,  52), S( -23,  47), S( -30,  59), S( -18,  59),
    S( -20,  57), S( -27,  50), S( -30,  56), S( -38,  66),
    S( -11,  53), S( -61,  96), S( -14,  51), S( -48,  95),
    S( -53, 100), S( -17,  47), S( -64, 103), S( -11,  60),
    S(   6,  31), S(  18,  33), S(  -6,  63), S(  -3,  59),
    S( -11,  69), S(   6,  46), S(  15,  56), S(  15,  37),
};

const int KingPSQT[SQUARE_NB] = {
    S(  53, -80), S(  45, -53), S(  -9, -13), S( -10, -36),
    S( -14, -36), S(  -7, -15), S(  45, -53), S(  53, -83),
    S(  27, -20), S( -17, -14), S( -44,   7), S( -70,  12),
    S( -68,  14), S( -46,  10), S( -17, -14), S(  25, -21),
    S(  -9, -28), S(   3, -25), S(   3,   1), S( -22,  20),
    S( -16,  20), S(   7,   0), S(   2, -25), S( -10, -26),
    S(  -6, -35), S(  86, -35), S(  54,   6), S( -23,  33),
    S(  -8,  32), S(  45,   7), S(  81, -34), S( -28, -30),
    S(  26, -13), S( 110, -26), S(  56,  18), S( -11,  29),
    S(  -5,  29), S(  57,  17), S( 100, -27), S(  -9, -10),
    S(  58, -25), S( 135,  -9), S( 106,  10), S(  36,   7),
    S(  20,  11), S(  98,   8), S( 133, -13), S(  50, -25),
    S(  31, -62), S(  60,  -4), S(  45,   6), S(  29,  -9),
    S( -17,   1), S(  42,   2), S(  64,  -6), S(  32, -66),
    S( -11,-134), S(  67, -66), S(  15, -42), S( -12, -19),
    S( -38, -26), S( -24, -30), S(  65, -73), S( -75,-113),
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -28, -16), S( -18,  15), S( -19,  29),
    S(  -7,  60), S(  30,  57), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -15,  18), S(  -5,  25), S(   2,  51),
    S(  24, 106), S(  53,  68), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated[FILE_NB] = {
    S(  -4,  -7), S(  -4, -15), S(  -5, -12), S(  -5, -14),
    S(  -7, -14), S(  -4, -14), S(  -6, -14), S(  -3,  -9),
};

const int PawnStacked[2][FILE_NB] = {
   {S(  -4, -32), S(  -2, -24), S(   3, -24), S(   2, -20),
    S(   1, -19), S(  -1, -24), S(  -1, -27), S(   4, -34)},
   {S(  -8, -16), S(  -3, -13), S(  -5,  -7), S(  -6,  -5),
    S(  -7,  -5), S(  -3,  -9), S(  -3, -14), S(  -4, -17)},
};

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   4,  -4), S(   9,  -7), S(  10, -16),
    S(  10, -23), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -14, -27), S(  -5, -25), S(   2, -26),
    S(  37, -36), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -2,  -7), S(  10,  -4), S(   3,   1), S(   7,   9),
    S(  11,   1), S(  22,  -4), S(  25,   4), S(  25,  11),
    S(   7,  -1), S(  18,   0), S(  10,   5), S(  16,  14),
    S(   6,  14), S(  18,  18), S(  28,  20), S(  31,  17),
    S(  37,  33), S(  35,  59), S(  59,  63), S(  67,  75),
    S( 113,  41), S( 214,  34), S( 185,  66), S( 212,  62),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightInSiberia[4] = {
    S(  -9,  -5), S( -11, -18), S( -22, -16), S( -37, -15),
};

const int KnightMobility[9] = {
    S( -91,-136), S( -45,-110), S( -25, -46), S( -12, -12),
    S(   1,  -2), S(   5,  15), S(  12,  18), S(  22,  17),
    S(  35,  -1),
};
...
const int BishopMobility[14] = {
    S( -86,-177), S( -43,-116), S( -16, -57), S(  -5, -22),
    S(   5, -10), S(  11,   6), S(  14,  19), S(  14,  22),
    S(  13,  31), S(  21,  30), S(  21,  30), S(  43,  16),
    S(  47,  29), S(  70, -13),
};
...
const int RookOnSeventh = S(  -1,  36);

const int RookMobility[15] = {
    S(-119,-152), S( -52,-109), S( -24, -74), S( -12, -27),
    S( -10,  -2), S( -14,  20), S( -12,  33), S(  -5,  36),
    S(   2,  41), S(   6,  43), S(   8,  52), S(  16,  55),
    S(  15,  60), S(  32,  48), S(  91,   6),
};
...
const int QueenMobility[28] = {
    S(-143,-292), S(-252,-422), S(-108,-224), S( -38,-215),
    S( -17,-158), S(  -7, -83), S(  -1, -36), S(   0,  -5),
    S(   6,   2), S(   8,  21), S(  14,  26), S(  16,  42),
    S(  19,  34), S(  21,  44), S(  19,  45), S(  17,  50),
    S(  19,  48), S(  12,  49), S(   9,  46), S(  13,  33),
    S(  21,  14), S(  34, -10), S(  30, -30), S(  23, -44),
    S(  -6, -48), S(  -8, -77), S( -29, -48), S( -10, -34),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -29,  -4), S( -13,   3), S(   1,   6), S(  11,   9),
    S(  19,   9), S(  30,  -1), S(  34, -13), S(  11, -27),
    S(  11,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  46,  47), S(  28,  35), S(  11,  17), S( -24, -24),
    S( -27, -66), S( -23, -78), S( -20, -83), S(  -8, -73),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -5,  -2), S(  15, -28), S(  15,  -4), S(  12,   2),
    S(   6,  -9), S( -21,   5), S( -34, -21), S( -38,  24)},
   {S(  15,  -7), S(  10, -15), S(  -4,  -4), S(  -6,  -3),
    S( -16,   4), S( -52,  62), S(  74,  92), S( -36,   7)},
   {S(  37,  -2), S(  10,  -7), S( -28,   3), S( -13, -15),
    S( -14,  -9), S( -21,   9), S(  20,  70), S( -19,   0)},
   {S(  19,   9), S(  21, -15), S(   4, -16), S(  10, -22),
    S(  18, -33), S( -65,   9), S(-140,  41), S(  -1,  -4)},
   {S(  -2,  12), S(  12,  -5), S( -28,   3), S( -12,   3),
    S( -12,  -6), S( -39,  -8), S(  68, -47), S(  -8,   2)},
   {S(  43, -13), S(  11, -11), S( -20,   1), S( -10, -26),
    S(   4, -36), S(  18, -18), S(  47, -39), S( -25,   4)},
   {S(  31, -18), S(   0, -22), S( -31,  -3), S( -17, -17),
    S( -29,  -3), S( -26,  27), S(  30,  56), S( -19,   3)},
   {S(   4, -17), S(   6, -23), S(   8,   1), S(  -1,  11),
    S( -14,  21), S( -21,  48), S(-163,  73), S( -19,  19)}},
  {{S(   0,   0), S( -10, -33), S(   6, -24), S( -43,  10),
    S( -33,   8), S( -36,  59), S(-165,  -4), S( -32,  16)},
   {S(   0,   0), S(  16, -20), S(   5, -13), S( -21,  -5),
    S(  -8, -20), S(  22,  67), S(-175,  37), S( -39,  13)},
   {S(   0,   0), S(  19,  -9), S(   3, -12), S(   3, -25),
    S(  13,  -8), S( -83,  52), S(-101, -68), S( -18,   0)},
   {S(   0,   0), S(   4,   3), S(  -1,  -5), S( -13,   6),
    S( -20,   6), S( -91,  33), S( -54, -39), S( -17,  -1)},
   {S(   0,   0), S(   9,  -2), S(   7, -11), S(   7, -15),
    S(   6, -24), S( -63,   4), S(-124, -36), S(  -7,  -2)},
   {S(   0,   0), S(   1,  -9), S( -15,  -3), S( -14, -12),
    S(  19, -26), S( -42,  11), S(  46,  46), S( -14,  -3)},
   {S(   0,   0), S(  26, -22), S(  15, -20), S(  -7, -18),
    S( -23,   3), S( -17,  21), S( -19, -32), S( -34,  15)},
   {S(   0,   0), S(  15, -49), S(  17, -33), S( -19,  -9),
    S( -34,  27), S( -21,  32), S(-194, -32), S( -22,   8)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -22,  34), S( 119, -10), S( -22,  21), S( -15,   2),
    S( -17,  -1), S(  -9,  -2), S( -15,   8), S( -26,  -1)},
   {S( -21,  58), S(  66,  12), S( -13,  16), S(  -3,   9),
    S(  -2,   7), S(   7,  -2), S(   1,   3), S( -16,   9)},
   {S(  -2,  42), S(  16,  24), S( -15,  14), S( -11,   6),
    S(   1,   4), S(   9,   0), S(   8,  -9), S(   4,   6)},
   {S(   1,  21), S(  12,  17), S( -25,   6), S( -20,  -3),
    S( -13,   0), S(   7, -11), S(  -1, -10), S( -13,   7)}},
  {{S(   0,   0), S( -28, -37), S( -14,  -2), S(  20, -20),
    S(   6,  -2), S(  10, -23), S(   6,   9), S(  24,  34)},
   {S(   0,   0), S(   5, -55), S(   0, -12), S(  43, -11),
    S(   2,   4), S(  18, -25), S(   7, -12), S( -45,  -9)},
   {S(   0,   0), S( -44, -67), S( -15, -12), S(  12,  -7),
    S(   9,   1), S(  -5, -12), S(   2, -27), S( -19,   2)},
   {S(   0,   0), S(  10, -34), S( -18, -19), S( -14,   2),
    S(  -4,  -8), S(   2, -33), S(  69, -26), S(  -7,  38)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -41,   4), S( -46,  34), S( -67,  35),
    S(   5,  22), S(  90,  -4), S( 169,  52), S(   0,   0)},
   {S(   0,   0), S( -31,  19), S( -45,  48), S( -61,  49),
    S(  -5,  55), S( 115,  43), S( 226,  91), S(   0,   0)}},
  {{S(   0,   0), S( -30,  33), S( -50,  43), S( -65,  59),
    S(   3,  63), S( 111,  63), S( 277, 117), S(   0,   0)},
   {S(   0,   0), S( -31,  26), S( -44,  40), S( -62,  63),
    S(   2,  85), S( 113, 146), S( 175, 281), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(  -2,   2), S(   2,  -5), S(   8, -13),
    S(   6, -19), S( -10, -18), S( -20,  -8), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   4,  -3), S(   5,  -2), S(   7,   8),
    S(   1,  22), S(   3,  34), S(  20,  35), S(   0,   0),
};

const int PassedSafePromotionPath = S( -35,  51);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -11, -35);
const int ThreatMinorAttackedByPawn  = S( -51, -72);
...
const int ThreatMinorAttackedByKing  = S( -36, -23);
const int ThreatRookAttackedByKing   = S( -31, -19);
const int ThreatQueenAttackedByOne   = S( -46, -11);
const int ThreatOverloadedPieces     = S(  -6, -15);
const int ThreatByPawnPush           = S(  14,  30);

/* Space Evaluation Terms */

const int SpaceRestrictPiece = S(  -3,  -1);
const int SpaceRestrictEmpty = S(  -4,  -2);
const int SpaceCenterControl = S(   5,  -2);

/* Closedness Evaluation Terms */

const int ClosednessKnightAdjustment[9] = {
    S( -13,  -3), S( -13,  15), S( -15,  21), S( -12,  22),
    S(  -9,  28), S(  -8,  20), S(  -6,  17), S( -17,  33),
    S( -14,  16),
};

const int ClosednessRookAdjustment[9] = {
    S(  38,  10), S(  -8,  43), S(  -4,  25), S( -11,  14),
    S( -14,  10), S( -13,  -4), S( -17, -13), S( -26,  -9),
    S( -41, -32),
};
