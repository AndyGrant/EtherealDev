...
const int PawnValue   = S( 111, 135);
const int KnightValue = S( 473, 425);
const int BishopValue = S( 497, 444);
const int RookValue   = S( 696, 739);
const int QueenValue  = S(1326,1423);
...
const int PawnPSQT32[32] = {
    ...
    S( -20,  11), S(   6,   3), S( -16,   8), S( -10,  -3),
    S( -21,   3), S( -14,   3), S( -10,  -8), S(  -6, -15),
    S( -17,  13), S( -11,  13), S(  16, -15), S(  14, -28),
    S(  -1,  20), S(   6,  13), S(   3,  -1), S(  19, -22),
    S(  -1,  35), S(   3,  35), S(  14,  23), S(  43,  -4),
    S( -17, -40), S( -65,  -7), S(   3, -21), S(  40, -34),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -50, -26), S(  -7, -44), S( -19, -30), S(  -3, -20),
    S(  -8, -22), S(   4, -13), S(  -2, -31), S(  11, -21),
    S(   2, -24), S(  22, -25), S(  13, -19), S(  24,  -3),
    S(  20,   4), S(  25,   7), S(  32,  17), S(  29,  23),
    S(  29,  18), S(  27,  13), S(  41,  26), S(  31,  40),
    S( -13,  16), S(   8,  14), S(  33,  28), S(  35,  31),
    S(   8, -10), S(  -3,   6), S(  40, -17), S(  44,   3),
    S(-166, -16), S( -81,  -2), S(-110,  20), S( -30,   3),
};

const int BishopPSQT32[32] = {
    S(  18, -17), S(  17, -19), S( -11,  -9), S(   8, -13),
    S(  34, -32), S(  27, -33), S(  25, -22), S(  11, -12),
    S(  18, -14), S(  32, -14), S(  17,  -4), S(  21,  -2),
    S(  19, -10), S(  18,  -2), S(  18,   7), S(  21,  11),
    S( -10,   9), S(  19,   4), S(   9,  13), S(  12,  22),
    S(   1,   5), S(   0,  15), S(  18,  12), S(  22,  10),
    S( -47,  14), S( -35,  12), S(  -2,   6), S( -19,   9),
    S( -39,   3), S( -48,   9), S( -87,  17), S( -91,  25),
};

const int RookPSQT32[32] = {
    S(  -5, -31), S( -12, -20), S(   1, -25), S(   8, -30),
    S( -54, -13), S( -15, -30), S(  -9, -30), S(  -1, -33),
    S( -27, -13), S(  -6, -12), S( -16, -15), S(  -3, -23),
    S( -14,   2), S(  -5,   8), S(  -6,   4), S(   5,  -3),
    S(   2,  14), S(  17,  10), S(  27,   6), S(  37,   0),
    S(  -5,  24), S(  31,  12), S(   6,  21), S(  37,   6),
    S(   4,  13), S( -16,  21), S(   9,  10), S(  21,   9),
    S(  37,  24), S(  26,  27), S(   6,  32), S(  16,  27),
};

const int QueenPSQT32[32] = {
    S(  19, -52), S(   2, -38), S(  10, -48), S(  21, -43),
    S(  16, -38), S(  30, -56), S(  34, -70), S(  22, -25),
    S(  14, -20), S(  29, -17), S(  11,   8), S(  11,   4),
    S(  15,   0), S(  17,  20), S(   2,  25), S( -14,  70),
    S(  -3,  21), S(  -3,  45), S( -11,  26), S( -29,  77),
    S( -19,  30), S( -10,  22), S( -18,  27), S(  -7,  31),
    S(  -4,  28), S( -59,  67), S(  -6,  20), S( -38,  57),
    S(  -3,  21), S(  20,  12), S(  11,  13), S(  -5,  22),
};

const int KingPSQT32[32] = {
    S(  40, -85), S(  40, -55), S( -10, -18), S( -25, -27),
    S(  29, -37), S(  -4, -26), S( -38,   1), S( -51,   3),
    S(   8, -35), S(  18, -32), S(  16,  -8), S( -10,   7),
    S(   2, -36), S(  82, -40), S(  40,  -1), S(  -6,  19),
    S(   4, -18), S(  95, -28), S(  46,  12), S(   2,  22),
    S(  47, -15), S( 120, -12), S(  95,  14), S(  39,  13),
    S(   7, -36), S(  48,   0), S(  33,  16), S(  10,   9),
...
const int PawnStacked = S( -21, -26);

const int PawnBackwards[2] = { S(   9,  -3), S(  -6, -19) };

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -2,  -8), S(  11,   1), S(   4,   1), S(   5,  18),
    S(  15,   0), S(  35,  -1), S(  23,  11), S(  27,  20),
    S(  11,   1), S(  23,   5), S(  11,  13), S(  15,  25),
    S(  17,   8), S(  25,  15), S(  32,  22), S(  35,  23),
...
const int KnightOutpost[2] = { S(   7, -24), S(  31,  -2) };

const int KnightBehindPawn = S(   5,  20);

const int KnightMobility[9] = {
    S( -78,-104), S( -27, -99), S( -12, -41), S(   2, -15),
    S(  13,  -2), S(  20,  15), S(  29,  18), S(  38,  20),
    S(  49,   9),
};
...
const int BishopOutpost[2] = { S(  10, -11), S(  42,   3) };

const int BishopBehindPawn = S(   4,  19);

const int BishopMobility[14] = {
    S( -64,-147), S( -25, -94), S(  -5, -55), S(   5, -29),
    S(  14, -15), S(  21,   1), S(  24,  11), S(  26,  16),
    S(  26,  21), S(  29,  23), S(  28,  24), S(  43,  14),
    S(  42,  19), S(  71,  -9),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  20,   7), S(  40,   4) };

const int RookOnSeventh = S(   1,  32);

const int RookMobility[15] = {
    S(-148,-113), S( -47,-113), S(  -8, -59), S(  -1, -16),
    S(  -1,   5), S(  -2,  21), S(  -1,  31), S(   5,  34),
    S(  12,  37), S(  16,  41), S(  19,  47), S(  23,  51),
    S(  24,  55), S(  31,  50), S(  74,  23),
};

/* Queen Evaluation Terms */

const int QueenMobility[28] = {
    S( -61,-263), S(-210,-387), S( -59,-201), S( -12,-192),
    S(   1,-139), S(   5, -85), S(  10, -53), S(   9, -26),
    S(  12, -15), S(  13,   7), S(  15,  19), S(  16,  32),
    S(  18,  33), S(  19,  42), S(  17,  47), S(  15,  50),
    S(  13,  55), S(   8,  55), S(   9,  51), S(   5,  46),
    S(  12,  27), S(  27,   3), S(  31, -27), S(  32, -46),
...
const int KingDefenders[12] = {
    S( -24,  -4), S( -10,  -1), S(   0,   2), S(   8,   5),
    S(  17,   6), S(  27,   4), S(  33,  -2), S(  13,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -12,   4), S(  19, -25), S(  20, -10), S(  11,   3),
    S(   5,   3), S(   2,   2), S(  -3, -33), S( -50,  19)},
   {S(  17,  -7), S(  25, -19), S(   0,  -5), S( -15,   4),
    S( -31,  15), S( -70,  69), S(  92,  82), S( -30,   3)},
   {S(  35,  -3), S(  17, -10), S( -27,   7), S( -12,  -8),
    S( -21,  -4), S( -10,   1), S(   0,  66), S( -15,  -1)},
   {S(   4,  12), S(  21, -10), S(   5, -12), S(  16, -23),
    S(  24, -36), S( -56,   5), S(-136,  52), S(   4,  -6)},
   {S( -14,   8), S(   3,  -4), S( -25,   1), S( -19,   4),
    S( -21,  -6), S( -40,   0), S(  33, -17), S(  -8,  -2)},
   {S(  46, -16), S(  26, -19), S( -22,   1), S( -13, -19),
    S(   5, -25), S(  18, -22), S(  41, -30), S( -26,   1)},
   {S(  25, -11), S(   3, -18), S( -23,  -4), S( -19, -10),
    S( -31,  -8), S( -35,  32), S(   0,  44), S( -13,   3)},
   {S(  -9, -10), S(   8, -18), S(   9,  -3), S(  -3,   8),
  ...
  {{S(   0,   0), S( -11, -22), S(   2, -18), S( -41,  16),
   ...
   {S(   0,   0), S(  11,   3), S(  13,  -6), S(  14, -10),
    S(  14, -26), S( -57,  15), S(-104, -61), S(  -2,  -7)},
   {S(   0,   0), S(  11,  -8), S( -20,   0), S( -30,  -7),
    S(  17, -24), S( -38,   3), S(  55,  38), S( -23,  -5)},
   {S(   0,   0), S(  26, -14), S(  13, -13), S(  -8, -10),
    S( -29,   7), S(  -9,  15), S( -56, -49), S( -35,  13)},
   {S(   0,   0), S(  16, -38), S(  19, -28), S( -18,  -5),
    S( -17,  18), S(  -4,  19), S(-228, -55), S( -22,   3)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S(  -3,  26), S( 117,  -8), S( -26,  25), S( -23,  11),
    S( -14,   3), S(  -8,  -3), S( -19,   7), S( -21,  -3)},
   {S(  -2,  47), S(  58,  14), S( -20,  25), S(  -6,  12),
    S(  -3,   5), S(   6,  -4), S(   1,   0), S( -11,   0)},
   {S(   9,  36), S(  18,  24), S( -26,  23), S( -12,  11),
    S(   4,   3), S(   9,   0), S(  10,  -8), S(   4,  -1)},
   {S(  -2,  23), S(  17,  21), S( -17,  10), S( -15,   4),
    S( -12,   2), S(   7, -10), S(   0,  -7), S( -15,   3)}},
  {{S(   0,   0), S( -14, -16), S( -16,  -2), S(  21, -17),
    S(  13,  -9), S(   1, -19), S(  -5,  -2), S(  17,  29)},
   {S(   0,   0), S( -16, -33), S(  -6, -11), S(  33, -14),
    S(  -1,  -2), S(  13, -24), S(  -7, -10), S( -17,   2)},
   {S(   0,   0), S( -27, -48), S( -30,  -8), S(  11, -11),
    S(   3,  -2), S(  -9, -14), S( -12, -15), S( -11,   6)},
   {S(   0,   0), S(  -2, -18), S( -18, -19), S( -11,  -4),
    S(  -3,  -7), S(   6, -27), S(  73, -10), S(  13,  20)}},
};
...
const int ThreatMinorAttackedByPawn  = S( -55, -48);
const int ThreatMinorAttackedByMinor = S( -27, -37);
const int ThreatMinorAttackedByMajor = S( -25, -47);
const int ThreatRookAttackedByLesser = S( -59, -12);
const int ThreatQueenAttackedByOne   = S( -46, -15);
