...
const int PawnValue   = S( 102, 114);
const int KnightValue = S( 437, 394);
const int BishopValue = S( 458, 410);
const int RookValue   = S( 652, 671);
const int QueenValue  = S(1268,1331);
...
const int PawnPSQT32[32] = {
    ...
    S( -18,   9), S(   6,   4), S( -11,   7), S(  -5,  -1),
    S( -20,   4), S( -10,   4), S(  -8,  -4), S(  -2, -12),
    S( -16,  12), S(  -9,  11), S(  14, -12), S(  11, -23),
    S(  -4,  16), S(   4,  11), S(   0,  -2), S(  14, -19),
    S(  -4,  31), S(   1,  27), S(   8,  18), S(  36,  -9),
    S( -16, -40), S( -65, -11), S(   2, -25), S(  40, -40),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -49, -26), S(  -7, -39), S( -15, -29), S(   0, -19),
    S(  -5, -22), S(   3, -13), S(   1, -31), S(  11, -20),
    S(   3, -26), S(  20, -25), S(  14, -18), S(  25,  -5),
    S(  14,   4), S(  22,   6), S(  28,  16), S(  29,  21),
    S(  22,  17), S(  25,  11), S(  37,  25), S(  29,  37),
    S( -15,  15), S(   5,  12), S(  31,  26), S(  31,  28),
    S(   6, -11), S(  -6,   1), S(  32, -21), S(  42,  -2),
    S(-170, -18), S( -81,  -2), S(-110,  18), S( -30,   0),
};

const int BishopPSQT32[32] = {
    S(  17, -19), S(  14, -20), S( -10, -10), S(   8, -14),
    S(  30, -34), S(  23, -32), S(  22, -23), S(  11, -13),
    S(  15, -14), S(  28, -16), S(  17,  -8), S(  21,  -7),
    S(  15,  -9), S(  17,  -3), S(  15,   4), S(  21,   7),
    S( -12,  10), S(  17,   3), S(   7,  11), S(  11,  18),
    S(   0,   6), S(   0,  15), S(  14,  12), S(  18,   9),
    S( -44,  15), S( -36,  11), S(  -6,   4), S( -21,   8),
    S( -40,   2), S( -48,   7), S( -87,  15), S( -90,  23),
};

const int RookPSQT32[32] = {
    S( -11, -28), S( -15, -19), S(  -3, -24), S(   6, -29),
    S( -54, -13), S( -14, -30), S(  -9, -30), S(   1, -34),
    S( -29, -13), S(  -8, -11), S( -16, -16), S(  -2, -24),
    S( -17,  -2), S(  -8,   5), S(  -6,   1), S(   5,  -5),
    S(  -2,   6), S(  13,   4), S(  23,   0), S(  37,  -5),
    S( -13,  16), S(  26,   3), S(   2,  15), S(  33,   0),
    S(   2,   6), S( -17,  14), S(   8,   4), S(  22,   5),
    S(  33,  20), S(  24,  22), S(   6,  26), S(  16,  23),
};

const int QueenPSQT32[32] = {
    S(  27, -53), S(   7, -41), S(  12, -51), S(  19, -42),
    S(  14, -39), S(  27, -57), S(  29, -73), S(  18, -28),
    S(  10, -24), S(  25, -23), S(   8,   2), S(   9,  -4),
    S(  11,  -7), S(  13,  13), S(  -1,  16), S( -15,  56),
    S(  -9,  14), S(  -7,  38), S( -17,  17), S( -31,  66),
    S( -26,  25), S( -18,  16), S( -27,  19), S( -16,  21),
    S(  -7,  25), S( -61,  61), S( -12,  12), S( -44,  49),
    S( -10,  14), S(  16,   6), S(   7,   6), S(  -8,  17),
};

const int KingPSQT32[32] = {
    S(  40, -75), S(  39, -48), S( -11, -12), S( -27, -19),
    S(  30, -28), S(  -3, -18), S( -36,   8), S( -50,  10),
    S(   8, -28), S(  16, -24), S(  16,  -2), S(  -9,  13),
    S(   3, -34), S(  84, -35), S(  41,   4), S(  -6,  23),
    S(   5, -19), S(  96, -30), S(  47,  11), S(   3,  20),
    S(  46, -25), S( 121, -22), S(  95,   4), S(  39,   3),
    S(   5, -47), S(  46,  -9), S(  31,   6), S(   9,  -2),
    S(   8,-101), S(  74, -54), S( -20, -14), S( -19, -12),
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -26,  -9), S( -11,  10), S( -15,  27),
    S(  -2,  58), S(  41,  64), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -15,  18), S(  -5,  19), S(   4,  42),
    S(  16,  79), S(  31,  53), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -7, -11);

const int PawnStacked = S( -17, -22);

const int PawnBackwards[2] = { S(   7,   0), S(  -6, -19) };

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -2,  -7), S(  11,   1), S(   3,   1), S(   5,  15),
    S(  14,  -1), S(  28,  -2), S(  20,   7), S(  23,  14),
    S(   9,   0), S(  22,   4), S(  10,  10), S(  15,  19),
    S(  13,   8), S(  21,  13), S(  27,  21), S(  31,  19),
    S(  56,  23), S(  49,  46), S(  66,  52), S(  82,  56),
    S( 112,   0), S( 204,   9), S( 228,  31), S( 240,  49),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

/* Knight Evaluation Terms */

const int KnightOutpost[2] = { S(   7, -26), S(  30,  -4) };

const int KnightBehindPawn = S(   4,  19);

const int KnightMobility[9] = {
    S( -77,-105), S( -35,-100), S( -21, -46), S(  -9, -22),
    S(   2, -12), S(   8,   5), S(  16,   7), S(  25,   7),
    S(  37,  -7),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  22,  67);

const int BishopRammedPawns = S( -10, -14);

const int BishopOutpost[2] = { S(  10, -12), S(  39,   0) };

const int BishopBehindPawn = S(   3,  17);

const int BishopMobility[14] = {
    S( -66,-147), S( -33, -95), S( -14, -57), S(  -4, -32),
    S(   6, -20), S(  13,  -6), S(  17,   4), S(  17,   8),
    S(  17,  14), S(  21,  15), S(  20,  16), S(  40,   4),
    S(  42,  15), S(  71, -18),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  15,   4), S(  34,   4) };

const int RookOnSeventh = S(  -4,  26);

const int RookMobility[15] = {
    S(-148,-113), S( -55,-113), S( -18, -63), S( -11, -23),
    S( -11,  -4), S( -11,  10), S( -10,  20), S(  -4,  23),
    S(   3,  26), S(   6,  30), S(   9,  35), S(  14,  38),
    S(  16,  42), S(  27,  32), S(  83,  -5),
};

/* Queen Evaluation Terms */

const int QueenMobility[28] = {
    S( -61,-263), S(-210,-387), S( -57,-201), S( -18,-192),
    S(  -7,-139), S(  -4, -82), S(   1, -52), S(   1, -24),
    S(   5, -20), S(   6,   0), S(   9,   6), S(  10,  22),
    S(  12,  16), S(  14,  24), S(  12,  27), S(   7,  33),
    S(   7,  32), S(  -1,  34), S(   0,  31), S(  -6,  26),
    S(   2,   5), S(  17, -17), S(  22, -43), S(  27, -58),
    S(  11, -72), S(  23, -98), S( -56, -40), S( -32, -63),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -24,   1), S(  -6,  -3), S(   1,   2), S(   8,   5),
    S(  16,   6), S(  26,   4), S(  30,  -2), S(  13,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -10,   2), S(  14, -25), S(  20,  -9), S(  12,   4),
    S(   7,   4), S(   0,   2), S(  -3, -33), S( -48,  17)},
   {S(  17,  -8), S(  19, -17), S(   0,  -4), S( -14,   4),
    S( -29,  15), S( -70,  67), S(  92,  82), S( -22,   1)},
   {S(  35,  -4), S(  14,  -9), S( -27,   7), S( -11,  -8),
    S( -19,  -4), S( -12,   1), S(   0,  66), S( -11,  -2)},
   {S(   4,  10), S(  21, -11), S(   4, -11), S(  15, -22),
    S(  25, -36), S( -58,   6), S(-136,  52), S(   6,  -7)},
   {S( -15,   7), S(   4,  -4), S( -25,   1), S( -17,   5),
    S( -19,  -6), S( -42,  -2), S(  33, -18), S(  -5,  -2)},
   {S(  45, -18), S(  22, -17), S( -21,   2), S( -12, -18),
    S(   5, -24), S(  17, -20), S(  41, -30), S( -23,   1)},
   {S(  24, -13), S(  -2, -16), S( -22,  -2), S( -19,  -7),
    S( -30,  -7), S( -37,  32), S(   0,  44), S(  -8,   0)},
   {S(  -8, -12), S(   6, -18), S(   7,   0), S(  -3,  11),
    S( -12,  16), S( -10,  36), S(-190,  87), S( -16,  14)}},
  {{S(   0,   0), S( -11, -23), S(   5, -19), S( -40,  16),
    S( -23,   2), S(   3,  42), S(-167,  -8), S( -45,   7)},
   {S(   0,   0), S(  23, -21), S(   6,  -7), S( -18,  -1),
    S(  -1, -12), S(  25,  65), S(-184,  -3), S( -36,   2)},
   {S(   0,   0), S(  29, -11), S(  -2,  -7), S(   6, -17),
    S(  15,  -7), S( -86,  47), S( -84, -73), S( -19,  -5)},
   {S(   0,   0), S(  -4,   8), S(  -3,   0), S( -17,   3),
    S( -27,   1), S( -99,  31), S(   7, -41), S( -20,  -7)},
   {S(   0,   0), S(  12,   2), S(  11,  -5), S(  14, -11),
    S(  14, -26), S( -58,  15), S(-104, -61), S(   0,  -7)},
   {S(   0,   0), S(   6,  -8), S( -20,   0), S( -26,  -7),
    S(  17, -24), S( -38,   3), S(  55,  38), S( -16,  -6)},
   {S(   0,   0), S(  21, -15), S(  11, -13), S(  -9,  -7),
    S( -28,  10), S( -10,  15), S( -56, -48), S( -30,  10)},
   {S(   0,   0), S(  11, -37), S(  18, -27), S( -18,  -4),
    S( -18,  18), S(  -6,  21), S(-228, -55), S( -21,   1)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S(  -6,  29), S( 116,  -9), S( -25,  26), S( -19,   8),
    S( -14,   2), S(  -8,  -4), S( -17,   5), S( -22,  -1)},
   {S(  -5,  50), S(  57,  11), S( -17,  24), S(  -4,  11),
    S(  -4,   5), S(   5,  -4), S(  -1,   1), S( -11,   0)},
   {S(   6,  39), S(  16,  23), S( -20,  20), S( -10,   9),
    S(   3,   2), S(   7,   0), S(   9,  -7), S(   3,  -1)},
   {S(  -3,  25), S(  15,  21), S( -16,   9), S( -13,   2),
    S( -13,   2), S(   7, -11), S(   2,  -9), S( -13,   2)}},
  {{S(   0,   0), S( -16, -17), S( -17,   0), S(  17, -17),
    S(   8,  -6), S(   3, -18), S(  -2,  -1), S(  17,  30)},
   {S(   0,   0), S( -17, -36), S(  -2,  -8), S(  34, -12),
    S(  -1,  -3), S(  13, -22), S(  -6, -10), S( -17,   2)},
   {S(   0,   0), S( -29, -51), S( -25,  -6), S(  11, -11),
    S(   3,  -2), S(  -8, -14), S( -13, -15), S( -11,   6)},
   {S(   0,   0), S(  -2, -19), S( -15, -17), S( -11,  -4),
    S(  -4,  -7), S(   4, -25), S(  73, -12), S(  13,  20)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -38,   3), S( -55,  20), S( -81,  27),
    S(  -6,  12), S(  72,  -5), S( 160,  58), S(   0,   0)},
   {S(   0,   0), S( -30,   8), S( -53,  25), S( -73,  31),
    S( -12,  31), S(  89,  32), S( 182, 101), S(   0,   0)}},
  {{S(   0,   0), S( -25,  17), S( -51,  20), S( -73,  33),
    S(  -4,  36), S(  87,  39), S( 260, 111), S(   0,   0)},
   {S(   0,   0), S( -31,  14), S( -48,  19), S( -67,  38),
    S(   0,  49), S( 100, 103), S( 161, 251), S(   0,   0)}},
};

const int PassedFriendlyDistance[RANK_NB] = {
    S(   0,   0), S(   0,   0), S(   3,  -4), S(   7,  -9),
    S(   5, -13), S(  -9, -12), S( -16,  -8), S(   0,   0),
};

const int PassedEnemyDistance[RANK_NB] = {
    S(   0,   0), S(   3,   0), S(   5,   1), S(  10,   8),
    S(   2,  20), S(   8,  28), S(  24,  25), S(   0,   0),
};

const int PassedSafePromotionPath = S( -20,  32);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -12, -28);
const int ThreatMinorAttackedByPawn  = S( -48, -55);
const int ThreatMinorAttackedByMinor = S( -25, -36);
const int ThreatMinorAttackedByMajor = S( -22, -45);
const int ThreatRookAttackedByLesser = S( -46, -23);
const int ThreatQueenAttackedByOne   = S( -35, -41);
const int ThreatOverloadedPieces     = S(  -8, -12);
const int ThreatByPawnPush           = S(  15,  20);
