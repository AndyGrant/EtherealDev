...
const int KingShelter[4][8] = {
   {S(  -6,   4), S(   0,  -4), S(   3,   0), S(   2,   0), S(   0,   3), S(   7,  11), S(   3,  21), S(   0,   0)},
   {S( -18,   0), S(   5,   0), S(  -1,  -1), S( -12,  -1), S(  -7,   2), S(  13,  13), S(  15,   7), S(   0,   0)},
   {S(  -5,   0), S(  12,   4), S( -10,   0), S(  -4,  -1), S(   2,   0), S(   3,   2), S(  19,   7), S(   0,   0)},
   {S(  -4,  -1), S(   1,   2), S(   0,   0), S(   8,   0), S(   3,   0), S( -17,   5), S(  16,  10), S(   0,   0)},
};
const int KingStorm[2][4][8] = {
  {{S(  -4,  -1), S(  81,  59), S(  -2,  27), S(  -7,  10), S(   2,   2), S(   0,   0), S(   0,   1), S(   0,   0)},
   {S(   2,   0), S(  10,  38), S( -28,  23), S(  -1,   4), S(   5,   1), S(   1,   0), S(  -7,   0), S(   0,   0)},
   {S(   6,   1), S( -47,  19), S( -22,  17), S(  -5,   4), S(   4,   3), S(   9,   0), S(  -5,   0), S(   0,   0)},
   {S(   0,   0), S(  14,  36), S(  -2,   6), S(  -7,   1), S(  -4,   0), S(   8,   0), S(   2,  -2), S(   0,   0)}},
  {{S(   0,   0), S(   0,   0), S(  -4, -15), S(  10,  -1), S(  16,   0), S(  -2,   0), S(   6,   7), S(   0,   0)},
   {S(   0,   0), S(   0,   0), S( -16, -21), S(  14,  -2), S(  -3,  -4), S( -12,  -4), S(  -5,  -5), S(   0,   0)},
   {S(   0,   0), S(   0,   0), S( -10, -13), S(   0,   0), S(  -2,  -2), S( -12,  -6), S( -15,  -3), S(   0,   0)},
   {S(   0,   0), S(   0,   0), S( -13, -17), S(  -2,  -4), S(   7,  -5), S(   5,  -2), S(  16,   3), S(   0,   0)}},
};
...
int evaluateKings(EvalInfo* ei, Board* board, int colour){
    ...
    // Shelter and Storm are saved in the Pawn King Evaluation table
    if (ei->pkentry != NULL) return eval;

    // Evaluate King Shelter & Storm for our King's file, and adjacent ones
    for (file = MAX(0, kingFile - 1); file <= MIN(7, kingFile + 1); file++){

        // Get friendly and enemy Pawns on this file, at or above our King's rank
        myFilePawns    =    myPawns & Files[file] & ranksAtOrAboveMasks(colour, kingRank);
        enemyFilePawns = enemyPawns & Files[file] & ranksAtOrAboveMasks(colour, kingRank);

        // Rank of closest friendly Pawn to our King
        myRank = myFilePawns     ?
                 colour == WHITE ? relativeRankOf(WHITE, getlsb(myFilePawns))
                                 : relativeRankOf(BLACK, getmsb(myFilePawns)) : 0;

        // Rank of closest enemy Pawn to our King
        enemyRank = enemyFilePawns  ?
                    colour == WHITE ? relativeRankOf(WHITE, getlsb(enemyFilePawns))
                                    : relativeRankOf(BLACK, getmsb(enemyFilePawns)) : 0;
