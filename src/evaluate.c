...
void initializeEvalInfo(EvalInfo* ei, Board* board, PawnTable* ptable){
    ...
    ei->mobilityAreas[WHITE] = ~((ei->pawnAttacks[BLACK] & ~(~ei->kingAreas[WHITE] & whitePawns & ~ei->pawnAttacks[WHITE])) | (white & kings) | ei->blockedPawns[WHITE]);
    ei->mobilityAreas[BLACK] = ~((ei->pawnAttacks[WHITE] & ~(~ei->kingAreas[BLACK] & blackPawns & ~ei->pawnAttacks[BLACK])) | (black & kings) | ei->blockedPawns[BLACK]);
