...
const int PawnValue   = S(  96, 124);
const int KnightValue = S( 437, 413);
const int BishopValue = S( 455, 426);
const int RookValue   = S( 638, 684);
const int QueenValue  = S(1283,1363);
...
const int PawnPSQT32[32] = {
    ...
    S( -20,   5), S(  -1,   0), S( -14,   3), S( -11,  -3),
    S( -22,   1), S( -15,   2), S( -10,  -8), S(  -6, -14),
    S( -18,  10), S( -14,  10), S(   8, -13), S(   9, -26),
    S(  -8,  12), S(  -4,   8), S(  -7,  -7), S(   1, -22),
    S(  -7,  25), S(  -6,  21), S(   0,  10), S(  27, -16),
    S( -18, -47), S( -67, -17), S(   0, -32), S(  37, -46),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -48, -26), S(  -6, -33), S( -12, -27), S(   0, -17),
    S(  -2, -21), S(   5, -12), S(   2, -28), S(   9, -18),
    S(   8, -24), S(  20, -22), S(  13, -19), S(  21,  -5),
    S(  22,   3), S(  21,   6), S(  29,  13), S(  24,  20),
    S(  25,  16), S(  24,  10), S(  36,  22), S(  28,  32),
    S( -13,  15), S(   8,  12), S(  31,  24), S(  29,  27),
    S(  10, -10), S(  -6,   3), S(  32, -19), S(  42,   1),
    S(-171, -18), S( -80,  -1), S(-109,  20), S( -30,   1),
};

const int BishopPSQT32[32] = {
    S(   3, -26), S(  11, -17), S( -10,  -9), S(   7, -11),
    S(  28, -31), S(   8, -37), S(  19, -18), S(   9, -10),
    S(  14, -11), S(  25, -12), S(   1, -18), S(  18,  -2),
    S(  15,  -8), S(  16,  -2), S(  14,   5), S(  21,   7),
    S( -10,  11), S(  17,   4), S(   7,  12), S(  11,  17),
    S(   1,   6), S(  -1,  15), S(  -2,   1), S(  16,  11),
    S( -45,  15), S( -54,   2), S(  -7,   8), S( -21,  12),
    S( -52,  -6), S( -46,  10), S( -86,  18), S( -88,  28),
};

const int RookPSQT32[32] = {
    S( -10, -29), S( -14, -20), S(  -3, -24), S(   4, -27),
    S( -49, -13), S( -13, -31), S(  -9, -30), S(   0, -32),
    S( -27, -14), S(  -9, -12), S( -17, -15), S(  -3, -23),
    S( -18,  -2), S(  -9,   6), S(  -9,   2), S(   2,  -4),
    S(  -5,  11), S(  11,   8), S(  19,   4), S(  31,  -2),
    S( -12,  21), S(  25,   7), S(   2,  19), S(  32,   3),
    S(   1,   7), S( -16,  17), S(   7,   6), S(  20,   6),
    S(  33,  17), S(  24,  20), S(   5,  27), S(  15,  23),
};

const int QueenPSQT32[32] = {
    S(  25, -52), S(   6, -39), S(  11, -49), S(  17, -39),
    S(  14, -38), S(  25, -56), S(  27, -66), S(  17, -27),
    S(  10, -23), S(  22, -21), S(   6,   1), S(   8,  -4),
    S(  10,  -7), S(  11,  14), S(  -2,  16), S( -15,  55),
    S(  -9,  15), S(  -5,  39), S( -14,  18), S( -31,  65),
    S( -22,  27), S( -15,  18), S( -23,  21), S( -13,  22),
    S(  -6,  26), S( -53,  62), S( -10,  14), S( -42,  51),
    S(  -9,  16), S(  17,   8), S(   7,   7), S(  -7,  19),
};

const int KingPSQT32[32] = {
    S(  41, -80), S(  39, -52), S( -11, -15), S( -26, -23),
    S(  28, -33), S(  -4, -22), S( -36,   4), S( -52,   7),
    S(   8, -30), S(  14, -26), S(  14,  -4), S( -11,  11),
    S(   4, -31), S(  83, -36), S(  41,   2), S(  -4,  22),
    S(   6, -15), S(  97, -29), S(  48,  12), S(   4,  22),
    S(  47, -22), S( 121, -20), S(  95,   6), S(  39,   6),
    S(   6, -44), S(  47,  -5), S(  32,  10), S(   9,   2),
    S(   8,-101), S(  74, -53), S( -19, -12), S( -18, -11),
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -26,  -9), S( -12,   9), S( -16,  28),
    S(  -3,  57), S(  36,  62), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -11,  19), S(  -4,  21), S(   0,  41),
    S(  10,  75), S(  31,  54), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -9, -11);

const int PawnStacked[2] = { S(  -2, -20), S(  -7,  -8) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   5,  -4), S(   8,  -6), S(   9, -11),
    S(  15, -10), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S(  -8, -23), S(  -6, -19), S(   5, -19),
    S(  23, -15), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -2,  -5), S(   9,   1), S(   3,   1), S(   3,  13),
    S(  12,   1), S(  26,  -2), S(  18,   8), S(  21,  15),
    S(   8,   0), S(  21,   3), S(   8,  10), S(  14,  18),
    S(  11,  10), S(  19,  15), S(  28,  20), S(  28,  21),
    S(  52,  23), S(  46,  46), S(  65,  53), S(  81,  56),
...
const int KnightInSiberia[4] = {
    S(  -8,   2), S(  -9,  -2), S( -16,  -4), S( -20,  -3),
};

const int KnightMobility[9] = {
    S( -73,-104), S( -31, -95), S( -20, -40), S(  -9, -17),
    S(   0,  -7), S(   5,   7), S(  11,  12), S(  20,  13),
    S(  29,   2),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  15,  71);
...
const int BishopBehindPawn = S(   3,  15);

const int BishopLongDiagonal = S(  20,  15);

const int BishopMobility[14] = {
    S( -62,-145), S( -30, -89), S( -14, -47), S(  -7, -24),
    S(   1, -14), S(   7,  -3), S(   9,   6), S(  10,  11),
    S(  10,  15), S(  12,  16), S(  13,  16), S(  32,   5),
    S(  40,  15), S(  69, -17),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  13,   5), S(  30,   4) };

const int RookOnSeventh = S(  -6,  31);

const int RookMobility[15] = {
    S(-147,-113), S( -48,-110), S( -18, -51), S( -11, -15),
    S( -11,   1), S( -12,  15), S( -11,  24), S(  -6,  26),
    S(   0,  29), S(   2,  33), S(   5,  38), S(   9,  41),
    S(   9,  44), S(  21,  35), S(  71,  -2),
};

/* Queen Evaluation Terms */

const int QueenMobility[28] = {
    S( -61,-263), S(-210,-387), S( -56,-201), S( -12,-191),
    S(  -2,-137), S(   0, -81), S(   4, -49), S(   3, -20),
    S(   6, -14), S(   7,   4), S(   9,   9), S(  10,  22),
    S(  12,  18), S(  13,  26), S(  11,  29), S(   7,  35),
    S(   6,  37), S(  -1,  39), S(   0,  36), S(  -4,  32),
    S(   5,  10), S(  19, -13), S(  23, -41), S(  28, -56),
    S(  11, -72), S(  23, -98), S( -56, -39), S( -32, -62),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -16,   0), S(  -4,  -2), S(   3,   2), S(   8,   5),
    S(  14,   6), S(  21,   3), S(  25,  -3), S(  13,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  35,  38), S(  15,  30), S(   7,  17), S( -16, -15),
    S( -19, -52), S( -17, -70), S( -16, -74), S( -11, -72),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -11,   2), S(  13, -23), S(  18, -10), S(  10,   3),
    S(   6,   3), S(   0,   2), S(  -3, -33), S( -44,  18)},
   {S(  17,  -8), S(  20, -16), S(   1,  -5), S( -14,   3),
    S( -30,  15), S( -70,  67), S(  92,  82), S( -24,   3)},
   {S(  34,  -5), S(  13,  -8), S( -23,   6), S( -11,  -8),
    S( -19,  -4), S( -12,   1), S(   0,  66), S( -12,  -1)},
   {S(   4,   9), S(  18, -11), S(   4, -12), S(  11, -19),
    S(  23, -36), S( -59,   6), S(-136,  52), S(   6,  -7)},
   {S( -12,   8), S(   4,  -5), S( -21,   0), S( -16,   3),
    S( -19,  -7), S( -42,  -2), S(  33, -18), S(  -7,  -1)},
   {S(  40, -15), S(  20, -15), S( -18,   1), S( -11, -18),
    S(   4, -26), S(  15, -21), S(  41, -30), S( -22,   0)},
   {S(  24, -13), S(   2, -15), S( -20,  -3), S( -19,  -9),
    S( -30,  -7), S( -37,  31), S(   0,  44), S( -11,   1)},
   {S(  -7, -10), S(   4, -15), S(   7,  -2), S(  -3,   8),
    S( -12,  13), S( -11,  35), S(-190,  87), S( -16,  13)}},
  {{S(   0,   0), S( -10, -22), S(   4, -20), S( -40,  16),
    S( -23,   2), S(   3,  42), S(-167,  -8), S( -42,  14)},
   {S(   0,   0), S(  22, -20), S(   5,  -9), S( -19,  -2),
    S(  -2, -12), S(  25,  65), S(-184,  -3), S( -34,   6)},
   {S(   0,   0), S(  25, -12), S(  -2, -10), S(   4, -19),
    S(  13,  -8), S( -86,  47), S( -84, -73), S( -15,  -2)},
   {S(   0,   0), S(  -5,   5), S(  -3,  -3), S( -19,   0),
    S( -27,   1), S( -99,  31), S(   7, -41), S( -12,   1)},
   {S(   0,   0), S(   9,   2), S(  11,  -7), S(  11, -11),
    S(  13, -27), S( -58,  15), S(-104, -61), S(   3,   1)},
   {S(   0,   0), S(   6,  -8), S( -18,  -2), S( -26,  -8),
    S(  16, -25), S( -38,   3), S(  55,  38), S( -16,  -3)},
   {S(   0,   0), S(  21, -13), S(  10, -13), S( -10, -11),
    S( -28,   7), S( -11,  15), S( -56, -49), S( -28,  13)},
   {S(   0,   0), S(  11, -36), S(  17, -29), S( -18,  -7),
    S( -18,  17), S(  -6,  20), S(-228, -55), S( -18,   6)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S(  -5,  26), S( 111, -12), S( -23,  22), S( -17,   6),
    S( -13,   2), S(  -9,  -1), S( -17,   8), S( -22,   4)},
   {S(  -4,  47), S(  55,  12), S( -16,  20), S(  -4,   8),
    S(  -4,   4), S(   3,  -3), S(  -3,   3), S( -11,   6)},
   {S(   7,  37), S(  14,  23), S( -21,  17), S( -10,   6),
    S(   1,   1), S(   5,  -1), S(   7,  -5), S(   2,   3)},
   {S(  -3,  25), S(  12,  16), S( -18,   7), S( -14,   0),
    S( -11,  -1), S(   5, -10), S(   0,  -7), S( -10,   5)}},
  {{S(   0,   0), S( -16, -17), S( -12,  -3), S(  18, -15),
    S(   8,  -4), S(   2, -17), S(  -4,  -1), S(  17,  29)},
   {S(   0,   0), S( -17, -35), S(   2, -12), S(  34, -12),
    S(   0,   0), S(  12, -22), S(  -7, -10), S( -17,   2)},
   {S(   0,   0), S( -29, -50), S( -22,  -9), S(  11, -12),
    S(   2,   0), S(  -8, -13), S( -14, -15), S( -11,   6)},
   {S(   0,   0), S(  -2, -19), S( -13, -17), S( -11,  -3),
    S(  -2,  -6), S(   6, -22), S(  72, -12), S(  13,  20)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -36,   4), S( -51,  25), S( -74,  32),
    S(  -6,  17), S(  70,  -1), S( 160,  61), S(   0,   0)},
   {S(   0,   0), S( -29,   9), S( -47,  29), S( -67,  37),
    S( -10,  34), S(  90,  28), S( 183,  96), S(   0,   0)}},
  {{S(   0,   0), S( -24,  16), S( -46,  29), S( -69,  43),
    S(  -2,  39), S(  85,  40), S( 259, 112), S(   0,   0)},
   {S(   0,   0), S( -29,  14), S( -41,  25), S( -62,  46),
    S(   6,  55), S( 102, 104), S( 160, 254), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   0,   2), S(   3,  -3), S(   6, -10),
    S(   5, -13), S( -10, -11), S( -19,  -7), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   3,  -1), S(   5,   0), S(  10,   8),
    S(   2,  21), S(   9,  30), S(  27,  27), S(   0,   0),
};

/* Threat Evaluation Terms */

const int PassedSafePromotionPath    = S( -15,  34);
const int ThreatWeakPawn             = S( -10, -31);
const int ThreatMinorAttackedByPawn  = S( -46, -51);
const int ThreatMinorAttackedByMinor = S( -20, -34);
const int ThreatMinorAttackedByMajor = S( -18, -48);
const int ThreatRookAttackedByLesser = S( -44, -19);
const int ThreatMinorAttackedByKing  = S( -27, -29);
const int ThreatRookAttackedByKing   = S( -22, -32);
const int ThreatQueenAttackedByOne   = S( -32, -35);
...
const int SpaceCenterControl = S(   1,   2);

/* Closedness Evaluation Terms */

const int ClosednessKnightAdjustment[9] = {
    S( -14,  -8), S( -13,   7), S( -14,  14), S( -10,  14),
    S(  -8,  19), S(  -6,  16), S(  -3,  13), S( -13,  27),
    S( -12,  16),
};

const int ClosednessRookAdjustment[9] = {
    S(  34,   5), S(   0,  26), S( -10,  14), S( -16,   1),
    S( -21,   0), S( -21, -12), S( -21, -14), S( -25, -17),
    S( -29, -18),
};
...
const int ComplexityPawnFlanks  = S(   0,  72);
const int ComplexityPawnEndgame = S(   0,  45);
const int ComplexityAdjustment  = S(   0,-132);
