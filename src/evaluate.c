...
const int PawnValue   = S( 100, 133);
const int KnightValue = S( 379, 420);
const int BishopValue = S( 410, 436);
const int RookValue   = S( 630, 690);
const int QueenValue  = S(1305,1354);
...
const int PawnPSQT32[32] = {
    ...
    S( -17,  10), S(   4,   3), S( -17,   7), S( -18,  -3),
    S( -19,   4), S( -17,   4), S( -11,  -5), S( -10, -11),
    S( -14,  11), S( -14,  12), S(   6, -11), S(   8, -23),
    S(  -3,  16), S(   2,   8), S(  -7,  -6), S(  -5, -17),
    S(  -7,  29), S(  -8,  23), S(  -4,  14), S(  26, -12),
    S( -20, -59), S( -61, -30), S(   2, -43), S(  45, -58),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -48, -31), S( -10, -32), S( -20, -31), S(  -6, -18),
    S(  -5, -21), S(   3, -11), S(   0, -29), S(   6, -18),
    S(  10, -26), S(  24, -20), S(  15, -14), S(  24,  -1),
    S(  27,   4), S(  25,   6), S(  37,  16), S(  22,  23),
    S(  30,  16), S(  30,  10), S(  40,  25), S(  21,  38),
    S( -16,  13), S(   3,  13), S(  18,  28), S(  23,  29),
    S(  12, -11), S(  -4,   1), S(  24, -19), S(  35,   2),
    S(-177, -24), S( -85,  -3), S(-112,  19), S( -32,  -1),
};

const int BishopPSQT32[32] = {
    S(   5, -27), S(  12, -18), S(  -8,  -9), S(   6, -12),
    S(  34, -32), S(  12, -35), S(  25, -18), S(  12,  -9),
    S(  21, -11), S(  34, -13), S(   7, -16), S(  26,  -1),
    S(  19,  -6), S(  17,  -2), S(  24,   5), S(  22,   7),
    S(  -9,  14), S(  22,   3), S(   1,  13), S(  10,  17),
    S(  -7,   6), S(  -7,  16), S( -30,   0), S(   4,  13),
    S( -49,  14), S( -55,   3), S(  -9,   9), S( -30,  14),
    S( -59,  -7), S( -47,   7), S( -87,  16), S( -90,  28),
};

const int RookPSQT32[32] = {
    S( -11, -32), S( -16, -24), S(  -2, -29), S(   7, -35),
    S( -57, -16), S( -17, -35), S( -14, -34), S(  -2, -38),
    S( -33, -14), S( -11, -13), S( -19, -17), S(  -4, -27),
    S( -18,   0), S( -11,   8), S(  -6,   4), S(   4,  -5),
    S(   0,  14), S(  13,  11), S(  24,   7), S(  39,  -1),
    S( -11,  25), S(  32,  10), S(   5,  21), S(  40,   5),
    S(   5,  11), S( -10,  17), S(  13,   7), S(  26,   5),
    S(  34,  22), S(  27,  24), S(  -1,  31), S(   7,  28),
};

const int QueenPSQT32[32] = {
    S(  31, -65), S(   4, -45), S(   6, -60), S(  14, -45),
    S(  14, -42), S(  25, -59), S(  29, -70), S(  16, -29),
    S(  11, -29), S(  24, -20), S(   5,   5), S(   7,   1),
    S(  13,  -8), S(  16,  12), S(   0,  24), S( -15,  64),
    S(  -3,  16), S(  -5,  47), S( -10,  23), S( -27,  74),
    S( -22,  33), S( -14,  21), S( -20,  26), S(  -9,  28),
    S(   1,  28), S( -58,  68), S(  -2,  13), S( -37,  53),
    S(  -9,  21), S(  21,  10), S(   9,  11), S(  -4,  22),
};

const int KingPSQT32[32] = {
    S(  40, -76), S(  37, -47), S( -14, -13), S( -35, -20),
    S(  26, -24), S( -14, -15), S( -41,   9), S( -64,  11),
    S(  -4, -21), S(   6, -20), S(  12,   0), S( -17,  15),
    S(   2, -27), S(  94, -36), S(  45,   5), S(  -4,  26),
    S(  11, -12), S( 108, -30), S(  56,  14), S(  13,  23),
    S(  51, -25), S( 135, -19), S( 106,   7), S(  44,   5),
    S(   7, -56), S(  49,  -7), S(  32,   7), S(   9,  -1),
    S(   4,-123), S(  73, -63), S( -23, -22), S( -20, -18),
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -26,  -8), S( -10,  10), S( -21,  28),
    S( -22,  48), S(  18,  56), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -13,  20), S(  -4,  23), S(   1,  44),
    S(  18,  84), S(  38,  62), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -8, -14);

const int PawnStacked[2] = { S( -11, -21), S( -13,  -6) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   9,  -5), S(   9,  -8), S(  12, -13),
    S(  13, -10), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -12, -27), S(  -9, -24), S(  14, -25),
    S(  33, -25), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -3,  -4), S(  12,   1), S(   2,   4), S(   5,  13),
    S(  12,   0), S(  31,   0), S(  19,   8), S(  23,  18),
    S(   9,   0), S(  21,   1), S(   9,  10), S(  14,  21),
    S(  14,  10), S(  24,  15), S(  34,  21), S(  42,  20),
    S(  58,  20), S(  45,  46), S(  65,  57), S(  78,  61),
...
const int KnightInSiberia[4] = {
    S( -13,   4), S( -15,  -1), S( -21,  -2), S( -26,  -6),
};

const int KnightMobility[9] = {
    S( -78,-106), S( -37,-102), S( -25, -43), S( -14, -15),
    S(  -2,  -3), S(   5,   9), S(  12,  15), S(  20,  14),
    S(  28,   3),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  -4,  75);
...
const int BishopBehindPawn = S(   6,  16);

const int BishopLongDiagonal = S(  23,  15);

const int BishopMobility[14] = {
    S( -68,-149), S( -31, -98), S( -13, -55), S(  -4, -28),
    S(   3, -13), S(   9,   0), S(  10,   9), S(  12,  14),
    S(  10,  17), S(  13,  19), S(  16,  19), S(  43,   8),
    S(  47,  19), S(  75, -16),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  20,   7), S(  34,   6) };

const int RookOnSeventh = S(  -2,  33);

const int RookMobility[15] = {
    S(-159,-116), S( -55,-115), S( -15, -63), S( -11, -16),
    S( -12,   3), S( -13,  19), S( -10,  27), S(  -4,  30),
    S(   4,  32), S(   8,  36), S(  12,  40), S(  14,  45),
    S(  16,  47), S(  31,  36), S( 101,  -6),
};

/* Queen Evaluation Terms */

const int QueenMobility[28] = {
    S( -61,-263), S(-213,-388), S( -60,-200), S( -19,-191),
    S(   1,-144), S(   2, -80), S(   5, -44), S(   4, -13),
    S(   7, -10), S(   7,  11), S(   9,  16), S(  10,  31),
    S(  14,  25), S(  15,  34), S(  14,  38), S(  12,  44),
    S(  14,  42), S(   6,  45), S(   8,  41), S(   8,  31),
    S(  17,   9), S(  30, -15), S(  31, -45), S(  30, -62),
...
const int KingDefenders[12] = {
    S( -25,   4), S(  -7,  -2), S(   3,   1), S(  11,   4),
    S(  16,   7), S(  24,   2), S(  30,  -4), S(  12,  -1),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  48,  28), S(  23,  22), S(  11,  13), S( -24, -14),
    S( -29, -49), S( -22, -65), S( -18, -68), S( -11, -65),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -10,   1), S(  18, -24), S(  26, -10), S(  15,   5),
    S(  14,   3), S(  -1,  -1), S(  -3, -34), S( -53,  20)},
   {S(  15,  -8), S(  24, -18), S(   1,  -5), S( -16,   4),
    S( -30,  16), S( -73,  67), S(  91,  81), S( -21,   2)},
   {S(  35,  -5), S(  15,  -6), S( -29,   7), S( -13,  -8),
    S( -18,  -5), S( -12,   1), S(   0,  68), S( -13,  -1)},
   {S(   2,  11), S(  18, -12), S(   8, -12), S(  13, -20),
    S(  28, -38), S( -58,   3), S(-135,  51), S(  11,  -7)},
   {S( -16,   9), S(   4,  -2), S( -27,   4), S( -19,   6),
    S( -21,  -6), S( -45,  -5), S(  32, -18), S(  -7,   0)},
   {S(  47, -17), S(  23, -13), S( -23,   4), S( -12, -18),
    S(   6, -25), S(  19, -21), S(  40, -32), S( -24,   1)},
   {S(  31, -16), S(   2, -16), S( -26,  -3), S( -21,  -8),
    S( -32,  -5), S( -41,  30), S(   1,  44), S( -10,   1)},
   {S(  -8, -10), S(   5, -13), S(  11,  -2), S(  -2,   9),
    S( -12,  15), S( -12,  35), S(-190,  89), S( -18,  15)}},
  {{S(   0,   0), S( -15, -26), S(   5, -24), S( -39,  19),
    S( -23,   1), S(   3,  42), S(-167,  -7), S( -43,  10)},
   {S(   0,   0), S(  27, -21), S(   7,  -8), S( -17,  -1),
    S(   3, -18), S(  26,  65), S(-184,  -3), S( -31,   6)},
   {S(   0,   0), S(  30, -11), S(  -3, -10), S(   6, -20),
    S(  20,  -4), S( -87,  48), S( -84, -74), S( -20,  -1)},
   {S(   0,   0), S( -10,   7), S(  -5,  -3), S( -25,   4),
    S( -27,   3), S(-100,  30), S(   7, -40), S( -18,   0)},
   {S(   0,   0), S(  11,   1), S(  15,  -7), S(   7, -13),
    S(  14, -31), S( -58,  14), S(-101, -60), S(   4,  -1)},
   {S(   0,   0), S(   5,  -8), S( -24,   0), S( -25, -11),
    S(  22, -24), S( -37,   3), S(  55,  39), S( -17,  -3)},
   {S(   0,   0), S(  25, -12), S(  13, -15), S( -12, -10),
    S( -31,  12), S(  -8,  19), S( -57, -50), S( -29,  13)},
   {S(   0,   0), S(  13, -39), S(  19, -32), S( -20,  -8),
    S( -20,  19), S(  -8,  21), S(-228, -56), S( -23,   6)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -13,  32), S( 111,  -7), S( -24,  23), S( -18,   5),
    S(  -9,   1), S(  -4,  -3), S( -18,   6), S( -26,   2)},
   {S(  -8,  53), S(  52,  16), S( -17,  24), S(  -3,  10),
    S(  -2,   3), S(   7,  -4), S(   0,   3), S( -13,   4)},
   {S(   3,  43), S(   5,  29), S( -19,  20), S( -12,   8),
    S(   2,   3), S(  12,  -2), S(  16,  -8), S(   5,   2)},
   {S(  -7,  28), S(   4,  23), S( -18,  10), S(  -7,   0),
    S( -11,  -2), S(   7, -13), S(   0,  -8), S( -16,   6)}},
  {{S(   0,   0), S( -18, -16), S( -15,  -2), S(  25, -19),
    S(  12,  -8), S(   4, -21), S(   3,   1), S(  16,  29)},
   {S(   0,   0), S( -17, -37), S(   1, -13), S(  41, -16),
    S(   2,  -4), S(  17, -25), S(  -6,  -5), S( -19,   0)},
   {S(   0,   0), S( -28, -52), S( -25,  -9), S(  16, -14),
    S(   4,  -2), S(  -8, -13), S( -10, -12), S( -11,   6)},
   {S(   0,   0), S(  -2, -19), S( -21, -17), S( -12,  -4),
    S(   0,  -8), S(   0, -23), S(  74, -12), S(  15,  20)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -36,   2), S( -49,  26), S( -75,  31),
    S(   8,  13), S(  85, -11), S( 159,  44), S(   0,   0)},
   {S(   0,   0), S( -27,  14), S( -46,  35), S( -64,  39),
    S(   0,  37), S( 107,  27), S( 189,  88), S(   0,   0)}},
  {{S(   0,   0), S( -26,  23), S( -48,  31), S( -70,  42),
    S(   5,  37), S( 100,  38), S( 272, 101), S(   0,   0)},
   {S(   0,   0), S( -29,  21), S( -43,  30), S( -61,  45),
    S(  13,  54), S( 104, 104), S( 156, 258), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   2,   1), S(   4,  -3), S(   7, -10),
    S(   2, -14), S( -11, -13), S( -21,  -6), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   2,  -2), S(   5,  -1), S(  10,   8),
    S(   6,  23), S(  14,  31), S(  31,  33), S(   0,   0),
};

const int PassedSafePromotionPath = S( -19,  32);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -28, -51);
const int ThreatMinorAttackedByPawn  = S(-158, -90);
const int ThreatMinorAttackedByMinor = S( -38, -44);
const int ThreatMinorAttackedByMajor = S(-142, -73);
const int ThreatRookAttackedByLesser = S( -70, -35);
const int ThreatMinorAttackedByKing  = S( -54, -42);
const int ThreatRookAttackedByKing   = S( -23, -24);
const int ThreatQueenAttackedByOne   = S(  -7, -17);
const int ThreatOverloadedPieces     = S(   6, -14);
const int ThreatByPawnPush           = S(  16,  19);

/* Space Evaluation Terms */

const int SpaceRestrictPiece = S(  -6,  -2);
const int SpaceRestrictEmpty = S(  -5,  -1);
const int SpaceCenterControl = S(  -1,  10);

/* Closedness Evaluation Terms */

const int ClosednessKnightAdjustment[9] = {
    S(  -8,  -3), S( -10,   9), S( -16,  16), S( -11,  17),
    S( -15,  22), S( -13,  17), S(  -9,   6), S( -13,  24),
    S( -10,   9),
};

const int ClosednessRookAdjustment[9] = {
    S(  60,   3), S(   1,  32), S(   0,  16), S(  -6,  -1),
    S( -14,   4), S( -13, -13), S( -14, -13), S( -24, -16),
    S( -35, -21),
};
...
const int ComplexityPawnEndgame = S(   0,  56);
const int ComplexityAdjustment  = S(   0,-141);
