...
int evaluateComplexity(EvalInfo *ei, Board *board, int eval) {
    ...
    complexity =  ComplexityTotalPawns  * popcount(board->pieces[PAWN])
               +  ComplexityPawnFlanks  * distanceBetweenOuterPawns(board->pieces[PAWN])
    ...
    if (TRACE) T.ComplexityPawnFlanks[WHITE]  += sign * distanceBetweenOuterPawns(board->pieces[PAWN]);
