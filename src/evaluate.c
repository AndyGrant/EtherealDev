...
const int PawnValue = S( 105, 117);

const int KnightValue = S( 455, 424);

const int BishopValue = S( 473, 434);

const int RookValue = S( 667, 698);

const int QueenValue = S(1334,1376);

const int KingValue = S(   0,   0);

const int PawnPSQT32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S( -20,   4), S(   1,   0), S( -12,   3), S(  -9,  -2),
    S( -23,   2), S( -14,   2), S( -10,  -7), S(  -5, -15),
    S( -19,   9), S( -15,   9), S(   8, -13), S(   8, -25),
    S( -12,   9), S(  -8,   6), S( -11,  -9), S(  -4, -23),
    S(  -9,  26), S(  -6,  21), S(  -2,  12), S(  26, -17),
    S( -20, -57), S( -71, -25), S(  -1, -41), S(  40, -57),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -49, -29), S(  -9, -31), S( -14, -29), S(  -1, -18),
    S(  -1, -21), S(   3, -13), S(   3, -30), S(   8, -20),
    S(   9, -25), S(  21, -25), S(  14, -21), S(  22,  -8),
    S(  20,   1), S(  20,   5), S(  29,  12), S(  27,  19),
    S(  25,  14), S(  26,   7), S(  37,  21), S(  29,  32),
    S( -14,  12), S(   4,  11), S(  33,  22), S(  25,  25),
    S(  13, -11), S(  -8,  -1), S(  29, -22), S(  45,  -5),
    S(-189, -20), S( -88,  -1), S(-120,  20), S( -34,  -3),
};

const int BishopPSQT32[32] = {
    S(  -1, -33), S(   9, -21), S( -12,  -9), S(   8, -13),
    S(  28, -37), S(   4, -41), S(  19, -21), S(  10, -13),
    S(  14, -14), S(  26, -17), S(  -2, -24), S(  21,  -7),
    S(  14,  -9), S(  18,  -4), S(  15,   2), S(  23,   4),
    S( -11,  11), S(  18,   3), S(   5,  12), S(  11,  16),
    S(   2,   5), S(  -4,  15), S( -10,   0), S(  15,  11),
    S( -47,  15), S( -62,  -1), S( -10,   6), S( -24,  10),
    S( -63, -13), S( -50,   7), S( -92,  15), S( -97,  27),
};

const int RookPSQT32[32] = {
    S( -18, -32), S( -21, -23), S(  -8, -28), S(   1, -33),
    S( -60, -18), S( -18, -37), S( -13, -36), S(  -2, -40),
    S( -34, -19), S( -13, -16), S( -22, -21), S(  -4, -30),
    S( -23,  -7), S( -15,   1), S( -13,  -4), S(   1, -10),
    S( -10,   2), S(   6,  -1), S(  16,  -5), S(  33, -10),
    S( -22,  13), S(  20,  -1), S(  -6,  13), S(  29,  -5),
    S(  -1,  -1), S( -18,   9), S(   6,  -2), S(  24,  -2),
    S(  29,  16), S(  23,  17), S(   4,  23), S(  16,  19),
};

const int QueenPSQT32[32] = {
    S(  22, -70), S(  -1, -53), S(   6, -65), S(  12, -51),
    S(   5, -50), S(  19, -68), S(  22, -84), S(  10, -38),
    S(   2, -37), S(  17, -36), S(  -2, -10), S(   0, -15),
    S(   3, -23), S(   6,   0), S( -10,   3), S( -24,  44),
    S( -18,   2), S( -15,  27), S( -25,   3), S( -41,  52),
    S( -37,  18), S( -31,   8), S( -34,   6), S( -24,   5),
    S( -14,  15), S( -68,  51), S( -18,  -2), S( -55,  40),
    S( -24,   4), S(   7,  -6), S(  -3,  -8), S( -17,   5),
};

const int KingPSQT32[32] = {
    S(  44, -76), S(  41, -48), S( -15,  -8), S( -34, -16),
    S(  33, -23), S(  -4, -14), S( -39,  13), S( -58,  15),
    S(   9, -24), S(  14, -21), S(  15,   4), S( -10,  19),
    S(   7, -32), S(  95, -36), S(  48,   7), S(  -3,  29),
    S(   8, -18), S( 109, -33), S(  57,  12), S(   8,  22),
    S(  51, -32), S( 136, -27), S( 107,   0), S(  46,   0),
    S(   4, -67), S(  50, -18), S(  33,   0), S(   9,  -9),
    S(   6,-125), S(  79, -70), S( -24, -26), S( -23, -20),
};

const int PawnCandidatePasser[2][8] = {
   {S(   0,   0), S( -26,  -9), S( -14,  11), S( -18,  30), S(  -1,  59), S(  37,  66), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -14,  24), S(  -5,  22), S(   1,  45), S(  11,  79), S(  32,  60), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -7, -12);

const int PawnStacked[2] = {
    S(  -5, -20), S(  -8,  -7),
};

const int PawnBackwards[2][8] = {
   {S(   0,   0), S(   2,  -8), S(   8,  -7), S(   9, -14), S(  17, -18), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -11, -31), S(  -6, -26), S(   7, -29), S(  43, -28), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -2,  -6), S(  12,   0), S(   3,   1), S(   4,  12),
    S(  13,   0), S(  27,  -3), S(  20,   5), S(  23,  12),
    S(   9,  -1), S(  23,   1), S(   9,   9), S(  16,  15),
    S(  11,   8), S(  21,  14), S(  28,  20), S(  31,  19),
    S(  54,  19), S(  41,  45), S(  62,  50), S(  75,  55),
    S( 123,  -5), S( 225,   1), S( 251,  31), S( 262,  47),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightInSiberia[4] = {
    S( -10,   0), S( -12,  -7), S( -24,  -6), S( -32, -10),
};

const int KnightMobility[9] = {
    S( -86,-118), S( -47,-108), S( -33, -55), S( -22, -31),
    S( -11, -22), S(  -6,  -7), S(   2,  -3), S(  11,  -3),
    S(  23, -17),
};

const int BishopPair = S(  23,  67);
...
const int BishopLongDiagonal = S(  28,  16);

const int BishopMobility[14] = {
    S( -76,-160), S( -44,-102), S( -24, -62), S( -15, -37),
    S(  -5, -27), S(   3, -15), S(   6,  -5), S(   6,   0),
    S(   5,   5), S(   9,   6), S(   9,   7), S(  29,  -5),
    S(  37,   5), S(  70, -32),
};
...
const int RookOnSeventh = S(  -8,  29);

const int RookMobility[15] = {
    S(-165,-125), S( -74,-125), S( -36, -73), S( -27, -37),
    S( -27, -19), S( -28,  -4), S( -27,   6), S( -21,   9),
    S( -14,  11), S( -11,  16), S(  -8,  21), S(  -3,  24),
    S(  -2,  28), S(  12,  16), S(  82, -33),
};

const int QueenMobility[28] = {
    S( -67,-290), S(-231,-427), S( -63,-220), S( -37,-210),
    S( -24,-151), S( -21, -88), S( -16, -62), S( -16, -38),
    S( -11, -38), S(  -9, -21), S(  -5, -18), S(  -5,  -2),
    S(  -2, -10), S(  -1,  -1), S(  -3,   1), S(  -8,   7),
    S(  -7,   5), S( -17,   9), S( -15,   5), S( -21,  -1),
    S( -10, -28), S(   5, -54), S(   7, -82), S(  13, -98),
    S(  -1,-105), S(  15,-130), S( -64, -47), S( -39, -76),
};

const int KingDefenders[12] = {
    S( -20,   5), S(  -4,  -2), S(   3,   2), S(   8,   6),
    S(  16,   5), S(  25,   2), S(  28,  -4), S(  13,   0),
    S(  13,   7), S(  13,   7), S(  13,   7), S(  13,   7),
};

const int KingPawnFileProximity[8] = {
    S(  55,  35), S(  34,  26), S(  16,  14), S( -32,  -9),
    S( -33, -49), S( -28, -72), S( -24, -85), S( -14, -79),
};

const int KingShelter[2][8][8] = {
  {{S(  -9,   1), S(  13, -26), S(  22,  -9), S(  13,   3), S(   9,   3), S(  -3,   0), S(  -4, -37), S( -50,  20)},
   {S(  19,  -9), S(  19, -16), S(   1,  -4), S( -15,   2), S( -31,  14), S( -79,  71), S( 100,  89), S( -20,   2)},
   {S(  36,  -4), S(  13,  -8), S( -27,   6), S( -11, -11), S( -18,  -8), S( -17,  -1), S(  -1,  72), S( -11,  -2)},
   {S(   2,  11), S(  22, -12), S(   4, -11), S(  14, -23), S(  27, -40), S( -63,   7), S(-150,  56), S(   8,  -8)},
   {S( -16,   9), S(   4,  -3), S( -26,   2), S( -18,   5), S( -19,  -9), S( -49,  -4), S(  35, -21), S(  -4,  -1)},
   {S(  46, -17), S(  21, -15), S( -20,   3), S( -12, -19), S(   5, -27), S(  17, -21), S(  45, -34), S( -23,   1)},
   {S(  26, -14), S(  -2, -15), S( -23,  -1), S( -21,  -8), S( -31,  -8), S( -43,  32), S(   0,  49), S(  -8,   0)},
   {S(  -8, -12), S(   5, -17), S(   7,   3), S(  -3,  12), S( -14,  16), S( -13,  38), S(-208,  96), S( -17,  16)}},
  {{S(   0,   0), S( -12, -26), S(   9, -25), S( -43,  14), S( -28,   2), S(   3,  46), S(-184,  -8), S( -44,  16)},
   {S(   0,   0), S(  22, -21), S(   7,  -8), S( -21,  -1), S(  -1, -16), S(  25,  68), S(-203,  -4), S( -33,   7)},
   {S(   0,   0), S(  26, -12), S(  -3, -11), S(   4, -22), S(  15, -10), S( -93,  51), S( -92, -80), S( -17,  -1)},
   {S(   0,   0), S(  -9,   8), S(  -5,  -3), S( -24,   2), S( -30,  -1), S(-109,  33), S(   8, -45), S( -11,  -2)},
   {S(   0,   0), S(  13,   2), S(  11,  -5), S(  13, -14), S(  16, -31), S( -61,  17), S(-114, -67), S(   2,   0)},
   {S(   0,   0), S(   1,  -7), S( -22,  -1), S( -26, -10), S(  18, -28), S( -41,   3), S(  61,  41), S( -13,  -4)},
   {S(   0,   0), S(  21, -15), S(  10, -14), S( -12,  -8), S( -29,   8), S( -12,  17), S( -62, -53), S( -30,  14)},
   {S(   0,   0), S(  11, -39), S(  20, -30), S( -19,  -8), S( -22,  16), S( -10,  23), S(-250, -60), S( -21,   6)}},
};

const int KingStorm[2][4][8] = {
  {{S( -10,  30), S( 119, -15), S( -24,  23), S( -16,   5), S( -14,   1), S(  -8,  -4), S( -17,   6), S( -25,   3)},
   {S( -10,  53), S(  59,   6), S( -18,  21), S(  -2,   9), S(  -4,   5), S(   4,  -4), S(  -3,   2), S( -13,   5)},
   {S(   2,  42), S(  13,  20), S( -18,  17), S( -10,   6), S(   2,   1), S(   6,   0), S(   9,  -5), S(   3,   3)},
   {S(  -6,  29), S(   7,  18), S( -19,   6), S( -15,  -1), S( -13,   0), S(   8, -12), S(   2,  -8), S( -10,   4)}},
  {{S(   0,   0), S( -20, -20), S( -16,  -1), S(  19, -16), S(   7,  -3), S(   6, -17), S(   1,   2), S(  20,  33)},
   {S(   0,   0), S( -20, -42), S(   3,  -9), S(  40, -10), S(   1,   0), S(  14, -19), S(  -6,  -9), S( -20,   1)},
   {S(   0,   0), S( -34, -60), S( -23,  -7), S(  13, -11), S(   3,   1), S(  -8, -11), S( -13, -12), S( -14,   5)},
   {S(   0,   0), S(  -3, -23), S( -14, -17), S( -12,  -2), S(  -2,  -4), S(   6, -21), S(  80, -15), S(  15,  21)}},
};
...
const int PassedPawn[2][2][8] = {
  {{S(   0,   0), S( -40,   9), S( -52,  26), S( -80,  33), S(  -2,  16), S(  85,  -9), S( 176,  51), S(   0,   0)},
   {S(   0,   0), S( -34,  14), S( -52,  35), S( -74,  39), S(  -9,  38), S( 104,  31), S( 207,  90), S(   0,   0)}},
  {{S(   0,   0), S( -28,  23), S( -50,  30), S( -74,  40), S(   1,  42), S( 101,  38), S( 279, 102), S(   0,   0)},
   {S(   0,   0), S( -35,  20), S( -47,  29), S( -69,  46), S(   3,  56), S( 115, 106), S( 181, 254), S(   0,   0)}},
};

const int PassedFriendlyDistance[8] = {
    S(   0,   0), S(   1,   1), S(   3,  -3), S(   6,  -8),
    S(   4, -12), S( -12, -10), S( -20,  -3), S(   0,   0),
};

const int PassedEnemyDistance[8] = {
    S(   0,   0), S(   3,  -1), S(   5,  -1), S(  11,   6),
    S(   2,  18), S(   8,  26), S(  25,  24), S(   0,   0),
};

const int PassedSafePromotionPath = S( -17,  33);

const int ThreatWeakPawn = S( -12, -27);

const int ThreatMinorAttackedByPawn = S( -51, -56);

const int ThreatMinorAttackedByMinor = S( -24, -37);

const int ThreatMinorAttackedByMajor = S( -21, -48);

const int ThreatRookAttackedByLesser = S( -46, -25);

const int ThreatMinorAttackedByKing = S( -33, -24);

const int ThreatRookAttackedByKing = S( -32, -27);

const int ThreatQueenAttackedByOne = S( -35, -53);

const int ThreatOverloadedPieces = S(  -7, -13);

const int ThreatByPawnPush = S(  14,  21);
