...
const int PawnValue   = S( 115, 134);
const int KnightValue = S( 446, 417);
const int BishopValue = S( 475, 435);
const int RookValue   = S( 675, 707);
const int QueenValue  = S(1264,1376);
const int KingValue   = S(   0,   0);

const int PieceValues[8][PHASE_NB] = {
    { 115, 134}, { 446, 417}, { 475, 435}, { 675, 707},
    {1264,1376}, {   0,   0}, {   0,   0}, {   0,   0},
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -26, -10), S( -12,   8), S( -13,  26),
    S(  -1,  61), S(  45,  67), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -14,  14), S(  -4,  21), S(   2,  44),
    S(  17,  86), S(  33,  52), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -8, -15);

const int PawnStacked = S( -30, -19);

const int PawnBackwards[2] = { S(   7,   0), S(  -7, -21) };

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(   0,  -3), S(  11,   5), S(   3,   4), S(   1,  18),
    S(  19,   0), S(  35,   1), S(  23,  12), S(  27,  20),
    S(  11,   0), S(  25,   6), S(   9,  11), S(  15,  24),
    S(  17,   6), S(  26,  13), S(  32,  22), S(  38,  22),
    S(  57,  25), S(  53,  45), S(  70,  54), S(  83,  59),
    S( 109,  -1), S( 202,   9), S( 226,  27), S( 239,  50),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

/* Knight Evaluation Terms */

const int KnightOutpost[2] = { S(   8, -18), S(  33,   0) };

const int KnightBehindPawn = S(   5,  22);

const int KnightMobility[9] = {
    S( -81,-101), S( -36, -99), S( -23, -45), S(  -9, -18),
    S(   5,  -6), S(  12,  10), S(  21,  16), S(  32,  15),
    S(  44,   4),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  20,  74);

const int BishopRammedPawns = S( -12, -19);

const int BishopOutpost[2] = { S(  11,  -6), S(  45,   3) };

const int BishopBehindPawn = S(   4,  20);

const int BishopMobility[14] = {
    S( -64,-146), S( -28, -95), S( -10, -58), S(  -1, -31),
    S(  10, -18), S(  17,   0), S(  19,  11), S(  21,  16),
    S(  20,  22), S(  24,  23), S(  23,  21), S(  40,  11),
    S(  40,  13), S(  65, -19),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  20,   4), S(  41,   0) };

const int RookOnSeventh = S(   2,  31);

const int RookMobility[15] = {
    S(-149,-111), S( -55,-116), S( -16, -63), S(  -8, -22),
    S(  -8,  -1), S(  -9,  11), S(  -8,  23), S(   0,  27),
    S(   8,  31), S(  15,  36), S(  19,  43), S(  23,  48),
    S(  20,  55), S(  27,  45), S(  70,   4),
};

/* Queen Evaluation Terms */

const int QueenMobility[28] = {
    S( -61,-263), S(-210,-386), S( -60,-202), S( -24,-192),
    S( -12,-140), S(  -7, -89), S(  -2, -62), S(  -3, -34),
    S(   0, -24), S(  -1,  -1), S(   2,  10), S(   5,  22),
    S(   7,  25), S(   8,  33), S(   8,  39), S(   6,  42),
    S(   4,  47), S(   0,  45), S(  -1,  39), S(  -4,  32),
    S(   3,  12), S(  17,  -7), S(  19, -36), S(  21, -54),
    S(   5, -69), S(  20, -95), S( -57, -39), S( -27, -54),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -21,   0), S(  -5,  -6), S(   0,   0), S(   6,   5),
    S(  14,   7), S(  26,   2), S(  31,   0), S(  14,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};


const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -12,   3), S(  16, -23), S(  21,  -8), S(   9,   2),
    S(   4,   3), S(   7,   2), S(  -1, -31), S( -50,  20)},
   {S(  13,  -8), S(  22, -17), S(   0,  -3), S( -16,   1),
    S( -29,  12), S( -69,  68), S(  92,  83), S( -27,   5)},
   {S(  33,   0), S(  17,  -7), S( -26,   6), S( -12,  -9),
    S( -21,  -4), S(  -6,   0), S(   0,  65), S( -16,   0)},
   {S(   3,  12), S(  23,  -8), S(   3, -10), S(  11, -21),
    S(  25, -34), S( -50,   1), S(-136,  51), S(   3,  -2)},
   {S( -13,   7), S(   0,  -1), S( -25,   1), S( -19,   1),
    S( -20,  -8), S( -38,   0), S(  33, -15), S( -10,   0)},
   {S(  45, -18), S(  26, -15), S( -20,   0), S( -12, -18),
    S(   7, -25), S(  17, -22), S(  41, -29), S( -28,   2)},
   {S(  21, -13), S(   0, -16), S( -23,  -5), S( -17, -11),
    S( -28,  -9), S( -34,  30), S(   0,  44), S( -12,   3)},
   {S( -12,  -9), S(   8, -17), S(  10,  -2), S(  -2,   8),
    S(  -9,  13), S(  -9,  36), S(-190,  87), S( -18,  15)}},
  {{S(   0,   0), S( -12, -23), S(   0, -16), S( -42,  15),
    S( -18,   0), S(   2,  42), S(-166,  -8), S( -45,   8)},
   {S(   0,   0), S(  26, -19), S(   5,  -8), S( -18,  -3),
    S(   0, -15), S(  27,  70), S(-183,  -4), S( -41,   7)},
   {S(   0,   0), S(  35,  -8), S(  -2,  -4), S(  10, -16),
    S(  12,  -8), S( -91,  47), S( -84, -74), S( -21,  -2)},
   {S(   0,   0), S(   0,   9), S(  -1,   1), S( -15,   0),
    S( -26,   0), S(-100,  29), S(   6, -42), S( -24,  -3)},
   {S(   0,   0), S(  10,   4), S(  12,  -4), S(  12, -10),
    S(  13, -24), S( -54,  15), S(-105, -62), S(   0,  -8)},
   {S(   0,   0), S(   6,  -6), S( -20,   0), S( -27,  -7),
    S(  17, -26), S( -39,   1), S(  56,  38), S( -22,  -3)},
   {S(   0,   0), S(  24, -11), S(  10, -11), S(  -6,  -8),
    S( -30,   4), S(  -7,  14), S( -55, -50), S( -33,  13)},
   {S(   0,   0), S(  15, -36), S(  19, -26), S( -17,  -5),
    S( -17,  14), S(  -1,  15), S(-228, -56), S( -22,   3)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S(   1,  25), S( 114, -11), S( -24,  25), S( -23,   9),
    S( -12,   0), S(  -8,  -4), S( -21,   6), S( -18,  -4)},
   {S(   0,  47), S(  58,  12), S( -16,  28), S(  -8,  11),
    S(  -3,   3), S(   4,  -5), S(  -1,  -1), S(  -8,  -4)},
   {S(  11,  37), S(  20,  19), S( -22,  24), S( -16,  14),
    S(   3,   3), S(   8,  -1), S(  10,  -6), S(   4,  -2)},
   {S(   0,  26), S(  20,  20), S( -11,  11), S( -10,   3),
    S( -15,   0), S(   5, -10), S(  -1,  -6), S( -13,   1)}},
  {{S(   0,   0), S( -11, -13), S( -16,  -1), S(  20, -17),
    S(  12,  -6), S(   0, -17), S(  -7,   0), S(  16,  27)},
   {S(   0,   0), S( -13, -28), S(  -7, -10), S(  31, -14),
    S(  -1,  -1), S(   9, -21), S( -10,  -7), S( -15,   2)},
   {S(   0,   0), S( -23, -42), S( -31,  -8), S(   7, -11),
    S(   0,  -1), S(  -7, -14), S( -12, -12), S( -10,   4)},
   {S(   0,   0), S(  -1, -14), S( -16, -18), S( -15,  -4),
    S(  -7,  -4), S(   7, -28), S(  67,  -5), S(  12,  18)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -38,   0), S( -54,  20), S( -84,  23),
    S(  -7,  13), S(  66,   0), S( 152,  59), S(   0,   0)},
   {S(   0,   0), S( -26,   2), S( -45,  22), S( -73,  25),
    S( -12,  27), S(  85,  31), S( 183,  95), S(   0,   0)}},
  {{S(   0,   0), S( -24,   8), S( -47,  13), S( -74,  27),
    S(  -4,  28), S(  87,  37), S( 258, 120), S(   0,   0)},
   {S(   0,   0), S( -27,   7), S( -38,  16), S( -63,  36),
    S(  -1,  55), S(  74, 129), S( 155, 296), S(   0,   0)}},
};

const int PassedFriendlyDistance[RANK_NB] = {
    S(   0,   0), S(   2,  -1), S(   5,  -4), S(   6,  -9),
    S(   4, -11), S(  -4, -12), S(  -7, -10), S(   0,   0),
};

const int PassedEnemyDistance[RANK_NB] = {
    S(   0,   0), S(   1,   3), S(   2,   3), S(  10,  10),
    S(   6,  21), S(  15,  24), S(  29,  23), S(   0,   0),
};

const int PassedSafePromotionPath = S( -26,  22);


/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -16, -35);
const int ThreatMinorAttackedByPawn  = S( -50, -48);
const int ThreatMinorAttackedByMinor = S( -28, -36);
const int ThreatMinorAttackedByMajor = S( -20, -42);
const int ThreatRookAttackedByLesser = S( -53, -11);
const int ThreatQueenAttackedByOne   = S( -38, -15);
const int ThreatOverloadedPieces     = S(  -5, -12);
const int ThreatByPawnPush           = S(  18,  18);
