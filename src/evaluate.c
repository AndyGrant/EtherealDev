...
void evaluateImbalanace(EvalInfo* ei, Board* board){
    ...
    int pawnavg = popcount(board->pieces[PAWN]) / 2;
        pawnavg = MIN(7, pawnavg);
    ...
    ei->midgame[WHITE] -= 0.6 * Imbalance[widx * (9+8) + bidx * 9 + pawnavg];
    ei->endgame[WHITE] -= 1.2 * Imbalance[widx * (9+8) + bidx * 9 + pawnavg];
}
