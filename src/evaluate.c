...
const int PawnValue   = S( 100, 112);
const int KnightValue = S( 418, 354);
const int BishopValue = S( 428, 376);
const int RookValue   = S( 578, 649);
const int QueenValue  = S(1185,1216);
const int KingValue   = S(   0,   0);

const int PieceValues[8][PHASE_NB] = {
    { 100, 112}, { 418, 354}, { 428, 376}, { 578, 649},
    {1185,1216}, {   0,   0}, {   0,   0}, {   0,   0},
};

// Definition of evaluation terms related to Pawns

const int PawnIsolated = S(  -1,  -6);

const int PawnStacked = S( -10, -29);
...
const int PawnConnected32[32] = {
    ...
    S(   0, -12), S(   5,   3), S(   2,  -1), S(   3,  18),
    S(   8,   1), S(  21,   1), S(  15,   9), S(  15,  21),
    S(   9,   1), S(  22,   4), S(  14,   8), S(  18,  18),
    S(  11,   9), S(  23,  19), S(  24,  22), S(  38,  24),
    S(  19,  31), S(  24,  42), S(  72,  55), S(  58,  69),
    S(  95, -53), S( 173, -22), S( 187, -17), S( 248,  53),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightRammedPawns = S(  -2,   5);
...
const int KnightMobility[9] = {
    S( -91,-111), S( -38, -89), S( -21, -42), S(  -6, -15),
    S(   0, -14), S(   5,  -2), S(  16,   0), S(  29,  -1),
    S(  43, -32),
};
...
const int BishopMobility[14] = {
    S( -63,-115), S( -43, -64), S( -18, -44), S(  -7, -22),
    S(   4, -10), S(  15,  -1), S(  20,   6), S(  25,   3),
    S(  27,   9), S(  33,   3), S(  34,   4), S(  40, -14),
    S(  44,  -2), S(   5, -51),
};
...
const int RookOnSeventh = S(   5,  26);

const int RookMobility[15] = {
    S(-156,-108), S( -61,-109), S( -16, -64), S(  -8, -25),
    S(  -7,  -4), S(  -8,  10), S(  -9,  20), S(  -3,  27),
    S(   2,  31), S(   8,  35), S(  11,  40), S(  19,  46),
    S(  20,  47), S(  24,  41), S(  13,  35),
};
...
const int QueenMobility[28] = {
    S( -56,-258), S(-159,-333), S( -54,-191), S( -36,-177),
    S( -20,-125), S( -22, -66), S( -14, -86), S( -16, -77),
    S( -13, -56), S(  -9, -50), S(  -8, -27), S(  -4, -25),
    S(  -3, -12), S(   1,  -7), S(   2,  -3), S(   0,   5),
    S(   3,  15), S(   1,  15), S(   8,  21), S(  -1,  21),
    S(   1,  20), S(  11,  20), S(  10,   4), S(  28,   5),
    S(  28,   8), S(  42, -10), S( -28,  -6), S(  -4, -10),
};
...
const int KingDefenders[12] = {
    S( -27,   4), S( -20,   0), S(   0,   0), S(  10,   1),
    S(  21,  -2), S(  32,   0), S(  33,   6), S(  52,-138),
...
const int KingShelter[2][8][8] = {
  {{S( -17,   10), S(  10,   -9), S(  16,    0), S(  17,   -1), S(   8,    3), S(  22,  -2), S(  21,  -23), S( -24,   9)},
   {S(   0,    4), S(  16,   -8), S(  14,   -9), S(  -3,  -12), S( -17,   -2), S( -73,  70), S(  21,   48), S( -28,   2)},
   {S(  11,    8), S(   8,   -2), S( -19,    1), S(  -5,   -2), S( -13,    0), S(  18, -12), S( -12,   46), S( -14,   2)},
   {S(   4,   24), S(  10,   -2), S(  -7,   -8), S(  21,  -12), S(  21,  -28), S( -17, -26), S(-118,   47), S(  -1,  -1)},
   {S( -18,   16), S(  -3,    1), S( -28,    2), S( -10,    1), S( -18,  -15), S( -32, -21), S(  24,  -14), S( -14,  -1)},
   {S(  19,    1), S(  17,   -2), S( -17,    0), S(  -6,  -19), S(  12,  -28), S(  25, -37), S(  42,  -28), S( -13,   0)},
   {S(  14,    0), S(   0,  -10), S( -27,   -6), S( -17,  -12), S( -24,  -20), S( -46,   6), S(  18,   44), S( -24,   9)},
   {S( -18,   -4), S(   0,   -7), S(   6,    1), S(   5,    4), S( -14,   10), S(   0,  33), S(-148,   81), S( -18,  15)}},
  {{S(   0,    0), S(  -1,  -17), S(   1,  -14), S( -44,   19), S(   9,   10), S(  12,  67), S(-110,   -3), S( -48,  13)},
   {S(   0,    0), S(  18,   -5), S(  11,   -5), S(  -5,   -5), S(  18,  -21), S(   5,  74), S(-215,  -15), S( -38,   8)},
   {S(   0,    0), S(  24,    0), S(   0,   -7), S(  24,  -22), S(  18,   -5), S(-120,  26), S( -88,  -77), S( -19,   0)},
   {S(   0,    0), S(  -6,    5), S( -11,   12), S( -12,    0), S( -23,   -1), S( -90,  13), S(   3,  -26), S( -29,  -2)},
   {S(   0,    0), S(   2,    3), S(   4,   -6), S(  23,   -5), S(  16,  -19), S( -40,   2), S( -80,  -38), S(  -4,  -4)},
   {S(   0,    0), S(   7,    0), S( -14,   -1), S(  -8,  -12), S(  27,  -33), S( -47,   4), S(  28,   56), S( -27,   1)},
   {S(   0,    0), S(  11,    0), S(  -1,    1), S( -15,   -3), S( -17,  -11), S(  12,  12), S( -39,  -60), S( -40,  14)},
   {S(   0,    0), S(   4,  -26), S(   7,  -15), S( -25,    0), S( -21,    5), S(   5,  -9), S(-172,  -34), S( -45,  15)}},
};
...
const int PassedPawn[2][2][8] = {
  {{S(   0,    0), S( -29,  -22), S( -24,    3), S( -14,   -5), S(  25,   -3), S(  61,   -7), S( 148,   33), S(   0,    0)},
   {S(   0,    0), S(   0,    3), S( -15,   20), S( -11,   31), S(   9,   35), S(  63,   49), S( 178,  112), S(   0,    0)}},
  {{S(   0,    0), S(  -2,   14), S( -10,    6), S(  -5,   21), S(  30,   28), S(  77,   53), S( 214,  116), S(   0,    0)},
   {S(   0,    0), S(  -1,   11), S(  -5,   18), S( -12,   51), S(   0,  104), S(  40,  192), S( 108,  334), S(   0,    0)}},
};


// Definition of evaluation terms related to Threats

const int ThreatPawnAttackedByOne = S( -15, -26);
const int ThreatMinorAttackedByPawn = S( -63, -47);
const int ThreatMinorAttackedByMajor = S( -36, -35);
const int ThreatQueenAttackedByOne = S( -71,   1);
