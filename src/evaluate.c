...
const int PawnValue   = S( 107, 115);
const int KnightValue = S( 458, 407);
const int BishopValue = S( 477, 425);
const int RookValue   = S( 661, 683);
...
const int PawnPSQT32[32] = {
    ...
    S( -21,   7), S(   3,   2), S(  -7,   4), S(  -4,  -2),
    S( -26,   4), S( -14,   4), S( -11,  -4), S(  -2, -13),
    S( -18,  10), S( -14,  10), S(   5,  -9), S(   5, -21),
    S( -11,  12), S(  -6,   7), S( -10,  -6), S(  -5, -20),
...
const int KnightPSQT32[32] = {
    ...
    S(  14, -26), S(  21, -22), S(  14, -15), S(  25,  -4),
    S(  20,   1), S(  25,   6), S(  32,  17), S(  35,  23),
...
const int BishopPSQT32[32] = {
    S(   8, -26), S(  12, -18), S(  -6,  -8), S(  12, -11),
    S(  30, -35), S(  10, -35), S(  21, -20), S(  14, -11),
...
const int QueenPSQT32[32] = {
    S(  31, -66), S(   6, -45), S(  15, -59), S(  22, -43),
...
const int KingPSQT32[32] = {
    S(  47, -77), S(  38, -50), S( -17,  -9), S( -31, -19),
    S(  27, -20), S( -14, -10), S( -44,  14), S( -63,  16),
...
const int PawnStacked[2] = { S(  -5, -18), S(  -6,  -9) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   4,  -7), S(  11,  -5), S(   8, -11),
    S(   7, -12), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S(  -9, -28), S(  -4, -25), S(   3, -28),
    S(  28, -30), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -2,  -6), S(  12,   0), S(   1,   1), S(   8,  12),
    S(  13,   0), S(  26,  -5), S(  21,   3), S(  23,  10),
    S(   8,  -1), S(  24,   1), S(   9,   8), S(  16,  15),
    S(  10,   9), S(  21,  14), S(  25,  20), S(  29,  19),
...
const int KnightMobility[9] = {
    S( -78,-106), S( -31,-102), S( -14, -42), S(  -3, -15),
    S(   9,  -6), S(  12,  10), S(  20,  13), S(  30,  12),
    S(  43,  -4),
};
...
const int BishopBehindPawn = S(   1,  19);

const int BishopLongDiagonal = S(  21,  10);

const int BishopMobility[14] = {
    S( -69,-149), S( -33, -99), S( -11, -54), S(  -2, -28),
...
const int RookFile[2] = { S(  11,   3), S(  34,   2) };
...
const int QueenMobility[28] = {
    ...
    S(  -6,-144), S(  -4, -80), S(   1, -44), S(   0, -12),
    S(   7, -11), S(   7,  11), S(  13,  14), S(  14,  32),
...
const int KingDefenders[12] = {
    S( -26,   4), S(  -6,  -1), S(   3,   3), S(  10,   5),
    S(  17,   5), S(  26,   1), S(  29,  -4), S(  12,  -1),
...
const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -13,  32), S( 112,  -6), S( -23,  25), S( -14,   3),
   ...
   {S(  -8,  53), S(  51,  14), S( -20,  22), S(  -3,   9),
    S(  -2,   4), S(   6,  -5), S(  -2,   2), S( -10,   3)},
   {S(   3,  43), S(   4,  28), S( -22,  20), S( -10,   7),
    S(   2,   1), S(   7,   0), S(  14,  -6), S(   6,   2)},
   {S(  -7,  28), S(   4,  25), S( -22,   9), S( -14,   0),
    S( -12,   1), S(  10, -12), S(   2,  -9), S( -12,   4)}},
...
const int PassedFriendlyDistance[FILE_NB] = {
    ...
    S(   4, -14), S( -10, -10), S( -17,  -2), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   3,   0), S(   4,   0), S(   9,   8),
