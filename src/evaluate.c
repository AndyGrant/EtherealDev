...
const int PawnValue   = S( 107, 133);
const int KnightValue = S( 458, 415);
const int BishopValue = S( 481, 436);
const int RookValue   = S( 661, 693);
const int QueenValue  = S(1284,1363);
...
const int PawnPSQT32[32] = {
    ...
    S( -20,   5), S(   4,  -1), S(  -8,   1), S(  -5,  -4),
    S( -27,   2), S( -13,   3), S( -11,  -6), S(  -3, -15),
    S( -19,  11), S( -13,  13), S(   6,  -9), S(   7, -22),
    S( -10,  12), S(  -5,   9), S(  -9,  -7), S(  -5, -20),
    S(  -6,  26), S(  -3,  22), S(   1,  11), S(  29, -16),
    S( -18, -49), S( -67, -19), S(   0, -33), S(  38, -47),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -49, -26), S(  -6, -29), S( -16, -30), S(   0, -16),
    S(  -1, -21), S(   2, -12), S(   1, -28), S(  10, -19),
    S(  11, -25), S(  21, -21), S(  15, -18), S(  26,  -4),
    S(  22,   4), S(  24,   6), S(  32,  16), S(  34,  22),
    S(  27,  16), S(  32,   8), S(  44,  27), S(  36,  36),
    S( -15,  14), S(   4,  11), S(  34,  27), S(  31,  28),
    S(  10, -10), S(  -8,   0), S(  29, -20), S(  43,   1),
    S(-173, -20), S( -81,  -2), S(-111,  18), S( -31,  -1),
};

const int BishopPSQT32[32] = {
    S(   3, -27), S(  11, -16), S(  -9,  -8), S(  10, -10),
    S(  32, -33), S(   7, -35), S(  22, -18), S(  14, -10),
    S(  15, -10), S(  30, -12), S(   3, -17), S(  27,  -3),
    S(  16,  -7), S(  23,  -2), S(  18,   6), S(  30,   7),
    S( -11,  13), S(  24,   6), S(   6,  15), S(  16,  18),
    S(   3,   6), S(  -3,  17), S(   0,   4), S(  18,  14),
    S( -46,  15), S( -57,   3), S( -10,  10), S( -21,  15),
    S( -53,  -8), S( -46,   8), S( -87,  15), S( -90,  29),
};

const int RookPSQT32[32] = {
    S( -12, -30), S( -15, -20), S(  -1, -23), S(   7, -27),
    S( -55, -15), S(  -9, -35), S(  -7, -33), S(   7, -37),
    S( -28, -17), S(  -3, -12), S( -16, -16), S(   2, -25),
    S( -17,  -3), S( -11,   8), S(  -7,   3), S(   8,  -2),
    S(  -5,  12), S(  14,  11), S(  23,   7), S(  44,   3),
    S( -16,  24), S(  28,  10), S(   5,  25), S(  39,   6),
    S(  -2,   6), S( -17,  15), S(   8,   4), S(  25,   8),
    S(  38,  21), S(  27,  24), S(   8,  31), S(  19,  27),
};

const int QueenPSQT32[32] = {
    S(  27, -55), S(   5, -40), S(  13, -54), S(  21, -43),
    S(  13, -40), S(  27, -55), S(  30, -69), S(  17, -29),
    S(   9, -25), S(  25, -21), S(   6,   4), S(   5,  -3),
    S(  11,  -6), S(  17,  13), S(  -4,  20), S( -17,  60),
    S(  -5,  16), S(  -7,  43), S( -12,  20), S( -31,  70),
    S( -24,  28), S( -17,  18), S( -23,  22), S( -12,  24),
    S(  -3,  27), S( -66,  63), S(  -8,  13), S( -42,  52),
    S( -11,  14), S(  18,   8), S(   8,   8), S(  -7,  19),
};

const int KingPSQT32[32] = {
    S(  55, -83), S(  49, -55), S( -12, -14), S( -25, -24),
    S(  30, -23), S( -12, -13), S( -44,  11), S( -63,  13),
    S(   3, -25), S(  11, -20), S(  16,   1), S( -14,  18),
    S(   3, -31), S(  89, -31), S(  45,  10), S(  -4,  29),
    S(   7, -12), S(  99, -29), S(  50,  14), S(   6,  23),
    S(  45, -31), S( 123, -24), S(  96,   2), S(  39,  -3),
    S(   4, -54), S(  46, -10), S(  31,   3), S(   8,  -5),
    S(   7,-107), S(  73, -57), S( -21, -18), S( -19, -15),
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -27, -11), S( -13,   9), S( -18,  31),
    S(   1,  61), S(  40,  67), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -14,  19), S(  -5,  22), S(   2,  48),
    S(  20,  83), S(  32,  56), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -7, -13);

const int PawnStacked[2] = { S(  -1, -25), S(  -6,  -9) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   4,  -5), S(   9,  -5), S(  11, -14),
    S(  10, -12), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -10, -25), S(  -5, -19), S(   5, -23),
    S(  29, -18), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -2,  -5), S(  14,  -1), S(   2,   1), S(   7,   8),
    S(  16,   0), S(  28,  -6), S(  22,   3), S(  25,   9),
    S(   9,  -2), S(  27,   0), S(   9,   7), S(  19,  11),
    S(   7,  12), S(  22,  14), S(  27,  22), S(  29,  20),
    S(  57,  20), S(  45,  43), S(  67,  54), S(  82,  58),
    S( 112,  -1), S( 204,   7), S( 228,  29), S( 240,  47),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightInSiberia[4] = {
    S( -11,  -1), S( -14,  -7), S( -27,  -7), S( -22,  -4),
};

const int KnightMobility[9] = {
    S( -75,-104), S( -29,-100), S( -13, -41), S(   0, -14),
    S(  12,  -3), S(  17,  13), S(  25,  18), S(  36,  17),
    S(  51,   1),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  23,  68);
...
const int BishopLongDiagonal = S(  26,  13);

const int BishopMobility[14] = {
    S( -69,-148), S( -32, -95), S(  -9, -47), S(   1, -22),
    S(  13, -12), S(  21,   1), S(  24,  11), S(  23,  16),
    S(  22,  23), S(  26,  24), S(  28,  23), S(  49,  12),
    S(  44,  21), S(  71, -16),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  12,   3), S(  36,   2) };

const int RookOnSeventh = S(  -3,  35);

const int RookMobility[15] = {
    S(-151,-113), S( -63,-114), S( -16, -60), S(  -7, -20),
    S(  -7,   0), S(  -7,  15), S(  -6,  28), S(   0,  31),
    S(   9,  35), S(  11,  41), S(  14,  47), S(  22,  51),
    S(  22,  57), S(  42,  42), S( 104,  -7),
};

/* Queen Evaluation Terms */

const int QueenMobility[28] = {
    S( -61,-263), S(-210,-387), S( -58,-201), S( -27,-192),
    S( -10,-140), S(  -7, -82), S(  -2, -49), S(  -2, -17),
    S(   5, -17), S(   6,   6), S(  12,   7), S(  13,  25),
    S(  16,  16), S(  19,  26), S(  15,  31), S(  11,  40),
    S(  13,  37), S(   2,  42), S(   6,  39), S(   1,  34),
    S(  10,   8), S(  22, -14), S(  25, -43), S(  26, -60),
    S(  10, -73), S(  22,-100), S( -56, -39), S( -31, -62),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -22,   0), S(  -4,  -1), S(   2,   4), S(   9,   7),
    S(  16,   5), S(  25,   0), S(  26,  -4), S(  13,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  39,  46), S(  21,  33), S(   6,  15), S( -20, -19),
    S( -23, -60), S( -19, -74), S( -17, -73), S( -11, -68),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -12,   1), S(  13, -26), S(  28,  -9), S(  18,   7),
    S(  10,   5), S(   0,   2), S(  -3, -33), S( -51,  18)},
   {S(  16,  -7), S(  24, -18), S(   4,  -7), S( -14,   3),
    S( -30,  15), S( -71,  68), S(  92,  82), S( -16,   2)},
   {S(  36,  -2), S(  11,  -8), S( -32,   4), S( -14,  -9),
    S( -19,  -4), S( -12,   1), S(   0,  66), S( -14,   0)},
   {S(   2,  10), S(  29, -16), S(   6, -13), S(  14, -19),
    S(  33, -37), S( -61,   4), S(-136,  52), S(   8,  -5)},
   {S( -19,  12), S(   3,  -4), S( -36,   2), S( -25,   7),
    S( -23,  -9), S( -43,  -3), S(  33, -17), S( -11,   2)},
   {S(  50, -16), S(  23, -14), S( -22,   3), S( -12, -21),
    S(   5, -25), S(  17, -19), S(  41, -30), S( -25,   2)},
   {S(  32, -16), S(   0, -16), S( -27,  -1), S( -25,  -7),
    S( -31,  -8), S( -38,  31), S(   0,  44), S( -10,   0)},
   {S( -11, -14), S(   5, -18), S(  11,   2), S(  -1,  11),
    S( -13,  16), S( -10,  38), S(-190,  87), S( -21,  15)}},
  {{S(   0,   0), S( -13, -26), S(   8, -21), S( -39,  17),
    S( -23,   2), S(   3,  42), S(-167,  -8), S( -42,  15)},
   {S(   0,   0), S(  27, -22), S(   9,  -7), S( -20,  -1),
    S(  -1, -14), S(  26,  65), S(-184,  -3), S( -30,   8)},
   {S(   0,   0), S(  26, -13), S(   1, -15), S(   5, -21),
    S(  19,  -4), S( -87,  47), S( -84, -73), S( -16,   1)},
   {S(   0,   0), S( -11,   4), S( -10,  -5), S( -25,   1),
    S( -28,   2), S( -99,  31), S(   7, -41), S( -21,   3)},
   {S(   0,   0), S(  15,   1), S(  13,  -8), S(  15, -14),
    S(  15, -28), S( -57,  15), S(-104, -61), S(   0,   3)},
   {S(   0,   0), S(  -1,  -4), S( -30,   2), S( -26,  -8),
    S(  20, -23), S( -38,   3), S(  55,  38), S( -14,  -2)},
   {S(   0,   0), S(  25, -17), S(  15, -16), S( -13,  -9),
    S( -32,  12), S(  -8,  17), S( -56, -49), S( -34,  16)},
   {S(   0,   0), S(  16, -44), S(  27, -35), S( -20,  -8),
    S( -19,  18), S(  -6,  20), S(-228, -55), S( -24,   6)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S(  -6,  31), S( 115,  -4), S( -25,  23), S( -12,   0),
    S( -13,  -1), S(  -7,  -5), S( -16,   3), S( -27,   6)},
   {S(  -5,  54), S(  54,  15), S( -20,  18), S(  -1,   4),
    S(   0,   1), S(   6,  -6), S(  -4,   3), S( -15,   8)},
   {S(   6,  44), S(  12,  29), S( -26,  17), S( -13,   4),
    S(   0,   0), S(   3,   0), S(   9,  -4), S(   2,   7)},
   {S(  -3,  29), S(   9,  23), S( -25,   6), S( -20,  -3),
    S( -16,   0), S(   7, -12), S(   2,  -9), S( -15,   7)}},
  {{S(   0,   0), S( -16, -16), S( -18,  -4), S(  27, -20),
    S(  12,  -4), S(   7, -17), S(  -1,   1), S(  17,  29)},
   {S(   0,   0), S( -16, -35), S(   2, -12), S(  47, -12),
    S(   1,  -1), S(  14, -21), S(  -7,  -9), S( -18,   1)},
   {S(   0,   0), S( -29, -51), S( -25,  -9), S(  15, -12),
    S(   3,  -1), S(  -9, -13), S( -12, -13), S( -11,   6)},
   {S(   0,   0), S(  -2, -18), S( -19, -17), S( -19,  -2),
    S(  -4,  -6), S(   4, -25), S(  72, -11), S(  13,  20)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -38,   1), S( -45,  25), S( -70,  31),
    S(   2,  11), S(  70, -11), S( 155,  49), S(   0,   0)},
   {S(   0,   0), S( -30,  11), S( -46,  39), S( -69,  44),
    S(  -6,  39), S(  98,  23), S( 186,  87), S(   0,   0)}},
  {{S(   0,   0), S( -23,  23), S( -46,  37), S( -69,  50),
    S(   4,  44), S(  92,  35), S( 261,  99), S(   0,   0)},
   {S(   0,   0), S( -31,  21), S( -43,  37), S( -66,  58),
    S(   7,  64), S( 100, 116), S( 160, 263), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   1,   3), S(   5,  -4), S(   6, -11),
    S(   6, -14), S(  -7,  -9), S( -17,  -1), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   3,  -1), S(   4,  -1), S(   9,   7),
    S(   1,  21), S(   5,  28), S(  31,  20), S(   0,   0),
};

/* Threat Evaluation Terms */

const int PassedSafePromotionPath    = S( -19,  41);
const int ThreatWeakPawn             = S( -11, -30);
const int ThreatMinorAttackedByPawn  = S( -51, -55);
const int ThreatMinorAttackedByMinor = S( -23, -37);
const int ThreatMinorAttackedByMajor = S( -21, -51);
const int ThreatRookAttackedByLesser = S( -46, -20);
const int ThreatMinorAttackedByKing  = S( -27, -23);
const int ThreatRookAttackedByKing   = S( -19, -26);
const int ThreatQueenAttackedByOne   = S( -35, -41);
const int ThreatOverloadedPieces     = S(  -8, -15);
const int ThreatByPawnPush           = S(  14,  25);
...
const int ClosednessKnightAdjustment[9] = {
    S(  -4,  -1), S(   0,  15), S(  -2,  22), S(   3,  19),
    S(   6,  21), S(   8,  12), S(  13,   7), S(  -4,  26),
    S(  -2,  15),
};

const int ClosednessRookAdjustment[9] = {
    S(  67,   3), S(  13,  42), S(   8,  24), S(   1,   5),
    S(  -3,   7), S(  -5,  -7), S( -10, -10), S( -22, -15),
    S( -30, -18),
};
...
const int ComplexityPawnFlanks  = S(   0, 101);
