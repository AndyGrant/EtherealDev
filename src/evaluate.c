...
const int PawnValue   = S( 100, 135);
const int KnightValue = S( 458, 393);
const int BishopValue = S( 462, 427);
const int RookValue   = S( 646, 768);
const int QueenValue  = S(1321,1363);
const int KingValue   = S( 147, 147);

const int PieceValues[8][PHASE_NB] = {
    { 100, 135}, { 458, 393}, { 462, 427}, { 646, 768},
    {1321,1363}, { 147, 147}, {   0,   0}, {   0,   0},
};
...
const int PawnStacked = S(   0, -37);

const int PawnBackwards[2] = { S(   6,  -6), S( -14, -11) };

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(   0, -14), S(   7,   5), S(   4,   0), S(   6,  25),
    S(   7,   4), S(  22,   7), S(  16,  13), S(  16,  27),
    S(   8,   6), S(  15,   9), S(  14,  13), S(  19,  21),
    S(   1,  20), S(  16,  25), S(  16,  29), S(  33,  30),
    S(  17,  56), S(  27,  74), S(  75,  71), S(  62,  89),
    S( 175,  27), S( 197,  16), S( 207,  49), S( 186,  90),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightRammedPawns = S(   4,  20);

const int KnightOutpost[2] = { S(  24, -16), S(  36,  27) };

const int KnightMobility[9] = {
    S( -88,-135), S( -33, -80), S(  -8, -21),
    S(   6,   9), S(  17,  12), S(  24,  27),
    S(  36,  29), S(  53,  31), S(  69,   2),
};


// Definition of evaluation terms related to Bishops

const int BishopPair = S(  64,  90);

const int BishopRammedPawns = S(  -6,   1);

const int BishopOutpost[2] = { S(  26,   1), S(  54,   5) };

const int BishopMobility[14] = {
    S( -69,-116), S( -54, -57), S( -15, -29), S(   3,  -1),
    S(  15,  15), S(  29,  25), S(  36,  38), S(  42,  36),
    S(  45,  43), S(  52,  38), S(  61,  36), S(  78,  18),
    S(  68,  35), S(  50, -10),
};
...
const int RookFile[2] = { S(  12,  16), S(  39,  17) };

const int RookOnSeventh = S(   0,  42);

const int RookMobility[15] = {
    S(-175, -91), S( -79,-108), S( -10, -59), S(   0, -12),
    S(   1,  13), S(   0,  35), S(   0,  50), S(   5,  58),
    S(   9,  66), S(  15,  67), S(  18,  76), S(  29,  83),
    S(  33,  85), S(  36,  82), S(  33,  74),
};
...
const int QueenMobility[28] = {
    S( -67,-230), S(-135,-380), S( -44,-173), S( -20,-152),
    S(  -7, -82), S( -10,  -9), S(   0, -25), S(   4, -34),
    S(  10,  -5), S(  15,   6), S(  18,  32), S(  21,  37),
    S(  23,  52), S(  25,  59), S(  32,  63), S(  33,  80),
    S(  37,  92), S(  38,  96), S(  49, 108), S(  45, 114),
    S(  50, 122), S(  69, 122), S(  79, 109), S(  92, 117),
    S( 127, 115), S( 111, 106), S(  92, 116), S( 100, 124),
};
...
const int KingDefenders[12] = {
    S( -48,  -6), S( -28,   4), S(  -1,   1), S(  12,   0),
    S(  31,   4), S(  44,  11), S(  58, -10), S(  68,  70),
    S(  10,   5), S(  10,   5), S(  10,   5), S(  10,   5),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -26,  20), S(   7, -12), S(  20,   2), S(  32,   6), S(   9,   4), S(  19,  -4), S( -41, -22), S( -37,   0)},
   {S(   4,  10), S(  18,  -9), S(   8,  -8), S( -13,  -6), S( -23,   3), S(-116,  89), S(   0, 117), S( -38,   2)},
   {S(   8,  21), S(   3,   0), S( -25,   4), S( -13,   1), S( -28,   1), S(  -8,  -5), S( -88,  72), S( -21,   2)},
   {S( -22,  39), S(   6,   1), S( -14,  -3), S(  21, -16), S(  19, -31), S( -23, -21), S(-117,   3), S(  -6,   3)},
   {S( -18,  24), S(  -9,   1), S( -39,   6), S(  -6,   1), S( -25, -19), S( -17, -21), S( -97,   4), S( -21,   4)},
   {S(  24,   4), S(  19,  -9), S( -25,  -2), S(  -1, -20), S(   8, -37), S(  28, -52), S(  80, -35), S( -16,   3)},
   {S(  30,   0), S(   3, -13), S( -26, -12), S( -13, -20), S( -18, -25), S( -36,   6), S( -67,  18), S( -29,  10)},
   {S( -17,  -4), S(   1, -13), S(   8,  -3), S(  12,  -1), S(  -4,   6), S(   3,  32), S(-141,  76), S( -17,  17)}},
  {{S(   0,   0), S(  -3, -29), S(   6, -25), S( -54,  15), S( -68,  -8), S(  31,  15), S(-196,  11), S( -61,  15)},
   {S(   0,   0), S(  18, -14), S(   7,  -9), S(  -9,  -6), S( -10, -28), S(  -9,  57), S(-179, -35), S( -46,   5)},
   {S(   0,   0), S(  19,   0), S(  -2,  -4), S(  26, -27), S(  31,   1), S( -89,  62), S(-105,-103), S( -21,   0)},
   {S(   0,   0), S( -18,  18), S( -35,  26), S( -23,   0), S( -20, -10), S(-112,  25), S( -99,  33), S( -32,   3)},
   {S(   0,   0), S(  -3,   6), S(   3,  -4), S(  20,  -6), S(  15, -21), S( -62,  37), S( -43, -96), S( -11,   1)},
   {S(   0,   0), S(   8,  -1), S( -20,   0), S( -31, -12), S(  11, -31), S( -71,  11), S(-118,  17), S( -32,   4)},
   {S(   0,   0), S(  17,  -6), S(   0,  -4), S( -16, -11), S( -29,  -8), S( -15,   0), S( -75, -35), S( -44,  15)},
   {S(   0,   0), S(  16, -37), S(  15, -23), S( -14,  -9), S( -23, -10), S(  10,  -9), S(-112, -69), S( -43,  20)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -28, -37), S( -16,   7), S(  -7,  -4), S(  29,   2), S(  59, -15), S( 152, -14), S(   0,   0)},
   {S(   0,   0), S(   3,   9), S(  -9,  37), S(  -8,  52), S(   9,  64), S(  79,  66), S( 198,  97), S(   0,   0)}},
  {{S(   0,   0), S(  -5,   9), S( -10,   4), S(  -5,  27), S(  34,  35), S(  94,  58), S( 257, 146), S(   0,   0)},
   {S(   0,   0), S(   1,  17), S(  -7,  27), S( -16,  67), S(  -5, 129), S(  48, 230), S( 155, 452), S(   0,   0)}},
};


// Definition of evaluation terms related to Threats

const int ThreatPawnAttackedByOne    = S( -22, -65);

const int ThreatMinorAttackedByPawn  = S( -81, -56);

const int ThreatMinorAttackedByMajor = S( -48, -72);

const int ThreatMajorAttackedByMinor = S( -51, -30);

const int ThreatQueenAttackedByOne   = S( -65, -14);
