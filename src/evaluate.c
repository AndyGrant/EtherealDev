...
const int PawnValue   = S( 100, 122);
const int KnightValue = S( 461, 391);
const int BishopValue = S( 471, 415);
const int RookValue   = S( 637, 714);
const int QueenValue  = S(1289,1331);
const int KingValue   = S( 166, 166);

const int PieceValues[8][PHASE_NB] = {
    { 100, 122}, { 461, 391}, { 471, 415}, { 637, 714},
    {1289,1331}, { 166, 166}, {   0,   0}, {   0,   0},
};
...
const int PawnStacked = S( -10, -31);

const int PawnBackwards[2] = { S(   6,  -2), S( -12, -11) };

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(   0, -16), S(   6,   1), S(   3,  -3), S(   5,  20),
    S(   7,   0), S(  20,   0), S(  14,   8), S(  17,  21),
    S(   6,   0), S(  19,   4), S(  14,   8), S(  17,  16),
    S(   6,  12), S(  19,  19), S(  18,  24), S(  37,  24),
    S(  24,  54), S(  25,  61), S(  74,  60), S(  55,  75),
    S(  91, -22), S( 168,   1), S( 208,  14), S( 181,  71),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightRammedPawns = S(   1,   5);

const int KnightOutpost[2] = { S(  18, -35), S(  35,   7) };

const int KnightMobility[9] = {
    S( -90, -96), S( -37, -88), S( -18, -42),
    S(  -6, -12), S(   3, -14), S(   8,   0),
    S(  17,  -1), S(  32,  -1), S(  48, -33),
};


// Definition of evaluation terms related to Bishops

const int BishopPair = S(  43,  69);

const int BishopRammedPawns = S( -10,  -6);

const int BishopOutpost[2] = { S(  19, -16), S(  51, -10) };

const int BishopMobility[14] = {
    S( -65,-126), S( -50, -67), S( -19, -46), S(  -6, -21),
    S(   4,  -7), S(  17,   0), S(  22,   8), S(  29,   4),
    S(  30,  10), S(  36,   3), S(  42,   4), S(  52, -14),
    S(  45,  -1), S(  39, -35),
};
...
const int RookFile[2] = { S(  12,   1), S(  40,  -8) };

const int RookOnSeventh = S(   0,  24);

const int RookMobility[15] = {
    S(-165,-103), S( -76,-122), S( -16, -68), S(  -9, -26),
    S(  -8,  -3), S(  -7,  14), S(  -7,  25), S(  -3,  31),
    S(   0,  36), S(   6,  36), S(  10,  42), S(  20,  48),
    S(  21,  50), S(  27,  47), S(  16,  45),
};
...
const int QueenMobility[28] = {
    S( -60,-260), S( -57,-232), S( -43,-198), S( -39,-191),
    S( -16,-123), S( -27, -63), S( -17, -89), S( -19, -84),
    S( -15, -59), S( -10, -53), S(  -8, -27), S(  -5, -29),
    S(  -5, -15), S(  -1, -10), S(   1,  -6), S(   0,   4),
    S(   4,  16), S(   1,  14), S(  13,  25), S(   0,  25),
    S(   4,  28), S(  19,  30), S(  20,  11), S(  36,  16),
    S(  54,  25), S(  52,   1), S( -14,  -3), S(  24,  12),
};
...
const int KingDefenders[12] = {
    S( -40,  -4), S( -20,   5), S(   0,   1), S(   9,   0),
    S(  24,  -2), S(  36,   3), S(  39,   9), S(  33,-142),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -16,  17), S(   5,  -9), S(  16,   3), S(  24,   4), S(  14,   2), S(  27,  -4), S( -11, -49), S( -34,   0)},
   {S(   4,   7), S(  15,  -6), S(  15,  -9), S(   1, -12), S( -29,  -3), S( -71,  82), S(  56,  89), S( -33,   0)},
   {S(  12,  14), S(   8,   0), S( -19,   0), S(  -8,  -1), S( -28,  -3), S(  -6, -17), S( -44,  61), S( -16,   1)},
   {S(  11,  29), S(  15,   0), S(  -4,  -8), S(  18, -13), S(  16, -33), S( -25, -31), S( -63,  28), S(  -1,   0)},
   {S( -15,  19), S(   0,   2), S( -29,   1), S( -13,   3), S( -34, -16), S( -56, -24), S(   6,   0), S( -16,   0)},
   {S(  21,   2), S(  17,  -4), S( -19,  -1), S(  -2, -20), S(   6, -32), S(  14, -49), S(  74, -35), S( -13,   0)},
   {S(  20,   2), S(   2,  -9), S( -29,  -8), S( -20, -13), S( -21, -16), S( -35,   2), S(  -2,  41), S( -29,   8)},
   {S( -17,  -3), S(  -1,  -9), S(   5,   0), S(   3,   3), S( -11,  13), S(   5,  32), S(-171,  75), S( -18,  15)}},
  {{S(   0,   0), S(   1, -17), S(   9, -15), S( -48,  16), S(  15, -11), S(   7,  41), S(-153,  11), S( -56,   9)},
   {S(   0,   0), S(  16,  -6), S(   7,  -6), S(   0,  -6), S(  12, -31), S(  33,  86), S(-157,  31), S( -47,   3)},
   {S(   0,   0), S(  23,   0), S(   0,  -6), S(  20, -25), S(  16,  -6), S(-102,  47), S(-162, -97), S( -22,  -1)},
   {S(   0,   0), S(  -7,   8), S( -12,  13), S( -17,   2), S( -25,   0), S(-105,   3), S(  27, -51), S( -26,   0)},
   {S(   0,   0), S(   5,   5), S(   8,  -7), S(  23,  -5), S(   8, -20), S( -53,   8), S( -86, -92), S(  -6,  -2)},
   {S(   0,   0), S(   9,   1), S( -16,  -2), S( -15, -13), S(  19, -33), S( -14,  11), S(  26,  21), S( -31,   0)},
   {S(   0,   0), S(  13,  -1), S(  -2,   0), S( -19,  -5), S( -19, -14), S(  21,   0), S( -96, -48), S( -45,  13)},
   {S(   0,   0), S(   8, -27), S(  11, -14), S( -22,   0), S( -24,  -4), S(  10, -21), S(-182, -74), S( -46,  16)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -31, -29), S( -24,   8), S( -13,  -1), S(  21,  -1), S(  62,  -6), S( 159,  34), S(   0,   0)},
   {S(   0,   0), S(  -1,   3), S( -15,  23), S( -12,  36), S(   5,  44), S(  70,  62), S( 194, 129), S(   0,   0)}},
  {{S(   0,   0), S(  -6,  14), S( -11,   8), S(  -9,  27), S(  27,  32), S(  80,  63), S( 221, 149), S(   0,   0)},
   {S(   0,   0), S(  -5,  10), S( -11,  19), S( -19,  54), S( -11, 110), S(  25, 202), S( 107, 367), S(   0,   0)}},
};
...
const int ThreatMinorAttackedByPawn  = S( -62, -43);

const int ThreatMinorAttackedByMajor = S( -33, -29);
