...
const int PawnValue[PHASE_NB] = {  63,  77};

const int PawnIsolated[PHASE_NB] = {  -2,  -3};

const int PawnStacked[PHASE_NB] = {  -6, -20};

const int PawnBackwards[2][PHASE_NB] = { {   3,  -3}, {  -9,  -8} };
...
const int PawnConnected32[32][PHASE_NB] = {
    ...
    {   5,   0}, {  12,   0}, {  10,   6}, {  12,  13},
    {   5,   0}, {  12,   2}, {   9,   5}, {  12,  10},
    {   3,   7}, {  12,  11}, {  14,  14}, {  25,  14},
    {  24,  27}, {  26,  33}, {  52,  40}, {  42,  47},
    { 155,  22}, { 101,   0}, { 130,  31}, { 151,  58},
    {   0,   0}, {   0,   0}, {   0,   0}, {   0,   0}
};

const int KnightValue[PHASE_NB] = { 300, 267};

const int KnightAttackedByPawn[PHASE_NB] = { -33, -18};

const int KnightOutpost[2][PHASE_NB] = { {  13, -23}, {  28,   6} };
...
const int BishopValue[PHASE_NB] = { 297, 265};

const int BishopWings[PHASE_NB] = {   1,   1};

const int BishopPair[PHASE_NB] = {  28,  44};

const int BishopAttackedByPawn[PHASE_NB] = { -36, -18};

const int BishopOutpost[2][PHASE_NB] = { {  15, -10}, {  37,  -8} };

const int BishopMobility[14][PHASE_NB] = {
    { -43, -81}, { -28, -51}, { -14, -32}, {  -4, -16},
    {   2,  -7}, {   9,  -1}, {  14,   5}, {  19,   1},
    {  18,   7}, {  25,   2}, {  27,   2}, {  43, -12},
    {  34,  -6}, {  13, -27}
};

const int RookValue[PHASE_NB] = { 398, 434};

const int RookFile[2][PHASE_NB] = { {   5,   1}, {  26,  -6} };

const int RookOnSeventh[PHASE_NB] = {   4,  16};

const int RookMobility[15][PHASE_NB] = {
    {-114, -50}, { -54, -83}, { -13, -48}, {  -6, -19},
    {  -5,  -2}, {  -5,   8}, {  -5,  17}, {  -1,  19},
    {   0,  23}, {   5,  21}, {   6,  26}, {  14,  30},
    {  14,  30}, {  19,  26}, {  13,  22}
};

const int QueenValue[PHASE_NB] = { 802, 833};

const int QueenChecked[PHASE_NB] = { -17,  61};

const int QueenCheckedByPawn[PHASE_NB] = { -31,  30};

const int QueenMobility[28][PHASE_NB] = {
    {-197, -66}, {  72,-481}, { -45,-252}, { -29, -91},
    {  -7,-155}, { -18, -29}, { -13, -54}, { -10, -54},
    {  -9, -33}, {  -7, -28}, {  -6, -14}, {  -3, -16},
    {   0,  -9}, {  -2,  -5}, {   1,  -4}, {   0,   3},
    {   0,   8}, {   1,   7}, {   7,  13}, {   2,  15},
    {   4,  16}, {   7,  12}, {  21,   8}, {  30,  12},
    {  32,  14}, {  31,   3}, {  13,   4}, {  17,   3}
};
...
const int KingDefenders[12][PHASE_NB] = {
    { -28,  -1}, { -20,   3}, {   0,   1}, {   8,  -1},
    {  17,  -1}, {  25,   3}, {  34,  12}, {  60, -53},
    {   8,   4}, {   8,   4}, {   8,   4}, {   8,   4}
};

const int KingShelter[2][FILE_NB][RANK_NB][PHASE_NB] = {
  {{{  -9,   8}, {   3,  -5}, {  10,   2}, {  11,   8}, {  12,  -6}, {  27,   0}, {   5,  57}, { -24,   2}},
   {{  -1,   6}, {  11,  -2}, {  14,  -5}, {   5, -10}, { -47,   0}, { -34,  31}, {  47,  76}, { -24,   1}},
   {{  17,   5}, {   6,   0}, { -12,   0}, {  -7,  -3}, { -16,   2}, {  22, -12}, {  46,   9}, { -12,   2}},
   {{   7,  17}, {   7,   0}, {  -1,  -7}, {  10,  -8}, {   9, -12}, { -21, -17}, { -59, -10}, {  -1,   0}},
   {{   3,  11}, {   5,   0}, { -17,   1}, {  -7,   1}, { -15, -18}, { -40,  -1}, { -18, -19}, { -11,   0}},
   {{  17,   1}, {  10,  -1}, { -11,   0}, {  -1, -16}, {   4, -27}, {  21, -36}, {  59, -25}, { -10,   0}},
   {{  16,   0}, {   2,  -6}, { -19,  -7}, { -16, -10}, { -15, -11}, { -50,  -2}, { -45,   7}, { -22,   5}},
   {{  -7,  -2}, {   0,  -6}, {   4,   0}, {   1,   1}, {  -9,   3}, {  -4,  23}, { -44,  18}, { -13,  10}}},
  {{{   0,   0}, { -10, -10}, {   5, -16}, { -54,   5}, {  -1,  -6}, {-148,  21}, {  38,  83}, { -31,  10}},
   {{   0,   0}, {  11,  -6}, {   0,  -2}, {  -9,   5}, {  10, -11}, { -63,  60}, {-363,   0}, { -28,   4}},
   {{   0,   0}, {  16,   1}, {   3,  -3}, {  16, -17}, {  -8,  -8}, { -60,  29}, {-322, -44}, { -14,   0}},
   {{   0,   0}, {   2,   6}, {  -3,  13}, {  -9,   0}, { -23,   0}, { -30,  27}, {  69,  12}, { -15,  -1}},
   {{   0,   0}, {   2,   2}, {   4,  -1}, {  12,  -3}, {   4, -12}, { -30,  -9}, {  24, -40}, {  -4,  -2}},
   {{   0,   0}, {   5,   0}, {  -2,   0}, { -15,  -6}, {   6, -25}, { -87,  -7}, {-241, -52}, { -18,   0}},
   {{   0,   0}, {   8,  -1}, {   2,  -1}, { -12,  -5}, { -18, -12}, {   4,  -7}, { -51, -38}, { -29,   9}},
   {{   0,   0}, {   5, -19}, {   8, -12}, { -19,   0}, { -34,   2}, {  -1, -18}, {-131,  52}, { -31,  13}}}
};

const int PassedPawn[2][2][RANK_NB][PHASE_NB] = {
  {{{   0,   0}, { -18, -22}, { -14,   6}, { -10,  -4}, {  17,  -1}, {  39,  -7}, {  93,  12}, {   0,   0}},
   {{   0,   0}, {  -3,   0}, {  -8,  17}, { -13,  25}, {   4,  28}, {  43,  33}, { 129,  80}, {   0,   0}}},
  {{{   0,   0}, {  -6,   5}, {  -8,   2}, {  -7,  17}, {  18,  20}, {  62,  39}, { 165,  88}, {   0,   0}},
   {{   0,   0}, {  -3,   7}, {  -8,  11}, { -12,  33}, { -12,  71}, {  18, 124}, {  58, 226}, {   0,   0}}}
};
