...
const int KingShelter[2][RANK_NB] = {
   {S(   5,  10), S(   9,  -4), S(  -7,  -3), S(  -1,  -6), S( -15,  -11), S( -19,  -1), S(-25,  21), S( -21,   3)},
   {S(   0,   0), S(   9,  -4), S(   2 , -5), S( -14,  -5), S(  -4,  -15), S( -38,  18), S(-99, -28), S( -35,   5)}
};
...
int evaluateKings(EvalInfo* ei, Board* board, int colour){
    ...
    for (file = MAX(0, kingFile - 1); file <= MIN(7, kingFile + 1); file++){
        ...
        if (TRACE) T.kingShelter[colour][file == kingFile][distance]++;
    }
