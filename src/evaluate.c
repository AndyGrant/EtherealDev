...
void evaluateRooks(EvalInfo* ei, Board* board, int colour){
    ...
    while (tempRooks){
        ...
        static const int RookSupportsPassedPawn[PHASE_NB] = {  27,  27};
        
        // Rook gains a bonus for supporting or attacks a passed pawn
        if (Files[File(sq)] & attacks & ei->passedPawns){
