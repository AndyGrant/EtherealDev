...
const int PawnValue   = S( 103, 121);
const int KnightValue = S( 463, 409);
const int BishopValue = S( 482, 429);
const int RookValue   = S( 664, 687);
const int QueenValue  = S(1302,1343);
...
const int PawnPSQT32[32] = {
    ...
    S( -22,   6), S(   2,   2), S(  -6,   3), S(  -3,  -3),
    S( -28,   4), S( -13,   4), S( -11,  -4), S(  -2, -13),
    S( -19,  10), S( -14,  12), S(   4,  -7), S(   6, -20),
    S( -12,  12), S(  -7,   9), S( -10,  -5), S(  -6, -18),
    S(  -8,  28), S(  -3,  24), S(   1,  15), S(  30, -12),
    S( -21, -62), S( -60, -31), S(   2, -45), S(  45, -60),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -49, -32), S(  -6, -27), S( -14, -30), S(   2, -14),
    S(   1, -20), S(   1, -11), S(   5, -26), S(  11, -16),
    S(  13, -26), S(  20, -21), S(  15, -15), S(  26,  -3),
    S(  20,   0), S(  27,   6), S(  32,  18), S(  37,  23),
    S(  26,  14), S(  33,   8), S(  42,  28), S(  36,  38),
    S( -18,  11), S(   2,  13), S(  31,  30), S(  33,  28),
    S(  14, -12), S(  -4,  -2), S(  30, -23), S(  40,   0),
    S(-178, -26), S( -85,  -5), S(-111,  18), S( -34,  -4),
};

const int BishopPSQT32[32] = {
    S(   8, -27), S(  13, -18), S(  -6, -10), S(  12, -11),
    S(  31, -35), S(  10, -35), S(  22, -22), S(  14, -10),
    S(  15, -13), S(  28, -15), S(   4, -17), S(  25,  -5),
    S(  15,  -7), S(  20,  -3), S(  17,   5), S(  28,   4),
    S(  -9,  15), S(  22,   6), S(   4,  14), S(  17,  16),
    S(   2,   7), S(  -4,  17), S(  -1,   7), S(  12,  15),
    S( -44,  15), S( -50,   6), S(  -7,   9), S( -23,  14),
    S( -59,  -8), S( -45,   5), S( -83,  15), S( -90,  28),
};

const int RookPSQT32[32] = {
    S( -14, -28), S( -16, -19), S(  -3, -21), S(   5, -26),
    S( -52, -15), S(  -9, -34), S(  -8, -32), S(   6, -37),
    S( -30, -15), S(  -4, -11), S( -15, -16), S(   3, -26),
    S( -18,  -2), S( -12,   7), S(  -6,   2), S(   9,  -3),
    S(  -5,  11), S(   8,   8), S(  21,   5), S(  44,   1),
    S( -16,  21), S(  29,   5), S(   7,  20), S(  40,   3),
    S(   0,   5), S( -11,  12), S(  13,   4), S(  29,   6),
    S(  36,  22), S(  28,  22), S(   1,  32), S(  10,  29),
};

const int QueenPSQT32[32] = {
    S(  32, -66), S(   7, -45), S(  15, -60), S(  21, -42),
    S(  12, -43), S(  27, -58), S(  28, -70), S(  18, -27),
    S(  10, -31), S(  25, -20), S(   7,   7), S(   7,   1),
    S(  12, -10), S(  17,  11), S(  -1,  24), S( -14,  65),
    S(  -2,  15), S(  -8,  48), S( -12,  22), S( -27,  76),
    S( -25,  32), S( -17,  20), S( -22,  25), S( -11,  27),
    S(   0,  27), S( -66,  67), S(  -1,  12), S( -37,  52),
    S( -11,  19), S(  20,   9), S(   9,  10), S(  -5,  21),
};

const int KingPSQT32[32] = {
    S(  50, -79), S(  40, -52), S( -17, -10), S( -30, -21),
    S(  28, -19), S( -16,  -9), S( -46,  16), S( -68,  18),
    S(  -5, -19), S(   5, -16), S(  13,   5), S( -18,  22),
    S(   1, -30), S(  96, -33), S(  47,   9), S(  -3,  29),
    S(  11, -12), S( 109, -31), S(  57,  14), S(  12,  20),
    S(  50, -29), S( 134, -23), S( 105,   2), S(  43,  -2),
    S(   7, -60), S(  48, -10), S(  32,   3), S(   9,  -5),
    S(   4,-125), S(  73, -64), S( -23, -24), S( -20, -20),
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -27, -10), S( -10,   9), S( -18,  33),
    S(   4,  60), S(  39,  65), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -15,  19), S(  -4,  21), S(   3,  46),
    S(  22,  88), S(  38,  63), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -4, -12);

const int PawnStacked[2] = { S(  -3, -20), S(  -7,  -8) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   2,  -5), S(  10,  -5), S(   9, -13),
    S(   6, -13), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -10, -27), S(  -5, -23), S(   4, -27),
    S(  32, -29), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -2,  -5), S(  11,   1), S(   1,   1), S(   6,  12),
    S(  14,   0), S(  25,  -6), S(  21,   2), S(  23,   9),
    S(   7,  -1), S(  25,  -1), S(   9,   6), S(  16,  11),
    S(   8,  10), S(  20,  15), S(  23,  20), S(  28,  18),
    S(  56,  18), S(  41,  44), S(  61,  55), S(  74,  60),
    S( 110, -10), S( 202,  -7), S( 226,  24), S( 235,  38),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightInSiberia[4] = {
    S( -10,  -1), S( -14,  -7), S( -27,  -3), S( -26,  -6),
};

const int KnightMobility[9] = {
    S( -79,-106), S( -32,-103), S( -14, -43), S(  -3, -14),
    S(   9,  -6), S(  13,  11), S(  21,  14), S(  31,  13),
    S(  45,  -5),
};
...
const int BishopMobility[14] = {
    S( -70,-149), S( -37,-100), S( -13, -53), S(  -3, -27),
    S(   9, -17), S(  17,  -3), S(  21,   7), S(  22,  13),
    S(  21,  19), S(  27,  21), S(  26,  20), S(  48,   8),
    S(  47,  19), S(  75, -18),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  10,   2), S(  34,   2) };

const int RookOnSeventh = S(  -2,  33);

const int RookMobility[15] = {
    S(-159,-116), S( -62,-116), S( -19, -66), S( -10, -23),
    S( -10,  -1), S( -10,  14), S(  -9,  27), S(  -2,  30),
    S(   6,  35), S(   9,  39), S(  14,  45), S(  21,  50),
    S(  22,  54), S(  41,  40), S( 110, -14),
};
...
const int QueenMobility[28] = {
    ...
    S(  -9,-144), S(  -7, -80), S(  -2, -43), S(  -2, -11),
    S(   6, -11), S(   8,  12), S(  14,  13), S(  15,  33),
    S(  18,  21), S(  21,  33), S(  18,  38), S(  16,  45),
    S(  19,  42), S(   8,  45), S(  10,  41), S(   9,  31),
    S(  17,   7), S(  30, -16), S(  31, -46), S(  29, -63),
    S(   8, -79), S(  19,-108), S( -55, -37), S( -32, -63),
};
...
const int KingDefenders[12] = {
    ...
    S(  17,   3), S(  27,   0), S(  28,  -4), S(  12,  -1),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  41,  33), S(  31,  21), S(  10,   8), S( -26, -18),
    S( -29, -50), S( -21, -62), S( -17, -65), S( -11, -63),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -9,   2), S(  14, -27), S(  29,  -9), S(  18,   7),
    S(  15,   3), S(  -1,  -1), S(  -3, -34), S( -52,  20)},
   {S(  16,  -7), S(  22, -20), S(   4,  -5), S( -16,   4),
    S( -30,  16), S( -73,  67), S(  91,  81), S( -18,   1)},
   {S(  36,  -4), S(   9,  -8), S( -32,   6), S( -14,  -8),
    S( -17,  -5), S( -12,   1), S(   0,  68), S( -13,   0)},
   {S(   2,  12), S(  26, -14), S(   5, -12), S(  15, -21),
    S(  33, -38), S( -58,   3), S(-135,  51), S(   6,  -6)},
   {S( -19,  12), S(   3,  -1), S( -35,   4), S( -21,   9),
    S( -22,  -6), S( -45,  -5), S(  32, -18), S( -10,   3)},
   {S(  49, -18), S(  21, -13), S( -21,   5), S( -10, -20),
    S(   9, -25), S(  19, -21), S(  40, -32), S( -23,   3)},
   {S(  32, -18), S(   0, -17), S( -27,  -1), S( -23,  -7),
    S( -31,  -4), S( -41,  30), S(   1,  44), S( -10,   0)},
   {S( -10, -13), S(   5, -19), S(   9,   3), S(   0,  12),
    S( -11,  16), S( -11,  36), S(-190,  89), S( -21,  15)}},
  {{S(   0,   0), S( -13, -27), S(   7, -23), S( -39,  20),
    S( -23,   1), S(   3,  42), S(-167,  -7), S( -43,  10)},
   {S(   0,   0), S(  26, -23), S(   8,  -6), S( -16,   0),
    S(   3, -18), S(  26,  65), S(-184,  -3), S( -29,   5)},
   {S(   0,   0), S(  24, -14), S(  -2, -11), S(   6, -20),
    S(  21,  -3), S( -87,  48), S( -84, -74), S( -19,  -1)},
   {S(   0,   0), S( -12,   8), S(  -8,  -3), S( -27,   4),
    S( -27,   4), S(-100,  30), S(   7, -40), S( -23,  -1)},
   {S(   0,   0), S(  16,   1), S(  13,  -7), S(  16, -13),
    S(  17, -31), S( -57,  14), S(-101, -60), S(  -3,  -1)},
   {S(   0,   0), S(   0,  -6), S( -28,   2), S( -23,  -9),
    S(  23, -23), S( -37,   3), S(  55,  39), S( -16,  -3)},
   {S(   0,   0), S(  24, -18), S(  13, -16), S( -12,  -7),
    S( -28,  15), S(  -8,  19), S( -57, -50), S( -33,  14)},
   {S(   0,   0), S(  14, -44), S(  23, -33), S( -19,  -8),
    S( -20,  19), S(  -8,  21), S(-228, -56), S( -26,   3)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -13,  33), S( 112,  -5), S( -23,  24), S( -12,   0),
    S( -12,  -1), S(  -5,  -7), S( -14,   2), S( -27,   4)},
   {S(  -8,  53), S(  51,  15), S( -21,  20), S(  -2,   7),
    S(  -1,   3), S(   7,  -5), S(  -3,   3), S( -11,   6)},
   {S(   3,  44), S(   3,  29), S( -23,  18), S( -12,   6),
    S(   1,   2), S(   7,   0), S(  14,  -6), S(   5,   4)},
   {S(  -7,  29), S(   3,  26), S( -24,   8), S( -16,  -1),
    S( -12,   1), S(   9, -12), S(   3,  -9), S( -13,   6)}},
  {{S(   0,   0), S( -18, -16), S( -16,  -2), S(  24, -21),
    S(  12,  -7), S(   7, -21), S(   4,   1), S(  16,  29)},
   {S(   0,   0), S( -17, -37), S(   4, -12), S(  47, -13),
    S(   3,  -4), S(  17, -25), S(  -6,  -5), S( -19,   0)},
   {S(   0,   0), S( -28, -52), S( -23,  -7), S(  17, -13),
    S(   6,  -2), S(  -7, -13), S( -10, -12), S( -11,   6)},
   {S(   0,   0), S(  -2, -19), S( -20, -16), S( -17,  -1),
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -36,   0), S( -48,  26), S( -72,  31),
    S(   7,   8), S(  82, -17), S( 157,  40), S(   0,   0)},
   {S(   0,   0), S( -29,  13), S( -46,  38), S( -65,  41),
    S(   1,  38), S( 107,  21), S( 190,  86), S(   0,   0)}},
  {{S(   0,   0), S( -23,  27), S( -47,  36), S( -68,  47),
    S(  11,  43), S( 103,  32), S( 271,  94), S(   0,   0)},
   {S(   0,   0), S( -29,  24), S( -42,  35), S( -61,  54),
    S(  12,  63), S( 105, 120), S( 157, 264), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   2,   2), S(   5,  -4), S(   7, -12),
    S(   5, -15), S( -10,  -9), S( -16,   2), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   2,  -1), S(   3,   0), S(   8,   9),
    S(   0,  21), S(   5,  28), S(  28,  21), S(   0,   0),
};

const int PassedSafePromotionPath = S( -17,  44);
...
const int ThreatMinorAttackedByMajor = S( -30, -49);
const int ThreatRookAttackedByLesser = S( -50, -24);
const int ThreatMinorAttackedByKing  = S( -17, -17);
const int ThreatRookAttackedByKing   = S( -18, -16);
const int ThreatQueenAttackedByOne   = S( -49, -27);
...
const int ClosednessKnightAdjustment[9] = {
    S(  -8,  -7), S(  -4,   9), S(  -7,  17), S(  -2,  16),
    S(  -1,  19), S(   2,  13), S(   6,   5), S(  -7,  24),
    S(  -4,  10),
};

const int ClosednessRookAdjustment[9] = {
    S(  73, -15), S(   6,  37), S(   4,  20), S(  -3,   0),
    S( -11,   6), S( -10, -12), S( -12, -12), S( -23, -16),
    S( -35, -21),
};
