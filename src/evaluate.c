...
const int PawnValue   = S( 118, 131);
const int KnightValue = S( 477, 417);
const int BishopValue = S( 510, 438);
const int RookValue   = S( 686, 700);
const int QueenValue  = S(1332,1420);
...
const int PawnPSQT32[32] = {
    ...
    S( -21,   6), S(   5,   0), S( -10,   2), S(  -6,  -3),
    S( -27,   3), S( -14,   2), S( -11,  -7), S(  -4, -16),
    S( -19,  11), S( -14,  11), S(  10, -13), S(   8, -27),
    S( -10,  13), S(  -5,   8), S(  -9,  -8), S(  -2, -22),
    S(  -6,  29), S(  -1,  25), S(   2,  15), S(  32, -14),
    S( -19, -50), S( -67, -18), S(   0, -32), S(  39, -47),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -51, -27), S(  -9, -29), S( -17, -30), S(  -1, -17),
    S(  -3, -21), S(   2, -11), S(   2, -31), S(  10, -20),
    S(  10, -26), S(  22, -25), S(  15, -20), S(  26,  -5),
    S(  22,   4), S(  27,   7), S(  35,  18), S(  35,  25),
    S(  31,  17), S(  34,  10), S(  47,  28), S(  39,  40),
    S( -16,  14), S(   5,  13), S(  36,  28), S(  35,  30),
    S(  13,  -8), S(  -7,   1), S(  30, -22), S(  47,  -2),
    S(-175, -21), S( -82,  -2), S(-111,  18), S( -31,  -2),
};

const int BishopPSQT32[32] = {
    S(   6, -27), S(  14, -17), S(  -9,  -5), S(  12, -11),
    S(  35, -36), S(  11, -39), S(  25, -21), S(  16, -11),
    S(  18, -12), S(  34, -15), S(   5, -19), S(  28,  -4),
    S(  19,  -7), S(  24,  -1), S(  21,   6), S(  31,   8),
    S(  -9,  15), S(  26,   7), S(   8,  17), S(  19,  20),
    S(   6,   8), S(  -1,  20), S(   1,   6), S(  21,  16),
    S( -47,  19), S( -60,   4), S(  -8,  10), S( -20,  14),
    S( -53,  -9), S( -45,   8), S( -86,  15), S( -89,  29),
};

const int RookPSQT32[32] = {
    S(  -9, -29), S( -13, -19), S(   1, -25), S(  10, -31),
    S( -59, -11), S(  -8, -35), S(  -5, -34), S(   7, -39),
    S( -28, -13), S(  -2, -10), S( -14, -16), S(   3, -26),
    S( -15,   0), S(  -7,  10), S(  -3,   5), S(  11,  -2),
    S(  -1,  13), S(  18,  10), S(  29,   5), S(  49,   0),
    S( -13,  24), S(  33,   9), S(   8,  24), S(  45,   4),
    S(   0,   9), S( -15,  18), S(  12,   7), S(  28,  10),
    S(  41,  26), S(  30,  28), S(   9,  35), S(  19,  30),
};

const int QueenPSQT32[32] = {
    S(  30, -56), S(   7, -40), S(  16, -57), S(  25, -47),
    S(  17, -39), S(  32, -56), S(  35, -77), S(  23, -26),
    S(  15, -24), S(  31, -17), S(  11,  13), S(  11,   6),
    S(  16,   1), S(  23,  20), S(   2,  32), S( -12,  78),
    S(   1,  23), S(  -2,  53), S(  -8,  31), S( -26,  85),
    S( -18,  35), S( -11,  26), S( -19,  32), S(  -6,  34),
    S(   1,  32), S( -67,  73), S(  -2,  21), S( -37,  60),
    S(  -5,  19), S(  23,  14), S(  13,  14), S(  -1,  27),
};

const int KingPSQT32[32] = {
    S(  52, -83), S(  47, -54), S( -12, -13), S( -25, -25),
    S(  32, -21), S( -12, -12), S( -44,  12), S( -62,  13),
    S(  -2, -21), S(   9, -20), S(  15,   2), S( -15,  19),
    S(   2, -31), S(  92, -33), S(  46,   9), S(  -6,  30),
    S(   6, -15), S( 103, -30), S(  51,  15), S(   6,  24),
    S(  46, -29), S( 126, -21), S(  99,   5), S(  41,   1),
    S(   4, -58), S(  47, -11), S(  31,   2), S(   8,  -5),
    S(   6,-111), S(  73, -60), S( -21, -20), S( -20, -18),
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -27, -12), S( -13,  12), S( -19,  35),
    S(   0,  68), S(  43,  70), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -15,  24), S(  -4,  23), S(   4,  52),
    S(  20,  91), S(  35,  59), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -7, -14);

const int PawnStacked[2] = { S(  -7, -19), S(  -9,  -9) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   4,  -8), S(  10,  -7), S(  10, -13),
    S(  10, -13), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -12, -32), S(  -6, -27), S(   4, -30),
    S(  29, -29), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -3,  -7), S(  14,   0), S(   2,   2), S(   6,  14),
    S(  17,  -1), S(  32,  -5), S(  24,   6), S(  27,  13),
    S(  10,  -2), S(  28,   2), S(  11,   9), S(  20,  16),
    S(  12,  10), S(  25,  14), S(  31,  23), S(  35,  20),
    S(  63,  21), S(  50,  46), S(  72,  56), S(  85,  61),
    S( 112,  -3), S( 204,   4), S( 227,  28), S( 239,  47),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightBehindPawn = S(   5,  22);

const int KnightInSiberia[4] = {
    S( -10,  -1), S( -13,  -7), S( -25,  -6), S( -27,  -7),
};

const int KnightMobility[9] = {
    S( -78,-105), S( -25,-108), S(  -7, -43), S(   6, -13),
    S(  19,  -2), S(  24,  16), S(  33,  20), S(  44,  20),
    S(  59,   2),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  27,  74);
...
const int BishopBehindPawn = S(   3,  22);

const int BishopLongDiagonal = S(  26,  13);

const int BishopMobility[14] = {
    S( -72,-150), S( -28,-104), S(  -3, -57), S(   8, -27),
    S(  20, -14), S(  29,   1), S(  33,  13), S(  33,  18),
    S(  32,  25), S(  37,  26), S(  37,  26), S(  60,  13),
    S(  50,  29), S(  78, -10),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  15,   2), S(  38,   3) };

const int RookOnSeventh = S(   2,  29);

const int RookMobility[15] = {
    S(-155,-115), S( -55,-119), S(  -8, -71), S(   2, -22),
    S(   2,   2), S(   1,  19), S(   2,  32), S(  10,  35),
    S(  18,  40), S(  22,  45), S(  26,  52), S(  33,  55),
    S(  34,  61), S(  51,  49), S( 109,   7),
};

/* Queen Evaluation Terms */

const int QueenMobility[28] = {
    S( -61,-263), S(-211,-387), S( -60,-201), S( -19,-192),
    S(  -2,-143), S(   3, -85), S(   9, -50), S(   9, -12),
    S(  16, -11), S(  17,  17), S(  22,  22), S(  24,  42),
    S(  27,  33), S(  29,  45), S(  26,  49), S(  22,  58),
    S(  23,  56), S(  12,  60), S(  16,  56), S(  12,  49),
    S(  22,  24), S(  35,   3), S(  36, -29), S(  34, -46),
    S(  14, -65), S(  25, -94), S( -55, -38), S( -31, -61),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -30,   4), S(  -6,  -3), S(   1,   4), S(   9,   7),
    S(  18,   5), S(  29,   0), S(  31,  -4), S(  12,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  41,  33), S(  26,  24), S(   6,  13), S( -23, -13),
    S( -23, -48), S( -19, -66), S( -18, -73), S( -12, -68),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -11,   3), S(  17, -30), S(  28, -11), S(  19,   6),
    S(  12,   4), S(  -1,   0), S(  -3, -34), S( -55,  21)},
   {S(  17,  -7), S(  25, -20), S(   4,  -5), S( -14,   5),
    S( -31,  15), S( -72,  68), S(  92,  82), S( -18,   1)},
   {S(  39,  -2), S(  13,  -8), S( -35,  10), S( -14, -10),
    S( -18,  -5), S( -12,   1), S(   0,  67), S( -14,   0)},
   {S(   3,  15), S(  28, -13), S(   6, -11), S(  17, -23),
    S(  33, -41), S( -62,   4), S(-136,  52), S(   7,  -6)},
   {S( -21,  13), S(   4,  -1), S( -35,   6), S( -24,  10),
    S( -24,  -8), S( -44,  -6), S(  32, -17), S( -10,   2)},
   {S(  54, -19), S(  25, -17), S( -24,   5), S( -13, -22),
    S(   6, -28), S(  18, -20), S(  40, -31), S( -27,   4)},
   {S(  31, -16), S(   0, -17), S( -27,  -1), S( -24,  -8),
    S( -32,  -9), S( -39,  30), S(   0,  44), S( -11,   2)},
   {S( -10, -14), S(   7, -22), S(  11,   2), S(   0,  13),
    S( -12,  16), S( -11,  38), S(-191,  87), S( -22,  17)}},
  {{S(   0,   0), S( -15, -27), S(   7, -23), S( -39,  18),
    S( -24,   1), S(   3,  42), S(-167,  -7), S( -44,  14)},
   {S(   0,   0), S(  30, -25), S(  10,  -7), S( -20,   1),
    S(   0, -16), S(  26,  66), S(-184,  -3), S( -32,   6)},
   {S(   0,   0), S(  32, -12), S(   1, -10), S(   6, -21),
    S(  21,  -3), S( -86,  48), S( -84, -73), S( -20,   0)},
   {S(   0,   0), S(  -9,  11), S(  -7,   0), S( -26,   5),
    S( -30,   3), S(-100,  30), S(   7, -41), S( -23,  -3)},
   {S(   0,   0), S(  15,   4), S(  13,  -6), S(  16, -15),
    S(  16, -34), S( -56,  15), S(-104, -61), S(  -3,  -2)},
   {S(   0,   0), S(   3,  -5), S( -29,   4), S( -27,  -8),
    S(  21, -24), S( -37,   3), S(  55,  38), S( -19,  -3)},
   {S(   0,   0), S(  28, -18), S(  15, -16), S( -11,  -7),
    S( -33,  14), S(  -7,  18), S( -58, -50), S( -35,  16)},
   {S(   0,   0), S(  16, -46), S(  26, -34), S( -18,  -8),
    S( -19,  20), S(  -7,  21), S(-228, -55), S( -27,   6)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S(  -7,  34), S( 120,  -7), S( -27,  27), S( -15,   3),
    S( -12,  -1), S(  -7,  -8), S( -16,   1), S( -26,   1)},
   {S(  -5,  57), S(  57,  12), S( -22,  23), S(  -3,   8),
    S(  -1,   2), S(   8,  -7), S(  -1,   0), S( -13,   4)},
   {S(   6,  45), S(  13,  28), S( -26,  21), S( -12,   8),
    S(   3,   1), S(   7,   1), S(  11,  -6), S(   4,   3)},
   {S(  -4,  31), S(  10,  26), S( -24,  10), S( -18,   2),
    S( -16,   3), S(   8, -12), S(   2, -11), S( -15,   6)}},
  {{S(   0,   0), S( -17, -17), S( -21,   0), S(  25, -22),
    S(  13,  -8), S(   8, -21), S(   1,   1), S(  16,  29)},
   {S(   0,   0), S( -17, -36), S(  -1, -10), S(  46, -14),
    S(   3,  -3), S(  16, -23), S(  -6,  -7), S( -19,   0)},
   {S(   0,   0), S( -29, -52), S( -27,  -6), S(  16, -13),
    S(   5,   0), S(  -9, -14), S( -12, -13), S( -12,   5)},
   {S(   0,   0), S(  -2, -18), S( -21, -17), S( -17,  -1),
    S(  -5,  -6), S(   4, -28), S(  73, -11), S(  13,  21)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -39,   1), S( -48,  22), S( -78,  29),
    S(   4,   9), S(  75, -16), S( 155,  40), S(   0,   0)},
   {S(   0,   0), S( -32,  11), S( -51,  35), S( -75,  39),
    S(  -6,  37), S( 100,  29), S( 187,  92), S(   0,   0)}},
  {{S(   0,   0), S( -25,  22), S( -49,  29), S( -73,  40),
    S(   6,  41), S(  96,  36), S( 266, 100), S(   0,   0)},
   {S(   0,   0), S( -33,  20), S( -46,  30), S( -70,  49),
    S(   6,  60), S( 101, 122), S( 158, 272), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   1,   2), S(   4,  -4), S(   7, -10),
    S(   5, -14), S(  -8, -11), S( -15,  -4), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   3,  -1), S(   4,   1), S(   9,   9),
    S(   1,  22), S(   7,  31), S(  29,  27), S(   0,   0),
};

const int PassedSafePromotionPath = S( -27,  42);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -14, -30);
const int ThreatMinorAttackedByPawn  = S( -55, -59);
const int ThreatMinorAttackedByMinor = S( -27, -40);
const int ThreatMinorAttackedByMajor = S( -24, -50);
const int ThreatRookAttackedByLesser = S( -51, -20);
const int ThreatMinorAttackedByKing  = S( -21, -17);
const int ThreatRookAttackedByKing   = S( -20, -18);
const int ThreatQueenAttackedByOne   = S( -39, -34);
const int ThreatOverloadedPieces     = S(  -9, -15);
const int ThreatByPawnPush           = S(  16,  25);
...
const int ClosednessKnightAdjustment[9] = {
    S(   2,  -5), S(   5,  12), S(   4,  19), S(   9,  19),
    S(  12,  23), S(  15,  15), S(  20,   7), S(   4,  31),
    S(   4,  17),
};

const int ClosednessRookAdjustment[9] = {
    S(  73,   6), S(  25,  49), S(  27,  27), S(  22,  10),
    S(  18,  12), S(  18,  -2), S(  10,  -3), S(  -9, -10),
    S( -25, -17),
};
