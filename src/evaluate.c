...
const int PawnValue   = S( 102, 122);
const int KnightValue = S( 463, 391);
const int BishopValue = S( 475, 416);
const int RookValue   = S( 639, 713);
const int QueenValue  = S(1308,1337);
const int KingValue   = S(   0,   0);

const int PieceValues[8][PHASE_NB] = {
    { 102, 122}, { 463, 391}, { 475, 416}, { 639, 713},
    {1308,1337}, {   0,   0}, {   0,   0}, {   0,   0},
};

// Definition of evaluation terms related to Pawns

const int PawnIsolated = S(  -5,  -7);

const int PawnStacked = S(  -4, -31);

const int PawnBackwards[2] = { S(   6,  -4), S( -11, -11) };

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(   0, -16), S(   6,   1), S(   4,  -5), S(   7,  24),
    S(   7,   0), S(  22,   2), S(  17,   9), S(  16,  20),
    S(   8,   2), S(  19,   5), S(  13,  10), S(  18,  18),
    S(   3,  13), S(  21,  19), S(  21,  23), S(  33,  22),
    S(  19,  29), S(  35,  52), S(  67,  55), S(  52,  66),
    S( 152,  12), S( 222,   7), S( 201,  32), S( 181,  42),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

// Definition of evaluation terms related to Knights

const int KnightRammedPawns = S(   0,  15);

const int KnightOutpost[2] = { S(  23, -28), S(  39,   0) };

const int KnightMobility[9] = {
    S( -94,-147), S( -48, -87), S( -26, -41), S( -11, -19),
    S(   0, -16), S(   6,  -2), S(  20,  -2), S(  36,  -4),
    S(  52, -23),
};
...
const int BishopRammedPawns = S(  -8,  -5);

const int BishopOutpost[2] = { S(  23, -14), S(  52,  -8) };

const int BishopMobility[14] = {
    S( -78,-125), S( -58, -70), S( -23, -47), S(  -6, -23),
    S(   5, -10), S(  16,  -2), S(  24,   6), S(  28,   5),
    S(  32,  10), S(  38,   4), S(  43,   4), S(  57, -11),
    S(  54,   1), S(  39, -34),
};

// Definition of evaluation terms related to Rooks

const int RookFile[2] = { S(  14,   0), S(  46,  -6) };

const int RookOnSeventh = S(   4,  28);

const int RookMobility[15] = {
    S(-138,-128), S( -66,-107), S( -17, -71), S( -11, -29),
    S(  -8, -10), S(  -9,   7), S(  -9,  23), S(  -5,  30),
    S(   1,  32), S(   7,  34), S(  12,  41), S(  20,  49),
    S(  23,  53), S(  26,  50), S(  31,  42),
};

// Definition of evaluation terms related to Queens

const int QueenMobility[28] = {
    S(-143,-263), S( -94,-433), S( -84,-223), S( -41,-209),
    S( -32,-171), S( -30, -89), S( -20, -91), S( -16, -87),
    S( -12, -68), S( -10, -53), S(  -8, -29), S(  -5, -25),
    S(  -5, -14), S(  -2,  -7), S(   0,  -5), S(   0,   2),
    S(   2,  11), S(   1,  13), S(   1,  12), S(  -2,  16),
    S(   0,  14), S(  11,   9), S(  13,  -2), S(  28, -10),
    S(  24, -18), S(  38, -41), S( -22,  -7), S( -46, -49),
};

// Definition of evaluation terms related to Kings

const int KingDefenders[12] = {
    S( -36,   0), S( -20,   5), S(  -1,   1), S(  11,  -3),
    S(  26,  -3), S(  36,   0), S(  54, -16), S(  59,  47),
...
const int KingShelter[2][8][8] = {
  {{S( -22,  12), S(   1,  -9), S(  17,   0), S(  23,  10), S(  15,  11), S(  18,   0), S( -49,   3), S( -29,   5)},
   {S(   5,   8), S(  15, -10), S(  -8,  -8), S( -20,  -2), S( -14,   8), S( -89,  83), S( -32,  61), S( -36,   1)},
   {S(  17,  13), S(   3,  -1), S( -24,   4), S( -10,  -1), S( -19,   0), S(   4,  -2), S( -57,  87), S( -15,   1)},
   {S(  -3,  26), S(  10,   0), S( -12,  -4), S(  21, -21), S(  14, -27), S( -38,  -6), S(-150,  41), S(  -4,   0)},
   {S( -13,  18), S(  -5,  -1), S( -34,   3), S(  -4,  -3), S(  -7, -17), S( -10,   3), S(  17, -19), S( -12,   0)},
   {S(  31,   1), S(  18,  -9), S( -25,  -2), S(  -2, -19), S(   7, -34), S(  20, -41), S(  41, -31), S( -15,   4)},
   {S(  24,   1), S(   2, -13), S( -29,  -5), S( -17, -17), S( -19, -24), S( -33,  15), S( -88, -19), S( -27,  11)},
   {S( -15,  -3), S(   0, -10), S(   4,   0), S(   4,  -1), S(  -5,   4), S(  -2,  24), S(-205,  72), S( -12,  18)}},
  {{S(   0,   0), S(  -9, -26), S(  -4, -14), S( -46,  25), S( -73,  -4), S( -57,  -2), S(-111, -37), S( -60,  19)},
   {S(   0,   0), S(  14,  -8), S(   7,  -6), S( -11, -12), S(  12, -20), S(  32,  24), S(-168,   6), S( -43,   8)},
   {S(   0,   0), S(  17,  -4), S(  -2,  -3), S(  22, -23), S(  19,   0), S( -54,  37), S( -41, -50), S( -24,  -1)},
   {S(   0,   0), S(  -4,  16), S( -18,  11), S(  -7,  -6), S( -24, -14), S( -94,  53), S(   4,  59), S( -28,   0)},
   {S(   0,   0), S(   1,   2), S(   6,  -3), S(  22, -12), S(  22, -17), S( -62,  40), S( -71,  -9), S( -10,  -3)},
   {S(   0,   0), S(  13,  -5), S(  -9,  -1), S( -11, -14), S(  26, -35), S( -55,  -5), S( -23, -40), S( -30,   2)},
   {S(   0,   0), S(  13,  -4), S(   1,   0), S( -19,  -7), S( -26,  -7), S( -28,  -9), S( -72, -28), S( -43,  16)},
   {S(   0,   0), S(  10, -32), S(  10, -17), S( -17,  -2), S( -16,   0), S(  -5,   5), S( -66,  13), S( -51,  23)}},
};

// Definition of evaluation terms related to Passed Pawns

const int PassedPawn[2][2][8] = {
  {{S(   0,   0), S( -28, -16), S( -15,  13), S( -12,   5), S(  19,  12), S(  46,  -2), S( 156,  30), S(   0,   0)},
   {S(   0,   0), S(  -1,  -5), S( -14,  20), S(  -9,  34), S(  10,  46), S(  62,  48), S( 194, 113), S(   0,   0)}},
  {{S(   0,   0), S( -10,   8), S( -12,   7), S(  -6,  27), S(  33,  35), S(  88,  55), S( 259, 163), S(   0,   0)},
   {S(   0,   0), S(  -5,   6), S( -14,  15), S( -21,  52), S(  -9, 110), S(  37, 233), S( 125, 388), S(   0,   0)}},
};

// Definition of evaluation terms related to Threats

const int ThreatPawnAttackedByOne    = S(   0, -23);

const int ThreatMinorAttackedByPawn  = S( -72, -37);

const int ThreatMinorAttackedByMajor = S( -36, -27);

const int ThreatQueenAttackedByOne   = S( -79,  13);
