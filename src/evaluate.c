...
void evaluatePieces(EvalInfo * ei, Board * board){
    ...
    ei->midgame[WHITE] += -6 * popcount(board->colours[WHITE] & ~ei->attacked[WHITE]);
    ei->endgame[WHITE] += -6 * popcount(board->colours[WHITE] & ~ei->attacked[WHITE]);
    ei->endgame[BLACK] += -6 * popcount(board->colours[BLACK] & ~ei->attacked[BLACK]);
    ei->midgame[BLACK] += -6 * popcount(board->colours[BLACK] & ~ei->attacked[BLACK]);
