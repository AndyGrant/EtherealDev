...
int evaluateKings(EvalInfo *ei, Board *board, int colour) {
    ...
    int count, safety, dist, blocked, mg, eg, eval = 0;
    ...
    for (int file = MAX(0, fileOf(kingSq) - 1); file <= MIN(FILE_NB - 1, fileOf(kingSq) + 1); file++) {
        ...
        const uint64_t myPawns     = board->pieces[PAWN ] & board->colours[  US];
        const uint64_t enemyPawns  = board->pieces[PAWN ] & board->colours[THEM];
