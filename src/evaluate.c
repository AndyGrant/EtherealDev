...
const int KnightValue[PHASE_NB] = { 281, 267};

const int BishopValue[PHASE_NB] = { 286, 267};

const int RookValue[PHASE_NB]   = { 375, 419};

const int QueenValue[PHASE_NB]  = { 765, 818};

const int KingValue[PHASE_NB]   = { 100, 100};

const int PawnIsolated[PHASE_NB] = {  20,   0};

const int PawnStacked[PHASE_NB] = { -27, -18};

const int PawnBackwards[2][PHASE_NB] = { {   0,  -2}, {  -8,  -5} };

const int PawnConnected32[32][PHASE_NB] = {
    {   0,   0}, {   0,   0}, {   0,   0}, {   0,   0},
    {   2,  -3}, {   5,   9}, {   7,   3}, {  11,  20},
    {   7,   5}, {  20,   6}, {  21,   9}, {  21,  19},
    {  11,   2}, {  15,   7}, {  13,   9}, {  22,  13},
    {  10,  16}, {  13,  16}, {  24,  14}, {  32,  20},
    {  -7,  49}, {  16,  34}, {  28,  29}, {  42,  32},
    { 220,  59}, {  68,  35}, {  57,  29}, { 168,  11},
    {   0,   0}, {   0,   0}, {   0,   0}, {   0,   0},
};

const int KnightAttackedByPawn[PHASE_NB] = { -38, -22};

const int KnightOutpost[2][PHASE_NB] = { {  12, -15}, {  23,  -1} };
...
const int BishopWings[PHASE_NB] = { -10,  -4};

const int BishopPair[PHASE_NB] = {  10,  37};

const int BishopAttackedByPawn[PHASE_NB] = { -34, -40};

const int BishopOutpost[2][PHASE_NB] = { {  15,  -8}, {  31,  -5} };

const int BishopMobility[14][PHASE_NB] = {
    { -45, -13}, { -25, -37}, { -13, -30}, {  -3, -14},
    {   4, -11}, {   8,   0}, {  12,   5}, {  13,   5},
    {  14,  10}, {  20,   4}, {  17,   6}, {  28, -11},
    {  22,  -9}, {  43, -34},
};

const int RookFile[2][PHASE_NB] = { {  10,   2}, {  24,  -2} };

const int RookOnSeventh[PHASE_NB] = {   0,   5};

const int RookMobility[15][PHASE_NB] = {
    { -90, -75}, { -19, -68}, {  -9, -37}, {  -3, -17},
    {  -3,  -8}, {  -3,   1}, {  -5,  10}, {  -3,  13},
    {  -3,  18}, {   0,  21}, {   3,  23}, {   4,  29},
    {   5,  30}, {  11,  27}, {   0,  27},
};

const int QueenChecked[PHASE_NB] = { -35,  -9};

const int QueenCheckedByPawn[PHASE_NB] = { -37, -17};

const int QueenMobility[28][PHASE_NB] = {
    {-197, -66}, {-102,-416}, { -16, -33}, { -20,-122},
    { -17, -96}, { -15, -28}, { -12, -42}, { -12, -38},
    {  -9, -45}, {  -8, -35}, {  -5, -30}, {  -3, -19},
    {  -4, -15}, {  -2,  -8}, {  -2,  -4}, {   0,   0},
    {  -2,  10}, {   0,   5}, {  -4,  13}, {  -5,  17},
    {  -9,  13}, {   6,  15}, {   0,   6}, {  15, -11},
...
const int KingShelter[2][FILE_NB][RANK_NB][PHASE_NB] = {
  {{{   0,   2}, {  16,  -7}, {  10,   0}, {  14,  -6}, {   2,  -4}, { -53,   9}, { -48,  61}, { -21,   3}},
   {{   0,   4}, {  13,  -3}, {  16,  -2}, {  -9,  -2}, {  -9,  26}, { -96,  41}, {-102,  53}, { -19,   0}},
   {{  18,   0}, {   4,   0}, { -10,   0}, {  -5,   0}, {  -5,  -1}, {  48,   5}, {  24,  30}, { -14,   4}},
   {{   7,  18}, {   5,   3}, {   0,  -8}, {  13,  -6}, {  10, -21}, {  -5, -26}, { -45, -12}, {  -3,   0}},
   {{ -11,  13}, {  -2,   0}, { -13,   1}, {  -5,  -3}, {  -6,  -3}, { -19, -21}, {  61,  27}, { -11,   1}},
   {{  13,   0}, {   7,   3}, {  -4,   0}, {   1, -21}, {   4, -32}, { -12,   1}, {  24, -41}, {  -7,  -1}},
   {{  16,   0}, {   4,  -6}, { -23,   0}, { -19, -12}, { -19, -10}, { -62,   5}, {  62,  -7}, { -21,   5}},
   {{   0,  -4}, {   1,  -2}, {   4,   0}, {  -1,   1}, {  -7,  -6}, {  -2,  23}, { -31,  -5}, { -21,   7}}},
  {{{   0,   0}, {   0,   2}, {  14, -14}, { -36,   5}, { -17,  19}, { -31,  64}, { -30, -15}, { -21,   2}},
   {{   0,   0}, {  15,  -3}, {  11,  -3}, { -21,  21}, {  -5,  12}, {-146,  18}, {-281,  -1}, { -22,   0}},
   {{   0,   0}, {  19,  -7}, {   0,  -8}, {   7,  -8}, {  21, -12}, { -80, -50}, {-202,  -4}, { -18,   4}},
   {{   0,   0}, {   0,  -3}, {   4,  10}, {   0,  -9}, { -10,  -6}, {  10,  10}, { -82,  14}, { -26,   0}},
   {{   0,   0}, {  -4,   4}, {   0,   3}, {  13,  -7}, {  12, -12}, {   2,  -7}, { -92, 100}, {  -1,  -2}},
   {{   0,   0}, {  10,   0}, {   0,   0}, {   1, -11}, { -14, -19}, { -54,  11}, {-135,  63}, { -23,   1}},
   {{   0,   0}, {   9,  -2}, {  -1,  -1}, {  -3,  -7}, { -13, -14}, {  -5,   2}, {  -7, -44}, { -29,   9}},
   {{   0,   0}, {  11, -17}, {   9,  -4}, { -41,  18}, { -23,  29}, { -20,  12}, {-124,  15}, { -38,   7}}},
};

const int PassedPawn[2][2][RANK_NB][PHASE_NB] = {
  {{{   0,   0}, { -30,  -7}, { -29,  -1}, { -11, -11}, {  15,   0}, {  29, -18}, {  89,  31}, {   0,   0}},
   {{   0,   0}, {  -8,   0}, { -19,   7}, { -14,  16}, {  11,  23}, {  26,  35}, { 130,  78}, {   0,   0}}},
  {{{   0,   0}, {  -8,   0}, {  -8,   0}, { -10,  11}, {  14,  15}, {  51,  35}, { 143,  87}, {   0,   0}},
   {{   0,   0}, {  -5,  -1}, { -10,   1}, { -18,  27}, {  -8,  64}, {  14, 124}, {  85, 203}, {   0,   0}}},
};
