...
const int PawnValue   = S( 108, 117);
const int KnightValue = S( 456, 408);
const int BishopValue = S( 478, 426);
const int RookValue   = S( 662, 686);
...
const int PawnPSQT32[32] = {
    ...
    S( -23,   7), S(   3,   3), S(  -7,   5), S(  -6,  -2),
    S( -25,   5), S( -15,   4), S( -12,  -4), S(  -5, -13),
    S( -18,  10), S( -13,  11), S(   6,  -9), S(   6, -22),
    S( -11,  11), S(  -5,   7), S(  -9,  -6), S(  -3, -20),
    S(  -8,  27), S(  -3,  24), S(   1,  14), S(  29, -12),
    S( -21, -61), S( -60, -29), S(   2, -43), S(  45, -58),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -48, -31), S(  -7, -28), S( -12, -29), S(   2, -15),
    S(   0, -20), S(   3, -11), S(   4, -27), S(  10, -17),
    S(  11, -26), S(  20, -22), S(  15, -16), S(  25,  -4),
    S(  20,   1), S(  25,   6), S(  32,  17), S(  35,  24),
    S(  26,  14), S(  32,   9), S(  42,  27), S(  36,  38),
...
const int BishopPSQT32[32] = {
    S(   8, -26), S(  12, -18), S( -10,  -9), S(  11, -12),
    S(  30, -35), S(  10, -35), S(  21, -20), S(  13, -11),
    S(  16, -13), S(  29, -15), S(   5, -17), S(  25,  -5),
    S(  15,  -7), S(  20,  -3), S(  18,   4), S(  26,   5),
    S( -10,  14), S(  22,   5), S(   5,  14), S(  15,  17),
...
const int RookPSQT32[32] = {
    S( -15, -29), S( -15, -19), S(  -3, -23), S(   7, -28),
    S( -53, -14), S( -10, -33), S(  -9, -31), S(   4, -36),
    S( -29, -14), S(  -5, -11), S( -15, -16), S(   2, -25),
    S( -17,  -2), S( -11,   7), S(  -6,   2), S(   8,  -4),
    S(  -4,  10), S(  10,   7), S(  22,   5), S(  41,  -1),
...
const int QueenPSQT32[32] = {
    S(  31, -66), S(   6, -45), S(  14, -59), S(  21, -43),
...
const int KingPSQT32[32] = {
    S(  45, -77), S(  40, -50), S( -18, -11), S( -34, -21),
    S(  28, -20), S( -13, -10), S( -44,  13), S( -63,  15),
...
const int PawnCandidatePasser[2][RANK_NB] = {
   ...
   {S(   0,   0), S( -14,  19), S(  -4,  21), S(   4,  46),
    S(  20,  86), S(  38,  62), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -6, -13);

const int PawnStacked[2] = { S(  -6, -19), S(  -7,  -9) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   2,  -6), S(   9,  -5), S(   8, -11),
    S(   7, -12), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -10, -28), S(  -6, -25), S(   4, -28),
    S(  28, -30), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -2,  -5), S(  13,   1), S(   3,   1), S(   6,  12),
    S(  15,   0), S(  27,  -4), S(  21,   4), S(  23,  11),
    S(   9,  -1), S(  24,   1), S(  10,   8), S(  16,  15),
    S(  10,   8), S(  21,  14), S(  25,  20), S(  30,  19),
...
const int KnightInSiberia[4] = {
    S(  -9,  -1), S( -12,  -7), S( -23,  -3), S( -25,  -6),
};

const int KnightMobility[9] = {
    S( -78,-106), S( -31,-102), S( -16, -42), S(  -6, -16),
    S(   8,  -7), S(  13,  10), S(  21,  13), S(  31,  13),
    S(  44,  -4),
};
...
const int BishopMobility[14] = {
    S( -69,-149), S( -34, -99), S( -13, -54), S(  -4, -29),
    S(   8, -18), S(  18,  -3), S(  22,   7), S(  22,  12),
    S(  21,  19), S(  26,  20), S(  25,  20), S(  46,   8),
    S(  47,  19), S(  75, -17),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  12,   3), S(  38,   5) };

const int RookOnSeventh = S(  -3,  31);

const int RookMobility[15] = {
    S(-159,-116), S( -58,-115), S( -19, -65), S( -11, -22),
    S( -10,  -1), S( -12,  13), S( -10,  25), S(  -2,  29),
    S(   8,  33), S(  11,  38), S(  15,  45), S(  21,  49),
    S(  22,  52), S(  38,  40), S( 106,  -9),
};
...
const int QueenMobility[28] = {
    ...
    S(  -7,-144), S(  -4, -80), S(   0, -44), S(   0, -12),
    S(   6, -11), S(   7,  11), S(  13,  14), S(  14,  32),
    S(  18,  23), S(  20,  34), S(  18,  38), S(  14,  44),
...
const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -9,   2), S(  14, -26), S(  26,  -9), S(  17,   6),
   ...
   {S(   2,  12), S(  24, -13), S(   4, -12), S(  15, -21),
    S(  31, -38), S( -58,   3), S(-135,  51), S(   5,  -6)},
   {S( -18,  11), S(   4,  -1), S( -31,   4), S( -20,   8),
    S( -21,  -6), S( -45,  -5), S(  32, -18), S(  -8,   2)},
   {S(  49, -18), S(  22, -13), S( -22,   4), S( -11, -19),
    S(   7, -25), S(  19, -21), S(  40, -32), S( -24,   2)},
   {S(  30, -16), S(  -1, -16), S( -26,  -1), S( -22,  -7),
    S( -31,  -5), S( -41,  30), S(   1,  44), S( -10,   0)},
   {S(  -9, -13), S(   6, -19), S(  10,   2), S(   0,  12),
    S( -11,  16), S( -11,  36), S(-190,  89), S( -20,  15)}},
  {{S(   0,   0), S( -13, -26), S(   7, -23), S( -39,  20),
    S( -23,   1), S(   3,  42), S(-167,  -7), S( -43,   9)},
   {S(   0,   0), S(  26, -22), S(   8,  -6), S( -16,   0),
...
const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -13,  32), S( 112,  -6), S( -23,  25), S( -14,   3),
    S( -12,   0), S(  -6,  -6), S( -14,   2), S( -24,   1)},
   {S(  -8,  53), S(  51,  14), S( -20,  22), S(  -3,   9),
    S(  -1,   4), S(   6,  -5), S(  -2,   2), S(  -9,   3)},
   {S(   3,  43), S(   4,  28), S( -22,  20), S( -10,   7),
    S(   3,   1), S(   7,   0), S(  10,  -6), S(   7,   2)},
   {S(  -7,  28), S(   4,  25), S( -22,   9), S( -15,   0),
    S( -13,   1), S(   9, -12), S(   3,  -9), S( -14,   4)}},
  {{S(   0,   0), S( -18, -16), S( -16,  -2), S(  22, -20),
   ...
   {S(   0,   0), S(  -2, -19), S( -20, -16), S( -15,  -2),
    S(  -4,  -7), S(   2, -23), S(  73, -12), S(  15,  20)}},
};
...
const int PassedFriendlyDistance[FILE_NB] = {
    ...
    S(   5, -15), S(  -7, -11), S( -15,  -1), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   3,   0), S(   3,   0), S(   9,   9),
    S(   2,  21), S(   8,  27), S(  28,  23), S(   0,   0),
};
...
const int ThreatMinorAttackedByMinor = S( -25, -37);
...
const int ComplexityTotalPawns  = S(   0,  12);
