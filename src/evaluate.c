...
const int PawnBackwards[2] = { S(   4,  -4), S(  -9, -16) };

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -1,  -8), S(  10,   2), S(   0,   0), S(   7,  17),
    S(  13,   2), S(  24,   0), S(  16,   6), S(  20,  15),
    S(   4,   0), S(  20,   4), S(  13,   8), S(  18,  15),
    S(   4,  10), S(  20,  15), S(  20,  22), S(  38,  19),
    S(  24,  53), S(  24,  64), S(  64,  61), S(  50,  73),
    S( 104, -13), S( 197,  15), S( 227,  20), S( 249,  74),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

/* Knight Evaluation Terms */

const int KnightOutpost[2] = { S(  13,  -5), S(  31,   3) };

const int KnightBehindPawn = S(   2,  17);

const int KnightMobility[9] = {
    S( -89, -85), S( -36, -94), S( -21, -40), S(  -7, -14),
    S(   2, -13), S(   7,   1), S(  17,   3), S(  30,  -1),
    S(  51, -39),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  30,  69);

const int BishopRammedPawns = S( -11, -13);

const int BishopOutpost[2] = { S(  21,   0), S(  40,   0) };

const int BishopBehindPawn = S(   2,  13);

const int BishopMobility[14] = {
    S( -60,-128), S( -47, -68), S( -15, -47), S(  -3, -21),
    S(   8, -11), S(  17,   0), S(  21,   6), S(  24,   6),
    S(  24,  11), S(  33,   6), S(  35,   3), S(  45, -12),
    S(  45,  -1), S(  40, -32),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  14,   0), S(  39,  -1) };

const int RookOnSeventh = S(   0,  25);

const int RookMobility[15] = {
    S(-147,-107), S( -70,-119), S( -20, -69), S( -10, -27),
    S(  -9,  -6), S(  -9,   8), S(  -8,  21), S(  -1,  26),
    S(   3,  34), S(   8,  36), S(   9,  44), S(  20,  48),
    S(  21,  51), S(  27,  47), S(  21,  48),
};

/* Queen Evaluation Terms */

const int QueenMobility[28] = {
    S( -61,-263), S(-216,-389), S( -48,-204), S( -38,-189),
    S( -15,-132), S( -26, -69), S( -16, -89), S( -19, -75),
    S( -11, -60), S(  -9, -50), S(  -4, -28), S(  -2, -26),
    S(  -2, -15), S(   2,  -6), S(   1,  -1), S(   0,   3),
    S(   5,  15), S(   0,  12), S(  11,  20), S(   0,  20),
...
const int KingDefenders[12] = {
    S( -31,  -2), S( -15,   5), S(  -1,   0), S(  10,   0),
    S(  22,  -5), S(  34,   2), S(  32,  11), S(  23,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -17,  14), S(   6, -10), S(  15,   0), S(  22,   1),
    S(   7,   6), S(  30,   4), S(   0, -33), S( -31,   2)},
   {S(   3,   5), S(  17,  -7), S(  11,  -9), S(  -1, -12),
    S( -26,   0), S( -66,  78), S( 100,  93), S( -29,   1)},
   {S(  13,  12), S(   7,  -1), S( -20,   0), S(  -6,  -2),
    S( -21,  -3), S(  13,  -9), S(   4,  65), S( -16,   2)},
   {S(   8,  30), S(  13,   0), S(  -3,  -7), S(  20, -16),
    S(  16, -35), S( -22, -32), S(-147,  41), S(  -1,   0)},
   {S( -19,  19), S(  -2,   2), S( -31,   2), S( -11,   4),
    S( -24, -16), S( -41, -19), S(  38,  -8), S( -13,   0)},
   {S(  22,   2), S(  18,  -4), S( -20,   0), S(  -3, -21),
    S(   7, -32), S(  16, -48), S(  49, -34), S( -13,   0)},
   {S(  18,   2), S(   1, -10), S( -29,  -7), S( -17, -13),
    S( -24, -20), S( -42,   9), S(   6,  47), S( -25,   7)},
   {S( -19,  -2), S(   0,  -8), S(   4,   0), S(   2,   3),
    S( -11,  13), S(   3,  32), S(-201,  79), S( -18,  14)}},
  {{S(   0,   0), S(   4, -15), S(   5, -14), S( -46,  18),
    S(   0,  -9), S(   9,  51), S(-170, -14), S( -50,  11)},
   {S(   0,   0), S(  16,  -7), S(   7,  -6), S(  -5,  -7),
    S(  10, -32), S(  51, 102), S(-187, -10), S( -45,   4)},
   {S(   0,   0), S(  25,   0), S(   0,  -6), S(  19, -26),
    S(  14,  -7), S(-113,  45), S( -89, -84), S( -21,   0)},
   {S(   0,   0), S(  -5,   9), S( -10,  12), S( -13,   3),
    S( -23,   0), S(-116,  10), S(   5, -50), S( -27,   1)},
   {S(   0,   0), S(   4,   5), S(   7,  -6), S(  23,  -4),
    S(  10, -23), S( -44,   7), S(-109, -73), S(  -3,  -3)},
   {S(   0,   0), S(   9,   0), S( -16,  -1), S( -12, -13),
    S(  29, -37), S( -45,   4), S(  59,  46), S( -30,   0)},
   {S(   0,   0), S(  12,   0), S(  -2,   0), S( -20,  -4),
    S( -20, -13), S(  22,  11), S( -58, -64), S( -43,  14)},
   {S(   0,   0), S(   7, -27), S(   8, -16), S( -22,   0),
    S( -26,  -2), S(   7, -16), S(-239, -73), S( -43,  15)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S(   2,   6), S(  33,  21), S(  -6,   9), S( -14,   4),
    S(  -2,  -1), S(  -5,  -1), S( -11,   3), S(  -3,   3)},
   {S(   5,  25), S(  12,  19), S(  -6,   4), S(  -4,  -1),
    S(   0,   0), S(   5,  -6), S(   1,   4), S(  -6,   7)},
   {S(   3,  17), S(   3,  20), S( -11,   4), S( -17,   0),
    S(   3,  -4), S(   1,  -3), S(   0,   0), S(   2,   3)},
   {S(   5,  16), S(   6,  19), S(  -5,  -5), S( -19,  -7),
    S(  -9,  -5), S(   3, -10), S(   0,  -3), S( -15,   2)}},
  {{S(   0,   0), S(   0,   0), S(   6, -12), S(  20, -14),
    S(   6,   0), S(  -1,  -6), S(  -6,   0), S(   0,   1)},
   {S(   0,   0), S(   0,   0), S(   0, -20), S(  28,  -9),
    S(   0,   2), S(  -3,   0), S(  -1,   0), S(  -2,   0)},
   {S(   0,   0), S(  -1,  -1), S(  -8, -21), S(  -2,  -4),
    S(  -5,   5), S( -19,  -4), S(  -4,  -1), S(   0,   0)},
...
const int PassedPawn[2][2][8] = {
  {{S(   0,   0), S( -27, -24), S( -22,   2), S( -12,   0),
    S(  15,   0), S(  54,   0), S( 140,  31), S(   0,   0)},
   {S(   0,   0), S(  -5,  -7), S( -23,  10), S( -15,  23),
    S(   2,  41), S(  65,  66), S( 191, 136), S(   0,   0)}},
  {{S(   0,   0), S( -12,   4), S( -18,   0), S(  -9,  20),
    S(  25,  37), S(  76,  75), S( 238, 158), S(   0,   0)},
   {S(   0,   0), S( -22,  -7), S( -16,  -1), S( -16,  34),
    S(  -1, 100), S(  43, 222), S( 124, 380), S(   0,   0)}},
};

const int PassedFriendlyDistance[RANK_NB] = {
    S(   0,   0), S(   2,   2), S(   2,   1), S(   2,  -3),
    S(   0,  -8), S(   0, -12), S(   0, -15), S(   0,   0),
};

const int PassedEnemyDistance[RANK_NB] = {
    S(   0,   0), S(   0,   2), S(   0,   1), S(   0,   8),
    S(   2,  16), S(   1,  21), S(   0,  30), S(   0,   0),
};

const int PassedSafePromotionPath = S(   0,  25);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -29, -36);
const int ThreatMinorAttackedByPawn  = S( -67, -53);
const int ThreatMinorAttackedByMinor = S( -25, -10);
const int ThreatMinorAttackedByMajor = S( -46, -43);
const int ThreatRookAttackedByLesser = S( -54, -24);
const int ThreatQueenAttackedByOne   = S( -94,   1);
const int ThreatOverloadedPieces     = S(  -7, -24);
