...
const int ThreatMinorWithoutPawn     = S(  -9,   0);
...
int evaluateThreats(EvalInfo *ei, Board *board, int colour) {
    ...
    // Penalty for minors not supported by a pawn
    count = popcount((knights | bishops) & ~ei->attackedBy[US][PAWN]);
    eval += count * ThreatMinorWithoutPawn;
    // if (TRACE) T.ThreatMinorWithoutPawn[US] += count;
