...
const int PawnPSQT32[32] = {
    ...
    S( -12,  15), S(  -8,   8), S( -14,  -7), S(  -8, -22),
    S(  -8,  33), S(  -5,  27), S(   0,  16), S(  24, -11),
...
const int BishopPSQT32[32] = {
    S(  11, -26), S(  12, -17), S(  -3, -14), S(   8, -11),
    S(  27, -36), S(  13, -38), S(  21, -22), S(  11, -11),
    S(  17, -13), S(  26, -15), S(   3, -17), S(  22,  -4),
    S(  12,  -8), S(  20,  -3), S(  17,   5), S(  26,   7),
    S( -10,  14), S(  20,   6), S(   6,  14), S(  17,  19),
...
const int RookPSQT32[32] = {
    S( -13, -30), S( -12, -21), S(  -4, -21), S(   3, -28),
    S( -52, -14), S(  -9, -33), S(  -9, -32), S(   0, -36),
    S( -28, -15), S(  -5, -12), S( -16, -16), S(   0, -25),
    S( -18,  -2), S( -11,   7), S(  -7,   3), S(   5,  -4),
...
const int QueenPSQT32[32] = {
    ...
    S(  14, -42), S(  27, -58), S(  27, -71), S(  17, -27),
...
const int KingPSQT32[32] = {
    S(  50, -77), S(  42, -49), S( -17, -11), S( -28, -26),
    S(  26, -20), S( -18, -13), S( -46,  10), S( -67,  13),
    S(  -5, -23), S(   3, -21), S(  12,   1), S( -17,  18),
    S(   0, -30), S(  94, -34), S(  46,   7), S(  -4,  29),
    S(  10, -13), S( 108, -29), S(  56,  16), S(  12,  22),
    S(  50, -26), S( 134, -18), S( 105,   7), S(  43,   2),
...
const int PawnConnected32[32] = {
    ...
    S(  54,  19), S(  41,  45), S(  62,  56), S(  75,  62),
...
const int KingDefenders[12] = {
    S( -28,  -3), S( -11,   0), S(   2,   6), S(  11,   7),
    S(  18,   7), S(  30,   1), S(  30,  -4), S(  12,  -1),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  42,  32), S(  29,  22), S(  10,   9), S( -24, -18),
...
const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -9,   2), S(  14, -26), S(  23,  -9), S(  18,   6),
   ...
   {S(   2,  13), S(  27, -13), S(   7, -14), S(  16, -22),
    S(  28, -39), S( -59,   3), S(-135,  51), S(   4,  -6)},
   {S( -17,  13), S(   3,  -2), S( -32,   4), S( -20,   8),
    S( -21,  -6), S( -45,  -5), S(  32, -18), S(  -9,   1)},
   {S(  48, -16), S(  20, -13), S( -21,   3), S(  -8, -22),
    S(   7, -27), S(  20, -21), S(  40, -32), S( -23,   1)},
   {S(  33, -15), S(   0, -17), S( -28,  -2), S( -22,  -8),
    S( -31,  -5), S( -41,  30), S(   1,  44), S( -11,   1)},
   {S(  -6, -13), S(   5, -21), S(   8,   2), S(  -2,  12),
...
const int SafetyUnsafeCheck     = S(   0,  18);
const int SafetyAdjustment      = S( -75, -26);
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -36,   1), S( -48,  26), S( -72,  30),
   ...
   {S(   0,   0), S( -28,  13), S( -45,  38), S( -65,  40),
    S(   0,  39), S( 106,  25), S( 190,  88), S(   0,   0)}},
  {{S(   0,   0), S( -24,  26), S( -48,  33), S( -67,  47),
    S(  10,  44), S( 103,  38), S( 271,  98), S(   0,   0)},
...
const int PassedSafePromotionPath = S( -23,  43);
...
const int ComplexityAdjustment  = S(   0,-147);
...
int evaluateKings(EvalInfo *ei, Board *board, int colour) {
    ...
    if (ei->kingAttackersCount[US] > 1 - popcount(enemyQueens)) {
        ...
        uint64_t unsafeChecks = (knightChecks | bishopChecks | rookChecks | queenChecks) & ~safe;
        ...
        safety += SafetyAttackValue     * scaledAttackCounts
                ...
                + SafetySafeQueenCheck  * popcount(safe & queenChecks)
                + SafetySafeRookCheck   * popcount(safe & rookChecks)
                + SafetySafeBishopCheck * popcount(safe & bishopChecks)
                + SafetySafeKnightCheck * popcount(safe & knightChecks)
                + SafetyUnsafeCheck     * popcount(unsafeChecks)
                + SafetyAdjustment;
        ...
        if (TRACE) T.SafetySafeQueenCheck[US]  = popcount(safe & queenChecks);
        if (TRACE) T.SafetySafeRookCheck[US]   = popcount(safe & rookChecks);
        if (TRACE) T.SafetySafeBishopCheck[US] = popcount(safe & bishopChecks);
        if (TRACE) T.SafetySafeKnightCheck[US] = popcount(safe & knightChecks);
        if (TRACE) T.SafetyUnsafeCheck[US]     = popcount(unsafeChecks);
