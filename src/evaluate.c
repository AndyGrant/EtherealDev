...
const int PawnValue   = S(  95, 103);
const int KnightValue = S( 408, 376);
const int BishopValue = S( 428, 387);
const int RookValue   = S( 597, 617);
const int QueenValue  = S(1183,1208);
...
const int PawnPSQT32[32] = {
    ...
    S( -18,   5), S(   1,   0), S( -11,   3), S(  -8,  -1),
    S( -21,   2), S( -12,   2), S(  -9,  -6), S(  -4, -13),
    S( -17,   8), S( -13,   9), S(   7, -11), S(   7, -22),
    S( -10,   9), S(  -7,   6), S(  -9,  -7), S(  -4, -20),
    S(  -8,  24), S(  -4,  20), S(  -2,  12), S(  22, -14),
    S( -18, -53), S( -63, -25), S(  -1, -39), S(  36, -53),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -43, -26), S(  -9, -27), S( -13, -26), S(  -1, -17),
    S(  -2, -19), S(   2, -12), S(   2, -28), S(   7, -19),
    S(   8, -23), S(  18, -23), S(  12, -19), S(  20,  -8),
    S(  18,   0), S(  17,   4), S(  26,   9), S(  24,  16),
    S(  23,  11), S(  23,   6), S(  33,  18), S(  26,  27),
    S( -12,   9), S(   3,   9), S(  29,  18), S(  22,  21),
    S(  12, -10), S(  -7,  -1), S(  24, -19), S(  40,  -5),
    S(-169, -16), S( -79,   0), S(-107,  18), S( -31,  -3),
};

const int BishopPSQT32[32] = {
    S(  -2, -30), S(   6, -19), S( -12,  -9), S(   6, -13),
    S(  24, -34), S(   2, -38), S(  16, -20), S(   8, -13),
    S(  11, -13), S(  23, -16), S(  -3, -23), S(  18,  -8),
    S(  12,  -9), S(  15,  -5), S(  12,   1), S(  20,   2),
    S( -11,   9), S(  15,   2), S(   3,  10), S(   9,  13),
    S(   0,   4), S(  -5,  13), S( -10,  -2), S(  12,   9),
    S( -43,  12), S( -56,  -2), S( -10,   5), S( -22,   8),
    S( -58, -12), S( -44,   5), S( -81,  12), S( -86,  23),
};

const int RookPSQT32[32] = {
    S( -16, -30), S( -19, -22), S(  -7, -27), S(   1, -31),
    S( -54, -17), S( -16, -34), S( -12, -34), S(  -2, -37),
    S( -30, -19), S( -11, -16), S( -20, -20), S(  -4, -28),
    S( -21,  -8), S( -14,   0), S( -12,  -5), S(   1, -10),
    S(  -9,   1), S(   5,  -2), S(  14,  -5), S(  30, -10),
    S( -20,  11), S(  17,  -2), S(  -6,  11), S(  25,  -5),
    S(   0,  -2), S( -16,   7), S(   6,  -3), S(  21,  -3),
    S(  24,  14), S(  19,  15), S(   2,  20), S(  13,  16),
};

const int QueenPSQT32[32] = {
    S(  17, -68), S(  -3, -53), S(   3, -63), S(   8, -50),
    S(   2, -48), S(  14, -64), S(  17, -79), S(   7, -38),
    S(  -1, -37), S(  13, -36), S(  -4, -13), S(  -3, -17),
    S(   1, -25), S(   3,  -4), S( -12,  -1), S( -25,  37),
    S( -18,  -3), S( -16,  19), S( -24,  -3), S( -39,  42),
    S( -36,  12), S( -31,   4), S( -33,   0), S( -23,  -1),
    S( -15,   9), S( -63,  39), S( -19,  -8), S( -51,  30),
    S( -25,   0), S(   2, -10), S(  -7, -12), S( -18,  -1),
};

const int KingPSQT32[32] = {
    S(  39, -67), S(  35, -43), S( -16,  -7), S( -34, -13),
    S(  29, -20), S(  -5, -12), S( -37,  12), S( -54,  14),
    S(   8, -21), S(  12, -18), S(  13,   4), S( -10,  18),
    S(   8, -29), S(  86, -32), S(  43,   7), S(  -1,  25),
    S(   9, -15), S( 101, -30), S(  54,  11), S(  11,  19),
    S(  47, -28), S( 125, -24), S( 100,   0), S(  44,   0),
    S(   4, -61), S(  46, -16), S(  30,   1), S(   8,  -7),
    S(   4,-117), S(  70, -67), S( -23, -25), S( -21, -19),
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -23,  -8), S( -13,  10), S( -16,  27),
    S(  -1,  53), S(  34,  58), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -13,  21), S(  -4,  20), S(   1,  40),
    S(   9,  71), S(  29,  55), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -6, -10);

const int PawnStacked[2] = { S(  -5, -18), S(  -7,  -6) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   2,  -7), S(   8,  -6), S(   8, -13),
    S(  16, -16), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -10, -28), S(  -6, -23), S(   6, -25),
    S(  42, -27), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -2,  -6), S(  10,   0), S(   2,   1), S(   4,  11),
    S(  12,   0), S(  24,  -3), S(  18,   4), S(  21,  11),
    S(   8,  -1), S(  21,   1), S(   8,   7), S(  15,  13),
    S(  10,   7), S(  19,  12), S(  25,  17), S(  28,  16),
    S(  48,  16), S(  35,  40), S(  55,  44), S(  65,  49),
    S( 111,  -8), S( 203,  -4), S( 227,  26), S( 236,  38),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightInSiberia[4] = {
    S(  -9,   0), S( -11,  -6), S( -22,  -6), S( -31, -11),
};

const int KnightMobility[9] = {
    S( -81,-108), S( -45,-101), S( -32, -53), S( -21, -32),
    S( -12, -23), S(  -7, -10), S(   0,  -6), S(   8,  -6),
    S(  19, -19),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  21,  59);

const int BishopRammedPawns = S(  -8, -12);
...
const int BishopBehindPawn = S(   3,  16);

const int BishopLongDiagonal = S(  25,  14);

const int BishopMobility[14] = {
    S( -74,-145), S( -43, -96), S( -26, -61), S( -17, -38),
    S(  -8, -29), S(  -2, -18), S(   1,  -9), S(   2,  -5),
    S(   1,   0), S(   4,   1), S(   4,   1), S(  22,  -9),
    S(  28,   0), S(  58, -33),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  12,   3), S(  30,   4) };

const int RookOnSeventh = S(  -8,  26);

const int RookMobility[15] = {
    S(-151,-114), S( -67,-115), S( -33, -71), S( -25, -37),
    S( -25, -21), S( -26,  -8), S( -25,   1), S( -19,   3),
    S( -13,   6), S( -10,  10), S(  -8,  15), S(  -3,  17),
    S(  -3,  20), S(  10,  10), S(  70, -33),
};

/* Queen Evaluation Terms */

const int QueenMobility[28] = {
    S( -61,-263), S(-209,-387), S( -59,-199), S( -39,-190),
    S( -27,-137), S( -24, -83), S( -19, -64), S( -19, -42),
    S( -14, -43), S( -13, -27), S(  -9, -23), S(  -9,  -9),
    S(  -7, -16), S(  -6,  -8), S(  -8,  -6), S( -12,   0),
    S( -12,  -2), S( -21,   1), S( -18,  -4), S( -24,  -9),
    S( -13, -36), S(   2, -60), S(   2, -88), S(   6,-104),
    S(  -8,-112), S(   7,-133), S( -60, -47), S( -38, -75),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -17,   4), S(  -3,  -2), S(   2,   3), S(   7,   5),
    S(  14,   5), S(  22,   2), S(  25,  -5), S(  11,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  56,  29), S(  38,  21), S(  22,  11), S( -32,  -7),
    S( -36, -42), S( -30, -64), S( -25, -76), S( -14, -72),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -8,   1), S(  11, -23), S(  19,  -8), S(  12,   2),
    S(   8,   2), S(  -6,  -1), S(  -4, -34), S( -44,  18)},
   {S(  16,  -7), S(  17, -13), S(   1,  -3), S( -13,   2),
    S( -27,  12), S( -73,  63), S(  90,  80), S( -17,   2)},
   {S(  31,  -3), S(  11,  -7), S( -25,   6), S( -10,  -9),
   S( -16,  -8), S( -17,  -1), S(  -1,  65), S( -10,  -2)},
   {S(   1,  10), S(  20, -10), S(   4, -10), S(  12, -20),
    S(  25, -36), S( -56,   7), S(-135,  50), S(   7,  -7)},
   {S( -15,   9), S(   4,  -2), S( -23,   2), S( -16,   4),
    S( -16,  -9), S( -46,  -5), S(  31, -20), S(  -3,  -1)},
   {S(  41, -15), S(  19, -14), S( -18,   3), S( -11, -17),
    S(   5, -24), S(  15, -19), S(  41, -31), S( -21,   1)},
   {S(  23, -12), S(  -2, -13), S( -21,  -1), S( -19,  -7),
    S( -28,  -8), S( -41,  27), S(   0,  44), S(  -7,   1)},
   {S(  -8, -10), S(   4, -15), S(   6,   3), S(  -2,  11),
    S( -13,  14), S( -14,  33), S(-189,  87), S( -15,  14)}},
  {{S(   0,   0), S( -11, -23), S(   9, -23), S( -38,  12),
    S( -27,   2), S(   2,  42), S(-166,  -7), S( -39,  15)},
   {S(   0,   0), S(  20, -18), S(   6,  -7), S( -19,  -1),
    S(  -1, -15), S(  21,  58), S(-184,  -4), S( -28,   6)},
   {S(   0,   0), S(  23, -11), S(  -3,  -9), S(   3, -20),
    S(  13,  -9), S( -83,  46), S( -83, -72), S( -15,  -1)},
   {S(   0,   0), S(  -8,   7), S(  -5,  -2), S( -23,   2),
    S( -27,  -1), S( -98,  30), S(   7, -41), S( -10,  -2)},
   {S(   0,   0), S(  12,   2), S(  10,  -5), S(  11, -12),
    S(  14, -28), S( -55,  16), S(-103, -60), S(   2,   0)},
   {S(   0,   0), S(   0,  -6), S( -21,  -1), S( -24,  -9),
    S(  16, -26), S( -36,   3), S(  55,  37), S( -11,  -4)},
   {S(   0,   0), S(  19, -13), S(  10, -12), S( -11,  -7),
    S( -26,   7), S( -12,  15), S( -56, -47), S( -26,  13)},
   {S(   0,   0), S(  10, -35), S(  18, -26), S( -17,  -7),
    S( -20,  14), S( -12,  21), S(-227, -54), S( -18,   6)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -12,  27), S( 104, -13), S( -20,  20), S( -14,   4),
    S( -12,   1), S(  -7,  -3), S( -15,   6), S( -22,   3)},
   {S( -12,  48), S(  53,   6), S( -15,  19), S(  -2,   8),
    S(  -3,   4), S(   4,  -4), S(  -2,   2), S( -12,   5)},
   {S(  -1,  38), S(  10,  18), S( -16,  15), S(  -8,   6),
    S(   3,   1), S(   6,   0), S(   8,  -4), S(   3,   3)},
   {S(  -7,  26), S(   4,  17), S( -15,   5), S( -13,  -1),
    S( -11,   0), S(   8, -10), S(   3,  -7), S(  -8,   4)}},
  {{S(   0,   0), S( -21, -20), S( -14,  -1), S(  17, -15),
    S(   7,  -3), S(   5, -14), S(   3,   3), S(  19,  30)},
   {S(   0,   0), S( -19, -40), S(   3,  -8), S(  36,  -9),
    S(   1,   0), S(  13, -16), S(  -4,  -7), S( -19,   0)},
   {S(   0,   0), S( -32, -57), S( -20,  -6), S(  13, -10),
    S(   4,   1), S(  -7,  -8), S( -10, -10), S( -13,   4)},
   {S(   0,   0), S(  -4, -22), S( -11, -15), S( -10,  -2),
    S(  -1,  -4), S(   6, -18), S(  73, -15), S(  14,  19)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -37,   9), S( -46,  23), S( -72,  29),
    S(  -1,  13), S(  81, -11), S( 161,  40), S(   0,   0)},
   {S(   0,   0), S( -32,  13), S( -47,  31), S( -66,  35),
    S(  -8,  33), S(  99,  24), S( 190,  74), S(   0,   0)}},
  {{S(   0,   0), S( -27,  21), S( -45,  26), S( -66,  36),
    S(   1,  37), S(  96,  31), S( 250,  86), S(   0,   0)},
   {S(   0,   0), S( -33,  19), S( -42,  26), S( -62,  41),
    S(   3,  49), S( 109,  91), S( 169, 218), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   1,   1), S(   3,  -3), S(   5,  -7),
    S(   4, -10), S( -12,  -8), S( -18,  -1), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   3,  -1), S(   5,  -1), S(  10,   5),
    S(   2,  16), S(   7,  23), S(  23,  21), S(   0,   0),
};

const int PassedSafePromotionPath = S( -15,  30);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -10, -25);
const int ThreatMinorAttackedByPawn  = S( -46, -49);
const int ThreatMinorAttackedByMinor = S( -22, -33);
const int ThreatMinorAttackedByMajor = S( -19, -44);
const int ThreatRookAttackedByLesser = S( -41, -22);
const int ThreatMinorAttackedByKing  = S( -31, -21);
const int ThreatRookAttackedByKing   = S( -35, -22);
const int ThreatQueenAttackedByOne   = S( -32, -45);
...
const int SpaceRestrictEmpty = S(  -4,  -1);
