...
const int PawnValue   = S(  97, 125);
const int KnightValue = S( 455, 406);
const int BishopValue = S( 474, 423);
const int RookValue   = S( 654, 686);
const int QueenValue  = S(1298,1341);
...
const int PawnPSQT32[32] = {
    ...
    S( -15,   6), S(   3,   1), S(  -6,   4), S(  -4,   0),
    S( -24,   5), S( -12,   4), S( -11,  -5), S(  -2, -12),
    S( -18,  12), S( -12,  11), S(   4,  -8), S(   6, -22),
    S( -14,  16), S(  -9,   8), S( -14,  -6), S( -10, -20),
    S(  -7,  30), S(  -5,  26), S(  -1,  13), S(  24, -12),
    S( -21, -60), S( -59, -30), S(   1, -44), S(  44, -60),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -49, -32), S(  -1, -29), S(  -9, -29), S(   3, -14),
    S(   2, -19), S(   2, -12), S(   5, -27), S(  11, -15),
    S(  14, -28), S(  18, -20), S(  16, -16), S(  23,  -3),
    S(  18,   2), S(  24,   7), S(  31,  17), S(  36,  25),
    S(  23,  12), S(  30,   9), S(  42,  28), S(  34,  38),
    S( -18,  11), S(   1,  12), S(  31,  29), S(  31,  27),
    S(  12, -13), S(  -6,  -2), S(  29, -23), S(  39,  -1),
    S(-178, -25), S( -85,  -4), S(-111,  17), S( -33,  -4),
};

const int BishopPSQT32[32] = {
    S(  10, -27), S(  13, -17), S(  -1, -15), S(  11, -10),
    S(  26, -36), S(  13, -38), S(  20, -21), S(  13, -10),
    S(  17, -12), S(  27, -14), S(   4, -17), S(  22,  -3),
    S(  11,  -7), S(  21,  -2), S(  17,   6), S(  27,   6),
    S( -11,  14), S(  20,   6), S(   5,  15), S(  17,  17),
    S(   3,   6), S(  -5,  17), S(   0,   7), S(  13,  14),
    S( -45,  14), S( -50,   5), S(  -8,   7), S( -23,  13),
    S( -58,  -7), S( -45,   6), S( -84,  16), S( -90,  27),
};

const int RookPSQT32[32] = {
    S( -13, -27), S( -13, -20), S(  -2, -21), S(   4, -24),
    S( -47, -14), S(  -9, -33), S(  -7, -30), S(   4, -34),
    S( -28, -16), S(  -6, -12), S( -13, -16), S(   2, -25),
    S( -19,  -3), S( -11,   7), S(  -6,   3), S(   7,  -3),
    S(  -6,   9), S(   9,   8), S(  21,   5), S(  41,   1),
    S( -18,  18), S(  27,   4), S(   5,  20), S(  38,   2),
    S(  -1,   4), S( -12,  12), S(  12,   4), S(  28,   6),
    S(  35,  20), S(  27,  23), S(   1,  32), S(  10,  28),
};

const int QueenPSQT32[32] = {
    S(  34, -66), S(   7, -44), S(  16, -59), S(  23, -44),
    S(  14, -42), S(  29, -57), S(  28, -70), S(  18, -26),
    S(  10, -30), S(  24, -18), S(   7,   7), S(   7,   2),
    S(  12,  -9), S(  17,  12), S(   0,  24), S( -13,  65),
    S(  -4,  14), S(  -8,  47), S( -13,  22), S( -27,  74),
    S( -25,  31), S( -18,  19), S( -23,  25), S( -11,  27),
    S(  -2,  27), S( -65,  67), S(  -3,  12), S( -39,  51),
    S( -12,  18), S(  20,   9), S(   8,   9), S(  -5,  20),
};

const int KingPSQT32[32] = {
    S(  51, -77), S(  41, -52), S( -18, -10), S( -28, -25),
    S(  27, -19), S( -17, -10), S( -45,  12), S( -65,  15),
    S(  -6, -20), S(   3, -17), S(  12,   3), S( -18,  20),
    S(   0, -30), S(  95, -32), S(  46,   9), S(  -3,  31),
    S(  10, -13), S( 109, -30), S(  57,  15), S(  12,  22),
    S(  50, -27), S( 134, -22), S( 105,   3), S(  43,   0),
    S(   7, -58), S(  48, -11), S(  32,   3), S(   9,  -5),
    S(   4,-123), S(  73, -63), S( -23, -24), S( -20, -20),
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -27, -10), S( -10,   9), S( -17,  33),
    S(   5,  61), S(  38,  65), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -16,  19), S(  -4,  23), S(   3,  47),
    S(  22,  88), S(  38,  62), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -5, -13);

const int PawnStacked[2] = { S(  -1, -21), S(  -5, -10) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   2,  -6), S(  10,  -6), S(   8, -11),
    S(   6, -13), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -15, -27), S(  -5, -25), S(   2, -25),
    S(  30, -30), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -6,  -7), S(  10,   1), S(   0,   1), S(   8,  12),
    S(  12,   1), S(  21,  -3), S(  22,   3), S(  24,  10),
    S(   7,  -1), S(  22,   0), S(   8,   6), S(  15,  14),
    S(   9,  10), S(  20,  16), S(  24,  20), S(  29,  18),
    S(  53,  18), S(  40,  44), S(  60,  54), S(  74,  61),
...
const int KnightInSiberia[4] = {
    S(  -9,  -2), S( -13,  -7), S( -26,  -6), S( -26,  -6),
};

const int KnightMobility[9] = {
    S( -78,-106), S( -31,-102), S( -15, -42), S(  -3, -16),
    S(   9,  -6), S(  11,  11), S(  19,  13), S(  29,  12),
    S(  43,  -6),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  18,  66);
...
const int BishopMobility[14] = {
    S( -69,-149), S( -33,-100), S( -10, -56), S(  -2, -28),
    S(   9, -18), S(  16,  -3), S(  19,   8), S(  19,  13),
    S(  18,  19), S(  25,  20), S(  24,  20), S(  46,   7),
    S(  47,  19), S(  75, -19),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(   8,   4), S(  30,   4) };

const int RookOnSeventh = S(  -4,  32);

const int RookMobility[15] = {
    S(-159,-116), S( -57,-115), S( -16, -67), S(  -8, -23),
    S( -10,  -1), S( -11,  14), S( -10,  26), S(  -2,  29),
    S(   5,  34), S(   8,  37), S(  11,  46), S(  18,  49),
    S(  20,  53), S(  38,  40), S( 106,  -9),
};
...
const int QueenMobility[28] = {
    S( -61,-263), S(-213,-388), S( -60,-200), S( -24,-191),
    S(  -6,-144), S(  -5, -80), S(   0, -43), S(  -1, -11),
    S(   5, -11), S(   7,  12), S(  13,  13), S(  15,  31),
    S(  18,  23), S(  20,  32), S(  17,  37), S(  14,  44),
    S(  16,  41), S(   8,  44), S(   9,  41), S(   8,  31),
    S(  16,   7), S(  30, -16), S(  31, -45), S(  30, -63),
...
const int KingDefenders[12] = {
    S( -27,  -1), S(  -6,  -1), S(   3,   5), S(  10,   7),
    S(  18,   6), S(  28,   1), S(  26,  -4), S(  12,  -1),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  40,  32), S(  31,  22), S(  10,   7), S( -25, -18),
    S( -28, -48), S( -21, -62), S( -17, -66), S( -11, -64),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -9,   2), S(  14, -26), S(  23,  -9), S(  19,   7),
    S(  14,   3), S(  -1,  -1), S(  -3, -34), S( -51,  21)},
   {S(  15,  -7), S(  20, -18), S(   4,  -5), S( -15,   4),
    S( -30,  16), S( -73,  67), S(  91,  81), S( -18,   0)},
   {S(  35,  -3), S(   9,  -7), S( -31,   7), S( -14,  -8),
    S( -17,  -5), S( -12,   1), S(   0,  68), S( -13,  -1)},
   {S(   1,  13), S(  27, -15), S(   5, -14), S(  15, -21),
    S(  30, -38), S( -59,   3), S(-135,  51), S(   7,  -7)},
   {S( -17,  13), S(   4,  -2), S( -33,   4), S( -20,   8),
    S( -21,  -6), S( -45,  -5), S(  32, -18), S(  -9,   2)},
   {S(  49, -17), S(  21, -15), S( -22,   5), S(  -8, -22),
    S(   5, -26), S(  19, -20), S(  40, -32), S( -22,   2)},
   {S(  34, -16), S(   0, -17), S( -31,  -1), S( -23,  -8),
    S( -31,  -4), S( -41,  30), S(   1,  44), S(  -7,   1)},
   {S( -11, -14), S(   4, -21), S(   8,   2), S(  -2,  12),
    S( -10,  17), S(  -9,  37), S(-190,  89), S( -18,  16)}},
  {{S(   0,   0), S( -13, -26), S(   7, -23), S( -39,  20),
    S( -23,   1), S(   3,  42), S(-167,  -7), S( -42,  11)},
   {S(   0,   0), S(  22, -22), S(   9,  -7), S( -17,   0),
    S(   3, -18), S(  26,  65), S(-184,  -3), S( -29,   5)},
   {S(   0,   0), S(  22, -13), S(  -1, -11), S(   6, -20),
    S(  21,  -3), S( -87,  48), S( -84, -74), S( -18,  -2)},
   {S(   0,   0), S( -10,   9), S(  -8,  -2), S( -25,   5),
    S( -27,   4), S(-100,  30), S(   7, -40), S( -22,  -4)},
   {S(   0,   0), S(  15,   0), S(  12,  -7), S(  15, -13),
    S(  18, -31), S( -57,  14), S(-101, -60), S(  -2,  -2)},
   {S(   0,   0), S(  -1,  -6), S( -26,   2), S( -22,  -9),
    S(  23, -22), S( -37,   3), S(  55,  39), S( -16,  -4)},
   {S(   0,   0), S(  25, -17), S(  14, -16), S( -14,  -8),
    S( -29,  14), S(  -8,  19), S( -57, -50), S( -31,  15)},
   {S(   0,   0), S(  12, -45), S(  22, -33), S( -19,  -7),
    S( -20,  19), S(  -8,  21), S(-228, -56), S( -23,   4)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -13,  34), S( 112,  -6), S( -22,  24), S( -12,   1),
    S( -14,  -1), S(  -7,  -7), S( -15,   4), S( -29,   2)},
   {S(  -8,  54), S(  51,  15), S( -21,  20), S(  -4,   6),
    S(  -1,   3), S(   6,  -5), S(  -4,   5), S( -13,   6)},
   {S(   3,  44), S(   4,  28), S( -22,  18), S( -10,   5),
    S(   1,   2), S(   7,   0), S(  11,  -7), S(   4,   5)},
   {S(  -7,  28), S(   3,  24), S( -23,   8), S( -15,  -2),
    S( -10,   0), S(  11, -13), S(   2,  -8), S( -13,   7)}},
  {{S(   0,   0), S( -18, -16), S( -15,  -1), S(  23, -20),
    S(  11,  -7), S(   7, -20), S(   6,   2), S(  16,  29)},
   {S(   0,   0), S( -17, -37), S(   4, -11), S(  47, -13),
    S(   3,  -3), S(  16, -25), S(  -6,  -5), S( -19,   0)},
   {S(   0,   0), S( -28, -52), S( -23,  -7), S(  19, -12),
    S(   8,  -1), S(  -9, -14), S( -10, -12), S( -11,   6)},
   {S(   0,   0), S(  -2, -19), S( -19, -18), S( -13,  -2),
    S(  -1,  -8), S(   0, -24), S(  72, -12), S(  15,  20)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -37,   0), S( -48,  26), S( -72,  31),
    S(   8,  10), S(  80, -16), S( 157,  41), S(   0,   0)},
   {S(   0,   0), S( -28,  13), S( -46,  37), S( -66,  40),
    S(   2,  37), S( 106,  23), S( 190,  88), S(   0,   0)}},
  {{S(   0,   0), S( -22,  27), S( -48,  35), S( -66,  47),
    S(  11,  44), S( 103,  33), S( 271,  94), S(   0,   0)},
   {S(   0,   0), S( -29,  23), S( -42,  33), S( -62,  54),
    S(  10,  64), S( 105, 118), S( 157, 264), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   0,   2), S(   4,  -3), S(   8, -12),
    S(   5, -15), S( -12,  -9), S( -17,   0), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   3,  -1), S(   4,   0), S(   7,   9),
    S(   1,  21), S(   7,  27), S(  28,  23), S(   0,   0),
};

const int PassedSafePromotionPath = S( -19,  44);
...
const int ThreatMinorAttackedByMajor = S( -27, -48);
...
const int ThreatQueenAttackedByOne   = S( -51, -27);
...
const int ClosednessKnightAdjustment[9] = {
    S(  -9,  -8), S(  -7,   8), S(  -7,  16), S(  -1,  15),
    S(  -1,  19), S(   1,  14), S(   6,   6), S(  -8,  24),
    S(  -7,   9),
};

const int ClosednessRookAdjustment[9] = {
    S(  67, -11), S(   3,  33), S(   3,  18), S(  -2,   1),
    S( -11,   5), S( -11, -12), S( -12, -13), S( -23, -16),
    S( -35, -21),
};
...
const int ComplexityPawnEndgame = S(   0,  63);
const int ComplexityAdjustment  = S(   0,-158);
