...
const int KnightValue = S( 458, 419);
const int BishopValue = S( 478, 438);
const int RookValue   = S( 665, 706);
const int QueenValue  = S(1309,1357);
...
const int PawnPSQT32[32] = {
    ...
    S( -21,   9), S(  -1,   2), S( -15,   5), S( -13,  -3),
    S( -23,   2), S( -17,   1), S( -12,  -7), S( -10, -12),
    S( -19,  12), S( -15,  13), S(   8, -12), S(  11, -25),
    S(  -9,  18), S(  -5,  11), S(  -7,  -3), S(   3, -20),
    S(  -8,  32), S(  -2,  27), S(   2,  17), S(  30, -11),
    S( -20, -59), S( -60, -29), S(   2, -43), S(  45, -58),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -48, -31), S(  -3, -30), S( -17, -30), S(  -2, -17),
    S(  -3, -20), S(   3, -11), S(   1, -28), S(  10, -19),
    S(   9, -25), S(  22, -21), S(  16, -17), S(  23,  -2),
    S(  28,   2), S(  25,   7), S(  32,  16), S(  28,  22),
    S(  28,  15), S(  29,  10), S(  41,  27), S(  34,  36),
    S( -16,  12), S(   6,  15), S(  31,  30), S(  33,  30),
    S(  14, -11), S(  -3,   1), S(  32, -19), S(  41,   2),
    S(-178, -24), S( -85,  -3), S(-110,  19), S( -33,  -2),
};

const int BishopPSQT32[32] = {
    S(   5, -27), S(  13, -18), S(  -5,  -9), S(   7, -12),
    S(  32, -33), S(  11, -37), S(  25, -20), S(  12,  -9),
    S(  18, -12), S(  29, -13), S(   0, -16), S(  24,  -2),
    S(  17,  -6), S(  18,  -3), S(  19,   7), S(  26,   7),
    S(  -9,  14), S(  19,   5), S(   7,  14), S(  14,  18),
    S(   3,   7), S(  -3,  18), S(  -2,   4), S(  15,  14),
    S( -45,  14), S( -53,   4), S(  -7,   8), S( -23,  13),
    S( -58,  -7), S( -45,   6), S( -84,  15), S( -89,  27),
};

const int RookPSQT32[32] = {
    S(  -6, -28), S( -13, -20), S(   2, -24), S(   8, -29),
    S( -52, -14), S( -13, -34), S(  -8, -31), S(   2, -36),
    S( -30, -15), S(  -7, -13), S( -16, -16), S(  -1, -26),
    S( -18,  -1), S( -10,   8), S(  -7,   3), S(   5,  -5),
    S(  -3,  13), S(  11,  11), S(  22,   7), S(  39,  -1),
    S( -13,  24), S(  30,   9), S(   6,  21), S(  39,   4),
    S(   2,  10), S( -12,  16), S(  12,   6), S(  27,   6),
    S(  35,  21), S(  27,  23), S(   0,  31), S(   9,  28),
};

const int QueenPSQT32[32] = {
    S(  31, -65), S(   7, -44), S(  11, -59), S(  21, -42),
    S(  15, -41), S(  27, -57), S(  32, -67), S(  21, -26),
    S(  14, -29), S(  25, -19), S(   7,   6), S(   9,   2),
    S(  19,  -8), S(  15,  12), S(  -2,  24), S( -18,  64),
    S(  -2,  15), S(  -7,  47), S( -12,  22), S( -31,  74),
    S( -23,  32), S( -16,  20), S( -23,  25), S( -11,  27),
    S(  -3,  27), S( -63,  67), S(  -3,  12), S( -38,  52),
    S( -10,  20), S(  20,   9), S(   9,  10), S(  -5,  21),
};

const int KingPSQT32[32] = {
    S(  43, -77), S(  36, -51), S( -15, -15), S( -33, -22),
    S(  26, -26), S( -14, -18), S( -43,   7), S( -64,  10),
    S(  -3, -21), S(   6, -20), S(  11,  -1), S( -18,  15),
    S(   2, -27), S(  95, -32), S(  45,   8), S(  -4,  28),
    S(  11, -11), S( 109, -27), S(  57,  17), S(  13,  25),
    S(  51, -24), S( 135, -20), S( 105,   7), S(  44,   5),
    S(   7, -56), S(  48,  -8), S(  32,   6), S(   9,  -1),
...
const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -27, -10), S( -10,   9), S( -18,  32),
    S(   2,  59), S(  39,  64), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -14,  20), S(  -4,  22), S(  -1,  45),
    S(  18,  85), S(  38,  62), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S( -11, -13);

const int PawnStacked[2] = { S(  -5, -24), S(  -9,  -7) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   5,  -2), S(   9,  -6), S(  12, -14),
    S(  12, -11), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -12, -24), S(  -7, -20), S(   9, -19),
    S(  33, -25), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -1,  -5), S(  10,   1), S(   4,   0), S(   5,  12),
    S(  14,   1), S(  28,  -1), S(  20,   9), S(  24,  16),
    S(   8,   0), S(  23,   0), S(  10,   8), S(  16,  17),
    S(  12,  10), S(  22,  15), S(  30,  21), S(  33,  20),
    S(  57,  20), S(  43,  46), S(  63,  56), S(  76,  61),
...
const int KnightInSiberia[4] = {
    S( -12,   3), S( -14,  -4), S( -22,  -3), S( -26,  -6),
};

const int KnightMobility[9] = {
    S( -78,-106), S( -30,-101), S( -15, -41), S(  -3, -16),
    S(   8,  -4), S(  14,   9), S(  21,  13), S(  29,  15),
    S(  39,   3),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  23,  73);
...
const int BishopLongDiagonal = S(  26,  14);

const int BishopMobility[14] = {
    S( -69,-149), S( -31, -97), S(  -8, -52), S(   0, -26),
    S(   9, -15), S(  15,  -1), S(  18,   8), S(  20,  13),
    S(  20,  18), S(  24,  20), S(  24,  21), S(  45,   8),
    S(  47,  19), S(  75, -17),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  16,   8), S(  31,   9) };

const int RookOnSeventh = S(  -5,  35);

const int RookMobility[15] = {
    S(-159,-116), S( -56,-115), S( -14, -62), S(  -7, -17),
    S(  -7,   3), S(  -8,  18), S(  -7,  28), S(  -1,  31),
    S(   6,  33), S(  10,  37), S(  13,  42), S(  18,  47),
    S(  19,  49), S(  34,  39), S( 102,  -3),
};

/* Queen Evaluation Terms */

const int QueenRelativePin = S( -17,  -9);

const int QueenMobility[28] = {
    S( -61,-263), S(-213,-388), S( -60,-200), S( -19,-191),
    S(  -2,-143), S(   1, -79), S(   5, -43), S(   4, -12),
    S(   8,  -9), S(   9,  11), S(  12,  15), S(  13,  31),
    S(  16,  25), S(  17,  34), S(  15,  38), S(  13,  44),
    S(  13,  42), S(   6,  45), S(   7,  41), S(   6,  31),
    S(  16,   9), S(  30, -14), S(  31, -45), S(  30, -62),
...
const int KingDefenders[12] = {
    S( -21,  -2), S(  -7,   2), S(   2,   4), S(   9,   6),
    S(  16,   6), S(  25,   1), S(  29,  -4), S(  12,  -1),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  49,  28), S(  24,  23), S(  10,  11), S( -25, -16),
    S( -29, -49), S( -22, -63), S( -17, -67), S( -11, -64),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -9,   2), S(  17, -24), S(  25, -10), S(  15,   5),
    S(  14,   3), S(  -1,  -1), S(  -3, -34), S( -51,  20)},
   {S(  16,  -8), S(  23, -18), S(   2,  -5), S( -16,   4),
    S( -30,  16), S( -73,  67), S(  91,  81), S( -21,   2)},
   {S(  36,  -5), S(  12,  -7), S( -29,   7), S( -13,  -8),
    S( -17,  -5), S( -12,   1), S(   0,  68), S( -14,   0)},
   {S(   2,  11), S(  22, -12), S(   6, -13), S(  14, -21),
    S(  28, -38), S( -58,   3), S(-135,  51), S(   6,  -6)},
   {S( -17,  11), S(   4,  -2), S( -27,   4), S( -19,   7),
    S( -21,  -6), S( -45,  -5), S(  32, -18), S(  -8,  -1)},
   {S(  46, -16), S(  23, -14), S( -21,   4), S( -12, -17),
    S(   8, -25), S(  19, -21), S(  40, -32), S( -26,   0)},
   {S(  30, -15), S(   2, -16), S( -25,  -3), S( -21,  -7),
    S( -31,  -5), S( -41,  30), S(   1,  44), S( -10,  -1)},
   {S(  -9, -11), S(   5, -14), S(  10,   0), S(  -2,  10),
    S( -11,  15), S( -12,  36), S(-190,  89), S( -19,  14)}},
  {{S(   0,   0), S( -13, -26), S(   6, -23), S( -39,  20),
    S( -23,   1), S(   3,  42), S(-167,  -7), S( -43,  11)},
   {S(   0,   0), S(  26, -22), S(   7,  -8), S( -16,  -1),
    S(   3, -18), S(  26,  65), S(-184,  -3), S( -31,   4)},
   {S(   0,   0), S(  27, -12), S(  -3, -10), S(   6, -20),
    S(  20,  -3), S( -87,  48), S( -84, -74), S( -20,  -2)},
   {S(   0,   0), S( -10,   8), S(  -6,  -3), S( -25,   4),
    S( -27,   4), S(-100,  30), S(   7, -40), S( -20,   1)},
   {S(   0,   0), S(  10,   0), S(  12,  -7), S(  12, -13),
    S(  16, -31), S( -57,  14), S(-101, -60), S(   2,   0)},
   {S(   0,   0), S(   5,  -7), S( -26,   1), S( -24, -10),
    S(  23, -23), S( -37,   3), S(  55,  39), S( -17,  -6)},
   {S(   0,   0), S(  26, -13), S(  13, -14), S( -12,  -9),
    S( -29,  13), S(  -8,  19), S( -57, -50), S( -31,  12)},
   {S(   0,   0), S(  13, -40), S(  19, -33), S( -19,  -8),
    S( -20,  19), S(  -8,  21), S(-228, -56), S( -22,   5)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -13,  31), S( 112,  -7), S( -23,  21), S( -15,   4),
    S( -10,   1), S(  -6,  -4), S( -15,   6), S( -25,   5)},
   {S(  -8,  52), S(  51,  14), S( -19,  20), S(  -2,   7),
    S(  -2,   5), S(   6,  -3), S(  -1,   3), S( -11,   9)},
   {S(   3,  42), S(   4,  28), S( -22,  17), S( -10,   6),
    S(   5,   2), S(   9,   0), S(  12,  -6), S(   5,   5)},
   {S(  -7,  28), S(   3,  23), S( -22,   8), S( -15,   0),
    S( -10,  -1), S(   8, -11), S(   0,  -8), S( -11,   8)}},
  {{S(   0,   0), S( -18, -16), S( -15,  -3), S(  23, -19),
    S(  11,  -8), S(   5, -21), S(   3,   1), S(  16,  29)},
   {S(   0,   0), S( -17, -37), S(   2, -14), S(  42, -15),
    S(   3,  -4), S(  17, -25), S(  -6,  -5), S( -19,   0)},
   {S(   0,   0), S( -28, -52), S( -24, -10), S(  16, -14),
    S(   5,  -1), S(  -7, -13), S( -10, -12), S( -11,   6)},
   {S(   0,   0), S(  -2, -19), S( -20, -18), S( -13,  -2),
    S(  -1,  -7), S(   2, -23), S(  73, -12), S(  15,  20)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -35,   2), S( -48,  26), S( -73,  32),
    S(   7,  13), S(  83, -12), S( 159,  43), S(   0,   0)},
   {S(   0,   0), S( -28,  13), S( -47,  35), S( -66,  39),
    S(  -1,  37), S( 106,  25), S( 189,  89), S(   0,   0)}},
  {{S(   0,   0), S( -25,  23), S( -47,  33), S( -69,  46),
    S(   7,  42), S( 101,  38), S( 271, 100), S(   0,   0)},
   {S(   0,   0), S( -29,  22), S( -42,  31), S( -61,  48),
    S(  14,  58), S( 105, 109), S( 156, 260), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   1,   2), S(   4,  -2), S(   8, -11),
    S(   3, -16), S( -11, -13), S( -20,  -2), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   3,  -3), S(   5,  -2), S(   8,   9),
    S(   3,  25), S(  11,  33), S(  29,  30), S(   0,   0),
};

const int PassedSafePromotionPath = S( -17,  37);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S(   2, -28);
const int ThreatMinorAttackedByPawn  = S( -34, -50);
const int ThreatMinorAttackedByMinor = S( -23, -35);
const int ThreatMinorAttackedByMajor = S(  -9, -39);
const int ThreatRookAttackedByLesser = S( -38, -12);
const int ThreatMinorAttackedByKing  = S( -17, -16);
const int ThreatRookAttackedByKing   = S( -18, -15);
const int ThreatQueenAttackedByOne   = S( -25, -22);
const int ThreatOverloadedPieces     = S(  -7, -16);
const int ThreatByPawnPush           = S(  16,  23);

/* Space Evaluation Terms */

const int SpaceRestrictPiece = S(  -3,   0);
const int SpaceRestrictEmpty = S(  -5,  -1);
const int SpaceCenterControl = S(   0,   3);

/* Closedness Evaluation Terms */

const int ClosednessKnightAdjustment[9] = {
    S( -10,  -3), S(  -7,   9), S(  -8,  16), S(  -3,  16),
    S(  -1,  20), S(   3,  15), S(   6,   7), S(  -5,  25),
    S(  -5,  10),
};

const int ClosednessRookAdjustment[9] = {
    S(  64,   4), S(   6,  35), S(   6,  20), S(  -1,   2),
    S(  -8,   6), S(  -9, -12), S( -12, -12), S( -23, -16),
    S( -35, -21),
};
...
const int ComplexityPawnFlanks  = S(   0,  78);
const int ComplexityPawnEndgame = S(   0,  50);
