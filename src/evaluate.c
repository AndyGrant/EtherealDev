...
const int PawnValue   = S(  99, 127);
const int KnightValue = S( 451, 408);
const int BishopValue = S( 471, 425);
const int RookValue   = S( 651, 690);
const int QueenValue  = S(1302,1347);
...
const int PawnPSQT32[32] = {
    ...
    S( -14,   7), S(   0,   2), S( -10,   5), S(  -7,   0),
    S( -22,   6), S( -15,   5), S( -12,  -5), S(  -3, -12),
    S( -18,  13), S( -12,  11), S(   5, -10), S(   7, -24),
    S( -11,  16), S(  -7,   9), S( -12,  -6), S(  -9, -21),
    S(  -7,  33), S(  -6,  27), S(  -1,  17), S(  24, -10),
    S( -20, -60), S( -58, -27), S(   2, -41), S(  45, -56),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -48, -31), S(  -2, -30), S( -12, -29), S(   1, -16),
    S(   0, -20), S(   1, -13), S(   3, -30), S(   8, -17),
    S(  12, -29), S(  18, -22), S(  14, -17), S(  22,  -4),
    S(  21,   2), S(  24,   6), S(  32,  19), S(  31,  24),
    S(  25,  13), S(  31,   9), S(  44,  28), S(  33,  40),
    S( -17,  12), S(   2,  14), S(  29,  30), S(  30,  29),
    S(  13, -13), S(  -4,  -1), S(  32, -21), S(  39,   2),
    S(-178, -25), S( -85,  -4), S(-111,  18), S( -33,  -3),
};

const int BishopPSQT32[32] = {
    S(  10, -27), S(  13, -18), S(  -2, -15), S(   8, -12),
    S(  28, -36), S(  14, -37), S(  22, -23), S(  11, -12),
    S(  18, -13), S(  27, -15), S(   3, -16), S(  21,  -3),
    S(  11,  -9), S(  19,  -3), S(  18,   6), S(  25,   7),
    S( -10,  14), S(  19,   5), S(   5,  14), S(  16,  19),
    S(   2,   8), S(  -5,  17), S(  -2,   5), S(  12,  14),
    S( -44,  15), S( -50,   4), S(  -7,   8), S( -24,  12),
    S( -59,  -6), S( -45,   6), S( -85,  16), S( -90,  27),
};

const int RookPSQT32[32] = {
    S( -12, -31), S( -12, -22), S(  -3, -22), S(   2, -29),
    S( -53, -15), S( -10, -34), S( -10, -33), S(   0, -37),
    S( -29, -16), S(  -6, -13), S( -17, -17), S(  -1, -26),
    S( -19,  -2), S( -10,   8), S(  -7,   4), S(   5,  -4),
    S(  -4,  12), S(  12,   8), S(  23,   6), S(  41,   0),
    S( -15,  22), S(  30,   6), S(   8,  21), S(  39,   3),
    S(   2,   7), S( -10,  15), S(  13,   6), S(  28,   7),
    S(  34,  22), S(  28,  26), S(   0,  34), S(   8,  28),
};

const int QueenPSQT32[32] = {
    S(  31, -66), S(   5, -46), S(  12, -60), S(  20, -45),
    S(  13, -42), S(  26, -59), S(  28, -72), S(  17, -28),
    S(  12, -30), S(  25, -19), S(   6,   5), S(   5,   0),
    S(  13,  -7), S(  19,  13), S(   1,  25), S( -12,  66),
    S(  -3,  16), S(  -5,  48), S( -11,  24), S( -27,  77),
    S( -24,  32), S( -15,  21), S( -22,  26), S( -10,  28),
    S(  -1,  27), S( -62,  68), S(  -3,  13), S( -36,  54),
    S(  -9,  20), S(  21,  10), S(  10,  12), S(  -3,  23),
};

const int KingPSQT32[32] = {
    S(  49, -78), S(  42, -49), S( -18, -12), S( -28, -26),
    S(  26, -21), S( -18, -13), S( -46,  10), S( -67,  12),
    S(  -6, -23), S(   2, -22), S(  11,   0), S( -18,  18),
    S(   0, -29), S(  94, -33), S(  46,   8), S(  -3,  30),
    S(  10, -12), S( 109, -28), S(  57,  16), S(  13,  22),
    S(  50, -25), S( 135, -18), S( 106,   7), S(  44,   3),
    S(   7, -57), S(  48,  -7), S(  32,   6), S(   9,  -2),
...
const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -27,  -9), S( -10,   9), S( -19,  31),
    S(  -2,  58), S(  37,  63), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -15,  20), S(  -4,  23), S(   3,  48),
    S(  22,  89), S(  38,  62), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -6, -13);

const int PawnStacked[2] = { S(  -3, -20), S(  -6, -10) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   4,  -3), S(   9,  -7), S(  10, -14),
    S(   7, -13), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -14, -26), S(  -7, -24), S(   4, -25),
    S(  30, -28), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -6,  -8), S(  10,  -1), S(   0,   1), S(   8,  13),
    S(  13,   1), S(  24,  -4), S(  24,   4), S(  26,  12),
    S(   8,  -1), S(  24,   0), S(  10,   7), S(  17,  15),
    S(  10,  11), S(  21,  16), S(  28,  21), S(  31,  18),
    S(  55,  20), S(  42,  46), S(  63,  57), S(  76,  63),
...
const int KnightBehindPawn = S(   2,  22);

const int KnightInSiberia[4] = {
    S(  -9,  -3), S( -12, -11), S( -23,  -7), S( -27,  -6),
};

const int KnightMobility[9] = {
    S( -78,-106), S( -34,-104), S( -16, -44), S(  -4, -16),
    S(   8,  -7), S(  11,  10), S(  18,  13), S(  28,  14),
    S(  41,  -4),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  17,  67);

const int BishopRammedPawns = S(  -8, -15);
...
const int BishopMobility[14] = {
    S( -69,-149), S( -32,-100), S( -10, -57), S(  -2, -30),
    S(   9, -19), S(  15,  -3), S(  17,   7), S(  18,  12),
    S(  17,  19), S(  25,  19), S(  23,  21), S(  45,   8),
    S(  47,  20), S(  75, -17),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  10,   5), S(  31,   6) };

const int RookOnSeventh = S(  -2,  35);

const int RookMobility[15] = {
    S(-159,-116), S( -58,-115), S( -16, -66), S(  -8, -24),
    S(  -8,  -3), S( -12,  15), S( -11,  27), S(  -4,  31),
    S(   4,  35), S(  10,  38), S(  12,  46), S(  18,  49),
    S(  19,  53), S(  35,  40), S( 105,  -8),
};

/* Queen Evaluation Terms */

const int QueenRelativePin = S( -20, -12);

const int QueenMobility[28] = {
    S( -61,-263), S(-213,-388), S( -60,-200), S( -21,-191),
    S(  -6,-144), S(  -3, -80), S(   1, -44), S(  -1, -13),
    S(   5, -12), S(   6,  10), S(  12,  14), S(  14,  31),
    S(  18,  24), S(  20,  34), S(  18,  39), S(  16,  45),
    S(  17,  42), S(   8,  45), S(   9,  42), S(   8,  32),
    S(  18,   9), S(  31, -14), S(  31, -45), S(  30, -62),
...
const int KingDefenders[12] = {
    S( -28,  -3), S( -11,  -1), S(   1,   6), S(  11,   8),
    S(  19,   8), S(  28,   0), S(  30,  -4), S(  12,  -1),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  42,  32), S(  28,  23), S(   9,   8), S( -24, -18),
    S( -28, -50), S( -21, -63), S( -17, -67), S( -11, -64),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -9,   2), S(  14, -25), S(  22,  -9), S(  18,   6),
    S(  14,   3), S(  -1,  -1), S(  -3, -34), S( -54,  20)},
   {S(  16,  -5), S(  22, -17), S(   2,  -5), S( -15,   4),
    S( -30,  16), S( -73,  67), S(  91,  81), S( -21,  -1)},
   {S(  35,  -2), S(   9,  -7), S( -30,   7), S( -14,  -8),
    S( -17,  -5), S( -12,   1), S(   0,  68), S( -14,  -1)},
   {S(   2,  14), S(  27, -12), S(   8, -14), S(  16, -22),
    S(  27, -39), S( -59,   3), S(-135,  51), S(   5,  -7)},
   {S( -16,  14), S(   3,  -1), S( -32,   4), S( -21,   7),
    S( -21,  -7), S( -45,  -5), S(  32, -18), S(  -9,   0)},
   {S(  47, -15), S(  21, -12), S( -21,   3), S(  -9, -22),
    S(   6, -28), S(  20, -21), S(  40, -32), S( -24,   0)},
   {S(  33, -14), S(  -1, -18), S( -29,  -3), S( -23,  -9),
    S( -31,  -5), S( -41,  30), S(   1,  44), S( -11,   0)},
   {S(  -8, -13), S(   6, -20), S(   9,   3), S(  -3,  11),
    S( -13,  17), S( -10,  37), S(-190,  89), S( -20,  15)}},
  {{S(   0,   0), S( -13, -26), S(   7, -23), S( -39,  20),
    S( -23,   1), S(   3,  42), S(-167,  -7), S( -43,  11)},
   {S(   0,   0), S(  22, -21), S(   9,  -7), S( -16,   0),
    S(   3, -18), S(  26,  65), S(-184,  -3), S( -32,   6)},
   {S(   0,   0), S(  23, -11), S(  -1, -10), S(   5, -20),
    S(  21,  -3), S( -87,  48), S( -84, -74), S( -20,  -2)},
   {S(   0,   0), S( -10,  10), S(  -6,  -2), S( -25,   5),
    S( -27,   4), S(-100,  30), S(   7, -40), S( -23,  -4)},
   {S(   0,   0), S(  14,   2), S(  14,  -8), S(  15, -13),
    S(  17, -31), S( -57,  14), S(-101, -60), S(  -2,  -4)},
   {S(   0,   0), S(   0,  -6), S( -26,   1), S( -22,  -9),
    S(  23, -23), S( -37,   3), S(  55,  39), S( -18,  -5)},
   {S(   0,   0), S(  26, -15), S(  14, -14), S( -11,  -9),
    S( -29,  12), S(  -8,  19), S( -57, -50), S( -34,  13)},
   {S(   0,   0), S(  13, -43), S(  20, -34), S( -20,  -7),
    S( -20,  19), S(  -8,  21), S(-228, -56), S( -24,   3)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -13,  33), S( 112,  -7), S( -24,  22), S( -13,   1),
    S( -14,  -1), S(  -7,  -4), S( -14,   7), S( -26,   2)},
   {S(  -8,  53), S(  51,  14), S( -20,  19), S(  -4,   7),
    S(  -2,   4), S(   5,  -4), S(  -2,   1), S( -12,   7)},
   {S(   3,  43), S(   4,  28), S( -22,  17), S( -11,   6),
    S(   3,   2), S(   9,   0), S(  10, -11), S(   5,   6)},
   {S(  -7,  27), S(   4,  23), S( -23,   7), S( -14,  -2),
    S(  -9,   0), S(  10, -13), S(   2,  -9), S( -14,   8)}},
  {{S(   0,   0), S( -18, -16), S( -16,  -3), S(  24, -21),
    S(  11,  -6), S(   5, -20), S(   5,   2), S(  16,  29)},
   {S(   0,   0), S( -17, -37), S(   2, -13), S(  43, -14),
    S(   3,  -3), S(  17, -25), S(  -6,  -5), S( -19,   0)},
   {S(   0,   0), S( -28, -52), S( -25, -10), S(  16, -12),
    S(   8,  -2), S(  -8, -13), S(  -9, -12), S( -11,   6)},
   {S(   0,   0), S(  -2, -19), S( -20, -18), S( -12,  -3),
    S(  -1,  -9), S(   0, -24), S(  73, -12), S(  15,  20)}},
};

/* Safety Evaluation Terms */

const int SafetyKnightWeight    = S(  46,  41);
const int SafetyBishopWeight    = S(  22,  36);
const int SafetyRookWeight      = S(  34,   9);
const int SafetyQueenWeight     = S(  28,   6);

const int SafetyAttackValue     = S(  43,  34);
const int SafetyWeakSquares     = S(  40,  41);
const int SafetyNoEnemyQueens   = S(-237,-258);
const int SafetySafeQueenCheck  = S(  91,  83);
const int SafetySafeRookCheck   = S(  89,  99);
const int SafetySafeBishopCheck = S(  58,  59);
const int SafetySafeKnightCheck = S( 110, 117);
const int SafetyAdjustment      = S( -76, -26);

/* Passed Pawn Evaluation Terms */

const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -37,   1), S( -48,  26), S( -71,  31),
    S(   9,  11), S(  83, -15), S( 158,  42), S(   0,   0)},
   {S(   0,   0), S( -27,  14), S( -44,  39), S( -64,  41),
    S(   1,  39), S( 107,  25), S( 191,  89), S(   0,   0)}},
  {{S(   0,   0), S( -24,  26), S( -47,  34), S( -66,  48),
    S(  11,  44), S( 104,  38), S( 272,  99), S(   0,   0)},
   {S(   0,   0), S( -28,  23), S( -42,  33), S( -62,  52),
    S(  12,  63), S( 105, 119), S( 157, 268), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(  -1,   4), S(   4,  -3), S(   8, -11),
    S(   6, -15), S( -10, -11), S( -16,  -3), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   5,  -1), S(   5,   0), S(   9,  10),
    S(   1,  24), S(   7,  33), S(  27,  31), S(   0,   0),
};

const int PassedSafePromotionPath = S( -21,  44);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -13, -32);
const int ThreatMinorAttackedByPawn  = S( -54, -59);
const int ThreatMinorAttackedByMinor = S( -24, -39);
const int ThreatMinorAttackedByMajor = S( -29, -48);
const int ThreatRookAttackedByLesser = S( -49, -23);
const int ThreatMinorAttackedByKing  = S( -18, -19);
const int ThreatRookAttackedByKing   = S( -19, -18);
const int ThreatQueenAttackedByOne   = S( -45, -23);
const int ThreatOverloadedPieces     = S(  -6, -12);
const int ThreatByPawnPush           = S(  15,  27);

/* Space Evaluation Terms */

const int SpaceRestrictPiece = S(  -4,  -2);
const int SpaceRestrictEmpty = S(  -5,  -3);
const int SpaceCenterControl = S(   3,  -2);

/* Closedness Evaluation Terms */

const int ClosednessKnightAdjustment[9] = {
    S( -10,  -8), S(  -6,   8), S(  -8,  15), S(  -4,  14),
    S(  -2,  19), S(   0,  13), S(   4,   4), S(  -9,  23),
    S(  -8,   9),
};

const int ClosednessRookAdjustment[9] = {
    S(  64,  -8), S(   3,  33), S(   2,  18), S(  -3,   2),
    S( -13,   5), S( -11, -12), S( -12, -12), S( -23, -16),
    S( -35, -21),
};
...
const int ComplexityPawnFlanks  = S(   0,  74);
const int ComplexityPawnEndgame = S(   0,  66);
const int ComplexityAdjustment  = S(   0,-145);
