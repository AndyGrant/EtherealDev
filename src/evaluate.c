...
const int PawnValue   = S(  97, 139);
const int KnightValue = S( 448, 486);
const int BishopValue = S( 466, 520);
const int RookValue   = S( 639, 803);
const int QueenValue  = S(1317,1652);
...
const int PawnPSQT[SQUARE_NB] = {
    ...
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S( -18,   6), S(  -3,   5), S(  -2,   9), S(  -6,   4),
    S(  -5,   9), S(   3,   9), S(  -5,  10), S( -22,   8),
    S( -16,   0), S( -20,   7), S(   0,  -7), S(   1, -18),
    S(   1, -11), S(  -8,  -2), S( -13,   7), S( -26,   8),
    S( -15,  10), S( -20,  15), S(   6,  -9), S(  15, -25),
    S(  14, -22), S(   6,  -9), S( -17,  20), S( -16,  12),
    S( -14,  16), S( -11,   8), S( -10,  -7), S(  12, -25),
    S(   9, -18), S(  -4,  -7), S( -10,  11), S( -18,  18),
    S( -18,  42), S( -14,  36), S( -10,  27), S(  37,  -9),
    S(  46,  -8), S( -10,  29), S(  -1,  34), S( -13,  39),
    S( -26, -64), S( -79, -11), S( -13, -26), S(  68, -25),
    S(  40, -24), S(  35, -19), S( -59,  -1), S( -53, -48),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT[SQUARE_NB] = {
    S( -26, -32), S( -22, -22), S( -18, -17), S( -12,  -7),
    S(  -4,  -4), S( -28, -19), S( -21, -20), S( -33, -23),
    S(  -1,  -8), S(  -6,   5), S(  -1, -20), S(  -6,  -5),
    S(  -1,  -5), S(  -5, -15), S(  -1,   0), S(  -6,  -4),
    S(   4, -20), S(   8,  -3), S(   5,   1), S(  11,  16),
    S(  13,  14), S(   3,   4), S(  20,  -3), S(   4, -14),
    S(  19,  18), S(  11,  28), S(  17,  35), S(  13,  44),
    S(  12,  50), S(  29,  32), S(  21,  28), S(  30,  23),
    S(  25,  29), S(  25,  30), S(  21,  45), S(  35,  58),
    S(  31,  57), S(  35,  46), S(  35,  36), S(  39,  30),
    S( -11,  31), S(  16,  42), S(  23,  50), S(  36,  61),
    S(  43,  54), S(  35,  52), S(  35,  40), S(  -2,  32),
    S(  29,   6), S(  -4,  24), S(  39,  11), S(  47,  33),
    S(  48,  30), S(  60,   2), S(  -3,  23), S(  25,   8),
    S(-174,  -4), S( -85,  24), S(-110,  51), S( -33,  27),
    S( -10,  28), S(-105,  51), S(-113,  32), S(-141,  -5),
};

const int BishopPSQT[SQUARE_NB] = {
    S(   4, -20), S(  23,   8), S(  -4,  -7), S(  -2,   6),
    S(   2,   4), S( -22,   6), S(  10,   6), S(  -8, -24),
    S(  40,  -7), S(   6, -26), S(  20,   2), S(   5,   4),
    S(   6,   5), S(  14,   0), S(  12, -28), S(  29, -18),
    S(  16,   6), S(  37,  11), S(   2,  -8), S(  19,  22),
    S(  19,  22), S(  -2, -11), S(  30,   6), S(  20,  11),
    S(   8,  12), S(   1,  18), S(  22,  30), S(  14,  33),
    S(  20,  33), S(  20,  27), S(   5,  19), S(  12,  12),
    S( -19,  26), S(  12,  30), S(   0,  36), S(  15,  45),
    S(   8,  45), S(   6,  35), S(  15,  33), S(  -2,  29),
    S( -24,  30), S(  -3,  41), S( -18,  19), S(  19,  43),
    S(  21,  40), S( -12,  33), S(  10,  46), S(  -6,  34),
    S( -32,  37), S( -48,  15), S( -10,  31), S( -27,  42),
    S( -25,  41), S(  -5,  34), S( -69,  17), S( -32,  38),
    S( -65,  19), S( -60,  45), S(-117,  57), S(-103,  60),
    S(-104,  59), S(-100,  48), S( -31,  29), S( -64,  17),
};

const int RookPSQT[SQUARE_NB] = {
    S( -29,   1), S( -21,   4), S( -15,   5), S(   0,  -3),
    S(   8,  -9), S(  -8,   1), S( -20,   0), S( -22, -15),
    S( -54,   2), S( -30,  -5), S( -15,  -2), S(  -7,  -5),
    S(  -5,  -8), S(  -7, -13), S( -24, -15), S( -73,  -2),
    S( -45,  10), S( -21,  13), S( -25,  14), S( -14,   7),
    S( -10,   4), S( -21,  12), S(  -7,   4), S( -37,  -1),
    S( -40,  32), S( -28,  35), S( -18,  37), S( -13,  27),
    S( -11,  27), S( -11,  33), S( -17,  29), S( -33,  24),
    S( -29,  45), S(  -2,  42), S(   8,  40), S(  18,  29),
    S(  24,  30), S(  11,  36), S(  17,  34), S( -14,  37),
    S( -26,  58), S(   9,  39), S(  -2,  51), S(  36,  37),
    S(  35,  34), S(   9,  48), S(  29,  29), S( -17,  54),
    S( -18,  41), S( -31,  47), S(   2,  40), S(  23,  39),
    S(  20,  38), S(  -2,  33), S( -33,  45), S(  -1,  34),
    S(  30,  54), S(  24,  62), S(   2,  71), S(  14,  65),
    S(  13,  64), S(   3,  67), S(  36,  57), S(  41,  55),
};

const int QueenPSQT[SQUARE_NB] = {
    S(  11, -38), S(   1, -27), S(  -6, -38), S(  -1, -23),
    S(   3, -26), S(  -5, -50), S(   3, -30), S(  25, -42),
    S(   8, -14), S(   5, -23), S(  19, -36), S(  12,  -8),
    S(  13,  -9), S(  22, -46), S(  19, -36), S(  20, -13),
    S(  10,  -1), S(  17,   2), S(   0,  36), S(   3,  29),
    S(   4,  36), S(  11,  31), S(  28,  12), S(  30,  -8),
    S(  13,  12), S(  -7,  42), S(  -7,  62), S( -29, 105),
    S( -17,  97), S(   2,  64), S(   8,  51), S(  20,  40),
    S(  -3,  40), S(  -9,  74), S( -17,  73), S( -39, 123),
    S( -30, 129), S( -12,  86), S(  -2,  97), S(  21,  66),
    S( -20,  57), S( -10,  56), S( -26,  75), S(  -8,  78),
    S(  -6,  79), S(  -5,  76), S(   5,  75), S(   1,  87),
    S( -17,  61), S( -55, 109), S( -22,  70), S( -41, 119),
    S( -49, 123), S(  -3,  61), S( -49, 125), S(  15,  82),
    S(  -1,  43), S(  19,  50), S(   1,  79), S(   4,  78),
    S(   0,  88), S(  15,  66), S(  18,  80), S(  25,  59),
};

const int KingPSQT[SQUARE_NB] = {
    S(  81, -74), S(  65, -45), S(  16,  -7), S( -29, -11),
    S(  -6, -24), S( -18,  -3), S(  59, -47), S(  82, -79),
    S(  33,  -3), S( -24,  -6), S( -54,  16), S( -65,  25),
    S( -65,  28), S( -46,  24), S( -27,  -8), S(  26, -11),
    S( -39, -13), S( -16, -18), S(  14,   7), S(  -5,  26),
    S(  -2,  31), S(  15,   8), S( -15, -16), S( -32,  -8),
    S( -44, -35), S(  95, -36), S(  63,   9), S( -25,  37),
    S(   3,  36), S(  73,   5), S( 103, -32), S( -53, -23),
    S(  -7, -16), S(  96, -36), S(  39,  13), S( -44,  23),
    S( -25,  25), S(  35,  12), S(  80, -33), S( -50, -13),
    S(  24, -42), S( 123, -27), S( 106,  -2), S( -18, -16),
    S( -35, -12), S(  76, -16), S( 101, -25), S( -18, -51),
    S(   1, -98), S(  28, -23), S(   8, -18), S(  -2, -39),
    S( -55, -36), S(  14, -24), S(  22, -25), S( -14,-117),
    S( -16,-153), S(  49, -96), S( -21, -74), S( -19, -34),
    S( -51, -56), S( -42, -63), S(  53, -94), S( -58,-133),
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S(  -6, -12), S(  -9,  19), S( -12,  25),
    S( -27,  48), S(   7,  47), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S(  -4,  21), S(  -5,  25), S(   1,  42),
    S(   9,  87), S(  49,  76), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated[FILE_NB] = {
    S( -13, -10), S(   0, -14), S(  -2, -16), S(  -5, -17),
    S(  -3, -18), S(   0, -15), S(  -6, -13), S(  -4, -15),
};

const int PawnStacked[2][FILE_NB] = {
   {S(   4, -33), S(   3, -24), S(  -2, -19), S(  -5, -13),
    S(  -5, -11), S(   8, -24), S(  14, -28), S(   3, -36)},
   {S(   0, -18), S(   1, -17), S(  -9,  -8), S( -14,   2),
    S( -10,   2), S(  -4,  -9), S(   5, -15), S(   0, -21)},
};

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(  -7,  -3), S(   8,  -9), S(   3, -13),
    S(   7, -23), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -15, -20), S(   1, -20), S(  11, -22),
    S(  33, -33), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -2,  -5), S(  10,   0), S(  -1,  -3), S(   8,  10),
    S(   8,   0), S(  20,  -3), S(  17,   5), S(  15,  10),
    S(   7,   1), S(  22,   2), S(  10,   7), S(   9,  12),
    S(  12,  13), S(  20,  16), S(  36,  21), S(  25,  18),
    S(  47,  38), S(  40,  59), S(  64,  71), S(  62,  75),
    S( 109,  37), S( 214,  47), S( 215,  70), S( 232,  60),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightInSiberia[4] = {
    S(  -9,  -1), S( -10, -11), S( -18, -22), S( -45, -20),
};

const int KnightMobility[9] = {
    S( -85,-133), S( -35, -76), S( -18, -19), S(  -5,   4),
    S(   7,  15), S(  16,  25), S(  25,  30), S(  34,  32),
    S(  44,  29),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  35,  79);

const int BishopRammedPawns = S(  -6, -13);
...
const int BishopBehindPawn = S(   8,  14);

const int BishopLongDiagonal = S(  26,  25);

const int BishopMobility[14] = {
    S( -66,-169), S( -29, -83), S( -13, -30), S(   2,  -8),
    S(   9,   8), S(  16,  19), S(  21,  27), S(  24,  32),
    S(  26,  36), S(  28,  39), S(  28,  40), S(  37,  33),
    S(  45,  38), S(  78,   9),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  24,  10), S(  38,   7) };

const int RookOnSeventh = S(   0,  37);

const int RookMobility[15] = {
    S(-138,-150), S( -56,-121), S( -22, -41), S( -15, -12),
    S(  -7,   8), S(  -5,  25), S(  -5,  35), S(  -1,  40),
    S(   4,  44), S(   8,  49), S(  12,  54), S(  16,  58),
    S(  18,  61), S(  23,  57), S(  64,  21),
};

/* Queen Evaluation Terms */

const int QueenRelativePin = S( -13, -10);

const int QueenMobility[28] = {
    S(-111,-273), S(-251,-401), S(-113,-226), S( -18,-230),
    S(  -4,-158), S(   1, -70), S(   5, -25), S(   8,   1),
    S(   9,  14), S(  10,  29), S(  11,  36), S(  13,  45),
    S(  15,  47), S(  16,  53), S(  17,  56), S(  17,  60),
    S(  16,  62), S(  14,  65), S(  13,  64), S(  15,  53),
    S(  22,  34), S(  33,  11), S(  35,  -7), S(  31, -26),
    S(  13, -41), S(   9, -77), S( -42, -30), S( -23, -50),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -23,  -1), S( -13,   0), S(  -1,   4), S(   8,   7),
    S(  18,   9), S(  29,   5), S(  39, -12), S(  12,  -5),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  41,  43), S(  18,  34), S(  10,  21), S(  -6, -21),
    S(  -6, -67), S(  -5, -82), S( -16, -84), S( -12, -75),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -4,  -7), S(  12, -22), S(  25,  -5), S(  20,   4),
    S(   3,   3), S( -11,   5), S( -16, -33), S( -49,  20)},
   {S(   7,  -5), S(  10, -13), S(   0,  -4), S(   1,  -6),
    S( -14,   3), S( -53,  67), S(  80,  81), S( -30,   3)},
   {S(  31,  -8), S(   3,  -4), S( -29,  10), S( -17,  -8),
    S(  -9,  -7), S( -25,  12), S(  10,  71), S( -19,  -3)},
   {S(  15,   4), S(  23, -13), S(  -3, -11), S(  15, -16),
    S(  20, -29), S( -25,  12), S(-136,  50), S(  -5, -12)},
   {S( -11,  11), S(  -5,  -4), S( -25,   2), S( -20,   4),
    S( -25,  -2), S( -37,  -4), S(  39, -25), S( -26,   1)},
   {S(  38, -12), S(  14, -14), S( -16,   2), S(  -7, -11),
    S(   2, -25), S(  29, -20), S(  47, -37), S( -21,  -4)},
   {S(  34, -16), S(   9, -22), S( -11,  -6), S( -17,  -6),
    S( -32,   0), S( -20,  26), S(   2,  47), S( -29,   1)},
   {S(   8, -14), S(   5, -14), S(  12,  -1), S(   5,  12),
    S( -14,  21), S( -14,  36), S(-183,  80), S( -25,  12)}},
  {{S(   0,   0), S(  -9, -31), S(   4, -29), S( -45,  13),
    S( -35,   6), S(  -9,  49), S(-168,  -3), S( -69,  16)},
   {S(   0,   0), S(  17, -11), S(  11, -10), S(  -7,  -6),
    S(  -7, -25), S(  23,  70), S(-185,   4), S( -37,   5)},
   {S(   0,   0), S(  22,  -9), S(  -7,  -7), S(  10, -23),
    S(  26, -10), S( -89,  55), S( -87, -73), S(  -6,  -5)},
   {S(   0,   0), S(  -4,   3), S(  -6,  -4), S( -27,   5),
    S( -45,  10), S( -98,  32), S(  -7, -45), S( -41,  -1)},
   {S(   0,   0), S(  11,  -3), S(  14, -11), S(  20, -10),
    S(  -1, -11), S( -30,   6), S(-101, -52), S( -19,  -4)},
   {S(   0,   0), S(   7,  -7), S( -11,  -7), S( -20,  -8),
    S(  11, -27), S( -28,   9), S(  51,  38), S( -32,  -3)},
   {S(   0,   0), S(  34, -17), S(  16, -10), S(  -6,  -4),
    S( -29,   8), S( -10,  19), S( -49, -38), S( -31,   8)},
   {S(   0,   0), S(  23, -47), S(  16, -27), S( -13, -10),
    S( -30,  19), S( -14,  22), S(-224, -48), S( -42,   1)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S(  -7,  33), S( 130,  -5), S( -10,  18), S(  -8,   6),
    S(  -6,   2), S(  -7,  -2), S( -16,   8), S( -23,   1)},
   {S( -17,  55), S(  79,  23), S(  -5,  24), S(   8,  11),
    S(   3,   8), S(   5,  -1), S(  -1,   0), S( -13,   8)},
   {S(   3,  47), S(  22,  31), S( -11,  17), S(  -5,   7),
    S(  -3,   6), S(   5,   1), S(   3,  -2), S(  11,   4)},
   {S(   0,  30), S(  12,  19), S( -34,  13), S( -20,   1),
    S( -15,  -1), S(   4,  -5), S(   7,  -3), S( -17,   6)}},
  {{S(   0,   0), S( -18, -16), S( -26,   0), S(  16, -18),
    S(   9, -11), S(   8, -27), S( -10,   1), S(  11,  30)},
   {S(   0,   0), S( -15, -42), S(  -5, -17), S(  39, -20),
    S(  13,  -6), S(  18, -28), S( -10, -19), S( -35,   4)},
   {S(   0,   0), S( -34, -62), S( -12, -16), S(  10, -10),
    S(  -1,   0), S(   7, -14), S(  -2, -19), S(  -5,   2)},
   {S(   0,   0), S(  -1, -26), S( -37, -15), S( -18,  -5),
    S( -12,  -7), S(   6, -23), S(  79, -28), S(  13,  25)}},
};

/* Safety Evaluation Terms */

const int SafetyKnightWeight    = S(  56,  47);
const int SafetyBishopWeight    = S(  36,  41);
const int SafetyRookWeight      = S(  30,  16);
const int SafetyQueenWeight     = S(  17,  11);

const int SafetyAttackValue     = S(  29,  60);
const int SafetyWeakSquares     = S(  25,  35);
const int SafetyNoEnemyQueens   = S(-222,-262);
const int SafetySafeQueenCheck  = S(  48,  87);
const int SafetySafeRookCheck   = S(  55, 100);
const int SafetySafeBishopCheck = S(  48,  64);
const int SafetySafeKnightCheck = S(  67, 114);
const int SafetyAdjustment      = S( -68, -19);
const int SafetyShelter[2][RANK_NB] = {
   {S( -11,   5), S( -11,  16), S(   3,  14), S(   6,  15),
    S(  12,   4), S(   3,   0), S(   3,   0), S(  11, -19)},
   {S(   0,   0), S(  -1,  11), S(   0,  15), S(   1,   9),
    S(   1,   3), S(  -2,   0), S(  -2,   0), S(   7, -11)},
};

const int SafetyStorm[2][RANK_NB] = {
   {S(  -1,  -1), S(  -3,   4), S(  16,   7), S(   7,   1),
    S(  -2,   8), S( -14,  23), S(  -9,  22), S(  16, -17)},
   {S(   0,   0), S(   1,   0), S(   2,   6), S(  -2,   2),
    S(  -2,   9), S(  -9,   2), S(   3,   0), S(   2,   0)},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -28,   7), S( -44,  33), S( -49,  40),
    S(   9,  34), S( 106,  22), S( 176,  67), S(   0,   0)},
   {S(   0,   0), S( -20,  17), S( -40,  38), S( -50,  44),
    S(   3,  48), S( 112,  54), S( 198,  97), S(   0,   0)}},
  {{S(   0,   0), S( -32,  29), S( -39,  41), S( -49,  57),
    S(   2,  60), S(  96,  70), S( 236, 127), S(   0,   0)},
   {S(   0,   0), S( -21,  22), S( -33,  34), S( -49,  58),
    S(   3,  72), S(  85, 125), S( 116, 254), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(  -4,  -4), S(   0,  -6), S(   5, -12),
    S(   6, -15), S(  -7, -15), S( -15,  -8), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   6,   0), S(   6,   0), S(   8,   7),
    S(   1,  18), S(   2,  27), S(  15,  28), S(   0,   0),
};
...
const int ThreatWeakPawn             = S(  -3, -23);
const int ThreatMinorAttackedByPawn  = S( -36, -54);
const int ThreatMinorAttackedByMinor = S( -19, -23);
const int ThreatMinorAttackedByMajor = S(  -4, -29);
const int ThreatRookAttackedByLesser = S( -19, -11);
const int ThreatMinorAttackedByKing  = S( -49, -21);
const int ThreatRookAttackedByKing   = S( -38, -22);
const int ThreatQueenAttackedByOne   = S(  -8,  -4);
const int ThreatOverloadedPieces     = S(  -3, -11);
const int ThreatByPawnPush           = S(  11,  19);

/* Space Evaluation Terms */

const int SpaceRestrictPiece = S(  -2,   1);
const int SpaceRestrictEmpty = S(  -3,   0);
const int SpaceCenterControl = S(   2,   2);

/* Closedness Evaluation Terms */

const int ClosednessKnightAdjustment[9] = {
    S( -14,  19), S(  -8,  31), S(  -1,  36), S(   5,  37),
    S(  10,  42), S(  13,  41), S(  15,  39), S(   9,  51),
    S(  10,  38),
};

const int ClosednessRookAdjustment[9] = {
    S(  27,  52), S( -12,  71), S(   3,  53), S(   2,  42),
    S(  13,  37), S(  16,  23), S(  11,  13), S(  -5,  13),
    S( -28, -12),
};
...
const int ComplexityPawnFlanks  = S(   0, 112);
const int ComplexityPawnEndgame = S(   0,  54);
const int ComplexityAdjustment  = S(   0,-181);
