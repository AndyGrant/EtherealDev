...
const int PawnValue   = S( 107, 136);
const int KnightValue = S( 457, 408);
const int BishopValue = S( 478, 427);
const int RookValue   = S( 663, 691);
const int QueenValue  = S(1301,1348);
...
const int PawnPSQT32[32] = {
    ...
    S( -18,   7), S(   2,   3), S(  -9,   7), S(  -4,  -2),
    S( -26,   5), S( -15,   6), S( -11,  -4), S(  -2, -11),
    S( -19,  12), S( -13,  12), S(   5,  -9), S(   7, -21),
    S( -12,  14), S(  -6,   9), S( -10,  -5), S(  -5, -20),
    S(  -8,  29), S(  -4,  25), S(   1,  14), S(  28, -13),
    S( -21, -61), S( -60, -29), S(   2, -43), S(  45, -58),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -48, -31), S(  -5, -28), S( -12, -29), S(   2, -15),
    S(   0, -20), S(   3, -11), S(   4, -28), S(  10, -17),
    S(  11, -26), S(  20, -22), S(  15, -16), S(  24,  -4),
    S(  19,   1), S(  25,   6), S(  33,  17), S(  35,  24),
    S(  26,  14), S(  32,   9), S(  42,  28), S(  36,  39),
...
const int BishopPSQT32[32] = {
    S(   8, -26), S(  12, -18), S(  -5,  -9), S(  11, -11),
    S(  30, -35), S(  13, -35), S(  21, -20), S(  11, -11),
    S(  16, -13), S(  29, -15), S(   4, -17), S(  23,  -5),
    S(  15,  -7), S(  20,  -3), S(  18,   5), S(  26,   6),
    S( -10,  14), S(  21,   5), S(   5,  14), S(  16,  18),
...
const int RookPSQT32[32] = {
    S( -13, -28), S( -14, -19), S(  -1, -21), S(   5, -28),
    S( -53, -14), S( -10, -33), S(  -8, -31), S(   4, -36),
    S( -29, -14), S(  -5, -11), S( -15, -16), S(   2, -25),
    S( -17,  -2), S( -11,   7), S(  -6,   2), S(   8,  -4),
    S(  -4,  10), S(  10,   8), S(  22,   5), S(  41,   0),
    S( -15,  21), S(  29,   6), S(   6,  20), S(  38,   3),
    S(   1,   7), S( -12,  14), S(  12,   5), S(  28,   7),
    S(  35,  22), S(  27,  23), S(   1,  32), S(   9,  28),
};

const int QueenPSQT32[32] = {
    S(  31, -66), S(   6, -45), S(  15, -59), S(  23, -43),
    S(  14, -42), S(  28, -58), S(  30, -71), S(  18, -26),
    S(  11, -30), S(  25, -20), S(   7,   7), S(   7,   1),
...
const int KingPSQT32[32] = {
    S(  46, -77), S(  40, -50), S( -18, -11), S( -31, -21),
    S(  27, -21), S( -15, -12), S( -45,  12), S( -64,  14),
    S(  -4, -20), S(   6, -18), S(  13,   3), S( -17,  19),
    S(   1, -30), S(  95, -33), S(  46,   8), S(  -4,  28),
...
const int PawnCandidatePasser[2][RANK_NB] = {
   ...
   {S(   0,   0), S( -14,  19), S(  -4,  21), S(   4,  47),
    S(  19,  87), S(  38,  62), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -5, -15);

const int PawnStacked[2] = { S(  -6, -18), S(  -6,  -8) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   5,  -6), S(  10,  -6), S(   8, -12),
    S(   7, -12), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -11, -27), S(  -6, -23), S(   3, -26),
    S(  28, -30), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -4,  -7), S(  10,   1), S(   1,   2), S(   8,  12),
    S(  14,   2), S(  26,  -2), S(  22,   5), S(  25,  13),
    S(   8,   0), S(  25,   1), S(   9,   8), S(  17,  16),
    S(  10,  10), S(  21,  14), S(  25,  21), S(  30,  19),
...
const int KnightInSiberia[4] = {
    S( -10,  -1), S( -12,  -6), S( -24,  -3), S( -25,  -6),
};

const int KnightMobility[9] = {
    S( -78,-106), S( -32,-102), S( -16, -42), S(  -4, -16),
    S(   9,  -7), S(  12,  10), S(  20,  13), S(  31,  13),
    S(  44,  -4),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  21,  69);
...
const int BishopLongDiagonal = S(  22,  11);

const int BishopMobility[14] = {
    S( -69,-149), S( -34, -99), S( -11, -54), S(  -3, -29),
    S(  10, -18), S(  17,  -3), S(  20,   7), S(  21,  12),
    S(  20,  18), S(  25,  21), S(  24,  20), S(  46,   8),
    S(  47,  19), S(  75, -17),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  11,   4), S(  36,   7) };

const int RookOnSeventh = S(  -3,  32);

const int RookMobility[15] = {
    S(-159,-116), S( -58,-115), S( -16, -65), S(  -8, -21),
    S( -10,   0), S( -11,  14), S( -10,  26), S(  -2,  29),
    S(   6,  34), S(   9,  38), S(  13,  45), S(  20,  49),
    S(  22,  53), S(  37,  40), S( 106,  -8),
};

/* Queen Evaluation Terms */

const int QueenRelativePin = S( -21, -10);

const int QueenMobility[28] = {
    S( -61,-263), S(-213,-388), S( -60,-200), S( -22,-191),
    S(  -7,-144), S(  -4, -80), S(   1, -44), S(   0, -12),
    S(   6, -11), S(   7,  11), S(  14,  14), S(  15,  32),
    S(  18,  23), S(  20,  34), S(  17,  38), S(  14,  44),
...
const int KingDefenders[12] = {
    S( -27,  -2), S(  -6,  -1), S(   2,   6), S(   9,   7),
    S(  18,   5), S(  28,   1), S(  29,  -4), S(  12,  -1),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  41,  27), S(  29,  19), S(  10,   8), S( -24, -14),
...
const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -9,   2), S(  14, -26), S(  26,  -9), S(  17,   6),
   ...
   {S(   2,  12), S(  26, -13), S(   4, -12), S(  15, -21),
    S(  30, -38), S( -58,   3), S(-135,  51), S(   6,  -6)},
   {S( -18,  12), S(   4,  -1), S( -31,   4), S( -20,   8),
    S( -21,  -6), S( -45,  -5), S(  32, -18), S(  -8,   2)},
   {S(  48, -17), S(  25, -13), S( -22,   4), S( -11, -19),
    S(   7, -25), S(  19, -21), S(  40, -32), S( -24,   1)},
   {S(  30, -16), S(  -1, -16), S( -27,  -1), S( -22,  -7),
    S( -31,  -5), S( -41,  30), S(   1,  44), S(  -9,  -1)},
   {S(  -9, -13), S(   5, -19), S(   9,   3), S(  -1,  12),
  ...
  {{S(   0,   0), S( -13, -26), S(   7, -23), S( -39,  20),
   ...
   {S(   0,   0), S(  15,   2), S(  13,  -6), S(  15, -13),
    S(  17, -31), S( -57,  14), S(-101, -60), S(  -2,  -3)},
   {S(   0,   0), S(   1,  -6), S( -26,   2), S( -23,  -9),
    S(  23, -23), S( -37,   3), S(  55,  39), S( -17,  -4)},
   {S(   0,   0), S(  25, -16), S(  12, -14), S( -12,  -7),
    S( -28,  14), S(  -8,  19), S( -57, -50), S( -32,  12)},
   {S(   0,   0), S(  12, -42), S(  21, -32), S( -19,  -7),
    S( -20,  19), S(  -8,  21), S(-228, -56), S( -25,   2)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -13,  32), S( 112,  -6), S( -23,  24), S( -14,   2),
    S( -12,   0), S(  -6,  -6), S( -14,   3), S( -25,   1)},
   {S(  -8,  53), S(  51,  14), S( -20,  21), S(  -3,   8),
    S(  -2,   3), S(   7,  -5), S(  -3,   2), S( -11,   5)},
   {S(   3,  43), S(   4,  28), S( -22,  19), S( -10,   6),
    S(   4,   1), S(   7,   0), S(  11,  -7), S(   6,   6)},
   {S(  -7,  28), S(   4,  25), S( -22,   8), S( -15,  -1),
    S( -11,   0), S(  11, -12), S(   1,  -9), S( -13,   7)}},
...
const int PassedPawn[2][2][RANK_NB] = {
  ...
  {{S(   0,   0), S( -24,  26), S( -48,  33), S( -68,  44),
    S(   9,  41), S( 102,  34), S( 271,  97), S(   0,   0)},
   {S(   0,   0), S( -29,  23), S( -43,  32), S( -62,  51),
    S(  11,  59), S( 104, 117), S( 157, 265), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   1,   3), S(   4,  -3), S(   5, -10),
    S(   3, -12), S( -11,  -7), S( -16,   2), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   1,   1), S(   3,   3), S(   8,  13),
    S(   1,  26), S(   6,  32), S(  28,  27), S(   0,   0),
};

const int PassedSafePromotionPath = S( -18,  42);
...
const int ThreatQueenAttackedByOne   = S( -50, -26);
...
const int ComplexityTotalPawns  = S(   0,  13);
const int ComplexityPawnFlanks  = S(   0,  80);
const int ComplexityPawnEndgame = S(   0,  52);
const int ComplexityAdjustment  = S(   0,-123);
