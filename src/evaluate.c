...
const int PawnValue   = S( 104, 119);
const int KnightValue = S( 456, 408);
const int BishopValue = S( 478, 422);
const int RookValue   = S( 659, 683);
const int QueenValue  = S(1288,1366);
...
const int PawnPSQT32[32] = {
    ...
    S( -20,   5), S(   3,   0), S(  -9,   2), S(  -5,  -3),
    S( -25,   2), S( -14,   3), S( -11,  -6), S(  -3, -15),
    S( -18,   9), S( -13,   9), S(   7, -12), S(   6, -24),
    S( -10,   9), S(  -6,   6), S(  -9,  -8), S(  -3, -21),
    S(  -7,  24), S(  -4,  20), S(   1,  11), S(  28, -17),
    S( -19, -49), S( -67, -17), S(   0, -31), S(  38, -45),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -49, -26), S(  -6, -31), S( -13, -28), S(   2, -16),
    S(   0, -21), S(   3, -12), S(   4, -28), S(  11, -18),
    S(  11, -26), S(  21, -24), S(  15, -18), S(  25,  -6),
    S(  19,   4), S(  23,   6), S(  31,  15), S(  33,  22),
    S(  25,  16), S(  30,   9), S(  41,  25), S(  33,  37),
    S( -15,  14), S(   4,  11), S(  32,  26), S(  31,  27),
    S(   8, -11), S(  -7,   0), S(  30, -23), S(  42,  -2),
    S(-171, -19), S( -81,  -3), S(-111,  18), S( -31,  -2),
};

const int BishopPSQT32[32] = {
    S(   7, -25), S(  12, -18), S(  -8,  -7), S(  12, -11),
    S(  30, -34), S(  10, -35), S(  21, -20), S(  14, -11),
    S(  16, -12), S(  29, -15), S(   5, -17), S(  24,  -6),
    S(  14,  -8), S(  21,  -3), S(  18,   4), S(  26,   6),
    S( -11,  13), S(  22,   5), S(   6,  13), S(  15,  17),
    S(   2,   7), S(  -2,  16), S(   1,   6), S(  18,  13),
    S( -45,  16), S( -53,   4), S(  -8,   7), S( -20,  11),
    S( -51,  -7), S( -47,   8), S( -87,  14), S( -90,  26),
};

const int RookPSQT32[32] = {
    S( -12, -27), S( -15, -18), S(  -2, -22), S(   6, -28),
    S( -52, -13), S( -10, -32), S(  -8, -31), S(   4, -35),
    S( -28, -13), S(  -5, -10), S( -15, -15), S(   1, -24),
    S( -17,  -2), S(  -9,   6), S(  -6,   2), S(   8,  -3),
    S(  -3,   8), S(  14,   5), S(  23,   2), S(  41,  -2),
    S( -14,  18), S(  27,   5), S(   4,  18), S(  37,   1),
    S(   0,   5), S( -16,  14), S(   9,   4), S(  25,   7),
    S(  36,  22), S(  26,  23), S(   7,  29), S(  18,  25),
};

const int QueenPSQT32[32] = {
    S(  24, -54), S(   4, -41), S(  13, -53), S(  20, -43),
    S(  13, -40), S(  27, -57), S(  29, -73), S(  18, -27),
    S(  10, -24), S(  25, -20), S(   8,   5), S(   8,  -1),
    S(  11,  -5), S(  16,  14), S(  -1,  20), S( -13,  62),
    S(  -6,  17), S(  -6,  42), S( -13,  21), S( -28,  71),
    S( -24,  28), S( -16,  18), S( -23,  22), S( -12,  24),
    S(  -4,  26), S( -64,  63), S(  -9,  14), S( -42,  51),
    S(  -9,  14), S(  18,   8), S(   8,   7), S(  -7,  18),
};

const int KingPSQT32[32] = {
    S(  48, -80), S(  44, -51), S( -12, -10), S( -25, -21),
    S(  30, -24), S(  -6, -14), S( -38,  11), S( -55,  13),
    S(   6, -26), S(  15, -22), S(  18,   1), S( -10,  17),
    S(   2, -34), S(  86, -34), S(  43,   6), S(  -5,  26),
    S(   5, -19), S(  97, -32), S(  47,  10), S(   3,  19),
    S(  45, -28), S( 121, -24), S(  94,   1), S(  38,  -1),
    S(   5, -49), S(  46, -10), S(  31,   4), S(   8,  -4),
    S(   8,-102), S(  74, -54), S( -20, -15), S( -19, -13),
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -27, -11), S( -12,   9), S( -15,  30),
    S(   1,  62), S(  42,  67), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -14,  19), S(  -5,  21), S(   4,  46),
    S(  21,  85), S(  32,  55), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -5, -13);

const int PawnStacked[2] = { S(  -6, -16), S(  -7,  -9) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   3,  -7), S(   9,  -6), S(   8, -11),
    S(   7, -12), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S(  -9, -30), S(  -4, -26), S(   3, -28),
    S(  14, -25), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -2,  -7), S(  12,   0), S(   2,   1), S(   6,  13),
    S(  14,  -1), S(  27,  -5), S(  21,   4), S(  24,  11),
    S(   9,  -1), S(  24,   2), S(   9,   9), S(  16,  15),
    S(  10,  10), S(  21,  14), S(  26,  22), S(  30,  19),
    S(  56,  22), S(  47,  44), S(  66,  53), S(  82,  58),
...
const int KnightInSiberia[4] = {
    S(  -8,  -1), S( -10,  -6), S( -21,  -4), S( -19,  -3),
};

const int KnightMobility[9] = {
    S( -74,-104), S( -29,-100), S( -13, -44), S(  -2, -16),
    S(   9,  -8), S(  14,   9), S(  22,  12), S(  32,  11),
    S(  45,  -5),
};
...
const int BishopBehindPawn = S(   2,  20);

const int BishopLongDiagonal = S(  21,  10);

const int BishopMobility[14] = {
    S( -67,-148), S( -30, -98), S(  -9, -56), S(   1, -29),
    S(  11, -18), S(  20,  -4), S(  23,   6), S(  23,  11),
    S(  22,  17), S(  27,  18), S(  27,  18), S(  46,   7),
    S(  43,  19), S(  71, -16),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  12,   3), S(  34,   2) };

const int RookOnSeventh = S(  -1,  27);

const int RookMobility[15] = {
    S(-150,-113), S( -57,-114), S( -16, -66), S(  -7, -24),
    S(  -7,  -2), S(  -8,  13), S(  -7,  25), S(   0,  28),
    S(   7,  32), S(  11,  36), S(  14,  43), S(  21,  46),
    S(  22,  50), S(  37,  39), S(  95,  -3),
};

/* Queen Evaluation Terms */

const int QueenMobility[28] = {
    S( -61,-263), S(-210,-387), S( -58,-201), S( -22,-192),
    S(  -8,-140), S(  -5, -85), S(   0, -52), S(   1, -21),
    S(   6, -18), S(   7,   6), S(  12,   9), S(  13,  27),
    S(  16,  19), S(  18,  29), S(  15,  33), S(  12,  39),
    S(  13,  39), S(   3,  41), S(   5,  38), S(   0,  33),
    S(   8,  10), S(  22, -12), S(  26, -39), S(  28, -55),
...
const int KingDefenders[12] = {
    S( -27,   3), S(  -5,  -2), S(   2,   3), S(   9,   5),
    S(  17,   4), S(  27,   1), S(  28,  -3), S(  13,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  31,  26), S(  20,  18), S(   3,   9), S( -17, -15),
    S( -18, -45), S( -16, -59), S( -15, -65), S( -11, -68),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -11,   2), S(  13, -27), S(  23,  -9), S(  15,   5),
    S(   8,   4), S(   1,   1), S(  -3, -33), S( -49,  18)},
   {S(  16,  -8), S(  20, -19), S(   3,  -6), S( -14,   4),
    S( -30,  15), S( -70,  68), S(  92,  82), S( -19,   1)},
   {S(  35,  -3), S(  11,  -8), S( -30,   7), S( -12,  -9),
    S( -19,  -4), S( -11,   1), S(   0,  66), S( -12,  -1)},
   {S(   4,  12), S(  25, -13), S(   5, -12), S(  15, -22),
    S(  29, -37), S( -60,   5), S(-136,  52), S(   6,  -7)},
   {S( -17,   9), S(   4,  -3), S( -31,   3), S( -20,   7),
    S( -21,  -7), S( -42,  -2), S(  33, -17), S(  -8,   0)},
   {S(  48, -18), S(  21, -16), S( -21,   3), S( -11, -20),
    S(   5, -24), S(  17, -20), S(  41, -30), S( -23,   2)},
   {S(  28, -15), S(  -1, -16), S( -25,  -1), S( -22,  -7),
    S( -30,  -7), S( -37,  32), S(   0,  44), S( -10,   1)},
   {S(  -9, -14), S(   5, -19), S(   7,   2), S(  -2,  12),
    S( -12,  16), S(  -9,  37), S(-190,  87), S( -18,  14)}},
  {{S(   0,   0), S( -12, -25), S(   6, -19), S( -39,  17),
    S( -22,   2), S(   3,  42), S(-167,  -8), S( -44,  10)},
   {S(   0,   0), S(  24, -23), S(   8,  -7), S( -18,   0),
    S(  -1, -13), S(  26,  66), S(-184,  -3), S( -34,   5)},
   {S(   0,   0), S(  26, -12), S(   0, -10), S(   6, -19),
    S(  18,  -6), S( -87,  47), S( -84, -73), S( -18,  -2)},
   {S(   0,   0), S(  -6,   8), S(  -6,  -1), S( -21,   2),
    S( -27,   2), S( -99,  31), S(   7, -41), S( -23,  -4)},
   {S(   0,   0), S(  14,   2), S(  12,  -6), S(  15, -13),
    S(  15, -27), S( -57,  15), S(-104, -61), S(  -1,  -4)},
   {S(   0,   0), S(   2,  -7), S( -24,   1), S( -25,  -7),
    S(  19, -23), S( -38,   3), S(  55,  38), S( -16,  -4)},
   {S(   0,   0), S(  22, -17), S(  11, -14), S( -12,  -6),
    S( -28,  12), S(  -8,  16), S( -56, -49), S( -32,  13)},
   {S(   0,   0), S(  12, -42), S(  21, -30), S( -19,  -5),
    S( -18,  19), S(  -5,  20), S(-228, -55), S( -24,   3)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S(  -5,  31), S( 117,  -7), S( -25,  26), S( -15,   4),
    S( -14,   0), S(  -8,  -6), S( -16,   2), S( -24,   1)},
   {S(  -4,  52), S(  56,  11), S( -21,  22), S(  -3,   9),
    S(  -2,   3), S(   5,  -6), S(  -2,   1), S( -12,   2)},
   {S(   7,  41), S(  16,  25), S( -24,  20), S( -12,   7),
    S(   1,   1), S(   5,   0), S(   9,  -7), S(   3,   1)},
   {S(  -3,  28), S(  14,  23), S( -20,   9), S( -16,   1),
    S( -14,   2), S(   8, -12), S(   2, -10), S( -14,   4)}},
  {{S(   0,   0), S( -15, -16), S( -18,   0), S(  20, -19),
    S(   9,  -7), S(   5, -19), S(  -1,   0), S(  17,  29)},
   {S(   0,   0), S( -16, -34), S(   0,  -7), S(  40, -11),
    S(   0,  -3), S(  13, -23), S(  -7,  -9), S( -17,   2)},
   {S(   0,   0), S( -28, -50), S( -25,  -5), S(  13, -11),
    S(   4,  -2), S(  -8, -14), S( -13, -14), S( -11,   6)},
   {S(   0,   0), S(  -2, -18), S( -17, -17), S( -15,  -2),
    S(  -4,  -7), S(   3, -27), S(  71, -11), S(  13,  20)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -38,   2), S( -50,  22), S( -76,  28),
    S(  -3,   9), S(  69, -11), S( 155,  50), S(   0,   0)},
   {S(   0,   0), S( -29,   9), S( -49,  30), S( -70,  35),
    S(  -9,  33), S(  92,  29), S( 184,  96), S(   0,   0)}},
  {{S(   0,   0), S( -23,  20), S( -48,  26), S( -70,  38),
    S(   1,  39), S(  91,  37), S( 263, 105), S(   0,   0)},
   {S(   0,   0), S( -29,  17), S( -44,  26), S( -66,  45),
    S(   3,  56), S(  95, 118), S( 160, 273), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   0,   1), S(   4,  -4), S(   6, -10),
    S(   6, -14), S(  -7, -11), S( -16,  -5), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   3,   0), S(   5,   1), S(  10,   9),
    S(   1,  21), S(   6,  29), S(  27,  23), S(   0,   0),
};

const int PassedSafePromotionPath = S( -25,  38);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -14, -26);
const int ThreatMinorAttackedByPawn  = S( -50, -56);
const int ThreatMinorAttackedByMinor = S( -24, -37);
const int ThreatMinorAttackedByMajor = S( -24, -47);
const int ThreatRookAttackedByLesser = S( -47, -21);
const int ThreatMinorAttackedByKing  = S( -18, -17);
const int ThreatRookAttackedByKing   = S( -14, -18);
const int ThreatQueenAttackedByOne   = S( -37, -36);
...
const int ClosednessKnightAdjustment[9] = {
    S(  -5, -12), S(  -4,   5), S(  -5,  14), S(   0,  13),
    S(   2,  17), S(   5,  12), S(   9,   8), S(  -5,  27),
    S(  -4,  15),
};

const int ClosednessRookAdjustment[9] = {
    S(  69, -21), S(  10,  30), S(   4,  16), S(  -2,   3),
    S(  -6,   4), S(  -8,  -8), S( -12, -10), S( -22, -15),
    S( -28, -17),
};
