...
const int PawnValue   = S(  99, 125);
const int KnightValue = S( 457, 406);
const int BishopValue = S( 476, 424);
const int RookValue   = S( 658, 687);
const int QueenValue  = S(1301,1342);
...
const int PawnPSQT32[32] = {
    ...
    S( -15,   6), S(   3,   1), S(  -6,   3), S(  -3,   0),
    S( -24,   6), S( -12,   4), S( -11,  -5), S(  -2, -12),
    S( -18,  13), S( -12,  11), S(   4,  -8), S(   6, -22),
    S( -14,  16), S(  -9,   9), S( -15,  -6), S( -10, -20),
    S(  -8,  32), S(  -5,  27), S(   0,  14), S(  23, -11),
    S( -21, -60), S( -58, -31), S(   1, -46), S(  43, -62),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -49, -32), S(   0, -30), S(  -9, -29), S(   3, -14),
    S(   3, -18), S(   2, -12), S(   5, -27), S(  11, -15),
    S(  15, -30), S(  18, -19), S(  16, -15), S(  23,  -2),
    S(  18,   3), S(  25,   7), S(  32,  18), S(  36,  25),
    S(  22,  12), S(  30,   9), S(  43,  28), S(  34,  38),
    S( -18,  11), S(   1,  12), S(  31,  30), S(  31,  28),
    S(  12, -13), S(  -6,  -2), S(  30, -22), S(  39,  -2),
    S(-178, -25), S( -85,  -4), S(-111,  17), S( -34,  -5),
};

const int BishopPSQT32[32] = {
    S(  11, -28), S(  14, -17), S(  -1, -16), S(  11, -10),
    S(  25, -37), S(  13, -40), S(  20, -22), S(  13, -11),
    S(  16, -11), S(  27, -14), S(   4, -18), S(  23,  -4),
    S(  11,  -7), S(  21,  -2), S(  18,   7), S(  27,   7),
    S( -11,  14), S(  20,   6), S(   5,  15), S(  17,  18),
    S(   2,   6), S(  -7,  18), S(  -1,   7), S(  12,  15),
    S( -45,  14), S( -50,   5), S(  -7,   7), S( -22,  13),
    S( -59,  -7), S( -44,   5), S( -84,  16), S( -90,  27),
};

const int RookPSQT32[32] = {
    S( -13, -27), S( -13, -20), S(  -2, -21), S(   4, -24),
    S( -45, -15), S(  -8, -34), S(  -7, -30), S(   4, -34),
    S( -28, -16), S(  -6, -12), S( -13, -16), S(   2, -25),
    S( -19,  -3), S( -11,   7), S(  -5,   4), S(   8,  -2),
    S(  -8,  10), S(   7,   9), S(  21,   6), S(  41,   1),
    S( -18,  19), S(  27,   4), S(   5,  20), S(  40,   2),
    S(  -1,   3), S( -13,  12), S(  11,   4), S(  29,   5),
    S(  35,  19), S(  28,  23), S(   1,  33), S(  11,  28),
};

const int QueenPSQT32[32] = {
    S(  35, -67), S(   7, -44), S(  16, -58), S(  24, -45),
    S(  14, -42), S(  29, -57), S(  28, -69), S(  18, -26),
    S(   9, -30), S(  24, -17), S(   7,   7), S(   6,   3),
    S(  12,  -9), S(  18,  12), S(   1,  23), S( -13,  65),
    S(  -4,  13), S(  -9,  47), S( -12,  22), S( -27,  74),
    S( -24,  31), S( -19,  19), S( -24,  24), S(  -9,  28),
    S(  -3,  27), S( -66,  67), S(  -3,  11), S( -38,  52),
    S( -13,  17), S(  20,   8), S(   8,   9), S(  -6,  20),
};

const int KingPSQT32[32] = {
    S(  53, -79), S(  43, -53), S( -16, -11), S( -27, -26),
    S(  27, -18), S( -17,  -9), S( -45,  13), S( -66,  15),
    S(  -7, -19), S(   1, -16), S(  12,   4), S( -19,  21),
    S(   0, -30), S(  95, -31), S(  46,  10), S(  -3,  32),
    S(  11, -13), S( 109, -29), S(  57,  16), S(  13,  24),
    S(  50, -27), S( 134, -23), S( 105,   2), S(  43,  -1),
    S(   7, -59), S(  48, -12), S(  32,   1), S(   8,  -7),
    S(   4,-123), S(  73, -64), S( -23, -24), S( -20, -20),
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -29, -10), S(  -9,  10), S( -16,  34),
    S(  11,  60), S(  40,  65), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -17,  19), S(  -4,  24), S(   3,  47),
    S(  25,  89), S(  39,  63), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -5, -13);

const int PawnStacked[2] = { S(  -2, -21), S(  -5, -10) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   2,  -6), S(  10,  -6), S(   8, -11),
    S(   6, -13), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -15, -26), S(  -5, -25), S(   2, -26),
    S(  31, -31), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -6,  -7), S(  10,   2), S(   0,   1), S(   8,  12),
    S(  12,   1), S(  21,  -3), S(  22,   2), S(  24,  10),
    S(   7,  -1), S(  22,   0), S(   8,   6), S(  15,  14),
    S(   9,  10), S(  20,  16), S(  25,  20), S(  28,  18),
    S(  50,  18), S(  39,  45), S(  58,  53), S(  73,  61),
    S( 110, -10), S( 202,  -7), S( 226,  23), S( 235,  38),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightInSiberia[4] = {
    S(  -9,  -2), S( -13,  -8), S( -26,  -6), S( -27,  -7),
};

const int KnightMobility[9] = {
    S( -77,-106), S( -31,-102), S( -15, -43), S(  -3, -15),
    S(   9,  -6), S(  12,  11), S(  19,  13), S(  29,  12),
    S(  43,  -6),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  18,  67);

const int BishopRammedPawns = S(  -6, -14);
...
const int BishopMobility[14] = {
    S( -68,-149), S( -33,-100), S( -10, -56), S(  -2, -27),
    S(   9, -17), S(  17,  -2), S(  19,   8), S(  20,  14),
    S(  18,  20), S(  25,  20), S(  25,  20), S(  47,   7),
    S(  47,  19), S(  74, -20),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(   8,   3), S(  31,   3) };

const int RookOnSeventh = S(  -3,  32);

const int RookMobility[15] = {
    S(-159,-116), S( -57,-116), S( -16, -69), S(  -9, -24),
    S( -10,  -1), S( -11,  14), S( -10,  27), S(  -2,  30),
    S(   5,  35), S(   8,  38), S(  11,  46), S(  18,  50),
    S(  21,  53), S(  39,  40), S( 106,  -9),
};

/* Queen Evaluation Terms */

const int QueenRelativePin = S( -20, -11);

const int QueenMobility[28] = {
    S( -61,-263), S(-213,-388), S( -60,-200), S( -25,-191),
    S(  -6,-144), S(  -5, -80), S(  -1, -42), S(  -1, -10),
    S(   5, -10), S(   7,  13), S(  13,  13), S(  15,  30),
    S(  18,  23), S(  20,  31), S(  17,  38), S(  14,  45),
    S(  16,  41), S(   9,  44), S(  10,  41), S(   9,  30),
    S(  17,   7), S(  30, -16), S(  31, -46), S(  29, -63),
...
const int KingDefenders[12] = {
    S( -26,   0), S(  -6,  -1), S(   3,   5), S(  10,   7),
    S(  18,   6), S(  28,   1), S(  26,  -4), S(  12,  -1),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  40,  34), S(  32,  23), S(  10,   7), S( -25, -20),
    S( -29, -50), S( -21, -63), S( -17, -66), S( -11, -64),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -10,   1), S(  14, -26), S(  22,  -8), S(  20,   7),
    S(  14,   3), S(  -1,  -1), S(  -3, -34), S( -52,  21)},
   {S(  14,  -7), S(  20, -18), S(   5,  -6), S( -16,   4),
    S( -29,  16), S( -73,  67), S(  91,  81), S( -18,   1)},
   {S(  35,  -3), S(   9,  -7), S( -31,   7), S( -14,  -8),
    S( -18,  -5), S( -12,   1), S(   0,  68), S( -13,   0)},
   {S(   1,  13), S(  27, -16), S(   6, -15), S(  15, -21),
    S(  31, -39), S( -59,   3), S(-135,  51), S(   7,  -7)},
   {S( -17,  14), S(   4,  -2), S( -34,   4), S( -21,   9),
    S( -22,  -6), S( -45,  -5), S(  32, -18), S(  -9,   2)},
   {S(  50, -17), S(  21, -14), S( -22,   5), S(  -8, -22),
    S(   6, -26), S(  19, -20), S(  40, -32), S( -22,   2)},
   {S(  36, -17), S(   0, -17), S( -32,   0), S( -24,  -8),
    S( -31,  -4), S( -40,  30), S(   1,  44), S(  -7,   1)},
   {S( -11, -15), S(   3, -21), S(   8,   2), S(  -3,  12),
    S( -10,  18), S(  -8,  37), S(-190,  89), S( -18,  16)}},
  {{S(   0,   0), S( -13, -26), S(   7, -23), S( -39,  20),
    S( -23,   1), S(   3,  42), S(-167,  -7), S( -41,  13)},
   {S(   0,   0), S(  22, -21), S(   9,  -8), S( -17,   1),
    S(   3, -18), S(  26,  65), S(-184,  -3), S( -29,   6)},
   {S(   0,   0), S(  21, -13), S(  -1, -12), S(   6, -21),
    S(  20,  -4), S( -87,  48), S( -84, -74), S( -17,  -1)},
   {S(   0,   0), S( -10,   8), S(  -9,  -2), S( -25,   5),
    S( -27,   4), S(-100,  30), S(   7, -40), S( -22,  -3)},
   {S(   0,   0), S(  15,  -1), S(  13,  -8), S(  15, -13),
    S(  19, -31), S( -57,  14), S(-101, -60), S(  -2,  -1)},
   {S(   0,   0), S(  -2,  -5), S( -26,   2), S( -22,  -9),
    S(  24, -22), S( -37,   3), S(  55,  39), S( -15,  -4)},
   {S(   0,   0), S(  25, -17), S(  14, -16), S( -14,  -9),
    S( -30,  14), S(  -8,  19), S( -57, -50), S( -30,  15)},
   {S(   0,   0), S(  12, -45), S(  22, -33), S( -19,  -7),
    S( -21,  19), S(  -8,  21), S(-228, -56), S( -24,   4)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -13,  35), S( 112,  -6), S( -22,  24), S( -12,   0),
    S( -15,  -1), S(  -7,  -7), S( -15,   4), S( -30,   2)},
   {S(  -8,  54), S(  50,  15), S( -21,  20), S(  -4,   6),
    S(  -2,   3), S(   6,  -5), S(  -3,   5), S( -13,   6)},
   {S(   3,  44), S(   3,  28), S( -21,  18), S( -10,   5),
    S(   1,   1), S(   7,   0), S(  11,  -7), S(   4,   5)},
   {S(  -8,  28), S(   3,  24), S( -24,   7), S( -15,  -2),
    S( -11,   0), S(  11, -13), S(   2,  -6), S( -13,   7)}},
  {{S(   0,   0), S( -18, -16), S( -15,   0), S(  24, -21),
    S(  11,  -7), S(   7, -21), S(   8,   2), S(  16,  29)},
   {S(   0,   0), S( -17, -37), S(   4, -11), S(  48, -14),
    S(   4,  -3), S(  17, -24), S(  -5,  -4), S( -19,   0)},
   {S(   0,   0), S( -28, -52), S( -23,  -8), S(  19, -13),
    S(   8,   0), S( -10, -14), S( -10, -12), S( -11,   6)},
   {S(   0,   0), S(  -2, -19), S( -19, -19), S( -13,  -2),
    S(  -1,  -8), S(  -1, -24), S(  72, -12), S(  15,  20)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -38,   0), S( -47,  26), S( -71,  31),
    S(   9,  11), S(  80, -17), S( 157,  40), S(   0,   0)},
   {S(   0,   0), S( -28,  13), S( -47,  39), S( -65,  42),
    S(   3,  40), S( 107,  23), S( 191,  88), S(   0,   0)}},
  {{S(   0,   0), S( -22,  27), S( -48,  36), S( -66,  49),
    S(  11,  46), S( 103,  32), S( 270,  92), S(   0,   0)},
   {S(   0,   0), S( -29,  23), S( -42,  35), S( -63,  56),
    S(  10,  68), S( 105, 119), S( 157, 264), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   1,   2), S(   5,  -3), S(   8, -12),
    S(   4, -15), S( -12,  -8), S( -17,   1), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   3,  -1), S(   4,   0), S(   7,   9),
    S(   1,  20), S(   8,  27), S(  29,  22), S(   0,   0),
};

const int PassedSafePromotionPath = S( -20,  44);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S(  -9, -31);
...
const int ThreatRookAttackedByLesser = S( -48, -26);
const int ThreatMinorAttackedByKing  = S( -11, -17);
const int ThreatRookAttackedByKing   = S( -19, -16);
const int ThreatQueenAttackedByOne   = S( -52, -26);
const int ThreatOverloadedPieces     = S(  -8, -14);
const int ThreatByPawnPush           = S(  12,  27);
...
const int ClosednessKnightAdjustment[9] = {
    S(  -8,  -8), S(  -6,   9), S(  -6,  16), S(  -1,  16),
    S(  -1,  18), S(   2,  14), S(   5,   6), S(  -8,  24),
    S(  -7,   9),
};

const int ClosednessRookAdjustment[9] = {
    S(  67, -10), S(   4,  34), S(   4,  19), S(  -1,   1),
    S( -10,   5), S( -11, -12), S( -12, -13), S( -24, -16),
    S( -35, -21),
};
...
const int ComplexityPawnFlanks  = S(   0,  76);
const int ComplexityPawnEndgame = S(   0,  67);
const int ComplexityAdjustment  = S(   0,-161);
