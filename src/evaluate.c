...
const int PawnValue[PHASE_NB]   = {  69,  77};

const int KnightValue[PHASE_NB] = { 299, 268};

const int BishopValue[PHASE_NB] = { 299, 269};

const int RookValue[PHASE_NB]   = { 399, 437};

const int QueenValue[PHASE_NB]  = { 751, 819};

const int KingValue[PHASE_NB]   = { 100, 100};

const int PawnIsolated[PHASE_NB] = {   4,  -1};

const int PawnStacked[PHASE_NB] = { -14, -20};

const int PawnBackwards[2][PHASE_NB] = { {   3,  -3}, {  -8,  -6} };
...
const int PawnConnected32[32][PHASE_NB] = {
    ...
    {   6,   1}, {  12,   1}, {  10,   6}, {  14,  15},
    {   4,   0}, {  15,   1}, {  10,   4}, {  14,  11},
    {   8,   6}, {  15,  12}, {  15,  14}, {  28,  16},
    {  20,  26}, {  20,  32}, {  43,  37}, {  40,  39},
    { 139,  -6}, {  94,  13}, { 136,  12}, { 188,  20},
    {   0,   0}, {   0,   0}, {   0,   0}, {   0,   0},
};

const int KnightAttackedByPawn[PHASE_NB] = { -48, -20};

const int KnightOutpost[2][PHASE_NB] = { {  14, -21}, {  29,   0} };
...
const int BishopWings[PHASE_NB] = {  -1,  -2};

const int BishopPair[PHASE_NB] = {  25,  40};
...
const int BishopMobility[14][PHASE_NB] = {
    { -51, -70}, { -31, -43}, { -14, -25}, {  -3, -11},
    {   4,  -4}, {  12,   1}, {  15,   6}, {  18,   3},
    {  21,   8}, {  24,   1}, {  24,   4}, {  33, -10},
    {  25,   0}, {  35, -20},
};

const int RookFile[2][PHASE_NB] = { {   9,   1}, {  27,  -6} };

const int RookOnSeventh[PHASE_NB] = {   3,   3};

const int RookMobility[15][PHASE_NB] = {
    { -87, -98}, { -41, -71}, {  -7, -37}, {  -4, -16},
    {  -6,  -4}, {  -5,   5}, {  -5,  12}, {  -2,  18},
    {   0,  20}, {   4,  22}, {   7,  25}, {   9,  29},
    {   9,  31}, {  12,  29}, {   1,  28},
};

const int QueenChecked[PHASE_NB] = { -40,   5};

const int QueenCheckedByPawn[PHASE_NB] = { -48, -13};

const int QueenMobility[28][PHASE_NB] = {
    {-197, -66}, { -84,-360}, { -35,-111}, { -20,-125},
    { -15,-109}, { -18, -54}, { -13, -49}, { -14, -49},
    { -11, -49}, { -10, -41}, {  -7, -31}, {  -5, -22},
    {  -6, -16}, {  -3,  -8}, {  -3,  -5}, {  -1,   0},
    {   0,   7}, {   0,   8}, {   0,  13}, {  -3,  14},
    {  -5,  17}, {  13,  19}, {  10,  14}, {  24,  10},
    {  22,  11}, {  55,   5}, {   0,   5}, {   3,   5},
};

const int KingDefenders[12][PHASE_NB] = {
    { -26,   0}, { -12,   3}, {   0,   0}, {   6,   0},
    {  15,   0}, {  22,   5}, {  29,  -7}, {  27, -15},
    {   8,   4}, {   8,   4}, {   8,   4}, {   8,   4},
};

const int KingShelter[2][FILE_NB][RANK_NB][PHASE_NB] = {
  {{{  -6,   4}, {   9,  -6}, {  16,  -1}, {  20,   0}, {   3,   1}, {   4,  -1}, {  26,  15}, { -17,   3}},
   {{   6,   5}, {  12,  -2}, {  17,  -5}, {  -6,  -8}, {  -1,   0}, { -29,  59}, { -36,  79}, { -18,   0}},
   {{   9,   9}, {   6,   3}, { -11,   0}, {  -5,  -4}, {  -3, -10}, {  28,  -6}, {  -2,  11}, {  -7,   0}},
   {{   4,  17}, {   8,   1}, {  -2,  -5}, {  12,  -6}, {   8, -11}, { -24, -23}, { -20,   8}, {   0,   0}},
   {{  -2,  12}, {   0,   2}, { -17,   0}, { -10,  -4}, { -20, -16}, { -11, -23}, {  65, -16}, { -11,   1}},
   {{  15,   3}, {  12,   0}, {  -6,  -2}, {  -3, -13}, {   0, -25}, {  -3, -17}, {  11,  -6}, { -12,   0}},
   {{  11,   3}, {   1,  -6}, { -17,  -6}, { -16, -13}, { -20, -14}, { -20,  12}, {  34,   1}, { -22,   5}},
   {{  -9,  -3}, {   0,  -5}, {   2,   0}, {   0,   0}, { -12,   0}, {  -1,  22}, { -28,  30}, { -17,   9}}},
  {{{   0,   0}, {  -3,  -5}, {   9,  -9}, { -27,  12}, { -36,  13}, { -26,  49}, { -14,  15}, { -35,   6}},
   {{   0,   0}, {  16,   0}, {  14,  -3}, { -11,  -1}, {   0, -31}, { -41,  18}, {-196, -10}, { -25,   1}},
   {{   0,   0}, {  19,   1}, {   3,   0}, {  18, -14}, {  18, -13}, { -60,   5}, {-124, -93}, { -13,  -1}},
   {{   0,   0}, {  -2,   4}, {  -2,   8}, { -16,  -6}, { -10,  -2}, {   0,   2}, { -22, -14}, { -15,  -2}},
   {{   0,   0}, {   2,   4}, {   5,   0}, {   9,  -6}, {   4, -13}, {  -4, -14}, { -84,  24}, {  -3,  -2}},
   {{   0,   0}, {   1,   3}, {  -9,   0}, { -12, -15}, { -22, -29}, { -52, -10}, { -19,  76}, { -18,   0}},
   {{   0,   0}, {   8,   0}, {  -1,   0}, {  -8,  -8}, { -14,  -9}, {   7,   8}, { -10, -66}, { -29,   7}},
   {{   0,   0}, {   5, -11}, {   6,  -8}, { -29,   0}, { -30,   0}, {   5, -15}, {-168, -16}, { -29,   9}}},
};

const int PassedPawn[2][2][RANK_NB][PHASE_NB] = {
  {{{   0,   0}, { -24,  -7}, { -20,   1}, { -16,  -1}, {  14,  -5}, {  39, -15}, {  81,   5}, {   0,   0}},
   {{   0,   0}, {  -1,   0}, {  -9,  13}, { -10,  20}, {   5,  27}, {  56,  36}, { 126,  77}, {   0,   0}}},
  {{{   0,   0}, {  -4,   6}, {  -7,   6}, {  -3,  15}, {  18,  17}, {  67,  35}, { 146,  88}, {   0,   0}},
   {{   0,   0}, {  -4,   5}, { -10,  10}, { -15,  34}, {  -6,  70}, {  25, 144}, {  92, 231}, {   0,   0}}},
};
