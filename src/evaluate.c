...
const int PawnCandidatePasser[2][8] = {
   {S(   0,   0), S( -26, -11), S( -12,   8), S( -12,  27), S(   3,  62), S(  47,  68), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -12,  14), S(  -5,  21), S(   4,  44), S(  16,  85), S(  33,  52), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -9,  -8);
...
const int PassedPawn[2][2][8] = {
  {{S(   0,   0), S( -38,   0), S( -53,  21), S( -83,  25), S(  -6,  15), S(  65,   0), S( 152,  58), S(   0,   0)},
   {S(   0,   0), S( -26,   0), S( -46,  19), S( -71,  24), S( -12,  26), S(  84,  26), S( 182,  93), S(   0,   0)}},
  {{S(   0,   0), S( -25,   9), S( -47,  16), S( -73,  31), S(  -2,  33), S(  87,  40), S( 258, 122), S(   0,   0)},
   {S(   0,   0), S( -27,   6), S( -39,  13), S( -64,  35), S(  -2,  58), S(  76, 139), S( 156, 301), S(   0,   0)}},
};

const int PassedFriendlyDistance[8] = {
    S(   0,   0), S(   0,   0), S(   3,  -3), S(   7, -10),
    S(   6, -16), S(  -6, -16), S( -12, -10), S(   0,   0),
};

const int PassedEnemyDistance[8] = {
    S(   0,   0), S(   3,   0), S(   4,   0), S(   8,  10),
    S(   1,  24), S(   8,  34), S(  24,  36), S(   0,   0),
};

const int PassedSafePromotionPath = S( -26,  35);

const int PassedIsolatedPawn = S(  -5,  -4);
...
int evaluatePassedPawns(EvalInfo* ei, Board* board, int colour){
    ...
    while (tempPawns){
        ...
        bitboard = ~forwardRanksMasks(US, rankOf(sq)) & Files[fileOf(sq)];
        flag = !(testBit(ei->pawnAttacks[US] | ei->attackedBy[US][KING], sq) || (bitboard & myRooks));
