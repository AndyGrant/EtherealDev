...
const int KnightValue = S( 456, 407);
const int BishopValue = S( 477, 424);
const int RookValue   = S( 662, 685);
...
const int PawnPSQT32[32] = {
    ...
    S( -20,   9), S(   2,   3), S(  -8,   5), S(  -6,  -2),
    S( -25,   4), S( -13,   4), S( -12,  -4), S(  -3, -13),
    S( -19,  10), S( -14,  10), S(   6,  -9), S(   6, -20),
    S( -11,  12), S(  -5,   7), S( -10,  -6), S(  -5, -20),
    S(  -8,  27), S(  -4,  24), S(   1,  14), S(  28, -13),
    S( -21, -61), S( -60, -29), S(   2, -43), S(  45, -58),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -48, -31), S(  -2, -28), S( -12, -29), S(   2, -15),
    S(   0, -20), S(   3, -11), S(   5, -27), S(  11, -17),
    S(  14, -26), S(  21, -22), S(  13, -15), S(  25,  -4),
    S(  21,   1), S(  25,   6), S(  32,  17), S(  33,  22),
    S(  26,  14), S(  30,   9), S(  41,  27), S(  32,  38),
...
const int BishopPSQT32[32] = {
    S(   8, -26), S(  12, -18), S(  -4,  -8), S(  12, -11),
    S(  30, -35), S(  10, -35), S(  21, -20), S(  13, -11),
    S(  17, -13), S(  29, -15), S(   5, -17), S(  24,  -5),
    S(  15,  -7), S(  20,  -3), S(  16,   4), S(  26,   5),
    S( -10,  14), S(  21,   5), S(   5,  14), S(  15,  17),
    S(   2,   7), S(  -4,  17), S(  -2,   6), S(  14,  14),
    S( -44,  15), S( -51,   5), S(  -7,   8), S( -23,  13),
    S( -58,  -7), S( -45,   6), S( -84,  15), S( -90,  27),
};

const int RookPSQT32[32] = {
    S( -11, -28), S( -15, -19), S(  -5, -22), S(   4, -29),
    S( -52, -14), S( -10, -33), S(  -8, -31), S(   4, -36),
    S( -29, -14), S(  -5, -11), S( -15, -16), S(   2, -25),
    S( -17,  -2), S( -11,   7), S(  -6,   2), S(   8,  -4),
    S(  -4,  10), S(  10,   7), S(  22,   4), S(  41,  -1),
    S( -15,  21), S(  29,   5), S(   6,  19), S(  38,   2),
    S(   1,   6), S( -12,  13), S(  12,   4), S(  28,   6),
    S(  35,  22), S(  27,  23), S(   1,  32), S(   9,  28),
};

const int QueenPSQT32[32] = {
    S(  31, -66), S(   6, -45), S(  15, -59), S(  19, -43),
...
const int KingPSQT32[32] = {
    S(  47, -77), S(  38, -48), S( -16,  -9), S( -30, -19),
    S(  27, -21), S( -14, -10), S( -44,  14), S( -63,  16),
    S(  -4, -20), S(   6, -18), S(  13,   4), S( -17,  20),
    S(   1, -30), S(  95, -34), S(  46,   7), S(  -4,  27),
    S(  10, -14), S( 108, -32), S(  56,  12), S(  12,  19),
...
const int PawnStacked[2] = { S(  -5, -18), S( -11,  -7) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   5,  -6), S(  10,  -6), S(   9, -11),
    S(   7, -12), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S(  -9, -28), S(  -5, -25), S(   4, -28),
    S(  28, -30), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -3,  -5), S(  10,   1), S(   0,   1), S(   6,  12),
    S(  13,   0), S(  27,  -4), S(  20,   4), S(  23,  11),
    S(   8,  -1), S(  24,   1), S(   9,   8), S(  14,  16),
    S(  10,   9), S(  21,  14), S(  26,  20), S(  30,  20),
...
const int KnightInSiberia[4] = {
    S(  -9,  -1), S( -11,  -6), S( -21,  -3), S( -25,  -6),
};

const int KnightMobility[9] = {
    S( -78,-106), S( -31,-102), S( -12, -42), S(  -1, -15),
    S(   8,  -6), S(  12,  10), S(  18,  13), S(  28,  12),
    S(  42,  -4),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  21,  67);
...
const int BishopLongDiagonal = S(  19,  10);

const int BishopMobility[14] = {
    S( -69,-149), S( -32, -99), S(  -9, -54), S(   1, -28),
    S(  10, -16), S(  15,  -3), S(  18,   7), S(  20,  12),
    S(  19,  17), S(  24,  19), S(  24,  20), S(  46,   8),
    S(  47,  19), S(  75, -17),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  14,   5), S(  32,   0) };

const int RookOnSeventh = S(  -4,  30);

const int RookMobility[15] = {
    S(-159,-116), S( -58,-115), S( -14, -65), S(  -8, -20),
    S(  -9,   0), S(  -9,  15), S(  -9,  27), S(  -2,  30),
    S(   5,  33), S(   9,  38), S(  13,  43), S(  18,  47),
    S(  21,  51), S(  37,  39), S( 105, -12),
};
...
const int QueenMobility[28] = {
    ...
    S(  -5,-144), S(  -3, -80), S(   2, -44), S(   1, -12),
    S(   7, -11), S(   7,  11), S(  12,  14), S(  13,  32),
...
const int KingDefenders[12] = {
    S( -26,   6), S(  -6,  -2), S(   3,   2), S(  10,   5),
    S(  17,   5), S(  25,   1), S(  29,  -4), S(  12,  -1),
...
const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -9,   2), S(  14, -26), S(  26,  -9), S(  17,   6),
   ...
   {S(  36,  -4), S(  10,  -8), S( -30,   7), S( -13,  -8),
    S( -17,  -5), S( -12,   1), S(   0,  68), S( -12,   0)},
   {S(   2,  12), S(  24, -13), S(   6, -12), S(  14, -21),
    S(  31, -38), S( -58,   3), S(-135,  51), S(   7,  -6)},
   {S( -18,  11), S(   4,  -1), S( -31,   4), S( -20,   8),
    S( -21,  -6), S( -45,  -5), S(  32, -18), S(  -8,   2)},
   {S(  48, -19), S(  19, -13), S( -21,   4), S(  -9, -19),
    S(   7, -25), S(  19, -21), S(  40, -32), S( -23,   2)},
   {S(  29, -18), S(  -1, -16), S( -26,  -1), S( -22,  -7),
    S( -31,  -5), S( -41,  30), S(   1,  44), S(  -9,   0)},
   {S(  -9, -13), S(   5, -17), S(   8,   2), S(  -1,  12),
  ...
  {{S(   0,   0), S( -13, -26), S(   7, -23), S( -39,  20),
   ...
   {S(   0,   0), S(  15,   2), S(  13,  -6), S(  14, -13),
    S(  17, -31), S( -57,  14), S(-101, -60), S(  -2,  -3)},
   {S(   0,   0), S(   0,  -6), S( -26,   2), S( -23,  -9),
    S(  23, -23), S( -37,   3), S(  55,  39), S( -17,  -4)},
   {S(   0,   0), S(  22, -16), S(  11, -15), S( -11,  -7),
...
const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -13,  32), S( 112,  -6), S( -23,  25), S( -14,   3),
    S( -12,   0), S(  -6,  -6), S( -16,   2), S( -24,   2)},
   {S(  -8,  53), S(  51,  14), S( -20,  22), S(  -3,   9),
    S(  -2,   4), S(   5,  -5), S(  -2,   3), S( -10,   3)},
   {S(   3,  43), S(   4,  28), S( -22,  20), S( -10,   7),
    S(   2,   1), S(   7,   0), S(  15,  -6), S(   6,   2)},
   {S(  -7,  28), S(   4,  25), S( -22,   9), S( -13,   0),
    S( -11,   0), S(   9, -12), S(   2,  -9), S( -13,   4)}},
...
const int PassedPawn[2][2][RANK_NB] = {
  ...
  {{S(   0,   0), S( -24,  26), S( -48,  33), S( -68,  42),
    S(   9,  39), S( 102,  33), S( 271,  96), S(   0,   0)},
   {S(   0,   0), S( -29,  22), S( -43,  31), S( -62,  49),
    S(  11,  58), S( 104, 116), S( 157, 265), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   1,   2), S(   4,  -3), S(   7, -10),
    S(   5, -13), S(  -8, -10), S( -17,  -3), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   3,   1), S(   4,   1), S(  10,  10),
    S(   2,  23), S(   8,  29), S(  28,  25), S(   0,   0),
};
...
const int ThreatWeakPawn             = S( -20, -32);
const int ThreatMinorAttackedByPawn  = S( -56, -58);
const int ThreatMinorAttackedByMinor = S( -22, -37);
const int ThreatMinorAttackedByMajor = S( -31, -48);
const int ThreatRookAttackedByLesser = S( -50, -22);
const int ThreatMinorAttackedByKing  = S( -17, -17);
const int ThreatRookAttackedByKing   = S( -18, -17);
const int ThreatQueenAttackedByOne   = S( -38, -21);
const int ThreatOverloadedPieces     = S(  -3, -12);
const int ThreatByPawnPush           = S(  11,  23);

/* Space Evaluation Terms */

const int SpaceRestrictPiece = S(  -5,  -2);
const int SpaceRestrictEmpty = S(  -4,   0);
const int SpaceCenterControl = S(   0,  -2);

/* Closedness Evaluation Terms */

const int ClosednessKnightAdjustment[9] = {
    S(  -9,  -8), S(  -6,   8), S(  -7,  16), S(  -3,  16),
    S(  -1,  20), S(   2,  14), S(   6,   5), S(  -7,  24),
    S(  -6,   9),
};

const int ClosednessRookAdjustment[9] = {
    S(  68, -16), S(   5,  34), S(   4,  18), S(  -2,   1),
