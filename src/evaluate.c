...
const int PawnValue[PHASE_NB]   = {  77,  73};

const int KnightValue[PHASE_NB] = { 312, 249};

const int BishopValue[PHASE_NB] = { 301, 245};

const int RookValue[PHASE_NB]   = { 439, 386};

const int QueenValue[PHASE_NB]  = { 845, 767};

const int KingValue[PHASE_NB]   = { 100, 100};

const int PawnIsolated[PHASE_NB] = {  -2,  -9};

const int PawnStacked[PHASE_NB] = { -11, -16};

const int PawnBackwards[2][PHASE_NB] = { {   3,   1}, {  -8, -13} };
...
const int PawnConnected32[32][PHASE_NB] = {
    ...
    {   6,   1}, {  17,   0}, {  14,   5}, {  15,  16},
    {   7,   0}, {  15,   2}, {  11,   4}, {  16,  12},
    {  14,   4}, {  16,   8}, {  22,   9}, {  28,  11},
    {  36,   4}, {  41,  13}, {  68,  28}, {  62,  39},
    { 187, -11}, { 288, -36}, { 191, -44}, { 253,  15},
    {   0,   0}, {   0,   0}, {   0,   0}, {   0,   0},
};

const int KnightAttackedByPawn[PHASE_NB] = { -38, -23};

const int KnightOutpost[2][PHASE_NB] = { {  18, -31}, {  33,   4} };
...
const int BishopWings[PHASE_NB] = {  16,  -3};

const int BishopPair[PHASE_NB] = {  43,  46};

const int BishopAttackedByPawn[PHASE_NB] = { -39, -24};

const int BishopOutpost[2][PHASE_NB] = { {  20, -15}, {  43, -11} };

const int BishopMobility[14][PHASE_NB] = {
    { -56, -82}, { -31, -57}, { -13, -40}, {  -5, -23},
    {   3, -13}, {  11,  -7}, {  14,   0}, {  19,  -2},
    {  21,   1}, {  27,  -3}, {  31,  -5}, {  48, -30},
    {  28, -11}, {  23, -69},
};

const int RookFile[2][PHASE_NB] = { {  12, -10}, {  30, -10} };

const int RookOnSeventh[PHASE_NB] = {   5,  18};

const int RookMobility[15][PHASE_NB] = {
    {-132, -73}, { -57, -92}, { -10, -54}, {  -5, -27},
    {  -4, -11}, {  -5,   2}, {  -5,  10}, {  -1,  14},
    {   0,  20}, {   6,  20}, {   9,  24}, {  17,  28},
    {  18,  27}, {  24,  23}, {  37,  10},
};

const int QueenChecked[PHASE_NB] = { -26,  49};

const int QueenCheckedByPawn[PHASE_NB] = { -34,  32};

const int QueenMobility[28][PHASE_NB] = {
    { -65, -66}, {  70,-400}, { -18,-226}, { -22,-128},
    { -15,-181}, { -15, -56}, { -14, -62}, { -11, -55},
    { -10, -39}, {  -7, -31}, {  -6, -21}, {  -3, -18},
    {   0, -11}, {  -1,  -7}, {   2,  -6}, {   3,   0},
    {   3,   4}, {   1,   4}, {  12,   3}, {   5,   4},
    {  15,  -1}, {  17,  -6}, {  29, -11}, {  37, -18},
    {  44, -23}, {  40, -33}, {  24, -35}, {  34, -47},
};

const int KingDefenders[12][PHASE_NB] = {
    { -35,   7}, { -20,  -2}, {   0,   0}, {   8,   0},
    {  19,   0}, {  29,   4}, {  33,   3}, {  69, -90},
    {   8,   4}, {   8,   4}, {   8,   4}, {   8,   4},
};

const int KingShelter[2][FILE_NB][RANK_NB][PHASE_NB] = {
  {{{   0,   1}, {  11, -11}, {  15,  -5}, {   9,   1}, {  -5,   5}, { -17,   0}, { -42,   8}, { -34,  13}},
   {{  -3,   4}, {  17,  -7}, {  18,  -6}, {   0, -10}, {   0, -17}, { -37,  22}, {-120,  95}, { -27,   4}},
   {{  19,   1}, {   6,  -1}, { -18,   2}, { -12,  -1}, { -19,  -3}, {  11,  -6}, { -54,  31}, { -12,   3}},
   {{  12,  13}, {   8,   0}, {  -7,  -9}, {  11,  -8}, {  15, -21}, { -16, -28}, {-118,  29}, {  -1,   0}},
   {{   5,   7}, {   0,   2}, { -24,   2}, {  -7,  -5}, { -18, -19}, { -21, -26}, { -26, -13}, { -12,   0}},
   {{  22,  -3}, {  12,  -3}, { -12,  -1}, {  -5, -15}, {   6, -28}, {  12, -26}, {   0,   9}, { -12,   2}},
   {{  17,  -4}, {   3,  -7}, { -23,  -4}, { -24,  -7}, { -20, -16}, { -44,   5}, { -67,  29}, { -21,   8}},
   {{  -4,  -5}, {   2,  -8}, {   5,   0}, {   0,   4}, { -19,  10}, { -17,  37}, {-102,  75}, { -17,  11}}},
  {{{   0,   0}, { -11, -14}, {   0, -16}, { -62,  14}, {  -7,  18}, {-130,  26}, {  70,  81}, { -57,  23}},
   {{   0,   0}, {  15,  -8}, {   7,  -9}, { -14,   6}, {  10, -13}, {-106,  16}, {-280,  39}, { -32,  10}},
   {{   0,   0}, {  24,  -5}, {   3,  -6}, {  18, -15}, {   6,  -6}, { -66,  14}, {-293, -57}, { -12,   0}},
   {{   0,   0}, {  -3,   3}, {  -5,   8}, {  -6,  -4}, { -18,  -1}, { -59,  -1}, {  62, -23}, { -21,  -1}},
   {{   0,   0}, {   0,   2}, {   3,  -3}, {  12, -11}, {  10, -23}, { -37,   5}, { -78,  17}, {  -4,  -2}},
   {{   0,   0}, {   7,  -1}, {  -4,  -1}, { -14,  -6}, {   0, -15}, {-141,  10}, {-116,  11}, { -20,   0}},
   {{   0,   0}, {  11,  -6}, {   0,  -1}, { -12,  -3}, { -23,  -7}, { -16,  37}, { -41,  -2}, { -30,  13}},
   {{   0,   0}, {   9, -21}, {  10, -14}, { -26,   1}, { -31,  17}, { -32,  10}, {-200,  41}, { -38,  15}}},
};

const int PassedPawn[2][2][RANK_NB][PHASE_NB] = {
  {{{   0,   0}, { -32, -15}, { -16, -10}, {   0, -29}, {  37, -39}, {  67, -53}, { 111, -25}, {   0,   0}},
   {{   0,   0}, {  -7,   3}, { -13,  16}, { -13,  23}, {  11,  21}, {  61,  19}, { 161,  58}, {   0,   0}}},
  {{{   0,   0}, { -10,   7}, {  -9,  -3}, {  -2,   5}, {  28,   5}, {  81,  19}, { 224,  43}, {   0,   0}},
   {{   0,   0}, {  -7,  12}, {  -7,  14}, { -10,  32}, {  -4,  68}, {  42, 115}, { 100, 220}, {   0,   0}}},
};
