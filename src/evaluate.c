...
const int PawnValue = S( 113, 137);

const int KnightValue = S( 463, 411);

const int BishopValue = S( 483, 432);

const int RookValue = S( 666, 694);

const int QueenValue = S(1302,1349);

const int KingValue = S(0, 0);

const int PawnPSQT32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S( -22,   7), S(   3,   2), S( -10,   4), S(  -5,  -2),
    S( -27,   3), S( -14,   3), S( -11,  -4), S(  -4, -13),
    S( -19,  12), S( -13,  13), S(   6,  -8), S(   7, -22),
    S( -11,  15), S(  -5,  10), S(  -9,  -4), S(  -2, -18),
    S(  -7,  29), S(  -3,  25), S(   2,  15), S(  29, -12),
    S( -21, -61), S( -60, -29), S(   2, -43), S(  45, -58),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -48, -31), S(  -8, -28), S( -13, -30), S(   1, -15),
    S(  -1, -20), S(   3, -11), S(   4, -27), S(  11, -18),
    S(  11, -26), S(  20, -22), S(  15, -16), S(  26,  -3),
    S(  21,   1), S(  25,   6), S(  33,  18), S(  36,  24),
    S(  26,  14), S(  33,   9), S(  43,  28), S(  39,  39),
    S( -18,  11), S(   2,  13), S(  30,  29), S(  32,  29),
    S(  13, -12), S(  -4,  -1), S(  30, -22), S(  40,   0),
    S(-178, -25), S( -85,  -4), S(-111,  18), S( -33,  -3),
};

const int BishopPSQT32[32] = {
    S(   8, -26), S(  12, -18), S(  -9,  -9), S(  11, -12),
    S(  30, -35), S(  10, -36), S(  22, -20), S(  14, -10),
    S(  15, -13), S(  29, -15), S(   4, -17), S(  27,  -4),
    S(  15,  -7), S(  20,  -3), S(  18,   5), S(  28,   6),
    S( -10,  14), S(  22,   6), S(   5,  15), S(  16,  18),
    S(   3,   7), S(  -4,  17), S(  -1,   6), S(  14,  14),
    S( -44,  15), S( -51,   5), S(  -7,   8), S( -23,  13),
    S( -58,  -7), S( -45,   6), S( -84,  15), S( -90,  27),
};

const int RookPSQT32[32] = {
    S( -12, -28), S( -16, -19), S(   0, -22), S(   8, -28),
    S( -55, -15), S( -10, -34), S(  -9, -32), S(   3, -38),
    S( -30, -15), S(  -5, -11), S( -16, -16), S(   2, -26),
    S( -17,  -2), S( -11,   8), S(  -6,   3), S(   8,  -3),
    S(  -3,  11), S(  10,   8), S(  23,   6), S(  42,   1),
    S( -14,  22), S(  30,   7), S(   7,  21), S(  39,   4),
    S(   2,   7), S( -12,  14), S(  13,   5), S(  29,   7),
    S(  35,  23), S(  27,  23), S(   1,  32), S(   9,  29),
};

const int QueenPSQT32[32] = {
    S(  31, -66), S(   6, -45), S(  13, -60), S(  22, -43),
    S(  14, -42), S(  28, -58), S(  30, -71), S(  19, -27),
    S(  11, -30), S(  26, -20), S(   7,   7), S(   7,   1),
    S(  13,  -8), S(  17,  12), S(  -1,  24), S( -15,  66),
    S(  -3,  15), S(  -7,  47), S( -12,  22), S( -29,  76),
...
const int KingPSQT32[32] = {
    S(  47, -78), S(  40, -53), S( -19, -13), S( -33, -23),
    S(  28, -21), S( -15, -13), S( -46,  11), S( -66,  12),
    S(  -4, -21), S(   6, -18), S(  13,   3), S( -17,  19),
    S(   1, -30), S(  96, -32), S(  47,  10), S(  -3,  30),
    S(  10, -13), S( 109, -29), S(  57,  15), S(  12,  22),
    S(  50, -26), S( 134, -21), S( 105,   5), S(  43,   2),
...
const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -27, -10), S( -10,   9), S( -17,  32),
    S(   3,  61), S(  40,  65), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -14,  19), S(  -4,  21), S(   4,  48),
    S(  21,  88), S(  38,  62), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -6, -12);

const int PawnStacked[2] = { S(  -7, -20), S(  -8,  -8) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   3,  -8), S(  10,  -6), S(   9, -12),
    S(   8, -12), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -13, -29), S(  -7, -23), S(   5, -23),
    S(  30, -28), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -1,  -6), S(  13,   1), S(   2,   2), S(   8,  12),
    S(  16,   1), S(  29,  -4), S(  22,   4), S(  25,  12),
    S(   8,   0), S(  25,   2), S(  10,   8), S(  18,  14),
    S(  10,  11), S(  22,  16), S(  27,  22), S(  32,  20),
    S(  57,  20), S(  42,  45), S(  62,  56), S(  75,  61),
...
const int KnightBehindPawn = S(   5,  23);

const int KnightInSiberia[4] = {
    S( -11,  -1), S( -15,  -8), S( -26,  -4), S( -25,  -6),
};

const int KnightMobility[9] = {
    S( -78,-106), S( -34,-103), S( -19, -43), S(  -4, -16),
    S(  11,  -6), S(  16,  12), S(  25,  15), S(  35,  16),
    S(  49,  -3),
};

const int BishopPair = S(  30,  74);
...
const int BishopLongDiagonal = S(  26,  12);

const int BishopMobility[14] = {
    S( -70,-149), S( -39,-100), S( -15, -55), S(  -3, -29),
    S(  11, -18), S(  20,  -2), S(  24,  10), S(  26,  15),
    S(  24,  21), S(  29,  24), S(  26,  23), S(  48,   9),
    S(  47,  19), S(  75, -17),
};

const int RookFile[2] = { S(  13,   7), S(  38,  13) };

const int RookOnSeventh = S(  -1,  37);

const int RookMobility[15] = {
    S(-159,-116), S( -61,-115), S( -22, -66), S( -10, -22),
    S(  -9,  -1), S( -10,  14), S(  -8,  27), S(   0,  30),
    S(  10,  35), S(  13,  41), S(  17,  48), S(  23,  52),
    S(  24,  56), S(  39,  42), S( 107,  -3),
};

const int QueenRelativePin = S( -23, -11);

const int QueenMobility[28] = {
    S( -61,-263), S(-213,-388), S( -60,-200), S( -23,-191),
    S(  -9,-144), S(  -8, -80), S(  -2, -44), S(  -2, -12),
    S(   6, -11), S(   9,  11), S(  15,  14), S(  17,  32),
    S(  19,  23), S(  22,  35), S(  19,  39), S(  16,  45),
    S(  17,  43), S(   6,  45), S(   8,  41), S(   7,  31),
    S(  16,   8), S(  30, -15), S(  31, -45), S(  30, -62),
    S(   8, -78), S(  19,-107), S( -55, -37), S( -32, -63),
};

const int KingDefenders[12] = {
    S( -28,  -5), S(  -6,   1), S(   1,   8), S(   9,   8),
    S(  18,   5), S(  29,   1), S(  29,  -4), S(  12,  -1),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  45,  31), S(  28,  22), S(   9,   7), S( -25, -17),
    S( -29, -48), S( -21, -62), S( -17, -66), S( -11, -64),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -9,   2), S(  15, -26), S(  28,  -9), S(  17,   6),
    S(  14,   3), S(  -1,  -1), S(  -3, -34), S( -52,  20)},
   {S(  16,  -6), S(  23, -19), S(   3,  -5), S( -15,   4),
    S( -30,  16), S( -73,  67), S(  91,  81), S( -18,   0)},
   {S(  36,  -3), S(  11,  -8), S( -31,   7), S( -13,  -8),
    S( -17,  -5), S( -12,   1), S(   0,  68), S( -14,  -1)},
   {S(   2,  13), S(  27, -13), S(   3, -12), S(  15, -22),
    S(  32, -38), S( -58,   3), S(-135,  51), S(   5,  -5)},
   {S( -18,  13), S(   3,  -1), S( -34,   4), S( -22,   8),
    S( -22,  -6), S( -45,  -5), S(  32, -18), S( -11,   3)},
   {S(  49, -16), S(  26, -16), S( -23,   4), S( -12, -20),
    S(   8, -25), S(  19, -21), S(  40, -32), S( -26,   2)},
   {S(  30, -14), S(  -2, -17), S( -28,  -2), S( -23,  -7),
    S( -31,  -5), S( -41,  30), S(   1,  44), S(  -8,  -1)},
   {S( -10, -13), S(   6, -21), S(  11,   2), S(   0,  12),
    S( -11,  16), S( -11,  36), S(-190,  89), S( -21,  15)}},
  {{S(   0,   0), S( -13, -26), S(   7, -23), S( -39,  20),
    S( -23,   1), S(   3,  42), S(-167,  -7), S( -43,  10)},
   {S(   0,   0), S(  27, -22), S(   8,  -6), S( -16,   0),
    S(   3, -18), S(  26,  65), S(-184,  -3), S( -30,   4)},
   {S(   0,   0), S(  27, -12), S(  -1, -10), S(   5, -20),
    S(  21,  -3), S( -87,  48), S( -84, -74), S( -19,  -2)},
   {S(   0,   0), S( -11,   9), S(  -7,  -2), S( -26,   5),
    S( -27,   4), S(-100,  30), S(   7, -40), S( -24,  -3)},
   {S(   0,   0), S(  14,   3), S(  13,  -7), S(  15, -13),
    S(  16, -31), S( -57,  14), S(-101, -60), S(  -4,  -2)},
   {S(   0,   0), S(  -2,  -5), S( -28,   2), S( -24,  -9),
    S(  23, -23), S( -37,   3), S(  55,  39), S( -17,  -3)},
   {S(   0,   0), S(  28, -18), S(  14, -16), S( -13,  -7),
    S( -29,  14), S(  -8,  19), S( -57, -50), S( -33,  13)},
   {S(   0,   0), S(  14, -43), S(  23, -33), S( -19,  -7),
    S( -20,  19), S(  -8,  21), S(-228, -56), S( -24,   2)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -13,  32), S( 112,  -6), S( -23,  23), S( -14,   1),
    S( -10,  -1), S(  -3,  -7), S( -13,   2), S( -26,   5)},
   {S(  -8,  53), S(  51,  14), S( -21,  20), S(  -2,   7),
    S(   1,   4), S(   9,  -5), S(  -2,   2), S( -12,   9)},
   {S(   3,  44), S(   4,  29), S( -23,  18), S( -12,   5),
    S(   3,   1), S(   7,   0), S(  14,  -6), S(   5,   8)},
   {S(  -7,  29), S(   4,  26), S( -23,   8), S( -18,  -1),
    S( -14,   1), S(   9, -12), S(   4,  -9), S( -15,  10)}},
  {{S(   0,   0), S( -18, -16), S( -18,  -4), S(  25, -21),
    S(  12,  -7), S(   6, -21), S(   5,   1), S(  16,  29)},
   {S(   0,   0), S( -17, -37), S(   2, -13), S(  46, -13),
    S(   3,  -4), S(  17, -25), S(  -6,  -5), S( -19,   0)},
   {S(   0,   0), S( -28, -52), S( -24,  -8), S(  16, -13),
    S(   5,  -2), S(  -8, -13), S( -10, -12), S( -11,   6)},
   {S(   0,   0), S(  -2, -19), S( -21, -17), S( -17,  -2),
    S(  -5,  -7), S(   1, -23), S(  73, -12), S(  15,  20)}},
};

const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -36,   1), S( -48,  25), S( -73,  30),
    S(   7,  10), S(  82, -15), S( 158,  41), S(   0,   0)},
   {S(   0,   0), S( -29,  13), S( -46,  37), S( -66,  40),
    S(   0,  38), S( 106,  24), S( 189,  89), S(   0,   0)}},
  {{S(   0,   0), S( -24,  25), S( -48,  32), S( -68,  45),
    S(  10,  44), S( 102,  35), S( 271,  98), S(   0,   0)},
   {S(   0,   0), S( -29,  24), S( -43,  34), S( -62,  53),
    S(  12,  62), S( 105, 118), S( 157, 265), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   3,   3), S(   6,  -4), S(   7, -14),
    S(   3, -17), S( -10,  -9), S( -17,   4), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   1,  -1), S(   2,   1), S(   7,  13),
    S(   1,  28), S(   7,  35), S(  28,  29), S(   0,   0),
};

const int PassedSafePromotionPath = S( -16,  51);

const int ThreatWeakPawn = S( -11, -38);

const int ThreatMinorAttackedByPawn = S( -54, -57);

const int ThreatMinorAttackedByMinor = S( -27, -40);

const int ThreatMinorAttackedByMajor = S( -27, -49);

const int ThreatRookAttackedByLesser = S( -52, -23);
...
const int SpaceCenterControl = S(   7,  -6);

const int ClosednessKnightAdjustment[9] = {
    S(  -7,  -5), S(  -4,  10), S(  -5,  17), S(   0,  17),
    S(   2,  21), S(   5,  14), S(   9,   6), S(  -6,  24),
    S(  -5,   9),
};

const int ClosednessRookAdjustment[9] = {
    S(  74,  11), S(   8,  38), S(   8,  21), S(   1,   3),
    S(  -7,   7), S(  -8, -11), S( -11, -12), S( -23, -16),
    S( -35, -21),
};

const int ComplexityTotalPawns = S(   0,  11);

const int ComplexityPawnFlanks = S(   0,  60);

const int ComplexityPawnEndgame = S(   0,  52);
