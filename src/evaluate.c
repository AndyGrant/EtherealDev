...
int evaluateBoard(Thread *thread, Board *board) {
    ...
    if (    USE_NNUE
        ...
        if (board->chess960) eval += evaluateFixFRC(board);
    }
...
int evaluateFixFRC(Board *board) {
    ...
    #define evaluate_fix_frc_helper(C, SQ1, SQ2) (                                \
          board->squares[relativeSquare(us, SQ1)] != EMPTY ? p1                   \
        : board->squares[relativeSquare(us, SQ2)] == makePiece(PAWN, C) ? p2 : p3 \
    )

    int adjustment = 0;

    if (   testBit(ours   & bishops, relativeSquare(us, SQ_A1))
        && testBit(ours   & pawns  , relativeSquare(us, SQ_B2)))
        adjustment -= evaluate_fix_frc_helper(us, SQ_B3, SQ_C3);

    if (   testBit(ours   & bishops, relativeSquare(us, SQ_H1))
        && testBit(ours   & pawns  , relativeSquare(us, SQ_G2)))
        adjustment -= evaluate_fix_frc_helper(us, SQ_G3, SQ_F3);

    if (   testBit(theirs & bishops, relativeSquare(us, SQ_A8))
        && testBit(theirs & pawns  , relativeSquare(us, SQ_B7)))
        adjustment += evaluate_fix_frc_helper(!us, SQ_B6, SQ_C6);

    if (   testBit(theirs & bishops, relativeSquare(us, SQ_H8))
