...
const int PawnValue   = S( 113, 132);
const int KnightValue = S( 471, 421);
const int BishopValue = S( 493, 440);
const int RookValue   = S( 694, 730);
const int QueenValue  = S(1314,1414);
const int KingValue   = S(   0,   0);

const int PieceValues[8][PHASE_NB] = {
    { 113, 132}, { 471, 421}, { 493, 440}, { 694, 730},
    {1314,1414}, {   0,   0}, {   0,   0}, {   0,   0},
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -27, -12), S( -15,   6), S(  -7,  30),
    S(   6,  63), S(  47,  68), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -13,  14), S(  -5,  21), S(   4,  43),
    S(  16,  85), S(  33,  52), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -8, -11);

const int PawnStacked = S( -20, -24);

const int PawnBackwards[2] = { S(   9,  -1), S(  -6, -14) };

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -2,  -7), S(  11,   0), S(   4,   0), S(   4,  18),
    S(  15,   0), S(  34,  -1), S(  23,  10), S(  26,  19),
    S(  10,   1), S(  23,   5), S(  10,  12), S(  15,  24),
    S(  16,   7), S(  24,  15), S(  31,  22), S(  34,  21),
    S(  56,  25), S(  51,  47), S(  69,  54), S(  83,  59),
...
const int KnightOutpost[2] = { S(   7, -24), S(  32,  -2) };

const int KnightBehindPawn = S(   4,  21);

const int KnightMobility[9] = {
    S( -80,-101), S( -31, -99), S( -15, -43), S(  -2, -17),
    S(   9,  -5), S(  16,  10), S(  25,  14), S(  35,  14),
    S(  46,   3),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  26,  71);

const int BishopRammedPawns = S( -10, -16);

const int BishopOutpost[2] = { S(  10, -10), S(  42,   1) };

const int BishopBehindPawn = S(   3,  20);

const int BishopMobility[14] = {
    S( -64,-146), S( -27, -94), S(  -7, -55), S(   3, -29),
    S(  12, -16), S(  20,  -1), S(  22,   8), S(  23,  14),
    S(  22,  19), S(  25,  21), S(  24,  21), S(  39,  11),
    S(  40,  14), S(  66, -15),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  18,   5), S(  39,   2) };

const int RookOnSeventh = S(   0,  31);

const int RookMobility[15] = {
    S(-148,-112), S( -51,-115), S( -11, -61), S(  -3, -19),
    S(  -4,   1), S(  -5,  16), S(  -3,  26), S(   2,  29),
    S(   9,  32), S(  13,  36), S(  15,  43), S(  20,  47),
    S(  20,  52), S(  28,  46), S(  70,  18),
};

/* Queen Evaluation Terms */

const int QueenMobility[28] = {
    S( -61,-263), S(-210,-386), S( -59,-201), S( -22,-191),
    S( -10,-140), S(  -5, -89), S(   0, -59), S(   0, -32),
    S(   2, -21), S(   3,   0), S(   5,  11), S(   6,  24),
    S(   8,  25), S(  10,  35), S(   8,  40), S(   7,  43),
    S(   5,  48), S(   0,  47), S(   1,  42), S(  -1,  36),
    S(   5,  15), S(  19,  -4), S(  21, -34), S(  23, -53),
    S(   6, -69), S(  20, -95), S( -56, -39), S( -27, -55),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -22,  -3), S(  -9,  -1), S(   0,   2), S(   7,   4),
    S(  17,   5), S(  27,   2), S(  32,   0), S(  13,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -12,   3), S(  16, -24), S(  19,  -9), S(  10,   2),
    S(   4,   3), S(   5,   2), S(  -2, -32), S( -50,  19)},
   {S(  15,  -6), S(  23, -18), S(   0,  -4), S( -16,   2),
    S( -30,  13), S( -69,  68), S(  92,  83), S( -29,   3)},
   {S(  32,  -2), S(  16,  -9), S( -27,   6), S( -11,  -8),
    S( -21,  -3), S(  -7,   0), S(   0,  66), S( -15,   0)},
   {S(   2,  11), S(  20,  -8), S(   3, -11), S(  15, -22),
    S(  23, -34), S( -52,   1), S(-136,  51), S(   3,  -5)},
   {S( -14,   7), S(   1,  -3), S( -26,   1), S( -18,   3),
    S( -20,  -6), S( -38,   0), S(  33, -15), S(  -8,  -1)},
   {S(  45, -15), S(  25, -18), S( -21,   0), S( -12, -19),
    S(   5, -24), S(  17, -22), S(  41, -29), S( -25,   1)},
   {S(  23, -10), S(   2, -17), S( -23,  -3), S( -19, -10),
    S( -29,  -8), S( -33,  31), S(   0,  44), S( -12,   3)},
   {S(  -9,  -9), S(   7, -18), S(   8,  -2), S(  -2,   8),
    S( -10,  13), S(  -9,  37), S(-190,  87), S( -19,  15)}},
  {{S(   0,   0), S( -10, -21), S(   0, -16), S( -41,  16),
    S( -18,   0), S(   2,  42), S(-166,  -8), S( -45,  10)},
   {S(   0,   0), S(  26, -20), S(   6,  -7), S( -18,  -2),
    S(   0, -14), S(  27,  70), S(-183,  -3), S( -42,   5)},
   {S(   0,   0), S(  35, -11), S(  -1,  -6), S(   9, -17),
    S(  13,  -7), S( -90,  47), S( -84, -74), S( -22,  -3)},
   {S(   0,   0), S(  -1,   9), S(  -1,   1), S( -15,   0),
    S( -26,   0), S(-100,  29), S(   7, -41), S( -24,  -4)},
   {S(   0,   0), S(  10,   3), S(  11,  -5), S(  13, -10),
    S(  13, -24), S( -53,  15), S(-104, -62), S(  -2,  -6)},
   {S(   0,   0), S(   9,  -7), S( -20,   0), S( -27,  -7),
    S(  17, -25), S( -38,   2), S(  55,  38), S( -21,  -3)},
   {S(   0,   0), S(  24, -13), S(  11, -12), S(  -9,  -9),
    S( -29,   5), S(  -8,  14), S( -56, -50), S( -35,  14)},
   {S(   0,   0), S(  14, -36), S(  19, -27), S( -17,  -4),
    S( -16,  15), S(  -1,  15), S(-228, -56), S( -22,   4)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S(   0,  24), S( 114, -10), S( -30,  23), S( -21,  10),
    S( -12,   3), S(  -7,  -2), S( -17,   7), S( -21,  -2)},
   {S(   0,  46), S(  57,  12), S( -20,  22), S(  -6,  12),
    S(  -2,   5), S(   5,  -3), S(   0,   0), S( -12,   0)},
   {S(  10,  35), S(  19,  22), S( -27,  19), S( -12,  10),
    S(   4,   3), S(   7,   0), S(   9,  -7), S(   2,  -1)},
   {S(   0,  23), S(  19,  19), S( -17,   7), S( -14,   2),
    S( -12,   2), S(   7,  -9), S(   1,  -6), S( -14,   2)}},
  {{S(   0,   0), S( -11, -13), S( -13,  -1), S(  21, -16),
    S(  12,  -7), S(   0, -17), S(  -7,   0), S(  16,  27)},
   {S(   0,   0), S( -13, -28), S(  -4,  -9), S(  33, -13),
    S(  -2,  -2), S(  10, -23), S( -10,  -8), S( -15,   2)},
   {S(   0,   0), S( -23, -43), S( -27,  -7), S(  10, -11),
    S(   2,  -2), S( -10, -15), S( -13, -13), S( -10,   5)},
   {S(   0,   0), S(  -1, -14), S( -16, -17), S( -11,  -3),
    S(  -3,  -7), S(   5, -29), S(  67,  -6), S(  12,  18)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -38,   0), S( -54,  21), S( -84,  25),
    S(  -7,  13), S(  64,  -1), S( 152,  58), S(   0,   0)},
   {S(   0,   0), S( -26,   1), S( -47,  19), S( -71,  24),
    S( -13,  24), S(  84,  25), S( 182,  93), S(   0,   0)}},
  {{S(   0,   0), S( -24,  10), S( -47,  16), S( -74,  30),
    S(  -3,  31), S(  87,  39), S( 259, 121), S(   0,   0)},
   {S(   0,   0), S( -27,   6), S( -40,  12), S( -65,  34),
    S(  -2,  56), S(  79, 138), S( 157, 302), S(   0,   0)}},
};

const int PassedFriendlyDistance[RANK_NB] = {
    S(   0,   0), S(   0,   0), S(   3,  -4), S(   8, -11),
    S(   5, -16), S(  -8, -16), S( -13, -10), S(   0,   0),
};

const int PassedEnemyDistance[RANK_NB] = {
    S(   0,   0), S(   3,   0), S(   4,   1), S(   8,  10),
    S(   1,  25), S(   9,  35), S(  25,  37), S(   0,   0),
};

const int PassedSafePromotionPath = S( -24,  35);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -14, -27);
