...
const int KingShelter[2][FILE_NB/2][5] = {
  {{S( -12,   0), S(  -3,  -3), S(   3,   1), S(   2,   3), S(  -6,   4)},
   {S(   5,   4), S(   1,  -2), S( -17,  -3), S( -15,  -5), S( -10,   0)},
   {S(   8,   6), S(  13,   1), S( -18,   0), S(  -5,  -6), S(  -7,   0)},
   {S( -10,  10), S(   5,   2), S(  -6,  -1), S(  11,  -5), S(  -2,   0)}},
  {{S(   0,   0), S(   0,  -5), S(   4,  -4), S(  -9,  -2), S(  -9,   3)},
   {S(   0,   0), S(   9,   1), S(  -4,   0), S( -18,  -3), S( -25,   1)},
   {S(   0,   0), S(   3,   4), S( -16,   0), S(  -3,  -7), S( -14,  -1)},
   {S(   0,   0), S(   0,   6), S(   5,   1), S(  12,   0), S(  -3,  -2)}},
};
...
int evaluateKings(EvalInfo* ei, Board* board, int colour){
    ...
    for (file = MAX(0, kingFile - 1); file <= MIN(7, kingFile + 1); file++){
        ...
        distance = filePawns ? 
                   colour == WHITE ? MIN(3, Rank(getlsb(filePawns)) - kingRank)
                                   : MIN(3, kingRank - Rank(getmsb(filePawns)))
                                   : 4;
