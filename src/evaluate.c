...
const int KnightValue = S( 464, 408);
const int BishopValue = S( 483, 428);
const int RookValue   = S( 665, 687);
const int QueenValue  = S(1301,1340);
...
const int PawnPSQT32[32] = {
    ...
    S( -21,   6), S(   2,   2), S(  -6,   3), S(  -3,  -3),
    S( -27,   4), S( -13,   4), S( -12,  -3), S(  -2, -13),
    S( -19,  11), S( -14,  13), S(   4,  -7), S(   5, -20),
    S( -12,  13), S(  -6,   9), S( -11,  -5), S(  -6, -18),
    S(  -8,  28), S(  -2,  24), S(   0,  16), S(  28, -12),
    S( -21, -63), S( -60, -31), S(   2, -45), S(  45, -61),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -49, -32), S(  -5, -25), S( -13, -29), S(   3, -14),
    S(   1, -20), S(   2, -11), S(   5, -27), S(  11, -17),
    S(  14, -27), S(  21, -21), S(  15, -15), S(  26,  -3),
    S(  20,   1), S(  26,   6), S(  32,  18), S(  36,  24),
    S(  25,  13), S(  33,   8), S(  43,  28), S(  37,  39),
    S( -19,  10), S(   1,  12), S(  31,  30), S(  33,  28),
    S(  13, -13), S(  -4,  -2), S(  28, -23), S(  40,  -1),
    S(-179, -26), S( -85,  -4), S(-111,  17), S( -34,  -5),
};

const int BishopPSQT32[32] = {
    S(   8, -27), S(  12, -18), S(  -6,  -9), S(  12, -11),
    S(  31, -36), S(  10, -35), S(  22, -21), S(  15, -11),
    S(  15, -13), S(  28, -15), S(   5, -17), S(  25,  -6),
    S(  15,  -7), S(  21,  -3), S(  17,   4), S(  28,   4),
    S( -10,  15), S(  23,   6), S(   4,  15), S(  16,  17),
    S(   3,   7), S(  -5,  18), S(   0,   7), S(  12,  15),
    S( -44,  15), S( -50,   6), S(  -8,   9), S( -23,  15),
    S( -59,  -8), S( -45,   5), S( -84,  14), S( -90,  28),
};

const int RookPSQT32[32] = {
    S( -14, -28), S( -15, -19), S(  -3, -21), S(   5, -26),
    S( -52, -15), S(  -8, -34), S(  -7, -31), S(   6, -37),
    S( -30, -15), S(  -5, -11), S( -15, -16), S(   3, -26),
    S( -18,  -3), S( -12,   7), S(  -6,   2), S(   9,  -3),
    S(  -5,  11), S(   9,   7), S(  21,   5), S(  43,   1),
    S( -17,  20), S(  28,   5), S(   6,  21), S(  40,   3),
    S(  -1,   5), S( -12,  12), S(  12,   3), S(  30,   6),
    S(  36,  22), S(  28,  23), S(   2,  32), S(  11,  29),
};

const int QueenPSQT32[32] = {
    S(  33, -67), S(   7, -46), S(  17, -60), S(  22, -42),
    S(  13, -43), S(  28, -58), S(  29, -70), S(  19, -27),
    S(  10, -31), S(  25, -20), S(   7,   7), S(   7,   1),
    S(  11,  -9), S(  17,  11), S(  -1,  24), S( -14,  65),
    S(  -2,  15), S(  -8,  48), S( -11,  22), S( -28,  75),
    S( -25,  33), S( -17,  20), S( -23,  25), S( -10,  27),
    S(  -1,  27), S( -68,  67), S(  -2,  11), S( -38,  52),
    S( -12,  17), S(  20,   8), S(   9,   9), S(  -5,  20),
};

const int KingPSQT32[32] = {
    S(  51, -79), S(  41, -52), S( -17, -10), S( -30, -21),
    S(  28, -18), S( -16,  -8), S( -46,  16), S( -69,  18),
    S(  -5, -18), S(   4, -15), S(  13,   5), S( -18,  22),
    S(   1, -30), S(  97, -32), S(  47,  10), S(  -3,  29),
    S(  11, -12), S( 109, -31), S(  57,  14), S(  13,  20),
    S(  50, -29), S( 135, -23), S( 105,   2), S(  43,  -3),
    S(   7, -61), S(  48, -10), S(  32,   3), S(   9,  -6),
    S(   4,-125), S(  73, -64), S( -23, -25), S( -20, -21),
};
...
const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -27, -10), S( -10,   9), S( -18,  33),
    S(   3,  61), S(  37,  65), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -16,  19), S(  -4,  21), S(   2,  47),
    S(  20,  88), S(  38,  63), S(   0,   0), S(   0,   0)},
};
...
const int PawnConnected32[32] = {
    ...
    S(   7,  -2), S(  24,  -1), S(   9,   6), S(  17,  11),
    S(   8,   9), S(  20,  15), S(  24,  21), S(  28,  18),
    S(  56,  18), S(  40,  43), S(  61,  55), S(  74,  61),
    S( 110, -11), S( 202,  -7), S( 226,  24), S( 235,  37),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightInSiberia[4] = {
    S(  -9,  -2), S( -13,  -7), S( -26,  -3), S( -26,  -6),
};

const int KnightMobility[9] = {
    S( -78,-106), S( -31,-104), S( -14, -43), S(  -3, -14),
...
const int BishopMobility[14] = {
    S( -71,-149), S( -36,-100), S( -12, -52), S(  -2, -26),
    S(  10, -17), S(  18,  -3), S(  21,   7), S(  22,  12),
    S(  20,  19), S(  26,  21), S(  27,  20), S(  50,   8),
    S(  47,  19), S(  75, -19),
};
...
const int RookMobility[15] = {
    S(-160,-116), S( -62,-116), S( -19, -67), S( -10, -22),
    S( -10,  -1), S( -10,  14), S(  -9,  27), S(  -2,  30),
    S(   6,  35), S(   9,  39), S(  13,  45), S(  21,  50),
    S(  22,  55), S(  42,  39), S( 112, -15),
};
...
const int QueenMobility[28] = {
    ...
    S(  -8,-144), S(  -6, -80), S(  -1, -43), S(  -1, -10),
    S(   6, -10), S(   8,  12), S(  14,  13), S(  15,  33),
    S(  18,  21), S(  21,  33), S(  18,  37), S(  15,  45),
    S(  19,  41), S(   7,  44), S(  10,  41), S(   9,  31),
    S(  17,   6), S(  30, -16), S(  30, -47), S(  29, -64),
...
const int KingDefenders[12] = {
    ...
    S(  17,   3), S(  28,   0), S(  27,  -4), S(  12,  -1),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  42,  34), S(  31,  22), S(  10,   8), S( -26, -18),
    S( -30, -52), S( -22, -64), S( -17, -66), S( -11, -63),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -9,   2), S(  13, -27), S(  29,  -9), S(  19,   7),
    S(  15,   3), S(  -1,  -1), S(  -3, -34), S( -53,  20)},
   {S(  16,  -6), S(  22, -20), S(   4,  -5), S( -16,   4),
    S( -30,  16), S( -73,  67), S(  91,  81), S( -17,   1)},
   {S(  36,  -3), S(   8,  -8), S( -32,   6), S( -14,  -9),
    S( -17,  -5), S( -12,   1), S(   0,  68), S( -13,   0)},
   {S(   1,  13), S(  27, -14), S(   5, -13), S(  15, -21),
    S(  33, -39), S( -58,   3), S(-135,  51), S(   7,  -6)},
   {S( -19,  12), S(   4,  -1), S( -35,   4), S( -21,   9),
    S( -22,  -6), S( -45,  -5), S(  32, -18), S( -10,   4)},
   {S(  50, -18), S(  20, -13), S( -21,   4), S( -10, -21),
    S(   8, -25), S(  19, -21), S(  40, -32), S( -23,   3)},
   {S(  32, -18), S(   0, -17), S( -27,   0), S( -23,  -7),
    S( -31,  -4), S( -41,  30), S(   1,  44), S( -10,   0)},
   {S(  -9, -14), S(   5, -20), S(   9,   3), S(   0,  12),
    S( -10,  16), S( -11,  37), S(-190,  89), S( -21,  15)}},
  {{S(   0,   0), S( -14, -27), S(   8, -23), S( -39,  20),
    S( -23,   1), S(   3,  42), S(-167,  -7), S( -43,  11)},
   {S(   0,   0), S(  26, -23), S(   8,  -6), S( -16,   0),
    S(   3, -18), S(  26,  65), S(-184,  -3), S( -29,   6)},
   {S(   0,   0), S(  24, -14), S(  -1, -11), S(   5, -21),
    S(  21,  -2), S( -87,  48), S( -84, -74), S( -18,   0)},
   {S(   0,   0), S( -12,   8), S(  -8,  -3), S( -27,   5),
    S( -27,   4), S(-100,  30), S(   7, -40), S( -23,  -1)},
   {S(   0,   0), S(  16,   1), S(  13,  -7), S(  16, -13),
    S(  17, -32), S( -57,  14), S(-101, -60), S(  -3,   0)},
   {S(   0,   0), S(   0,  -6), S( -29,   2), S( -22,  -9),
    S(  23, -23), S( -37,   3), S(  55,  39), S( -16,  -2)},
   {S(   0,   0), S(  24, -18), S(  13, -16), S( -12,  -8),
    S( -28,  15), S(  -8,  19), S( -57, -50), S( -33,  14)},
   {S(   0,   0), S(  13, -44), S(  24, -34), S( -19,  -8),
    S( -20,  19), S(  -8,  21), S(-228, -56), S( -25,   4)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -13,  33), S( 112,  -5), S( -23,  24), S( -11,   0),
    S( -12,   0), S(  -6,  -6), S( -14,   2), S( -26,   4)},
   {S(  -8,  53), S(  50,  15), S( -20,  20), S(  -1,   7),
    S(  -1,   4), S(   6,  -5), S(  -3,   4), S( -12,   6)},
   {S(   3,  44), S(   3,  30), S( -23,  18), S( -11,   6),
    S(   2,   1), S(   6,   0), S(  13,  -5), S(   6,   4)},
   {S(  -7,  29), S(   3,  26), S( -24,   8), S( -17,  -1),
    S( -12,   1), S(   9, -13), S(   4,  -9), S( -13,   6)}},
  {{S(   0,   0), S( -18, -16), S( -16,  -3), S(  24, -21),
    S(  10,  -7), S(   7, -21), S(   4,   2), S(  16,  29)},
   {S(   0,   0), S( -17, -37), S(   5, -12), S(  47, -13),
    S(   3,  -4), S(  17, -25), S(  -6,  -5), S( -19,   0)},
   {S(   0,   0), S( -28, -52), S( -23,  -6), S(  17, -13),
    S(   6,  -2), S(  -8, -13), S( -10, -12), S( -11,   6)},
   {S(   0,   0), S(  -2, -19), S( -20, -16), S( -17,  -1),
    S(  -3,  -7), S(   2, -24), S(  73, -12), S(  15,  20)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -37,   0), S( -47,  26), S( -71,  31),
    S(   8,   9), S(  81, -18), S( 157,  39), S(   0,   0)},
   {S(   0,   0), S( -29,  13), S( -46,  38), S( -65,  41),
    S(   1,  38), S( 108,  20), S( 190,  85), S(   0,   0)}},
  {{S(   0,   0), S( -24,  28), S( -47,  37), S( -67,  48),
    S(  11,  43), S( 103,  32), S( 271,  93), S(   0,   0)},
   {S(   0,   0), S( -29,  24), S( -42,  35), S( -62,  55),
    S(  12,  63), S( 105, 121), S( 157, 265), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   1,   2), S(   5,  -4), S(   7, -12),
    S(   4, -15), S( -10,  -9), S( -17,   2), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   3,  -1), S(   4,   0), S(   8,   8),
    S(   0,  21), S(   5,  28), S(  28,  22), S(   0,   0),
};

const int PassedSafePromotionPath = S( -17,  43);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -11, -28);
...
const int ThreatQueenAttackedByOne   = S( -39, -23);
...
const int ClosednessKnightAdjustment[9] = {
    S(  -8,  -7), S(  -4,   9), S(  -7,  17), S(  -3,  16),
    S(  -1,  19), S(   3,  12), S(   7,   5), S(  -6,  24),
    S(  -4,  10),
};

const int ClosednessRookAdjustment[9] = {
    S(  73, -15), S(   6,  37), S(   4,  19), S(  -2,   0),
