...
int evaluateKings(EvalInfo *ei, Board *board, int colour) {
    ...
    if (ei->kingAttackersCount[THEM] > 1 - popcount(enemyQueens)) {
        ...
        count += KSAttackValue     * scaledAttackCounts
               ...
               + KSSafeKnightCheck * popcount(knightChecks & safe)
               + KSSafeBishopCheck * popcount(bishopChecks & safe)
               + KSSafeRookCheck   * popcount(rookChecks   & ~ei->attackedBy2[US])
               + KSSafeQueenCheck  * popcount(queenChecks  & ~ei->attackedBy2[US])
               + KSAdjustment;
