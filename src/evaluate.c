...
void evaluatePieces(EvalInfo * ei, Board * board){
    ...
    ei->midgame[WHITE] += -12 * popcount(board->colours[WHITE] & ~ei->attacked[WHITE]);
    ei->endgame[WHITE] += -3  * popcount(board->colours[WHITE] & ~ei->attacked[WHITE]);
    ei->endgame[BLACK] += -12 * popcount(board->colours[BLACK] & ~ei->attacked[BLACK]);
    ei->midgame[BLACK] += -3  * popcount(board->colours[BLACK] & ~ei->attacked[BLACK]);
