...
const int PawnValue[PHASE_NB]   = { 100, 123};

const int KnightValue[PHASE_NB] = { 466, 395};

const int BishopValue[PHASE_NB] = { 473, 418};

const int RookValue[PHASE_NB]   = { 643, 720};

const int QueenValue[PHASE_NB]  = {1305,1346};

const int KingValue[PHASE_NB]   = { 168, 168};
...
const int PawnBackwards[2][PHASE_NB] = { {   6,  -3}, { -12, -12} };
...
const int PawnConnected32[32][PHASE_NB] = {
    ...
    {   7,   0}, {  21,   0}, {  15,   8}, {  17,  21},
    {   7,   0}, {  19,   4}, {  15,   8}, {  17,  17},
    {   5,  12}, {  19,  19}, {  17,  24}, {  37,  24},
    {  24,  54}, {  23,  62}, {  71,  62}, {  55,  76},
    {  78, -27}, { 164,   0}, { 223,  13}, { 191,  68},
    {   0,   0}, {   0,   0}, {   0,   0}, {   0,   0},
};


const int KnightAttackedByPawn[PHASE_NB] = { -49, -34};

const int KnightOutpost[2][PHASE_NB] = { {  19, -36}, {  37,   7} };
...
const int BishopPair[PHASE_NB] = {  42,  69};

const int BishopAttackedByPawn[PHASE_NB] = { -50, -38};

const int BishopOutpost[2][PHASE_NB] = { {  19, -17}, {  51, -11} };

const int BishopMobility[14][PHASE_NB] = {
    { -65,-129}, { -50, -68}, { -20, -47}, {  -6, -22},
    {   4,  -8}, {  17,   0}, {  22,   7}, {  29,   4},
    {  30,  10}, {  36,   3}, {  42,   4}, {  51, -15},
    {  47,  -1}, {  39, -38},
};


const int RookFile[2][PHASE_NB] = { {  13,   1}, {  40,  -9} };

const int RookOnSeventh[PHASE_NB] = {   1,  25};

const int RookMobility[15][PHASE_NB] = {
    {-166,-105}, { -76,-123}, { -16, -69}, {  -9, -27},
    {  -8,  -3}, {  -7,  14}, {  -7,  25}, {  -3,  32},
    {   0,  36}, {   6,  36}, {  10,  42}, {  19,  48},
    {  21,  50}, {  26,  47}, {  14,  45},
};


const int QueenChecked[PHASE_NB] = { -36, -32};

const int QueenCheckedByPawn[PHASE_NB] = { -53, -47};

const int QueenMobility[28][PHASE_NB] = {
    { -61,-263}, {-305,-329}, { -45,-206}, { -39,-197},
    { -16,-126}, { -27, -64}, { -17, -90}, { -19, -84},
    { -15, -60}, { -10, -54}, {  -8, -27}, {  -5, -29},
    {  -5, -15}, {   0, -10}, {   2,  -6}, {   0,   4},
    {   5,  17}, {   0,  15}, {  13,  26}, {   0,  26},
    {   6,  28}, {  20,  30}, {  22,  11}, {  35,  16},
    {  53,  23}, {  55,   1}, { -20,  -6}, {  17,  10},
};


const int KingDefenders[12][PHASE_NB] = {
    { -42,  -4}, { -22,   6}, {   0,   1}, {  10,   0},
    {  25,  -2}, {  37,   3}, {  40,  12}, {  30,-165},
    {  12,   6}, {  12,   6}, {  12,   6}, {  12,   6},
};

const int KingShelter[2][FILE_NB][RANK_NB][PHASE_NB] = {
  {{{ -14,  17}, {   7, -10}, {  17,   2}, {  22,   3}, {  13,   3}, {  24,  -5}, { -40, -58}, { -36,   1}},
   {{   4,   7}, {  16,  -6}, {  16,  -9}, {   0, -13}, { -31,  -3}, { -62,  81}, {  76, 101}, { -34,   0}},
   {{  13,  15}, {   9,  -1}, { -19,  -1}, {  -8,  -2}, { -30,  -5}, {   2, -16}, { -13,  67}, { -17,   2}},
   {{  10,  31}, {  15,   0}, {  -3,  -8}, {  20, -14}, {  16, -35}, { -26, -31}, { -73,  27}, {  -2,   0}},
   {{ -15,  19}, {   1,   3}, { -29,   1}, { -13,   4}, { -34, -17}, { -56, -26}, {  30,   0}, { -17,   0}},
   {{  23,   3}, {  18,  -4}, { -19,  -1}, {  -3, -21}, {   6, -33}, {  12, -51}, {  66, -39}, { -15,   0}},
   {{  20,   2}, {   3,  -9}, { -29,  -8}, { -22, -13}, { -25, -18}, { -31,   4}, {  17,  52}, { -31,   8}},
   {{ -17,  -3}, {   0,  -9}, {   5,   0}, {   1,   2}, { -14,  12}, {   3,  32}, {-190,  76}, { -20,  15}}},
  {{{   0,   0}, {   3, -17}, {  11, -14}, { -49,  18}, {  18,  -6}, {  -1,  40}, {-153,   9}, { -57,  10}},
   {{   0,   0}, {  17,  -6}, {   9,  -6}, {  -1,  -8}, {   6, -33}, {  33,  90}, {-166,  21}, { -48,   3}},
   {{   0,   0}, {  24,   0}, {   1,  -6}, {  19, -26}, {  17,  -7}, {-119,  49}, {-162, -99}, { -23,  -1}},
   {{   0,   0}, {  -6,   8}, { -11,  14}, { -17,   3}, { -31,   1}, {-117,   7}, {  14, -58}, { -27,   1}},
   {{   0,   0}, {   5,   5}, {   9,  -6}, {  23,  -5}, {   7, -22}, { -51,   9}, {-115, -90}, {  -6,  -2}},
   {{   0,   0}, {  10,   2}, { -15,  -2}, { -16, -13}, {  21, -35}, { -11,  12}, {  24,  18}, { -33,   0}},
   {{   0,   0}, {  13,  -1}, {  -1,   0}, { -21,  -5}, { -22, -14}, {  17,   3}, { -79, -53}, { -47,  13}},
   {{   0,   0}, {   8, -28}, {  10, -15}, { -24,   0}, { -28,  -5}, {  12, -19}, {-241, -81}, { -47,  16}}},
};


const int PassedPawn[2][2][RANK_NB][PHASE_NB] = {
  {{{   0,   0}, { -31, -30}, { -24,   8}, { -13,  -2}, {  20,  -1}, {  58,  -6}, { 158,  34}, {   0,   0}},
   {{   0,   0}, {  -1,   3}, { -16,  23}, { -13,  36}, {   5,  45}, {  71,  63}, { 196, 131}, {   0,   0}}},
  {{{   0,   0}, {  -7,  14}, { -11,   8}, {  -9,  27}, {  26,  33}, {  78,  64}, { 222, 151}, {   0,   0}},
   {{   0,   0}, {  -7,   9}, { -12,  19}, { -19,  54}, { -10, 111}, {  27, 205}, { 115, 372}, {   0,   0}}},
};
