...
const int PawnValue   = S( 100, 116);
const int KnightValue = S( 444, 411);
const int BishopValue = S( 465, 423);
const int RookValue   = S( 647, 678);
const int QueenValue  = S(1284,1361);
const int KingValue   = S(   0,   0);

const int PawnPSQT32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S( -19,   4), S(   3,   2), S(  -8,   3), S(  -5,  -1),
    S( -22,   3), S( -13,   3), S(  -9,  -5), S(  -2, -14),
    S( -18,   8), S( -14,  10), S(   8, -11), S(   8, -24),
    S( -12,   8), S(  -9,   5), S( -11, -10), S(  -5, -24),
    S( -10,  24), S(  -6,  20), S(  -1,   9), S(  27, -20),
    S( -19, -49), S( -67, -17), S(   1, -32), S(  38, -45),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -49, -26), S(  -8, -29), S( -13, -27), S(   1, -16),
    S(   0, -20), S(   3, -12), S(   5, -28), S(   8, -17),
    S(  12, -25), S(  21, -22), S(  14, -17), S(  22,  -6),
    S(  19,   4), S(  22,   6), S(  29,  14), S(  29,  21),
    S(  25,  16), S(  28,   9), S(  39,  24), S(  30,  35),
    S( -14,  14), S(   5,  12), S(  32,  25), S(  28,  26),
    S(   9, -11), S(  -8,   0), S(  30, -22), S(  42,  -1),
    S(-172, -19), S( -81,  -2), S(-110,  19), S( -31,  -1),
};

const int BishopPSQT32[32] = {
    S(   3, -26), S(  11, -18), S(  -9,  -7), S(  10,  -9),
    S(  29, -34), S(   5, -35), S(  20, -18), S(  13, -10),
    S(  15, -11), S(  28, -15), S(   0, -20), S(  24,  -6),
    S(  14,  -8), S(  20,  -3), S(  17,   2), S(  27,   5),
    S( -11,  13), S(  20,   5), S(   6,  13), S(  14,  16),
    S(   2,   7), S(  -2,  17), S(  -5,   3), S(  17,  12),
    S( -44,  17), S( -56,   4), S(  -8,   7), S( -21,  12),
    S( -52,  -7), S( -46,   9), S( -87,  16), S( -90,  28),
};

const int RookPSQT32[32] = {
    S( -13, -27), S( -16, -18), S(  -3, -22), S(   6, -27),
    S( -54, -13), S( -11, -32), S(  -7, -31), S(   4, -34),
    S( -28, -15), S(  -6, -11), S( -16, -16), S(   2, -24),
    S( -18,  -4), S( -10,   6), S(  -7,   1), S(   6,  -4),
    S(  -5,   6), S(  11,   4), S(  21,   1), S(  39,  -3),
    S( -16,  17), S(  25,   3), S(   1,  19), S(  35,   0),
    S(  -1,   4), S( -18,  15), S(   7,   4), S(  23,   6),
    S(  35,  21), S(  25,  23), S(   7,  30), S(  18,  27),
};

const int QueenPSQT32[32] = {
    S(  28, -53), S(   8, -40), S(  14, -51), S(  20, -38),
    S(  14, -39), S(  27, -56), S(  30, -71), S(  18, -25),
    S(  10, -24), S(  25, -22), S(   6,   2), S(   7,  -4),
    S(  11,  -5), S(  14,  14), S(  -2,  17), S( -15,  57),
    S(  -7,  17), S(  -7,  40), S( -16,  19), S( -31,  68),
    S( -25,  27), S( -18,  18), S( -25,  20), S( -14,  22),
    S(  -6,  26), S( -62,  62), S(  -9,  13), S( -43,  51),
    S( -10,  15), S(  16,   7), S(   7,   6), S(  -7,  18),
};

const int KingPSQT32[32] = {
    S(  45, -78), S(  43, -50), S( -12, -11), S( -26, -21),
    S(  32, -23), S(  -4, -14), S( -38,  11), S( -54,  13),
    S(   8, -26), S(  15, -21), S(  17,   1), S( -10,  17),
    S(   2, -35), S(  85, -34), S(  43,   6), S(  -5,  26),
    S(   4, -19), S(  96, -32), S(  48,  11), S(   3,  19),
    S(  45, -28), S( 120, -23), S(  94,   0), S(  37,  -3),
    S(   4, -51), S(  46,  -8), S(  31,   5), S(   8,  -4),
    S(   8,-103), S(  74, -56), S( -20, -16), S( -19, -13),
};

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -26, -10), S( -12,   9), S( -18,  28),
    S(  -1,  59), S(  40,  65), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -13,  19), S(  -4,  20), S(   3,  43),
    S(  17,  80), S(  32,  54), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -6, -10);

const int PawnStacked[2] = { S(  -6, -21), S(  -7,  -8) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   0,  -9), S(   9,  -6), S(   8, -14),
    S(   9, -14), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -12, -33), S(  -6, -26), S(   6, -28),
    S(  17, -24), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -2,  -7), S(  12,   0), S(   2,   0), S(   4,  12),
    S(  13,   0), S(  25,  -3), S(  21,   3), S(  22,  10),
    S(   8,  -2), S(  24,   1), S(  10,   7), S(  16,  13),
    S(   9,   9), S(  20,  13), S(  25,  20), S(  30,  18),
    S(  55,  21), S(  45,  43), S(  64,  50), S(  80,  55),
...
const int KnightInSiberia[4] = {
    S( -11,  -1), S( -14,  -7), S( -27,  -6), S( -21,  -3),
};

const int KnightMobility[9] = {
    S( -74,-104), S( -33, -96), S( -19, -38), S(  -8, -14),
    S(   3,  -6), S(   8,   8), S(  16,  12), S(  25,  12),
    S(  38,  -5),
};

const int BishopPair = S(  20,  68);
...
const int BishopLongDiagonal = S(  29,  12);

const int BishopMobility[14] = {
    S( -67,-147), S( -34, -95), S( -14, -53), S(  -5, -27),
    S(   5, -17), S(  13,  -4), S(  16,   6), S(  17,  11),
    S(  16,  17), S(  20,  18), S(  21,  18), S(  43,   6),
    S(  43,  18), S(  70, -18),
};

const int RookFile[2] = { S(  12,   3), S(  32,   4) };

const int RookOnSeventh = S(  -4,  29);

const int RookMobility[15] = {
    S(-149,-113), S( -57,-113), S( -20, -58), S( -11, -20),
    S( -11,  -2), S( -11,  12), S( -11,  23), S(  -4,  26),
    S(   3,  29), S(   6,  33), S(   9,  39), S(  15,  42),
    S(  16,  47), S(  32,  34), S(  92, -15),
};

const int QueenMobility[28] = {
    S( -61,-263), S(-210,-387), S( -58,-201), S( -20,-192),
    S(  -7,-139), S(  -4, -82), S(   2, -49), S(   2, -18),
    S(   7, -14), S(   8,   5), S(  12,   9), S(  13,  25),
    S(  15,  17), S(  17,  26), S(  14,  30), S(  10,  36),
    S(  10,  36), S(   0,  40), S(   3,  37), S(  -3,  32),
    S(   6,   8), S(  19, -15), S(  23, -43), S(  26, -59),
    S(   9, -73), S(  22,-100), S( -56, -39), S( -31, -62),
};

const int KingDefenders[12] = {
    S( -25,   1), S(  -5,  -4), S(   2,   2), S(   8,   6),
    S(  16,   6), S(  26,   3), S(  29,  -2), S(  13,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  39,  34), S(  21,  26), S(   5,  16), S( -18, -11),
    S( -21, -49), S( -19, -69), S( -17, -74), S( -12, -72),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -11,   1), S(  13, -27), S(  23,  -9), S(  15,   5),
    S(   8,   5), S(   1,   2), S(  -3, -33), S( -51,  18)},
   {S(  17,  -9), S(  18, -17), S(   3,  -6), S( -14,   4),
    S( -30,  15), S( -71,  68), S(  92,  82), S( -21,   0)},
   {S(  35,  -5), S(  12,  -8), S( -29,   6), S( -11,  -9),
    S( -19,  -4), S( -12,   1), S(   0,  66), S( -12,  -2)},
   {S(   3,  10), S(  23, -13), S(   4, -11), S(  14, -21),
    S(  27, -36), S( -60,   4), S(-136,  52), S(   7,  -7)},
   {S( -17,   8), S(   4,  -4), S( -29,   1), S( -19,   6),
    S( -20,  -6), S( -42,  -2), S(  33, -17), S(  -7,   0)},
   {S(  47, -18), S(  19, -15), S( -21,   3), S( -12, -17),
    S(   5, -24), S(  17, -20), S(  41, -30), S( -24,   2)},
   {S(  27, -15), S(  -3, -15), S( -24,  -1), S( -20,  -6),
    S( -29,  -6), S( -37,  32), S(   0,  44), S(  -9,  -1)},
   {S(  -9, -14), S(   5, -19), S(   7,   2), S(  -1,  13),
    S( -12,  16), S(  -8,  38), S(-190,  87), S( -18,  14)}},
  {{S(   0,   0), S( -11, -24), S(   7, -19), S( -40,  16),
    S( -22,   2), S(   3,  42), S(-167,  -8), S( -45,   9)},
   {S(   0,   0), S(  23, -22), S(   8,  -7), S( -18,   0),
    S(  -1, -12), S(  26,  66), S(-184,  -3), S( -35,   4)},
   {S(   0,   0), S(  26, -13), S(  -1, -10), S(   5, -19),
    S(  17,  -6), S( -87,  47), S( -84, -73), S( -17,  -2)},
   {S(   0,   0), S(  -7,   6), S(  -5,  -3), S( -21,   2),
    S( -28,   1), S( -99,  31), S(   7, -41), S( -18,  -2)},
   {S(   0,   0), S(  13,   0), S(  10,  -6), S(  13, -13),
    S(  15, -27), S( -57,  15), S(-104, -61), S(   0,  -2)},
   {S(   0,   0), S(   2,  -7), S( -23,   0), S( -25,  -6),
    S(  17, -24), S( -38,   3), S(  55,  38), S( -15,  -4)},
   {S(   0,   0), S(  21, -15), S(  11, -14), S( -10,  -6),
    S( -27,  11), S(  -9,  16), S( -56, -49), S( -31,  12)},
   {S(   0,   0), S(  12, -41), S(  22, -29), S( -18,  -4),
    S( -18,  18), S(  -5,  21), S(-228, -55), S( -24,   2)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S(  -4,  30), S( 116,  -9), S( -24,  24), S( -15,   4),
    S( -13,   1), S(  -8,  -4), S( -16,   5), S( -25,   2)},
   {S(  -3,  52), S(  56,   8), S( -20,  21), S(  -3,   9),
    S(  -3,   4), S(   5,  -5), S(  -3,   2), S( -12,   4)},
   {S(   8,  40), S(  16,  20), S( -22,  18), S( -11,   6),
    S(   1,   2), S(   4,   1), S(   8,  -4), S(   2,   3)},
   {S(  -2,  27), S(  13,  17), S( -20,   5), S( -16,  -1),
    S( -14,   0), S(   7, -11), S(   1,  -7), S( -13,   5)}},
  {{S(   0,   0), S( -16, -16), S( -18,  -1), S(  19, -18),
    S(   8,  -5), S(   7, -17), S(  -1,   1), S(  17,  29)},
   {S(   0,   0), S( -16, -35), S(  -1,  -8), S(  40, -11),
    S(   0,  -2), S(  14, -21), S(  -7,  -9), S( -17,   2)},
   {S(   0,   0), S( -28, -50), S( -25,  -6), S(  12, -11),
    S(   3,  -1), S(  -8, -13), S( -13, -14), S( -11,   6)},
   {S(   0,   0), S(  -2, -18), S( -16, -17), S( -14,  -3),
    S(  -4,  -5), S(   3, -24), S(  71, -12), S(  13,  20)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -37,   4), S( -50,  23), S( -78,  31),
    S(  -2,  13), S(  71,  -9), S( 156,  51), S(   0,   0)},
   {S(   0,   0), S( -30,  11), S( -50,  33), S( -72,  39),
    S( -10,  37), S(  92,  30), S( 184,  93), S(   0,   0)}},
  {{S(   0,   0), S( -23,  20), S( -48,  29), S( -71,  42),
    S(   0,  41), S(  89,  38), S( 261, 102), S(   0,   0)},
   {S(   0,   0), S( -29,  17), S( -45,  27), S( -67,  46),
    S(  -1,  55), S(  93, 113), S( 161, 270), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   0,   1), S(   3,  -3), S(   5,  -9),
    S(   6, -12), S(  -9,  -9), S( -15,  -5), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   3,   0), S(   5,   0), S(  10,   5),
    S(   1,  17), S(   5,  25), S(  24,  20), S(   0,   0),
};

const int PassedSafePromotionPath = S( -25,  37);

const int ThreatWeakPawn = S( -11, -25);
...
const int ThreatMinorAttackedByKing = S( -22, -21);

const int ThreatRookAttackedByKing = S( -17, -26);

const int ThreatQueenAttackedByOne = S( -35, -38);
...
const int SpaceCenterControl = S(   5,  -4);

const int ClosednessKnightAdjustment[9] = {
    S( -15, -20), S( -13,   2), S( -13,  13), S(  -6,  15),
    S(  -3,  22), S(   0,  19), S(   5,  15), S(  -9,  31),
    S(  -9,  18),
};

const int ClosednessRookAdjustment[9] = {
    S(  64, -19), S(   8,  28), S(  -1,  13), S( -10,  -1),
    S( -18,  -2), S( -21, -14), S( -22, -16), S( -27, -19),
    S( -31, -19),
};

const int ComplexityTotalPawns = S(   0,  11);

const int ComplexityPawnFlanks = S(   0,  96);

const int ComplexityPawnEndgame = S(   0,  70);

const int ComplexityAdjustment = S(   0,-188);
