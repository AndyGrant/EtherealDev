...
const int PawnValue   = S(  92, 131);
const int KnightValue = S( 427, 451);
const int BishopValue = S( 440, 469);
const int RookValue   = S( 616, 751);
const int QueenValue  = S(1306,1444);
...
const int PawnPSQT32[32] = {
    ...
    S( -15,   5), S(  -3,  -2), S(  -9,   4), S(  -6,  -1),
    S( -21,   2), S( -15,   2), S( -13,  -5), S(  -6, -12),
    S( -15,   8), S( -11,   3), S(  -1,  -9), S(   4, -22),
    S(  -9,  11), S(  -9,   4), S( -13, -11), S( -10, -23),
    S( -17,  28), S(   4,  26), S(   1,  10), S(  14,  -6),
    S( -16, -57), S( -51,  -8), S(   1, -36), S(  42, -42),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -33, -35), S(   1, -38), S(  -7, -36), S(   1, -16),
    S(   5, -16), S(  -3, -10), S(   4, -29), S(   9, -15),
    S(  11, -31), S(  15, -19), S(  14, -13), S(  20,  -1),
    S(  18,   6), S(  20,  11), S(  27,  19), S(  29,  27),
    S(  16,  12), S(  24,  12), S(  37,  28), S(  28,  41),
    S( -17,  11), S(   9,  12), S(  28,  30), S(  27,  32),
    S(   8, -14), S(  -4,  -6), S(  32, -19), S(  36,   4),
    S(-168, -23), S( -80,   7), S(-108,  23), S( -39,   9),
};

const int BishopPSQT32[32] = {
    S(   9, -33), S(   8, -16), S(   1, -18), S(   6,  -9),
    S(  20, -33), S(   9, -47), S(  16, -16), S(  12,  -8),
    S(  14,  -8), S(  20,  -6), S(   2, -23), S(  19,   0),
    S(   3,  -3), S(  16,  -2), S(  18,  11), S(  22,  14),
    S( -13,  16), S(  16,  12), S(   6,  18), S(  15,  25),
    S(  -2,   8), S(   0,  23), S(   0,   6), S(  13,  22),
    S( -44,   9), S( -42,  -3), S(  -7,  15), S( -23,  10),
    S( -51, -15), S( -38,   4), S( -92,  17), S( -80,  30),
};

const int RookPSQT32[32] = {
    S( -15, -26), S( -10, -19), S(  -5, -16), S(   0, -23),
    S( -64, -18), S( -11, -35), S( -11, -29), S(  -6, -28),
    S( -25, -23), S(  -7,  -9), S( -16, -15), S(  -4, -19),
    S( -22,  -1), S(  -8,   8), S( -10,   5), S(   0,  -3),
    S(  -3,  14), S(  14,   7), S(  18,   9), S(  37,   1),
    S( -13,  23), S(  31,   8), S(  16,  17), S(  36,   6),
    S( -12,  12), S( -15,  17), S(   3,   9), S(  16,   8),
    S(  38,  23), S(  30,  30), S(  16,  33), S(  25,  31),
};

const int QueenPSQT32[32] = {
    S(  25, -53), S(   9, -48), S(  15, -62), S(  21, -43),
    S(  10, -42), S(  21, -51), S(  25, -67), S(  18, -26),
    S(  11, -31), S(  23, -22), S(   8,   1), S(   6,  -4),
    S(   9,   4), S(  16,  19), S(   3,  16), S( -12,  62),
    S(  -4,  15), S(  -7,  54), S( -14,  31), S( -24,  83),
    S( -21,  25), S( -16,  21), S( -18,  25), S( -10,  38),
    S(  -7,  31), S( -56,  69), S(  -9,  19), S( -41,  55),
    S(   6,  16), S(  18,  25), S(   7,  24), S(   5,  34),
};

const int KingPSQT32[32] = {
    S(  54, -71), S(  54, -45), S(  -4, -10), S( -17, -26),
    S(  29, -16), S(   2, -13), S( -26,   8), S( -56,  14),
    S(  -1, -24), S(  23, -22), S(  27,   0), S(   4,  17),
    S(  -6, -34), S(  80, -26), S(  55,   5), S(   7,  27),
    S(   3, -11), S(  97, -22), S(  43,  14), S(  -5,  22),
    S(  41, -32), S( 110,  -5), S(  84,   5), S(  22,   1),
    S(   5, -61), S(  48, -10), S(  29,   5), S(  -1,  -5),
    S(   1,-124), S(  68, -75), S( -26, -24), S( -24, -21),
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -32,  -6), S( -10,   5), S( -17,  25),
    S(  -6,  57), S(  27,  69), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -18,  15), S(  -3,  24), S(   1,  48),
    S(  29, 105), S(  35,  59), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -5, -15);

const int PawnStacked[2] = { S(  -4, -17), S(  -5,  -6) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   4,  -4), S(  10,  -8), S(  10, -16),
    S(  14, -36), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -13, -27), S(  -4, -28), S(   6, -33),
    S(  45, -56), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -4,  -5), S(  10,  -2), S(   2,   3), S(   7,  11),
    S(   9,   2), S(  21,  -4), S(  23,   4), S(  23,  10),
    S(   6,  -1), S(  16,   3), S(   9,   5), S(  14,  13),
    S(   2,  15), S(  16,  17), S(  26,  25), S(  30,  17),
    S(  40,  40), S(  61,  62), S(  71,  80), S(  79,  92),
    S( 117,   9), S( 208,  25), S( 227,  41), S( 239,  58),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightBehindPawn = S(   2,  21);

const int KnightInSiberia[4] = {
    S(  -6,  -6), S(  -8, -18), S( -19, -17), S( -34, -21),
};

const int KnightMobility[9] = {
    S( -65,-107), S( -34,-112), S( -17, -45), S(  -6,  -8),
    S(   5,   4), S(   8,  19), S(  14,  23), S(  22,  22),
    S(  34,   7),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  20,  70);

const int BishopRammedPawns = S(  -6, -16);
...
const int BishopLongDiagonal = S(  19,  21);

const int BishopMobility[14] = {
    S( -72,-199), S( -27,-120), S(  -9, -59), S(  -2, -21),
    S(   7,  -7), S(  12,   9), S(  15,  20), S(  16,  22),
    S(  14,  31), S(  23,  29), S(  22,  31), S(  43,  20),
    S(  48,  31), S(  60,  -5),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(   6,   8), S(  29,   7) };

const int RookOnSeventh = S(   9,  31);

const int RookMobility[15] = {
    S(-133,-113), S( -50,-111), S( -16, -72), S(  -7, -27),
    S(  -8,   0), S( -11,  19), S( -10,  31), S(  -4,  35),
    S(   3,  39), S(   8,  41), S(  10,  48), S(  18,  50),
    S(  18,  54), S(  33,  45), S(  52,  22),
};
...
const int QueenMobility[28] = {
    S( -79,-265), S(-220,-389), S( -65,-226), S( -18,-227),
    S(  -7,-161), S(  -3, -84), S(   0, -38), S(   1,  -6),
    S(   4,   6), S(   6,  22), S(  10,  29), S(  12,  45),
    S(  16,  35), S(  18,  44), S(  15,  48), S(  15,  50),
    S(  17,  50), S(  11,  50), S(  11,  45), S(  18,  32),
    S(  24,   7), S(  39,  -6), S(  33, -18), S(  25, -55),
    S(  13, -69), S(  18,-105), S( -54, -37), S( -33, -64),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -19,  -3), S( -13,   3), S(   2,   5), S(  11,   6),
    S(  18,   7), S(  27,   1), S(  28,  -4), S(  10,  -2),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  47,  39), S(  30,  26), S(  20,  13), S( -31, -21),
    S( -31, -57), S( -23, -66), S( -19, -65), S( -13, -67),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -3,  -2), S(  13, -23), S(  16,  -5), S(  12,   6),
    S(   3,   8), S(  -9,   0), S(   2, -35), S( -44,  18)},
   {S(  13,  -6), S(  16, -15), S(   0,  -1), S(  -5,   0),
    S( -17,  13), S( -70,  59), S(  87,  76), S( -26,   2)},
   {S(  30,  -2), S(   8,  -3), S( -23,   3), S(  -9,  -9),
    S( -14,  -6), S(  -3,  -6), S(   4,  67), S( -14,  -1)},
   {S(  11,   9), S(  21, -14), S(   2, -14), S(   6, -18),
    S(  16, -33), S( -65,   7), S(-137,  45), S(  -2,  -2)},
   {S(  -6,  12), S(   8,  -3), S( -24,   2), S( -13,  -3),
    S(  -4, -13), S( -43, -13), S(  34, -14), S(  -5,   2)},
   {S(  36, -11), S(  13,  -8), S( -18,   2), S(  -7, -25),
    S(   3, -29), S(  13,  -7), S(  41, -29), S( -21,   0)},
   {S(  29, -15), S(   7, -20), S( -24,   1), S( -15,  -7),
    S( -37,   4), S( -47,  23), S(   2,  45), S( -16,   0)},
   {S(   4, -17), S(   3, -19), S(   4,   2), S(  -4,  11),
    S( -15,  19), S( -13,  46), S(-184,  95), S( -18,  14)}},
  {{S(   0,   0), S( -13, -28), S(   3, -17), S( -31,   1),
    S( -30,   2), S(   7,  40), S(-165,  -6), S( -35,  13)},
   {S(   0,   0), S(  12, -17), S(   4,  -5), S( -18,   1),
    S(  -2, -27), S(  36,  65), S(-181,  -2), S( -35,  10)},
   {S(   0,   0), S(  17,  -8), S(   1,  -9), S(   5, -20),
    S(  15,  -4), S( -88,  41), S( -84, -74), S( -15,   1)},
   {S(   0,   0), S(  -2,   2), S(  -6,  -7), S( -21,   5),
    S(  -8,  -1), S(-100,  27), S(   6, -40), S( -18,  -1)},
   {S(   0,   0), S(  11,   0), S(  12, -11), S(   7, -11),
    S(   5, -25), S( -54,  13), S(-102, -62), S(  -8,   2)},
   {S(   0,   0), S(   0,  -8), S( -18,  -1), S( -10, -15),
    S(  20, -27), S( -40,   6), S(  56,  41), S( -10,  -4)},
   {S(   0,   0), S(  23, -15), S(  13, -10), S( -10, -11),
    S( -25,  19), S(  -7,  22), S( -56, -46), S( -31,  12)},
   {S(   0,   0), S(  14, -40), S(  15, -25), S( -15, -10),
    S( -33,  19), S( -20,  18), S(-227, -56), S( -25,   8)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -10,  27), S( 105,  -3), S( -22,  17), S( -11,   4),
    S( -16,  -1), S( -11,  -2), S( -15,   6), S( -24,  -1)},
   {S(   4,  52), S(  63,  16), S( -14,  17), S(  -9,  13),
    S(  -9,  10), S(   1,  -1), S(  -3,   3), S( -16,  10)},
   {S(  16,  39), S(  16,  22), S( -16,  13), S( -14,   7),
    S(  -4,   4), S(   5,   2), S(   6,  -8), S(   3,   4)},
   {S(  12,  21), S(  21,  14), S( -27,   6), S( -19,  -4),
    S( -10,  -3), S(  10, -12), S(   5, -10), S(  -9,   3)}},
  {{S(   0,   0), S( -18, -16), S( -15,   0), S(  18, -13),
    S(   2,  -1), S(   2, -13), S(  -3,  -5), S(  29,  35)},
   {S(   0,   0), S( -17, -36), S(  -8,  -3), S(  34,  -7),
    S(   0,  -5), S(  16, -33), S(   9, -21), S( -19,   0)},
   {S(   0,   0), S( -26, -49), S( -14, -13), S(   7,  -8),
    S(   5,   0), S(  -8, -15), S(  -9,  -6), S(  -8,   8)},
   {S(   0,   0), S(  -1, -18), S( -25, -12), S( -14,   3),
    S(   4,  -9), S(  -1, -20), S(  68, -20), S(  15,  17)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -30,  -7), S( -37,  18), S( -61,  24),
    S(  11,  11), S(  81, -17), S( 147,  59), S(   0,   0)},
   {S(   0,   0), S( -25,  10), S( -35,  36), S( -45,  30),
    S(   5,  41), S( 113,  33), S( 188, 101), S(   0,   0)}},
  {{S(   0,   0), S( -21,  25), S( -41,  32), S( -54,  41),
    S(  10,  53), S( 102,  60), S( 260, 122), S(   0,   0)},
   {S(   0,   0), S( -20,  17), S( -34,  33), S( -48,  49),
    S(  14,  70), S( 123, 129), S( 188, 257), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(  -2,   1), S(   3,  -5), S(   8, -12),
    S(   7, -18), S(  -7, -18), S( -14, -12), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   3,   1), S(   4,   1), S(   5,  11),
    S(  -1,  25), S(   3,  37), S(  22,  40), S(   0,   0),
};

const int PassedSafePromotionPath = S(   0,  42);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -13, -31);
const int ThreatMinorAttackedByPawn  = S( -47, -65);
const int ThreatMinorAttackedByMinor = S( -20, -36);
const int ThreatMinorAttackedByMajor = S( -28, -46);
const int ThreatRookAttackedByLesser = S( -45, -21);
const int ThreatMinorAttackedByKing  = S( -36, -20);
const int ThreatRookAttackedByKing   = S( -57,  -6);
const int ThreatQueenAttackedByOne   = S( -45, -17);
...
const int ClosednessKnightAdjustment[9] = {
    S( -12,   0), S(  -9,  15), S( -10,  19), S(  -6,  20),
    S(  -1,  24), S(  -1,  16), S(   4,  12), S( -11,  31),
    S(  -5,  13),
};

const int ClosednessRookAdjustment[9] = {
    S(  60,   4), S(   3,  45), S(   6,  22), S(  -6,  17),
    S( -12,  10), S( -17,   6), S( -19, -15), S( -29, -12),
    S( -46, -26),
};
