...
const int PawnValue   = S(  85, 139);
const int KnightValue = S( 415, 463);
const int BishopValue = S( 426, 488);
const int RookValue   = S( 596, 769);
const int QueenValue  = S(1318,1428);
...
const int PawnPSQT32[32] = {
    ...
    S( -15,   6), S(  -1,   0), S(  -7,   4), S(  -6,   0),
    S( -21,   5), S( -14,   2), S( -10,  -6), S(  -4, -11),
    S( -15,   9), S( -11,   6), S(   2, -10), S(   4, -23),
    S( -11,  11), S(  -8,   3), S( -13, -11), S(  -9, -24),
    S( -13,  32), S(  -8,  27), S(  -5,  16), S(   8,  -5),
    S( -18, -59), S( -52, -18), S(   2, -35), S(  42, -47),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -35, -35), S(   2, -34), S(  -6, -33), S(   3, -14),
    S(   4, -20), S(   2, -13), S(   7, -30), S(  11, -15),
    S(  15, -32), S(  17, -18), S(  14, -14), S(  21,   0),
    S(  16,   5), S(  19,  11), S(  27,  20), S(  29,  28),
    S(  16,  15), S(  23,  12), S(  34,  31), S(  24,  44),
    S( -18,  10), S(  -3,  15), S(  26,  32), S(  25,  33),
    S(  12, -13), S(  -4,  -3), S(  32, -18), S(  36,   9),
    S(-177, -25), S( -83,  -1), S(-108,  25), S( -31,   4),
};

const int BishopPSQT32[32] = {
    S(  12, -31), S(   7, -16), S(   0, -15), S(   7,  -8),
    S(  22, -33), S(  11, -38), S(  17, -18), S(  11,  -8),
    S(  14, -10), S(  21, -10), S(   5, -19), S(  17,   1),
    S(   7,  -6), S(  18,   0), S(  14,  10), S(  20,  13),
    S( -13,  14), S(  15,  11), S(   6,  16), S(  11,  24),
    S(  -3,   8), S(  -3,  19), S(   3,   5), S(  10,  17),
    S( -46,  15), S( -39,   0), S(  -7,  10), S( -23,  15),
    S( -58,  -5), S( -44,   8), S( -84,  19), S( -88,  30),
};

const int RookPSQT32[32] = {
    S( -13, -28), S( -11, -19), S(  -6, -17), S(  -1, -23),
    S( -48, -20), S( -10, -33), S( -11, -31), S(  -5, -32),
    S( -23, -19), S(  -7, -10), S( -17, -14), S(  -3, -21),
    S( -18,  -2), S(  -9,   7), S(  -9,   5), S(   2,  -1),
    S(  -5,  13), S(  10,   9), S(  16,   9), S(  31,   4),
    S( -15,  24), S(  24,  11), S(   8,  21), S(  32,   7),
    S(  -5,  12), S( -13,  18), S(   7,   9), S(  20,  10),
    S(  35,  25), S(  27,  30), S(   6,  37), S(  13,  32),
};

const int QueenPSQT32[32] = {
    S(  28, -61), S(   9, -47), S(  14, -58), S(  20, -38),
    S(  13, -41), S(  24, -52), S(  25, -62), S(  18, -25),
    S(  11, -31), S(  23, -18), S(   8,   2), S(   8,  -4),
    S(  11,  -7), S(  15,  15), S(   2,  19), S(  -9,  59),
    S(  -8,  20), S(  -7,  49), S( -13,  29), S( -25,  77),
    S( -26,  30), S( -15,  21), S( -21,  27), S( -10,  32),
    S(  -5,  29), S( -54,  65), S(  -5,  19), S( -37,  60),
    S(  -3,  21), S(  22,  15), S(  12,  21), S(   1,  34),
};

const int KingPSQT32[32] = {
    S(  49, -73), S(  40, -44), S( -14,  -9), S( -23, -30),
    S(  27, -16), S(  -9, -15), S( -34,   7), S( -59,  11),
    S(  -6, -22), S(  11, -25), S(  20,  -1), S(  -8,  17),
    S(  -8, -33), S(  84, -33), S(  50,   4), S(  -3,  28),
    S(   7, -16), S( 103, -29), S(  51,  14), S(   4,  22),
    S(  48, -27), S( 130, -12), S(  98,   9), S(  38,   4),
    S(   7, -57), S(  49,  -4), S(  31,   5), S(   7,  -4),
    S(   4,-123), S(  72, -64), S( -23, -23), S( -20, -19),
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -28, -10), S( -11,   9), S( -17,  28),
    S(  -3,  58), S(  36,  62), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -18,  20), S(  -5,  25), S(   3,  49),
    S(  23, 102), S(  41,  64), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -4, -15);

const int PawnStacked[2] = { S(  -3, -19), S(  -4,  -8) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   4,  -5), S(   9,  -7), S(   7, -14),
    S(   6, -23), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -11, -30), S(  -2, -30), S(   4, -32),
    S(  36, -39), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -5,  -8), S(   9,   0), S(   0,  -1), S(   6,  13),
    S(   9,   0), S(  19,  -4), S(  19,   3), S(  19,  10),
    S(   7,  -1), S(  17,   1), S(   6,   6), S(  12,  14),
    S(   5,  12), S(  16,  18), S(  22,  25), S(  24,  20),
    S(  42,  34), S(  41,  58), S(  69,  70), S(  81,  81),
    S( 112,  -3), S( 205,   6), S( 227,  29), S( 236,  44),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightInSiberia[4] = {
    S(  -5,  -5), S(  -6, -15), S( -13, -17), S( -30, -11),
};

const int KnightMobility[9] = {
    S( -75,-107), S( -30,-110), S( -14, -41), S(  -6,  -7),
    S(   4,   3), S(   7,  19), S(  12,  23), S(  19,  24),
    S(  29,   9),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  14,  71);

const int BishopRammedPawns = S(  -5, -15);
...
const int BishopLongDiagonal = S(  15,  18);

const int BishopMobility[14] = {
    S( -72,-160), S( -25,-116), S(  -8, -54), S(  -2, -24),
    S(   5, -10), S(  11,   6), S(  12,  16), S(  13,  22),
    S(  12,  29), S(  18,  30), S(  17,  30), S(  36,  17),
    S(  48,  26), S(  70, -13),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(   6,   8), S(  25,   9) };

const int RookOnSeventh = S(   1,  34);

const int RookMobility[15] = {
    S(-153,-116), S( -49,-115), S( -14, -71), S(  -6, -21),
    S(  -7,   1), S( -10,  19), S(  -9,  32), S(  -4,  37),
    S(   2,  41), S(   6,  44), S(   7,  51), S(  13,  55),
    S(  16,  58), S(  29,  48), S(  58,  18),
};

/* Queen Evaluation Terms */

const int QueenRelativePin = S( -15, -16);

const int QueenMobility[28] = {
    S( -63,-263), S(-214,-388), S( -74,-203), S( -17,-197),
    S(  -4,-148), S(   0, -83), S(   5, -43), S(   4, -12),
    S(   8,  -5), S(   8,  16), S(  11,  22), S(  13,  38),
    S(  15,  30), S(  17,  39), S(  15,  43), S(  13,  49),
    S(  15,  46), S(   7,  48), S(   9,  45), S(  11,  35),
    S(  20,  13), S(  32,  -8), S(  35, -36), S(  31, -58),
    S(  10, -75), S(  19,-106), S( -55, -37), S( -32, -63),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -21,  -3), S( -11,  -1), S(   3,   3), S(  11,   5),
    S(  17,   8), S(  26,   6), S(  28,  -4), S(  12,  -1),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  42,  40), S(  28,  28), S(  15,  14), S( -24, -19),
    S( -29, -56), S( -23, -69), S( -18, -70), S( -11, -64),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -6,   2), S(  16, -23), S(  20,  -5), S(  14,   8),
    S(   7,   3), S(  -7,  -2), S(  -2, -34), S( -48,  18)},
   {S(  12,  -7), S(  17, -15), S(   0,  -3), S(  -8,   0),
    S( -24,  13), S( -72,  65), S(  91,  79), S( -27,   0)},
   {S(  31,  -1), S(   8,  -4), S( -23,   4), S( -10, -11),
    S( -14,  -5), S( -11,   0), S(   0,  68), S( -14,  -1)},
   {S(  10,  11), S(  22, -12), S(   7, -15), S(  12, -21),
    S(  21, -38), S( -62,   3), S(-135,  50), S(   3,  -5)},
   {S(  -9,   8), S(   7,  -4), S( -28,   2), S( -15,   5),
    S( -12,  -7), S( -45,  -6), S(  32, -17), S(  -5,   1)},
   {S(  38, -12), S(  13,  -8), S( -16,   2), S(  -6, -22),
    S(   5, -29), S(  22, -19), S(  40, -32), S( -19,   0)},
   {S(  26, -13), S(   0, -16), S( -25,  -2), S( -18, -11),
    S( -30,  -3), S( -39,  30), S(   1,  44), S( -13,   2)},
   {S(   0, -15), S(   3, -18), S(   4,   3), S(  -3,  11),
    S( -13,  17), S( -10,  42), S(-188,  90), S( -15,  15)}},
  {{S(   0,   0), S( -13, -29), S(   4, -23), S( -40,  14),
    S( -24,   2), S(   3,  43), S(-167,  -7), S( -40,  12)},
   {S(   0,   0), S(  16, -17), S(   8,  -7), S( -16,   0),
    S(  -1, -21), S(  27,  65), S(-184,  -3), S( -34,   8)},
   {S(   0,   0), S(  18,  -9), S(   2, -12), S(   6, -22),
    S(  18,  -4), S( -87,  47), S( -84, -74), S( -16,  -2)},
   {S(   0,   0), S(  -3,   4), S(  -6,  -6), S( -21,   3),
    S( -21,   2), S(-100,  30), S(   7, -40), S( -17,  -2)},
   {S(   0,   0), S(  11,   0), S(  13, -10), S(  13, -13),
    S(  12, -30), S( -57,  14), S(-101, -60), S(  -2,  -1)},
   {S(   0,   0), S(  -2,  -6), S( -20,   0), S( -16, -11),
    S(  20, -24), S( -37,   3), S(  55,  39), S( -10,  -5)},
   {S(   0,   0), S(  21, -12), S(  13, -10), S(  -7, -10),
    S( -23,  12), S(  -7,  20), S( -57, -49), S( -29,  13)},
   {S(   0,   0), S(  12, -41), S(  15, -27), S( -20, -10),
    S( -26,  20), S( -10,  20), S(-228, -56), S( -24,   6)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -13,  32), S( 107,  -3), S( -22,  22), S( -13,   3),
    S( -15,  -1), S( -10,  -3), S( -16,   5), S( -23,  -2)},
   {S(  -9,  53), S(  55,  15), S( -16,  18), S(  -4,  10),
    S(  -3,   7), S(   2,  -2), S(  -2,   1), S( -13,   6)},
   {S(   6,  45), S(  11,  27), S( -13,  15), S( -10,   7),
    S(  -1,   4), S(   7,   2), S(   9,  -9), S(   4,   5)},
   {S(  -1,  26), S(   9,  18), S( -22,   6), S( -16,  -2),
    S(  -7,  -1), S(  11, -12), S(   4,  -9), S(  -9,   3)}},
  {{S(   0,   0), S( -18, -16), S( -11,  -2), S(  20, -17),
    S(   5,  -3), S(   3, -17), S(  -2,   4), S(  19,  30)},
   {S(   0,   0), S( -17, -37), S(   0,  -9), S(  37,  -8),
    S(   1,  -3), S(  17, -29), S(  -2,  -7), S( -19,   0)},
   {S(   0,   0), S( -28, -52), S( -18, -10), S(  11,  -9),
    S(   6,  -3), S(  -5, -17), S( -10, -14), S( -10,   6)},
   {S(   0,   0), S(  -2, -19), S( -18, -16), S( -10,  -2),
    S(   2, -10), S(   7, -26), S(  70, -13), S(  15,  19)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -37,  -1), S( -47,  26), S( -66,  28),
    S(   7,  13), S(  84, -12), S( 158,  48), S(   0,   0)},
   {S(   0,   0), S( -27,  14), S( -38,  40), S( -55,  37),
    S(   0,  44), S( 105,  34), S( 194,  91), S(   0,   0)}},
  {{S(   0,   0), S( -24,  26), S( -44,  36), S( -61,  47),
    S(   8,  53), S( 103,  56), S( 267, 112), S(   0,   0)},
   {S(   0,   0), S( -25,  23), S( -36,  36), S( -51,  53),
    S(  15,  72), S( 113, 132), S( 164, 271), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(  -3,   2), S(   2,  -5), S(   7, -14),
    S(   5, -20), S(  -7, -19), S( -15, -10), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   5,   0), S(   5,   0), S(   6,  12),
    S(  -1,  27), S(   3,  40), S(  23,  43), S(   0,   0),
};

const int PassedSafePromotionPath = S(  -9,  44);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -12, -31);
const int ThreatMinorAttackedByPawn  = S( -44, -62);
const int ThreatMinorAttackedByMinor = S( -18, -40);
const int ThreatMinorAttackedByMajor = S( -26, -46);
const int ThreatRookAttackedByLesser = S( -44, -24);
const int ThreatMinorAttackedByKing  = S( -31, -19);
const int ThreatRookAttackedByKing   = S( -31, -14);
const int ThreatQueenAttackedByOne   = S( -43, -21);
const int ThreatOverloadedPieces     = S(  -6, -14);
const int ThreatByPawnPush           = S(  11,  28);

/* Space Evaluation Terms */

const int SpaceRestrictPiece = S(  -3,  -2);
const int SpaceRestrictEmpty = S(  -3,  -3);
const int SpaceCenterControl = S(   3,  -1);

/* Closedness Evaluation Terms */

const int ClosednessKnightAdjustment[9] = {
    S( -15,   4), S( -11,  16), S( -10,  22), S(  -7,  20),
    S(  -4,  25), S(  -3,  19), S(   1,  13), S( -10,  27),
    S(  -8,  12),
};

const int ClosednessRookAdjustment[9] = {
    S(  52,  18), S(  -1,  51), S(  -3,  32), S(  -8,  16),
    S( -18,  11), S( -17,  -5), S( -18, -12), S( -26, -16),
    S( -40, -24),
};
