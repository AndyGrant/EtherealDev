...
const int PawnValue[PHASE_NB] = {  91,  85};

const int PawnIsolated[PHASE_NB] = {   0,  -9};

const int PawnStacked[PHASE_NB] = { -18, -22};

const int PawnBackwards[2][PHASE_NB] = { {   1,   2}, { -11, -16} };
...
const int PawnConnected32[32][PHASE_NB] = {
    ...
    {   8,   0}, {  11,   2}, {   8,   6}, {  14,  15},
    {   7,   0}, {  14,   4}, {  10,   4}, {  17,  11},
    {  14,   4}, {  19,  10}, {  21,  10}, {  26,  15},
    {  32, -14}, {  42,   0}, {  73,  26}, {  69,  36},
    { 124, -48}, { 160, -58}, { 108, -72}, { 143,  -6},
    {   0,   0}, {   0,   0}, {   0,   0}, {   0,   0},
};

const int KnightValue[PHASE_NB] = { 296, 269};

const int KnightAttackedByPawn[PHASE_NB] = { -38, -32};

const int KnightOutpost[2][PHASE_NB] = { {  13, -29}, {  34,  12} };
...
const int BishopValue[PHASE_NB] = { 298, 270};

const int BishopWings[PHASE_NB] = {   0,  -8};

const int BishopPair[PHASE_NB] = {  25,  41};

const int BishopAttackedByPawn[PHASE_NB] = { -36, -30};

const int BishopOutpost[2][PHASE_NB] = { {  14, -15}, {  37,  -9} };

const int BishopMobility[14][PHASE_NB] = {
    { -69, -69}, { -37, -54}, { -16, -30}, {  -5, -13},
    {   6,  -5}, {  13,   4}, {  17,  11}, {  20,   7},
    {  24,  12}, {  22,   0}, {  23,   5}, {  15, -26},
    {  13,  -1}, {  -8, -38},
};

const int RookValue[PHASE_NB] = { 412, 442};

const int RookFile[2][PHASE_NB] = { {   8,  -3}, {  25,  -4} };

const int RookOnSeventh[PHASE_NB] = {   7,   7};

const int RookMobility[15][PHASE_NB] = {
    {-134, -99}, { -57, -93}, { -14, -50}, {  -5, -21},
    {  -5,  -8}, {  -6,   1}, {  -6,  11}, {  -3,  17},
    {   0,  24}, {   9,  27}, {  12,  33}, {  17,  38},
    {  17,  39}, {  14,  31}, {  14,  26},
};

const int QueenValue[PHASE_NB] = { 824, 847};

const int QueenChecked[PHASE_NB] = { -54, -30};

const int QueenCheckedByPawn[PHASE_NB] = { -63, -38};

const int QueenMobility[28][PHASE_NB] = {
    {-175, -66}, { -70,-302}, { -62,-125}, { -39, -85},
    { -27,-114}, { -24, -54}, { -18, -51}, { -15, -52},
    { -12, -47}, {  -8, -39}, {  -5, -28}, {  -1, -17},
    {   0, -11}, {   0,  -1}, {   2,   1}, {   4,  13},
    {   2,  18}, {   2,  19}, {   1,  18}, {   6,  24},
    {   5,  22}, {   8,  19}, {  16,  22}, {  18,  14},
    {  18,  16}, {  10,  -2}, {  22,  23}, {  10,  10},
};
...
const int KingDefenders[12][PHASE_NB] = {
    { -14,  12}, { -17,  -5}, {  -1,  -1}, {   6,   1},
    {  17,   0}, {  26,   6}, {  35,  -4}, {  38, -28},
    {   8,   4}, {   8,   4}, {   8,   4}, {   8,   4},
};

const int KingShelter[2][FILE_NB][RANK_NB][PHASE_NB] = {
  {{{  -4,  -8}, {  12, -11}, {  19,  -5}, {  11,  -7}, {  13,  -9}, { -14, -21}, {  67,  -1}, { -17,  14}},
   {{  -2,   3}, {  12,  -3}, {  24,  -6}, {  -4, -11}, {   3, -17}, { -75,  18}, { -86,   7}, { -18,   1}},
   {{   9,  -2}, {   7,  -1}, { -11,  -1}, {  -8,  -5}, {  -2,  -6}, {  43, -23}, {  61, -42}, {  -6,   0}},
   {{  15,  14}, {   8,   0}, {  -4,  -6}, {  12,  -2}, {  10,  -9}, { -20, -26}, { -13,  23}, {   0,  -2}},
   {{   2,   9}, {   1,   4}, { -16,   0}, { -14, -11}, { -16, -22}, { -33, -45}, {  52, -30}, { -10,  -1}},
   {{   7,  -1}, {  13,   2}, {  -4,  -1}, {  -7, -13}, {   9, -26}, {  -2,  -7}, {   1,  -4}, { -11,   1}},
   {{   3,  -3}, {   1,  -8}, { -15,  -7}, { -20, -13}, { -20, -21}, { -28,  -1}, {  40,  11}, { -16,   9}},
   {{ -12, -10}, {   2,  -6}, {   3,   0}, {  -3,   1}, { -18,  -4}, {  -7,  29}, {   3,  45}, { -14,  13}}},
  {{{   0,   0}, { -12, -14}, {   3, -14}, { -44,  -2}, { -19,  24}, { -24,  86}, { -30, -80}, { -31,  15}},
   {{   0,   0}, {  16,   0}, {  17,  -6}, { -16,   0}, {   8,  -2}, {-103,  -8}, {-257,  15}, { -21,   6}},
   {{   0,   0}, {  17,   0}, {   2,  -7}, {  14, -13}, {  14,  -7}, {-102, -23}, {-122,-113}, {  -8,  -1}},
   {{   0,   0}, {   4,   1}, {   0,  15}, { -17,  -7}, {  -9,  -7}, {  -1,  -2}, { -11,  28}, { -16,  -4}},
   {{   0,   0}, {   0,   8}, {   6,   0}, {   9,  -9}, {   7, -16}, { -18, -16}, { -34,   2}, {  -2,  -4}},
   {{   0,   0}, {   0,   0}, {  -7,  -2}, { -13, -10}, {   1, -10}, { -55,  -2}, { -58,  74}, { -15,   0}},
   {{   0,   0}, {   8,  -4}, {  -1,   0}, {  -9,  -8}, { -21,  -7}, {  -8,  28}, {  24, -66}, { -27,  10}},
   {{   0,   0}, {   2, -13}, {   5, -10}, { -37,  -3}, { -21,  18}, { -17, -16}, {-126, -85}, { -27,  12}}},
};

const int PassedPawn[2][2][RANK_NB][PHASE_NB] = {
  {{{   0,   0}, { -25, -14}, { -24,  -9}, { -20, -27}, {  16, -28}, {  40, -38}, { 100, -15}, {   0,   0}},
   {{   0,   0}, {  -5,   0}, { -11,  13}, {  -8,  23}, {  11,  22}, {  61,  26}, { 126,  46}, {   0,   0}}},
  {{{   0,   0}, {  -4,   2}, { -13,  -3}, {  -8,   5}, {  21,   9}, {  74,  35}, { 145,  71}, {   0,   0}},
   {{   0,   0}, {   0,  10}, {  -3,  14}, {  -7,  37}, {   8,  79}, {  56, 152}, { 130, 238}, {   0,   0}}},
};
