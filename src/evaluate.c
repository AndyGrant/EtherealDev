...
int evaluatePawns(EvalInfo *ei, Board *board, int colour) {
    ...
    int sq, flag, ourKing, theirKing, eval = 0, pkeval = 0;
    ...
    while (tempPawns) {
        ...
        if (TRACE) T.PawnKingPSQT[0][ourRankDist][ourFileDist][US]++;
        if (TRACE) T.PawnKingPSQT[1][theirRankDist][theirFileDist][US]++;
    }
...
void initEvalInfo(Thread *thread, Board *board, EvalInfo *ei) {
    ...
    ei->pkentry       = TRACE ? NULL : getCachedPawnKingEval(thread, board);
