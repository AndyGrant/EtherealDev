...
const int PawnValue   = S( 130, 139);
const int KnightValue = S( 426, 417);
const int BishopValue = S( 460, 433);
const int RookValue   = S( 665, 717);
const int QueenValue  = S(1255,1367);
const int KingValue   = S(   0,   0);

const int PieceValues[8][PHASE_NB] = {
    { 130, 139}, { 426, 417}, { 460, 433}, { 665, 717},
    {1255,1367}, {   0,   0}, {   0,   0}, {   0,   0},
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -26, -11), S( -13,   8), S( -13,  28),
    S(   6,  65), S(  50,  71), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -14,  13), S(  -5,  21), S(   8,  50),
    S(  25,  94), S(  34,  53), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S( -11, -10);

const int PawnStacked = S( -18, -31);

const int PawnBackwards[2] = { S(   8,  -1), S( -13, -20) };

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -1,  -9), S(  15,   2), S(   3,   1), S(   9,  18),
    S(  23,   0), S(  45,  -1), S(  29,  10), S(  37,  16),
    S(   9,   0), S(  26,   5), S(  12,  10), S(  22,  21),
    S(  16,   7), S(  27,  13), S(  35,  21), S(  43,  15),
    S(  59,  27), S(  55,  49), S(  72,  58), S(  83,  60),
    S( 109,  -1), S( 202,  10), S( 227,  28), S( 239,  50),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

/* Knight Evaluation Terms */

const int KnightOutpost[2] = { S(   4, -21), S(  37,   3) };

const int KnightBehindPawn = S(   4,  28);

const int KnightMobility[9] = {
    S( -82,-101), S( -47,-102), S( -35, -48), S( -19, -19),
    S(  -3,  -5), S(   4,  17), S(  16,  20), S(  30,  20),
    S(  50,  -1),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  14,  82);

const int BishopRammedPawns = S( -16, -16);

const int BishopOutpost[2] = { S(   8,  -6), S(  50,   1) };

const int BishopBehindPawn = S(   2,  26);

const int BishopMobility[14] = {
    S( -67,-147), S( -45,-101), S( -24, -70), S( -11, -36),
    S(   3, -21), S(  14,   0), S(  19,  14), S(  19,  22),
    S(  19,  27), S(  25,  29), S(  24,  27), S(  42,  13),
    S(  41,  15), S(  65, -19),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  20,   7), S(  49,   8) };

const int RookOnSeventh = S(  -2,  35);

const int RookMobility[15] = {
    S(-149,-112), S( -70,-120), S( -27, -75), S( -13, -35),
    S( -11, -10), S( -11,  10), S( -11,  25), S(  -2,  28),
    S(   7,  35), S(  12,  42), S(  17,  51), S(  25,  56),
    S(  24,  62), S(  33,  59), S(  76,  20),
};

/* Queen Evaluation Terms */

const int QueenMobility[28] = {
    S( -61,-263), S(-211,-387), S( -60,-202), S( -26,-192),
    S( -21,-141), S( -20, -90), S( -10, -63), S( -11, -36),
    S(  -6, -25), S(  -2,  -3), S(   3,   7), S(   6,  20),
    S(   9,  21), S(  13,  31), S(   9,  39), S(   9,  43),
    S(   7,  46), S(   0,  45), S(   1,  38), S(  -2,  33),
    S(   3,  10), S(  17,  -9), S(  18, -37), S(  20, -55),
    S(   4, -70), S(  19, -95), S( -57, -39), S( -27, -54),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -23,  -5), S( -12,   0), S(  -3,   2), S(   7,   6),
    S(  16,   5), S(  32,   0), S(  35,   0), S(  13,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -13,   3), S(  18, -26), S(  23,  -8), S(  12,   4),
    S(   4,   3), S(   6,   1), S(  -1, -31), S( -53,  21)},
   {S(  16,  -4), S(  26, -20), S(  -1,  -3), S( -17,   2),
    S( -30,  13), S( -69,  68), S(  92,  83), S( -31,   4)},
   {S(  34,   1), S(  23, -10), S( -27,   5), S( -11, -10),
    S( -22,  -4), S(  -7,  -1), S(   0,  65), S( -20,   2)},
   {S(   4,  17), S(  21, -10), S(   2, -13), S(  14, -25),
    S(  26, -36), S( -49,   1), S(-137,  50), S(  -2,   0)},
   {S( -10,  11), S(   0,  -2), S( -30,   0), S( -21,   3),
    S( -21,  -9), S( -38,   0), S(  33, -15), S( -10,   2)},
   {S(  49, -13), S(  31, -21), S( -27,   1), S( -16, -24),
    S(   8, -24), S(  18, -22), S(  41, -29), S( -35,   5)},
   {S(  29,  -9), S(   4, -22), S( -28,  -4), S( -19, -11),
    S( -29,  -9), S( -34,  30), S(   0,  44), S( -13,   5)},
   {S( -12,  -9), S(   7, -25), S(  10,   0), S(  -2,  10),
    S( -10,  14), S(  -7,  38), S(-189,  87), S( -22,  20)}},
  {{S(   0,   0), S( -12, -24), S(   0, -16), S( -42,  16),
    S( -18,   0), S(   2,  43), S(-166,  -8), S( -46,  12)},
   {S(   0,   0), S(  30, -22), S(   8,  -6), S( -19,  -3),
    S(   0, -14), S(  27,  70), S(-183,  -3), S( -42,   8)},
   {S(   0,   0), S(  39,  -7), S(  -2,  -6), S(   9, -18),
    S(  12,  -7), S( -91,  47), S( -85, -75), S( -22,   0)},
   {S(   0,   0), S(  -2,   9), S(  -2,   1), S( -16,   0),
    S( -26,   0), S(-101,  28), S(   7, -41), S( -22,  -3)},
   {S(   0,   0), S(   7,   4), S(  13,  -8), S(  16, -11),
    S(  11, -24), S( -53,  14), S(-104, -63), S(  -5,  -7)},
   {S(   0,   0), S(   7,  -4), S( -23,   2), S( -28,  -8),
    S(  18, -25), S( -39,   2), S(  55,  38), S( -23,   0)},
   {S(   0,   0), S(  29, -14), S(  19, -13), S( -12,  -7),
    S( -34,   5), S(  -7,  14), S( -56, -50), S( -42,  19)},
   {S(   0,   0), S(  16, -46), S(  26, -29), S( -16,  -5),
    S( -17,  15), S(  -1,  14), S(-229, -57), S( -21,   6)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S(   1,  24), S( 116, -10), S( -27,  25), S( -21,   8),
    S(  -9,  -1), S(  -5,  -4), S( -20,   8), S( -22,  -4)},
   {S(   0,  47), S(  59,  14), S( -17,  22), S(   0,  10),
    S(   0,   6), S(   4,  -2), S(  -3,   0), S( -13,  -1)},
   {S(  12,  38), S(  21,  22), S( -23,  22), S( -15,  12),
    S(   1,   7), S(   7,   2), S(   8, -12), S(  -2,   0)},
   {S(   0,  23), S(  20,  18), S( -13,   8), S( -16,   2),
    S( -17,   2), S(   9,  -9), S(   0,  -6), S( -23,   4)}},
  {{S(   0,   0), S( -11, -13), S( -21,  -6), S(  25, -18),
    S(  17,  -5), S(   0, -17), S(  -5,   1), S(  16,  27)},
   {S(   0,   0), S( -13, -28), S(  -8, -13), S(  34, -11),
    S(  -2,   0), S(   8, -22), S( -10,  -7), S( -15,   2)},
   {S(   0,   0), S( -23, -42), S( -33, -10), S(   9,  -9),
    S(   2,   0), S(  -9, -14), S( -11, -12), S( -10,   4)},
   {S(   0,   0), S(   0, -13), S( -20, -21), S( -14,  -1),
    S(  -6,  -7), S(   8, -30), S(  69,  -5), S(  11,  17)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -38,  -2), S( -54,  20), S( -83,  23),
    S(  -5,  12), S(  64,  -3), S( 151,  58), S(   0,   0)},
   {S(   0,   0), S( -26,   1), S( -49,  22), S( -76,  26),
    S( -14,  29), S(  87,  33), S( 187, 101), S(   0,   0)}},
  {{S(   0,   0), S( -25,   8), S( -46,  14), S( -75,  32),
    S(   0,  32), S(  91,  41), S( 256, 121), S(   0,   0)},
   {S(   0,   0), S( -29,   8), S( -41,  13), S( -67,  34),
    S(  -5,  63), S(  74, 138), S( 154, 294), S(   0,   0)}},
};

const int PassedFriendlyDistance[RANK_NB] = {
    S(   0,   0), S(   2,   0), S(   6,  -5), S(  10, -14),
    S(   6, -19), S(  -2, -19), S( -13, -13), S(   0,   0),
};

const int PassedEnemyDistance[RANK_NB] = {
    S(   0,   0), S(   0,   1), S(   0,   3), S(   3,  15),
    S(   1,  29), S(   7,  37), S(  28,  35), S(   0,   0),
};

const int PassedSafePromotionPath = S( -32,  46);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -10, -27);
const int ThreatMinorAttackedByPawn  = S( -60, -44);
const int ThreatMinorAttackedByMinor = S( -24, -30);
const int ThreatMinorAttackedByMajor = S( -24, -37);
...
const int ThreatByPawnPush           = S(  21,  24);
