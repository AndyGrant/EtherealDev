...
const int PawnValue   = S(  93, 122);
const int KnightValue = S( 437, 409);
const int BishopValue = S( 451, 421);
const int RookValue   = S( 635, 681);
const int QueenValue  = S(1277,1356);
...
const int PawnPSQT32[32] = {
    ...
    S( -20,   5), S(   0,   0), S( -12,   3), S(  -8,  -3),
    S( -22,   2), S( -14,   2), S( -10,  -7), S(  -5, -14),
    S( -18,  10), S( -14,  10), S(   7, -13), S(   7, -25),
    S(  -9,  12), S(  -5,   7), S(  -8,  -7), S(  -1, -22),
    S(  -7,  24), S(  -6,  20), S(  -1,  10), S(  26, -17),
    S( -18, -48), S( -67, -17), S(   0, -32), S(  37, -46),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -48, -26), S(  -6, -33), S( -11, -27), S(   1, -17),
    S(  -1, -21), S(   4, -12), S(   3, -28), S(   9, -18),
    S(   8, -24), S(  20, -23), S(  13, -18), S(  22,  -6),
    S(  19,   3), S(  20,   6), S(  28,  13), S(  26,  21),
    S(  23,  16), S(  24,   9), S(  35,  23), S(  27,  34),
    S( -14,  15), S(   6,  12), S(  30,  24), S(  27,  26),
    S(   9, -11), S(  -7,   1), S(  30, -20), S(  41,  -1),
    S(-171, -18), S( -81,  -1), S(-110,  20), S( -30,   0),
};

const int BishopPSQT32[32] = {
    S(   4, -26), S(  10, -18), S( -10,  -8), S(   8, -11),
    S(  26, -33), S(   8, -36), S(  17, -19), S(   9, -11),
    S(  12, -12), S(  24, -14), S(   2, -18), S(  18,  -4),
    S(  13,  -8), S(  16,  -3), S(  13,   4), S(  20,   6),
    S( -11,  11), S(  17,   4), S(   5,  12), S(  10,  16),
    S(   1,   6), S(  -2,  15), S(  -2,   2), S(  15,  10),
    S( -44,  16), S( -53,   3), S(  -8,   7), S( -22,  11),
    S( -52,  -6), S( -46,   9), S( -87,  17), S( -89,  28),
};

const int RookPSQT32[32] = {
    S( -12, -29), S( -15, -19), S(  -5, -23), S(   3, -27),
    S( -49, -13), S( -13, -31), S( -10, -30), S(   1, -33),
    S( -27, -14), S(  -9, -12), S( -17, -15), S(  -2, -24),
    S( -18,  -2), S( -10,   6), S(  -9,   2), S(   3,  -4),
    S(  -6,   9), S(  10,   7), S(  17,   3), S(  32,  -3),
    S( -14,  20), S(  23,   5), S(   0,  18), S(  30,   1),
    S(   0,   6), S( -16,  15), S(   6,   5), S(  20,   5),
    S(  33,  17), S(  24,  20), S(   6,  27), S(  15,  23),
};

const int QueenPSQT32[32] = {
    S(  24, -53), S(   5, -40), S(  11, -50), S(  17, -40),
    S(  13, -39), S(  24, -57), S(  26, -69), S(  16, -27),
    S(   9, -24), S(  22, -22), S(   6,   0), S(   7,  -4),
    S(  10,  -8), S(  11,  13), S(  -2,  15), S( -15,  56),
    S( -10,  15), S(  -6,  38), S( -16,  17), S( -31,  65),
    S( -24,  26), S( -17,  16), S( -24,  19), S( -15,  20),
    S(  -6,  25), S( -55,  62), S( -11,  12), S( -43,  50),
    S( -11,  15), S(  16,   7), S(   6,   6), S(  -8,  17),
};

const int KingPSQT32[32] = {
    S(  42, -79), S(  39, -51), S( -12, -13), S( -27, -21),
    S(  28, -29), S(  -4, -19), S( -35,   7), S( -51,   9),
    S(   8, -29), S(  15, -24), S(  15,  -2), S( -10,  13),
    S(   4, -32), S(  84, -35), S(  42,   4), S(  -4,  23),
    S(   6, -16), S(  96, -31), S(  48,  11), S(   4,  21),
    S(  46, -25), S( 121, -22), S(  95,   4), S(  39,   3),
    S(   5, -46), S(  47,  -7), S(  31,   8), S(   8,   0),
    S(   8,-101), S(  74, -54), S( -20, -14), S( -18, -12),
};

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -26,  -9), S( -11,   9), S( -15,  28),
    S(  -3,  58), S(  36,  63), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -12,  19), S(  -4,  21), S(   1,  42),
    S(  12,  77), S(  31,  54), S(   0,   0), S(   0,   0)},
};
...
const int PawnStacked[2] = { S(  -3, -19), S(  -7,  -8) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   4,  -5), S(   8,  -5), S(   8, -11),
    S(  11, -10), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S(  -8, -25), S(  -5, -21), S(   4, -22),
    S(  18, -18), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -2,  -6), S(  10,   0), S(   2,   1), S(   4,  13),
    S(  12,   0), S(  24,  -3), S(  18,   7), S(  21,  14),
    S(   8,  -1), S(  21,   2), S(   8,  10), S(  14,  17),
    S(  10,   9), S(  19,  15), S(  26,  20), S(  26,  20),
    S(  52,  22), S(  45,  45), S(  64,  52), S(  79,  55),
...
const int KnightInSiberia[4] = {
    S(  -7,   1), S(  -8,  -3), S( -16,  -4), S( -19,  -3),
};

const int KnightMobility[9] = {
    S( -73,-104), S( -34, -96), S( -22, -41), S( -12, -18),
    S(  -3,  -8), S(   2,   7), S(   8,  11), S(  16,  11),
    S(  26,  -2),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  14,  70);
...
const int BishopLongDiagonal = S(  19,  13);

const int BishopMobility[14] = {
    S( -63,-146), S( -33, -92), S( -17, -51), S(  -9, -27),
    S(  -1, -16), S(   5,  -4), S(   8,   5), S(   8,  10),
    S(   8,  15), S(  11,  16), S(  11,  16), S(  31,   4),
    S(  39,  14), S(  69, -18),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  12,   5), S(  29,   4) };

const int RookOnSeventh = S(  -7,  30);

const int RookMobility[15] = {
    S(-147,-113), S( -52,-111), S( -21, -56), S( -13, -19),
    S( -14,  -1), S( -14,  13), S( -13,  23), S(  -8,  26),
    S(  -2,  29), S(   1,  33), S(   3,  38), S(   8,  41),
    S(   9,  45), S(  22,  34), S(  75,  -5),
};

/* Queen Evaluation Terms */

const int QueenMobility[28] = {
    S( -61,-263), S(-210,-387), S( -57,-201), S( -15,-192),
    S(  -5,-138), S(  -3, -82), S(   1, -50), S(   0, -21),
    S(   4, -17), S(   5,   2), S(   8,   7), S(   9,  21),
    S(  11,  15), S(  12,  23), S(  10,  27), S(   6,  33),
    S(   6,  33), S(  -2,  36), S(  -1,  33), S(  -5,  28),
    S(   3,   6), S(  18, -16), S(  22, -43), S(  26, -59),
    S(  10, -73), S(  22, -99), S( -56, -39), S( -32, -62),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -17,   0), S(  -3,  -2), S(   3,   2), S(   8,   5),
    S(  14,   6), S(  22,   3), S(  25,  -3), S(  13,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  32,  33), S(  16,  26), S(   7,  14), S( -15, -15),
    S( -18, -50), S( -17, -66), S( -16, -71), S( -11, -71),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -10,   2), S(  11, -24), S(  18,  -9), S(  11,   3),
    S(   6,   4), S(   0,   2), S(  -3, -33), S( -43,  18)},
   {S(  17,  -8), S(  18, -17), S(   1,  -6), S( -14,   3),
    S( -29,  15), S( -70,  68), S(  92,  82), S( -21,   2)},
   {S(  33,  -5), S(  11,  -8), S( -24,   6), S( -11,  -8),
    S( -19,  -4), S( -12,   1), S(   0,  66), S( -10,  -1)},
   {S(   4,   9), S(  19, -12), S(   4, -12), S(  12, -20),
    S(  24, -36), S( -59,   5), S(-136,  52), S(   6,  -7)},
   {S( -13,   8), S(   4,  -4), S( -23,   1), S( -16,   5),
    S( -19,  -6), S( -42,  -2), S(  33, -17), S(  -5,  -1)},
   {S(  40, -17), S(  18, -15), S( -18,   2), S( -11, -18),
    S(   4, -25), S(  15, -21), S(  41, -30), S( -21,   0)},
   {S(  24, -14), S(   0, -16), S( -21,  -2), S( -19,  -8),
    S( -29,  -7), S( -37,  32), S(   0,  44), S( -10,   1)},
   {S(  -7, -12), S(   3, -17), S(   6,   0), S(  -3,   9),
    S( -12,  14), S( -10,  36), S(-190,  87), S( -15,  13)}},
  {{S(   0,   0), S( -10, -23), S(   5, -20), S( -39,  16),
    S( -22,   2), S(   3,  42), S(-167,  -8), S( -42,  13)},
   {S(   0,   0), S(  21, -21), S(   5,  -8), S( -19,  -1),
    S(  -1, -12), S(  25,  65), S(-184,  -3), S( -33,   5)},
   {S(   0,   0), S(  23, -12), S(  -1, -10), S(   4, -19),
    S(  14,  -7), S( -87,  47), S( -84, -73), S( -15,  -2)},
   {S(   0,   0), S(  -6,   6), S(  -4,  -3), S( -19,   1),
    S( -27,   1), S( -99,  31), S(   7, -41), S( -14,  -1)},
   {S(   0,   0), S(  10,   2), S(  11,  -6), S(  12, -12),
    S(  14, -27), S( -57,  15), S(-104, -61), S(   2,  -1)},
   {S(   0,   0), S(   4,  -7), S( -19,  -1), S( -24,  -7),
    S(  17, -25), S( -38,   3), S(  55,  38), S( -14,  -4)},
   {S(   0,   0), S(  19, -15), S(   9, -14), S( -11,  -9),
    S( -27,   8), S( -10,  15), S( -56, -49), S( -28,  13)},
   {S(   0,   0), S(  10, -38), S(  17, -29), S( -18,  -6),
    S( -18,  17), S(  -6,  20), S(-228, -55), S( -20,   4)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S(  -5,  28), S( 111, -10), S( -23,  24), S( -16,   6),
    S( -14,   2), S(  -9,  -3), S( -17,   6), S( -22,   2)},
   {S(  -4,  49), S(  54,  12), S( -16,  21), S(  -3,   8),
    S(  -4,   4), S(   3,  -4), S(  -3,   2), S( -11,   5)},
   {S(   7,  38), S(  14,  24), S( -20,  18), S( -10,   6),
    S(   1,   1), S(   4,  -1), S(   7,  -6), S(   2,   3)},
   {S(  -2,  26), S(  12,  18), S( -17,   8), S( -14,   0),
    S( -12,   0), S(   6, -11), S(   1,  -8), S( -10,   4)}},
  {{S(   0,   0), S( -16, -16), S( -12,  -3), S(  17, -16),
    S(   7,  -5), S(   2, -18), S(  -3,  -1), S(  17,  29)},
   {S(   0,   0), S( -17, -35), S(   3, -11), S(  35, -12),
    S(   0,  -1), S(  12, -22), S(  -7, -10), S( -17,   2)},
   {S(   0,   0), S( -28, -50), S( -21,  -8), S(  12, -12),
    S(   2,  -1), S(  -8, -13), S( -14, -15), S( -11,   6)},
   {S(   0,   0), S(  -2, -18), S( -13, -17), S( -11,  -3),
    S(  -2,  -6), S(   5, -23), S(  71, -12), S(  13,  20)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -36,   4), S( -50,  24), S( -73,  32),
    S(  -5,  15), S(  69,  -4), S( 159,  57), S(   0,   0)},
   {S(   0,   0), S( -29,   9), S( -47,  30), S( -66,  37),
    S( -10,  34), S(  91,  26), S( 183,  94), S(   0,   0)}},
  {{S(   0,   0), S( -24,  18), S( -47,  29), S( -68,  43),
    S(  -1,  39), S(  87,  38), S( 258, 109), S(   0,   0)},
   {S(   0,   0), S( -29,  15), S( -42,  25), S( -62,  46),
    S(   5,  55), S(  99, 108), S( 161, 260), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   0,   2), S(   3,  -3), S(   6, -10),
    S(   5, -13), S(  -9, -11), S( -18,  -6), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   3,  -1), S(   5,   0), S(  10,   7),
    S(   1,  20), S(   7,  28), S(  24,  25), S(   0,   0),
};

const int PassedSafePromotionPath = S( -17,  34);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -11, -29);
const int ThreatMinorAttackedByPawn  = S( -45, -54);
const int ThreatMinorAttackedByMinor = S( -20, -36);
const int ThreatMinorAttackedByMajor = S( -20, -47);
const int ThreatRookAttackedByLesser = S( -42, -22);
const int ThreatMinorAttackedByKing  = S( -26, -24);
const int ThreatRookAttackedByKing   = S( -20, -28);
const int ThreatQueenAttackedByOne   = S( -32, -39);
...
const int SpaceCenterControl = S(   2,  -1);

/* Closedness Evaluation Terms */

const int ClosednessKnightAdjustment[9] = {
    S( -17, -10), S( -16,   5), S( -17,  13), S( -14,  13),
    S( -12,  18), S(  -9,  14), S(  -6,  11), S( -16,  26),
    S( -15,  14),
};

const int ClosednessRookAdjustment[9] = {
    S(  36,  -6), S(  -5,  24), S( -16,  12), S( -21,  -1),
    S( -27,  -1), S( -26, -13), S( -23, -15), S( -26, -17),
    S( -30, -18),
};
...
const int ComplexityPawnFlanks  = S(   0,  67);
const int ComplexityPawnEndgame = S(   0,  45);
const int ComplexityAdjustment  = S(   0,-136);
