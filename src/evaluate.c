...
const int PawnValue   = S( 104, 126);
const int KnightValue = S( 452, 413);
const int BishopValue = S( 474, 426);
const int RookValue   = S( 655, 690);
const int QueenValue  = S(1293,1378);
...
const int PawnPSQT32[32] = {
    ...
    S( -21,   6), S(   2,   1), S( -14,   4), S( -10,  -3),
    S( -23,   1), S( -16,   2), S( -10,  -7), S(  -6, -14),
    S( -19,   8), S( -14,   9), S(   9, -13), S(  10, -26),
    S(  -8,  11), S(  -4,   7), S(  -6,  -7), S(   3, -23),
    S(  -8,  23), S(  -5,  22), S(   3,  11), S(  30, -16),
    S( -19, -48), S( -67, -15), S(   1, -29), S(  38, -43),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -49, -26), S(  -6, -34), S( -14, -27), S(   0, -17),
    S(  -2, -21), S(   4, -12), S(   3, -29), S(   9, -18),
    S(   8, -25), S(  23, -23), S(  15, -18), S(  23,  -5),
    S(  22,   5), S(  23,   6), S(  32,  14), S(  27,  20),
    S(  26,  17), S(  28,  10), S(  39,  24), S(  32,  35),
    S( -14,  15), S(   7,  13), S(  31,  26), S(  31,  28),
    S(   8, -11), S(  -7,   1), S(  34, -20), S(  42,  -1),
    S(-170, -18), S( -81,  -2), S(-110,  19), S( -30,   0),
};

const int BishopPSQT32[32] = {
    S(   7, -24), S(  13, -18), S( -10,  -7), S(   9, -10),
    S(  31, -32), S(  11, -36), S(  22, -18), S(  11, -11),
    S(  17, -12), S(  30, -14), S(   3, -17), S(  22,  -4),
    S(  16,  -8), S(  19,  -3), S(  17,   4), S(  24,   7),
    S( -11,  12), S(  20,   4), S(   7,  12), S(  13,  17),
...
const int RookPSQT32[32] = {
    S(  -8, -29), S( -14, -20), S(  -1, -23), S(   7, -27),
    S( -53, -13), S( -13, -30), S(  -9, -30), S(   1, -32),
    S( -28, -13), S(  -7, -11), S( -16, -15), S(  -1, -23),
    S( -17,   0), S(  -7,   6), S(  -6,   2), S(   5,  -4),
    S(  -1,  10), S(  14,   7), S(  24,   2), S(  37,  -3),
    S( -11,  20), S(  27,   7), S(   3,  18), S(  35,   2),
    S(   3,   9), S( -16,  18), S(   8,   7), S(  22,   7),
    S(  35,  21), S(  25,  24), S(   6,  29), S(  16,  25),
};

const int QueenPSQT32[32] = {
    S(  23, -52), S(   5, -39), S(  11, -49), S(  19, -41),
    S(  15, -38), S(  28, -56), S(  33, -70), S(  19, -26),
    S(  12, -22), S(  27, -19), S(   6,   4), S(   9,   0),
    S(  13,  -3), S(  14,  16), S(  -1,  20), S( -17,  61),
    S(  -6,  18), S(  -5,  42), S( -14,  21), S( -31,  70),
    S( -22,  28), S( -14,  19), S( -23,  23), S( -12,  25),
    S(  -5,  27), S( -59,  64), S(  -9,  16), S( -41,  53),
    S(  -7,  17), S(  18,   9), S(   9,   9), S(  -6,  20),
};

const int KingPSQT32[32] = {
    S(  42, -79), S(  41, -50), S( -11, -14), S( -27, -21),
    S(  31, -31), S(  -5, -21), S( -37,   4), S( -51,   7),
    S(   8, -31), S(  17, -27), S(  17,  -4), S(  -9,  11),
    S(   2, -34), S(  83, -37), S(  41,   1), S(  -6,  21),
    S(   4, -19), S(  95, -30), S(  46,  10), S(   2,  19),
    S(  46, -21), S( 120, -19), S(  94,   6), S(  38,   6),
    S(   6, -42), S(  47,  -5), S(  32,  10), S(   9,   3),
...
const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -27, -10), S( -11,   9), S( -15,  28),
    S(   0,  60), S(  45,  66), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -14,  17), S(  -6,  21), S(   2,  43),
    S(  17,  82), S(  32,  54), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -9, -11);

const int PawnStacked[2] = { S(  -7, -17), S(  -9,  -6) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   7,  -4), S(   8,  -6), S(  10, -11),
    S(  11,  -9), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -10, -26), S(  -6, -21), S(   7, -20),
    S(   9, -20), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -2,  -6), S(  11,   1), S(   3,   0), S(   2,  15),
    S(  14,   0), S(  30,  -2), S(  19,   8), S(  22,  16),
    S(   9,   1), S(  21,   3), S(   9,  11), S(  15,  19),
    S(  13,  10), S(  22,  16), S(  30,  21), S(  32,  21),
    S(  57,  25), S(  51,  48), S(  69,  55), S(  84,  58),
...
const int KnightInSiberia[4] = {
    S( -10,   3), S( -11,  -3), S( -16,  -3), S( -18,  -1),
};

const int KnightMobility[9] = {
    S( -74,-104), S( -30, -95), S( -15, -40), S(  -2, -16),
    S(   8,  -5), S(  14,   8), S(  21,  13), S(  29,  14),
    S(  39,   0),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  23,  70);

const int BishopRammedPawns = S(  -8, -10);
...
const int BishopLongDiagonal = S(  23,  14);

const int BishopMobility[14] = {
    S( -65,-147), S( -28, -94), S(  -7, -53), S(   2, -27),
    S(  10, -14), S(  16,  -2), S(  19,   6), S(  20,  12),
    S(  20,  16), S(  23,  17), S(  23,  19), S(  41,   7),
    S(  42,  18), S(  71, -15),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  17,   8), S(  35,   7) };

const int RookOnSeventh = S(   0,  32);

const int RookMobility[15] = {
    S(-148,-113), S( -51,-113), S( -12, -58), S(  -4, -18),
    S(  -5,   1), S(  -7,  17), S(  -5,  27), S(   1,  30),
    S(   7,  32), S(  10,  36), S(  12,  41), S(  15,  45),
    S(  17,  48), S(  28,  39), S(  78,   5),
};

/* Queen Evaluation Terms */

const int QueenMobility[28] = {
    S( -61,-263), S(-210,-387), S( -58,-201), S( -15,-192),
    S(  -2,-139), S(   1, -84), S(   6, -51), S(   5, -23),
    S(   9, -15), S(   9,   5), S(  11,  12), S(  12,  27),
    S(  14,  24), S(  15,  32), S(  13,  36), S(  11,  41),
    S(  10,  43), S(   3,  44), S(   3,  41), S(  -1,  36),
    S(   7,  15), S(  22,  -7), S(  26, -35), S(  30, -52),
    S(  12, -69), S(  24, -96), S( -56, -39), S( -31, -61),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -23,   0), S(  -7,  -1), S(   2,   1), S(   9,   5),
    S(  15,   6), S(  24,   4), S(  31,  -2), S(  13,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB]  = {
    S(  35,  26), S(   9,  21), S(   4,  11), S( -14, -17),
    S( -16, -45), S( -15, -59), S( -14, -66), S( -11, -70),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -11,   3), S(  16, -25), S(  20,  -9), S(  12,   4),
    S(   6,   4), S(   1,   2), S(  -3, -33), S( -49,  18)},
   {S(  17,  -8), S(  22, -17), S(   0,  -6), S( -14,   4),
    S( -30,  15), S( -70,  68), S(  92,  82), S( -26,   1)},
   {S(  35,  -4), S(  15,  -9), S( -27,   7), S( -11,  -8),
    S( -20,  -4), S( -11,   1), S(   0,  66), S( -13,  -3)},
   {S(   4,  11), S(  19, -11), S(   4, -11), S(  14, -21),
    S(  25, -36), S( -58,   5), S(-136,  52), S(   6,  -9)},
   {S( -14,   8), S(   3,  -5), S( -26,   0), S( -18,   4),
    S( -20,  -6), S( -41,  -1), S(  33, -17), S(  -6,  -5)},
   {S(  44, -16), S(  22, -16), S( -22,   1), S( -12, -16),
    S(   5, -24), S(  17, -21), S(  41, -30), S( -26,  -2)},
   {S(  27, -12), S(   2, -16), S( -23,  -3), S( -19,  -8),
    S( -30,  -7), S( -36,  32), S(   0,  44), S( -13,   0)},
   {S(  -9, -10), S(   6, -15), S(   8,  -2), S(  -3,   9),
    S( -12,  15), S(  -9,  37), S(-190,  87), S( -18,  12)}},
  {{S(   0,   0), S( -11, -23), S(   4, -18), S( -40,  16),
    S( -22,   2), S(   3,  42), S(-167,  -8), S( -45,  10)},
   {S(   0,   0), S(  25, -20), S(   6,  -8), S( -18,  -1),
    S(  -1, -12), S(  26,  66), S(-184,  -3), S( -39,   4)},
   {S(   0,   0), S(  29, -12), S(  -2,  -7), S(   7, -17),
    S(  15,  -7), S( -87,  47), S( -84, -73), S( -19,  -5)},
   {S(   0,   0), S(  -4,   7), S(  -3,  -2), S( -18,   1),
    S( -27,   1), S( -99,  31), S(   7, -41), S( -19,  -3)},
   {S(   0,   0), S(   9,   0), S(  11,  -6), S(  11, -11),
    S(  15, -26), S( -57,  15), S(-104, -61), S(   3,  -4)},
   {S(   0,   0), S(   6, -10), S( -20,  -2), S( -27,  -7),
    S(  17, -24), S( -38,   3), S(  55,  38), S( -18,  -5)},
   {S(   0,   0), S(  24, -13), S(  12, -13), S( -11,  -9),
    S( -28,   8), S(  -9,  15), S( -56, -49), S( -33,  11)},
   {S(   0,   0), S(  15, -38), S(  19, -28), S( -18,  -4),
    S( -17,  18), S(  -5,  20), S(-228, -55), S( -21,   5)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S(  -4,  27), S( 116,  -8), S( -26,  23), S( -19,   8),
    S( -12,   3), S(  -7,  -2), S( -17,   6), S( -22,   4)},
   {S(  -3,  48), S(  57,  12), S( -20,  21), S(  -4,  10),
    S(  -3,   4), S(   5,  -4), S(  -1,   2), S( -11,   6)},
   {S(   8,  37), S(  17,  23), S( -24,  18), S( -13,   7),
    S(   3,   1), S(   7,  -2), S(  10,  -6), S(   3,   4)},
   {S(  -2,  24), S(  15,  20), S( -18,   7), S( -16,   0),
    S( -12,  -1), S(   6, -11), S(   0,  -7), S( -13,   6)}},
  {{S(   0,   0), S( -15, -16), S( -16,  -2), S(  20, -16),
    S(  10,  -7), S(   2, -18), S(  -3,  -1), S(  17,  29)},
   {S(   0,   0), S( -16, -34), S(  -4, -10), S(  36, -12),
    S(  -1,  -2), S(  13, -23), S(  -7, -10), S( -17,   2)},
   {S(   0,   0), S( -28, -49), S( -28,  -8), S(  11, -11),
    S(   3,  -1), S(  -8, -14), S( -13, -15), S( -11,   6)},
   {S(   0,   0), S(  -2, -18), S( -17, -19), S( -13,  -6),
    S(  -3,  -7), S(   6, -25), S(  72, -11), S(  13,  20)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -38,   3), S( -55,  21), S( -81,  28),
    S(  -6,  13), S(  71,  -4), S( 157,  56), S(   0,   0)},
   {S(   0,   0), S( -29,   8), S( -50,  25), S( -72,  33),
    S( -12,  32), S(  88,  29), S( 182,  98), S(   0,   0)}},
  {{S(   0,   0), S( -25,  15), S( -49,  22), S( -72,  38),
    S(  -2,  37), S(  90,  39), S( 263, 114), S(   0,   0)},
   {S(   0,   0), S( -28,  12), S( -44,  17), S( -67,  38),
    S(   0,  55), S(  93, 123), S( 162, 277), S(   0,   0)}},
};

const int PassedFriendlyDistance[8] = {
    S(   0,   0), S(  -1,   2), S(   3,  -3), S(   7, -11),
    S(   6, -15), S(  -6, -13), S( -14,  -8), S(   0,   0),
};

const int PassedEnemyDistance[8] = {
    S(   0,   0), S(   4,  -1), S(   5,   2), S(   9,  11),
    S(   2,  26), S(   7,  36), S(  25,  35), S(   0,   0),
};

const int PassedSafePromotionPath = S( -29,  48);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S(  -6, -32);
const int ThreatMinorAttackedByPawn  = S( -52, -52);
const int ThreatMinorAttackedByMinor = S( -24, -35);
const int ThreatMinorAttackedByMajor = S( -21, -45);
const int ThreatRookAttackedByLesser = S( -52, -20);
const int ThreatMinorAttackedByKing  = S( -20, -21);
const int ThreatRookAttackedByKing   = S( -15, -23);
const int ThreatQueenAttackedByOne   = S( -42, -30);
...
const int SpaceCenterControl = S(   1,   4);

/* Closedness Evaluation Terms */

const int ClosednessKnightAdjustment[9] = {
    S( -11,  -6), S(  -8,   7), S(  -6,  13), S(  -2,  15),
    S(   3,  19), S(   5,  17), S(   7,  13), S(  -5,  28),
    S(  -7,  16),
};

const int ClosednessRookAdjustment[9] = {
    S(  47,  13), S(   7,  26), S(   5,  16), S(  -2,   6),
    S(  -6,   4), S(  -8,  -7), S( -13, -11), S( -21, -15),
    S( -26, -16),
};
...
const int ComplexityPawnFlanks  = S(   0,  83);
