...
const int KnightValue = S( 423, 429);
const int BishopValue = S( 436, 444);
const int RookValue   = S( 616, 714);
...
const int PawnPSQT32[32] = {
    ...
    S( -15,   5), S(  -4,  -1), S( -10,   4), S(  -4,  -2),
    S( -21,   4), S( -14,   2), S( -12,  -6), S(  -4, -16),
    S( -15,  10), S(  -9,   7), S(  -1, -10), S(   7, -26),
    S(  -8,  14), S(  -8,   7), S( -12, -10), S(  -9, -24),
    S( -19,  36), S(   2,  33), S(  -2,  19), S(  16,  -5),
    S( -19, -61), S( -56, -20), S(   0, -42), S(  46, -51),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -33, -31), S(  -2, -34), S( -10, -32), S(  -4, -18),
    S(   7, -17), S(  -3, -14), S(   2, -32), S(   8, -15),
    S(  10, -33), S(  13, -20), S(  15, -12), S(  17,  -1),
    S(  22,   3), S(  21,   9), S(  28,  19), S(  29,  29),
    S(  15,   9), S(  27,  12), S(  40,  25), S(  34,  42),
    S( -17,  12), S(   5,  13), S(  28,  31), S(  26,  33),
    S(  12, -14), S(  -2,  -4), S(  34, -21), S(  39,   3),
    S(-177, -25), S( -84,  -2), S(-110,  22), S( -32,  -1),
};

const int BishopPSQT32[32] = {
    S(   7, -36), S(   5, -17), S(   4, -17), S(   6, -12),
    S(  25, -35), S(   8, -48), S(  18, -19), S(  15, -10),
    S(  18, -12), S(  23, -10), S(   1, -21), S(  22,  -1),
    S(   4,  -8), S(  17,  -5), S(  22,  10), S(  23,  11),
    S(  -9,  16), S(  16,  11), S(   7,  15), S(  17,  23),
    S(  -1,   9), S(  -1,  21), S(  -5,   5), S(  11,  16),
    S( -45,  14), S( -47,   3), S(  -5,  12), S( -25,  11),
    S( -59,  -5), S( -45,   7), S( -87,  16), S( -89,  30),
};

const int RookPSQT32[32] = {
    S( -16, -28), S(  -6, -21), S(  -3, -20), S(   2, -26),
    S( -70, -20), S( -13, -36), S( -11, -33), S(  -3, -33),
    S( -30, -23), S(  -7, -12), S( -15, -18), S(  -4, -24),
    S( -24,  -3), S(  -9,   8), S( -11,   6), S(  -1,  -3),
    S(  -6,  16), S(  17,   7), S(  21,   7), S(  40,   0),
...
const int QueenPSQT32[32] = {
    S(  23, -65), S(  10, -48), S(  16, -62), S(  21, -43),
    S(   8, -41), S(  18, -55), S(  24, -73), S(  19, -28),
    S(  11, -30), S(  23, -24), S(   7,   4), S(   6,  -4),
    S(  10,  -7), S(  17,  15), S(  -1,  20), S( -14,  65),
    S(  -5,  14), S(  -6,  49), S( -15,  23), S( -23,  78),
    S( -23,  29), S( -18,  18), S( -21,  24), S(  -8,  28),
    S(  -5,  26), S( -59,  68), S(  -7,  12), S( -41,  54),
    S(  -1,  20), S(  22,  12), S(  11,  15), S(   2,  30),
};

const int KingPSQT32[32] = {
    S(  52, -79), S(  42, -53), S( -10, -15), S( -21, -33),
    S(  23, -18), S( -18, -15), S( -43,   7), S( -73,  12),
    S(  -8, -25), S(   4, -26), S(  10,   0), S( -16,  18),
    S(  -3, -33), S(  90, -35), S(  48,   8), S(  -5,  29),
    S(  10,  -9), S( 109, -26), S(  57,  19), S(  10,  25),
    S(  50, -23), S( 135, -10), S( 105,   9), S(  43,   5),
...
const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -27, -10), S(  -9,   9), S( -20,  29),
    S(  -8,  56), S(  25,  60), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -16,  18), S(  -5,  22), S(   1,  50),
    S(  28,  94), S(  39,  63), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -5, -12);

const int PawnStacked[2] = { S(   1, -22), S(  -4,  -8) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   5,  -3), S(   9,  -8), S(  10, -18),
    S(   8, -17), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -13, -24), S(  -5, -22), S(   2, -26),
    S(  34, -32), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(   0,  -4), S(  10,  -6), S(   6,   2), S(   6,   9),
    S(  10,   2), S(  24,  -5), S(  27,   3), S(  26,  11),
    S(   6,  -2), S(  16,   0), S(  12,   4), S(  17,  12),
    S(   6,  13), S(  16,  17), S(  28,  21), S(  32,  14),
    S(  48,  25), S(  45,  49), S(  63,  61), S(  73,  68),
...
const int KnightInSiberia[4] = {
    S(  -9,  -6), S( -12, -19), S( -24, -14), S( -31,  -9),
};

const int KnightMobility[9] = {
    S( -76,-106), S( -39,-107), S( -20, -46), S(  -7, -10),
    S(   6,  -1), S(   9,  16), S(  16,  18), S(  27,  17),
    S(  40,  -1),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  21,  73);
...
const int BishopMobility[14] = {
    S( -79,-158), S( -40,-114), S( -14, -62), S(  -3, -26),
    S(   7, -14), S(  12,   3), S(  15,  16), S(  17,  17),
    S(  15,  27), S(  24,  25), S(  25,  24), S(  46,  12),
    S(  49,  22), S(  73, -18),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(   6,   9), S(  30,   6) };

const int RookOnSeventh = S(  -4,  36);

const int RookMobility[15] = {
    S(-153,-115), S( -52,-110), S( -24, -74), S( -11, -30),
    S(  -9,  -4), S( -13,  18), S( -11,  31), S(  -4,  33),
    S(   2,  38), S(   6,  39), S(   8,  48), S(  18,  50),
    S(  16,  55), S(  35,  43), S(  92,   3),
};

/* Queen Evaluation Terms */

const int QueenRelativePin = S( -20, -13);

const int QueenMobility[28] = {
    S( -63,-263), S(-214,-388), S( -81,-203), S( -32,-197),
    S( -15,-147), S(  -7, -82), S(   0, -44), S(   1, -14),
    S(   7,  -3), S(   8,  14), S(  15,  18), S(  17,  35),
    S(  20,  25), S(  23,  34), S(  20,  37), S(  17,  44),
    S(  19,  42), S(  12,  44), S(   7,  41), S(  11,  30),
...
const int KingDefenders[12] = {
    S( -26,  -7), S( -15,   4), S(   1,   6), S(  12,   8),
    S(  20,   7), S(  29,   0), S(  32,  -4), S(  12,  -1),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  50,  44), S(  27,  32), S(  11,  14), S( -27, -25),
    S( -30, -60), S( -22, -69), S( -17, -69), S( -11, -64),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S(  -8,   1), S(  13, -25), S(  15,  -6), S(  12,   6),
    S(   6,   1), S(  -5,  -1), S(  -2, -34), S( -41,  24)},
   {S(  17,  -7), S(  13, -15), S(  -2,  -2), S(  -4,   2),
    S( -23,  15), S( -72,  66), S(  91,  80), S( -30,   4)},
   {S(  37,  -2), S(  11,  -6), S( -25,   5), S( -11, -10),
    S( -13,  -3), S( -11,   0), S(   0,  68), S( -15,   0)},
   {S(  13,  10), S(  18, -16), S(  -2, -13), S(   5, -19),
    S(  19, -37), S( -61,   3), S(-135,  51), S(  -5,  -2)},
   {S(  -9,  14), S(  11,  -4), S( -25,   3), S( -11,   2),
    S( -10,  -5), S( -45,  -6), S(  32, -17), S(  -6,   1)},
   {S(  43, -12), S(  10,  -9), S( -21,   3), S(  -8, -25),
    S(   5, -33), S(  18, -21), S(  40, -32), S( -23,   1)},
   {S(  34, -15), S(   8, -22), S( -26,   1), S( -19,  -9),
    S( -33,  -4), S( -41,  29), S(   1,  44), S( -17,   2)},
   {S(   1, -15), S(   7, -23), S(   6,   1), S(   0,  12),
    S( -11,  17), S( -12,  39), S(-189,  90), S( -18,  17)}},
  {{S(   0,   0), S( -14, -29), S(   7, -22), S( -39,  16),
    S( -24,   2), S(   3,  42), S(-167,  -7), S( -36,  17)},
   {S(   0,   0), S(  17, -19), S(   6, -11), S( -18,   0),
    S(   0, -20), S(  27,  65), S(-184,  -3), S( -34,  12)},
   {S(   0,   0), S(  17,  -9), S(   2, -11), S(   2, -21),
    S(  17,  -4), S( -87,  47), S( -84, -74), S( -17,   1)},
   {S(   0,   0), S(   1,   2), S(  -5,  -5), S( -18,   6),
    S( -20,   4), S(-100,  30), S(   7, -40), S( -18,  -1)},
   {S(   0,   0), S(   9,   0), S(   9, -11), S(   6, -13),
    S(  12, -31), S( -57,  14), S(-101, -60), S(  -8,   2)},
   {S(   0,   0), S(   1,  -8), S( -19,   0), S( -14,  -8),
    S(  21, -24), S( -37,   3), S(  55,  39), S( -13,  -2)},
   {S(   0,   0), S(  28, -18), S(  16, -14), S( -10, -11),
    S( -27,  14), S(  -8,  19), S( -57, -49), S( -31,  15)},
   {S(   0,   0), S(  12, -46), S(  13, -32), S( -17,  -8),
    S( -25,  19), S( -10,  20), S(-228, -56), S( -23,   8)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S( -14,  29), S( 112,  -9), S( -20,  19), S( -11,   2),
    S( -15,  -1), S(  -8,  -1), S( -12,   7), S( -26,   1)},
   {S(  -9,  53), S(  56,  14), S( -15,  14), S(  -4,   9),
    S(  -5,   9), S(   6,  -2), S(   0,   2), S( -17,  11)},
   {S(   3,  40), S(   6,  25), S( -19,  14), S( -13,   8),
    S(  -1,   6), S(   9,   2), S(   8,  -8), S(   4,   7)},
   {S(  -7,  22), S(   4,  19), S( -26,   7), S( -15,  -4),
    S(  -9,   0), S(  10, -10), S(   3,  -8), S(  -9,   7)}},
  {{S(   0,   0), S( -18, -16), S( -12,  -4), S(  19, -17),
    S(   5,  -3), S(   6, -18), S(  -1,   3), S(  18,  30)},
   {S(   0,   0), S( -17, -37), S(  -1, -11), S(  41, -13),
    S(   4,  -3), S(  19, -27), S(  -2,  -6), S( -19,   0)},
   {S(   0,   0), S( -28, -52), S( -17, -11), S(  13,  -9),
    S(  10,  -2), S(  -4, -16), S( -11, -13), S( -11,   6)},
   {S(   0,   0), S(  -2, -19), S( -16, -16), S( -10,   2),
...
const int SafetyBishopWeight    = S(  26,  35);
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -36,   1), S( -45,  26), S( -70,  29),
   ...
   {S(   0,   0), S( -30,  13), S( -44,  42), S( -60,  42),
    S(   3,  44), S( 109,  28), S( 195,  93), S(   0,   0)}},
  {{S(   0,   0), S( -26,  27), S( -49,  36), S( -66,  52),
    S(   9,  51), S( 104,  49), S( 271, 107), S(   0,   0)},
   {S(   0,   0), S( -26,  19), S( -43,  33), S( -62,  55),
    S(   9,  72), S( 110, 126), S( 158, 266), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(  -3,   3), S(   1,  -3), S(   8, -12),
    S(   5, -17), S( -10, -16), S( -18,  -5), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   5,  -2), S(   6,  -2), S(   7,   9),
    S(   0,  23), S(   3,  35), S(  24,  35), S(   0,   0),
};

const int PassedSafePromotionPath = S( -32,  52);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -10, -36);
const int ThreatMinorAttackedByPawn  = S( -52, -68);
const int ThreatMinorAttackedByMinor = S( -22, -35);
const int ThreatMinorAttackedByMajor = S( -29, -50);
const int ThreatRookAttackedByLesser = S( -45, -24);
const int ThreatMinorAttackedByKing  = S( -28, -26);
...
const int ComplexityPawnFlanks  = S(   0,  86);
const int ComplexityPawnEndgame = S(   0,  87);
