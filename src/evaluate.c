...
const int PawnValue   = S( 110, 130);
const int KnightValue = S( 461, 412);
const int BishopValue = S( 481, 430);
const int RookValue   = S( 676, 713);
const int QueenValue  = S(1264,1376);
const int KingValue   = S(   0,   0);

const int PieceValues[8][PHASE_NB] = {
    { 110, 130}, { 461, 412}, { 481, 430}, { 676, 713},
...
const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -26, -11), S( -12,   8), S( -12,  27),
    S(   3,  61), S(  47,  68), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -12,  14), S(  -5,  20), S(   4,  44),
    S(  16,  84), S(  33,  52), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -8, -10);

const int PawnStacked = S( -18, -25);

const int PawnBackwards[2] = { S(   8,  -2), S(  -6, -18) };

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -1,  -7), S(  10,   0), S(   4,   1), S(   4,  18),
    S(  15,   0), S(  33,  -1), S(  23,   9), S(  26,  18),
    S(   9,   0), S(  23,   4), S(  10,  11), S(  15,  23),
    S(  16,   7), S(  23,  14), S(  30,  20), S(  34,  21),
...
const int KnightOutpost[2] = { S(   6, -24), S(  30,  -2) };

const int KnightBehindPawn = S(   4,  21);

const int KnightMobility[9] = {
    S( -80,-101), S( -31, -98), S( -16, -42), S(  -3, -17),
    S(   9,  -6), S(  15,   8), S(  25,  12), S(  34,  12),
    S(  43,   0),
};
...
const int BishopOutpost[2] = { S(   9, -10), S(  41,   0) };

const int BishopBehindPawn = S(   4,  19);

const int BishopMobility[14] = {
    S( -63,-145), S( -26, -94), S(  -6, -55), S(   2, -29),
    S(  12, -16), S(  19,  -1), S(  21,   8), S(  21,  14),
    S(  21,  19), S(  24,  20), S(  22,  20), S(  39,   9),
    S(  40,  13), S(  65, -19),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  19,   5), S(  40,   2) };

const int RookOnSeventh = S(   0,  32);

const int RookMobility[15] = {
    S(-148,-112), S( -51,-115), S( -14, -62), S(  -5, -20),
    S(  -5,   0), S(  -4,  14), S(  -4,  25), S(   1,  28),
    S(   8,  31), S(  12,  35), S(  14,  41), S(  18,  46),
...
const int QueenMobility[28] = {
    S( -61,-263), S(-210,-386), S( -59,-201), S( -25,-191),
    S( -13,-140), S(  -7, -89), S(  -2, -61), S(  -2, -34),
    S(   0, -23), S(   0,   0), S(   3,  10), S(   4,  23),
    S(   6,  24), S(   8,  34), S(   5,  39), S(   5,  42),
    S(   2,  46), S(  -1,  45), S(   0,  39), S(  -3,  33),
    S(   4,  12), S(  18,  -6), S(  19, -36), S(  21, -54),
    S(   5, -69), S(  20, -95), S( -56, -39), S( -27, -54),
};

/* King Evaluation Terms */

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -11,   4), S(  17, -23), S(  18,  -8), S(   8,   1),
    S(   3,   2), S(   6,   1), S(  -2, -32), S( -49,  18)},
   {S(  16,  -5), S(  22, -17), S(   0,  -4), S( -17,   1),
    S( -30,  12), S( -69,  68), S(  92,  83), S( -29,   1)},
   {S(  32,   0), S(  17,  -8), S( -25,   6), S( -11,  -9),
    S( -22,  -4), S(  -7,   0), S(   0,  66), S( -16,  -1)},
   {S(   3,  13), S(  21,  -7), S(   7, -10), S(  12, -23),
    S(  21, -35), S( -51,   1), S(-136,  51), S(   0,  -5)},
   {S( -12,   8), S(   1,  -1), S( -24,   0), S( -18,   1),
    S( -20,  -8), S( -38,   0), S(  33, -15), S(  -9,  -2)},
   {S(  45, -12), S(  28, -17), S( -19,   0), S( -15, -20),
    S(   3, -25), S(  16, -23), S(  41, -29), S( -28,   0)},
   {S(  24,  -9), S(   2, -15), S( -22,  -3), S( -19, -11),
    S( -29,  -9), S( -34,  30), S(   0,  44), S( -13,   1)},
   {S(  -9,  -7), S(   9, -16), S(  10,  -1), S(  -5,   6),
    S( -12,  12), S(  -9,  36), S(-190,  87), S( -20,  14)}},
  {{S(   0,   0), S(  -9, -21), S(   0, -15), S( -42,  15),
    S( -18,   0), S(   2,  42), S(-166,  -8), S( -45,  10)},
   {S(   0,   0), S(  26, -19), S(   6,  -7), S( -19,  -3),
    S(   0, -15), S(  27,  70), S(-183,  -3), S( -42,   5)},
   {S(   0,   0), S(  35, -10), S(  -1,  -5), S(   7, -18),
    S(  12,  -8), S( -90,  47), S( -84, -74), S( -22,  -4)},
   {S(   0,   0), S(   0,   9), S(   0,   1), S( -15,   0),
    S( -26,   0), S(-100,  29), S(   7, -41), S( -23,  -5)},
   {S(   0,   0), S(  12,   4), S(  14,  -4), S(  11, -11),
    S(  12, -24), S( -54,  14), S(-104, -62), S(  -5,  -6)},
   {S(   0,   0), S(  10,  -5), S( -19,   0), S( -27,  -8),
    S(  17, -26), S( -38,   2), S(  55,  38), S( -21,  -3)},
   {S(   0,   0), S(  23, -11), S(  14, -11), S( -12,  -9),
    S( -29,   4), S(  -8,  13), S( -56, -50), S( -35,  13)},
   {S(   0,   0), S(  14, -35), S(  19, -26), S( -18,  -5),
    S( -17,  14), S(  -1,  15), S(-228, -56), S( -23,   4)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S(   0,  23), S( 114, -11), S( -26,  23), S( -21,  10),
    S( -11,   2), S(  -8,  -2), S( -18,   6), S( -21,  -2)},
   {S(   0,  45), S(  57,  11), S( -18,  23), S(  -5,  11),
    S(  -1,   4), S(   4,  -3), S(   0,   0), S( -11,   0)},
   {S(  11,  34), S(  19,  20), S( -24,  20), S( -12,   9),
    S(   4,   2), S(   6,   0), S(   6,  -7), S(   2,  -1)},
   {S(   0,  22), S(  19,  17), S( -13,   7), S( -14,   2),
    S( -11,   0), S(   6, -10), S(   0,  -7), S( -14,   2)}},
  {{S(   0,   0), S( -11, -13), S( -15,   0), S(  21, -15),
    S(  10,  -7), S(  -1, -17), S(  -7,   0), S(  16,  27)},
   {S(   0,   0), S( -13, -28), S(  -6,  -9), S(  32, -12),
    S(  -1,   0), S(   9, -22), S( -11,  -8), S( -15,   1)},
   {S(   0,   0), S( -23, -42), S( -29,  -7), S(   9,  -9),
    S(   0,  -1), S(  -9, -14), S( -13, -13), S( -10,   4)},
   {S(   0,   0), S(  -1, -14), S( -16, -17), S( -10,  -2),
    S(  -4,  -5), S(   4, -29), S(  67,  -6), S(  12,  18)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -37,   0), S( -53,  21), S( -82,  26),
    S(  -5,  16), S(  66,   0), S( 152,  59), S(   0,   0)},
   {S(   0,   0), S( -26,   1), S( -45,  20), S( -70,  25),
    S( -12,  27), S(  84,  27), S( 183,  94), S(   0,   0)}},
  {{S(   0,   0), S( -25,  10), S( -47,  16), S( -74,  32),
    S(  -2,  34), S(  87,  41), S( 258, 122), S(   0,   0)},
   {S(   0,   0), S( -27,   6), S( -39,  13), S( -64,  35),
    S(  -2,  57), S(  76, 139), S( 156, 301), S(   0,   0)}},
};

const int PassedFriendlyDistance[RANK_NB] = {
    S(   0,   0), S(   0,   0), S(   2,  -4), S(   7, -11),
    S(   6, -16), S(  -6, -16), S( -12, -11), S(   0,   0),
};

const int PassedEnemyDistance[RANK_NB] = {
    S(   0,   0), S(   3,   0), S(   4,   1), S(   8,  10),
    S(   1,  25), S(   7,  34), S(  24,  37), S(   0,   0),
};
...
const int ThreatMinorAttackedByPawn  = S( -56, -46);
const int ThreatMinorAttackedByMinor = S( -27, -35);
const int ThreatMinorAttackedByMajor = S( -25, -44);
const int ThreatRookAttackedByLesser = S( -57, -10);
