...
const int KingSafetyThreatWeight[PIECE_NB] = { 5, 14, 11, 11, 10, 0 };

const int KingSafetyBaseLine = 15;

const int KingSafetyWeakSquares = 9;

const int KingSafetyFriendlyPawns = -11;

const int KingSafetyNoEnemyQueens = -133;
...
int evaluateKings(EvalInfo* ei, Board* board, int colour){
    ...
    if (ei->attackerCounts[!colour] >= 2){
        ...
        int mgCount = -MAX(0, count) * MAX(0, count) / 64;
        int egCount = -MAX(0, count);

        eval += MakeScore(mgCount, egCount);
