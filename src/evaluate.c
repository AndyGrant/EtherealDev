...
const int PawnStacked = S( -21, -17);
const int PawnBackwards[2] = {
    S(   8,  -5), S(  -7, -12),
};
const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -4,   0), S(  10,   2), S(   3,   7), S(   5,  20),
    S(  15,   7), S(  32,   4), S(  22,  16), S(  28,  21),
    S(   9,   3), S(  23,  11), S(  12,  15), S(  15,  27),
    S(  13,  11), S(  24,  17), S(  33,  26), S(  35,  21),
    S(  31,  55), S(  29,  67), S(  65,  63), S(  55,  74),
    S( 106, -13), S( 199,  17), S( 227,  22), S( 249,  75),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightMobility[9] = {
    S( -90, -86), S( -49, -96), S( -37, -43), S( -23, -21),
    S( -13,  -9), S(  -6,   1), S(   2,   6), S(  11,   2),
    S(  25, -13),
};
const int BishopPair = S(  15,  81);
const int BishopRammedPawns = S( -11, -17);
const int BishopOutpost[2] = {
    S(  10,  -4), S(  40,   0),
};
const int BishopBehindPawn = S(   4,  15);
const int BishopMobility[14] = {
    S( -62,-129), S( -32, -73), S( -14, -53), S(  -4, -29),
    S(   3, -17), S(  12,  -4), S(  14,   4), S(  12,  10),
    S(  12,  12), S(  15,  15), S(  19,   5), S(  39,  -6),
    S(  45,  -3), S(  41, -32),
};
const int RookFile[2] = {
    S(  19,   0), S(  36,   0),
};
const int RookOnSeventh = S(  -8,  31);
const int RookMobility[15] = {
    S(-147,-107), S( -59,-119), S( -24, -68), S( -14, -25),
    S( -13,  -5), S( -15,  10), S( -14,  22), S(  -7,  25),
    S(   0,  29), S(   4,  33), S(   5,  41), S(  12,  44),
    S(  17,  46), S(  23,  43), S(  22,  46),
};
const int QueenMobility[28] = {
    S( -61,-263), S(-216,-389), S( -48,-204), S( -34,-189),
    S( -23,-132), S( -17, -70), S( -11, -90), S( -11, -76),
    S(  -6, -61), S(  -5, -48), S(  -4, -30), S(  -1, -23),
    S(   0, -16), S(   2,  -8), S(   0,   0), S(  -1,   6),
    S(  -4,  14), S( -10,  14), S(  -3,  17), S(  -3,  22),
    S(   0,  19), S(  14,  20), S(   6,   0), S(  31,   4),
...
const int KingDefenders[12] = {
    S( -25,  -3), S(  -7,   2), S(   0,   1), S(   8,   1),
    S(  16,   0), S(  25,  -2), S(  33,  11), S(  23,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};
const int KingShelter[2][8][8] = {
  {{S( -18,  13), S(  12, -10), S(  11,  -2), S(  17,   1), S(   7,   7), S(  30,   4), S(   0, -33), S( -36,  11)},
   {S(   2,   2), S(  22,  -8), S(   5,  -9), S(  -2,  -9), S( -25,   0), S( -66,  78), S( 100,  93), S( -32,   5)},
   {S(  15,  10), S(  12,   0), S( -23,   2), S(  -8,  -3), S( -22,  -3), S(  13,  -8), S(   4,  65), S( -16,   4)},
   {S(   5,  24), S(  16,   1), S(   1,  -6), S(  11, -16), S(  25, -35), S( -20, -32), S(-147,  41), S(  -1,  -1)},
   {S( -18,  16), S(   1,   3), S( -29,   4), S( -15,   4), S( -25, -15), S( -41, -19), S(  38,  -8), S( -10,   0)},
   {S(  33,  -3), S(  22, -11), S( -21,   6), S( -11, -23), S(   9, -33), S(  17, -48), S(  49, -34), S( -21,   3)},
   {S(  17,  -1), S(  -3,  -9), S( -28,  -1), S( -19, -12), S( -25, -20), S( -42,   9), S(   6,  47), S( -15,   4)},
   {S( -17,  -2), S(   5, -12), S(   5,   2), S(  -2,   6), S( -10,   9), S(   3,  31), S(-201,  79), S( -19,  15)}},
  {{S(   0,   0), S(  -2, -15), S(   1, -15), S( -46,  18), S(   0,  -9), S(   9,  51), S(-170, -14), S( -49,  14)},
   {S(   0,   0), S(  18, -11), S(   0,  -4), S(  -9,  -7), S(  10, -32), S(  51, 102), S(-187, -10), S( -44,  10)},
   {S(   0,   0), S(  36,  -3), S(   3,  -7), S(  11, -26), S(  14,  -7), S(-113,  45), S( -89, -84), S( -23,   3)},
   {S(   0,   0), S(  -4,  10), S(  -6,  10), S( -10,   4), S( -22,   1), S(-116,  10), S(   5, -50), S( -27,  -2)},
   {S(   0,   0), S(  10,   4), S(  10,  -4), S(  13,  -7), S(  15, -25), S( -44,   7), S(-109, -73), S(  -6,  -5)},
   {S(   0,   0), S(   3,   0), S( -15,   1), S( -14, -13), S(  29, -37), S( -45,   4), S(  59,  46), S( -30,   0)},
   {S(   0,   0), S(  16,  -4), S(   5,  -5), S( -16,  -3), S( -28, -12), S(  19,  10), S( -58, -64), S( -44,  19)},
   {S(   0,   0), S(   7, -25), S(  12, -18), S( -24,  -1), S( -25,  -1), S(   7, -16), S(-239, -73), S( -36,  15)}},
};
const int KingStorm[2][4][8] = {
  {{S(   3,  11), S(  41,  24), S(  -6,  12), S( -15,   6), S(  -7,   1), S(  -2,  -3), S( -13,   5), S( -17,  -2)},
   {S(   6,  32), S(  16,  20), S(  -4,   6), S(  -5,   3), S(  -3,   2), S(   7,  -7), S(   1,   1), S( -14,   0)},
   {S(   4,  22), S(   5,  22), S( -10,   4), S( -12,   0), S(   0,  -3), S(   1,  -2), S(   2,  -6), S(   0,   0)},
   {S(   6,  19), S(   7,  20), S(  -4,  -4), S( -11,  -8), S( -11,  -2), S(  11, -12), S(   0,  -3), S( -16,   1)}},
  {{S(   0,   0), S(   0,   0), S(   2, -16), S(  20, -16), S(   6,   0), S(   0,  -8), S(  -6,  -1), S(   0,   1)},
   {S(   0,   0), S(   0,   0), S(  -1, -21), S(  24, -10), S(  -2,   1), S(  -1,   1), S(  -3,   0), S(  -2,   0)},
   {S(   0,   0), S(  -1,  -2), S( -10, -20), S(  -1,  -5), S(  -5,   5), S( -21,  -4), S(  -4,  -1), S(   0,   0)},
   {S(   0,   0), S(   0,   0), S(  -9, -22), S(  -5,  -5), S(  -3,  -6), S(   5, -13), S(  20,   1), S(   0,   0)}},
};
...
const int PassedPawn[2][2][8] = {
  {{S(   0,   0), S( -29, -26), S( -26,   2), S( -12,   1), S(  19,   7), S(  57,   6), S( 140,  37), S(   0,   0)},
   {S(   0,   0), S( -12, -17), S( -26,   4), S( -15,  20), S(   3,  47), S(  66,  72), S( 192, 142), S(   0,   0)}},
  {{S(   0,   0), S( -21,  -5), S( -23,  -5), S( -14,  22), S(  17,  46), S(  70,  89), S( 242, 171), S(   0,   0)},
   {S(   0,   0), S( -26, -17), S( -21, -12), S( -12,  32), S(   0,  92), S(  41, 204), S( 126, 375), S(   0,   0)}},
};
const int PassedFriendlyDistance = S(   3,  -7);
const int PassedEnemyDistance = S(   0,  11);
const int PassedSafePromotionPath = S(  -2,  26);


/* Threat Evaluation Terms */

const int ThreatWeakPawn = S( -14, -36);
const int ThreatMinorAttackedByPawn = S( -53, -51);
const int ThreatMinorAttackedByMajor = S( -35, -40);
const int ThreatRookAttackedByLesser = S( -45, -25);
const int ThreatQueenAttackedByOne = S( -51,   0);
const int ThreatOverloadedPieces = S( -14, -15);
