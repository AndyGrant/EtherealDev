...
const int PawnValue   = S( 100, 125);
const int KnightValue = S( 468, 397);
const int BishopValue = S( 478, 422);
const int RookValue   = S( 644, 724);
const int QueenValue  = S(1322,1359);
const int KingValue   = S( 170, 170);

const int PieceValues[8][PHASE_NB] = {
    { 100, 125}, { 468, 397}, { 478, 422}, { 644, 724},
    {1322,1359}, { 170, 170}, {   0,   0}, {   0,   0},
};
...
const int PawnStacked = S( -11, -33);

const int PawnBackwards[2] = { S(   7,  -3), S( -12, -12) };

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(   0, -16), S(   6,   2), S(   3,  -3), S(   5,  21),
    S(   7,   0), S(  21,   0), S(  15,   8), S(  17,  21),
    S(   6,   0), S(  20,   4), S(  14,   7), S(  16,  16),
    S(   6,  11), S(  19,  19), S(  18,  24), S(  37,  24),
    S(  24,  53), S(  23,  61), S(  71,  62), S(  53,  75),
    S(  68, -28), S( 174,  -6), S( 245,  16), S( 173,  60),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightOutpost[2] = { S(  17, -37), S(  36,   6) };

const int KnightMobility[9] = {
    S( -91, -96), S( -38, -93), S( -18, -43),
    S(  -6, -14), S(   2, -14), S(   7,   0),
    S(  17,  -2), S(  32,  -2), S(  49, -35),
};


// Definition of evaluation terms related to Bishops

const int BishopPair = S(  42,  70);

const int BishopRammedPawns = S( -11,  -7);

const int BishopOutpost[2] = { S(  19, -17), S(  51, -10) };

const int BishopMobility[14] = {
    S( -63,-134), S( -51, -69), S( -19, -48), S(  -6, -22),
    S(   4,  -9), S(  17,   0), S(  22,   7), S(  29,   4),
    S(  29,  10), S(  36,   3), S(  39,   4), S(  51, -15),
    S(  45,  -4), S(  36, -35),
};
...
const int RookFile[2] = { S(  12,   0), S(  40,  -8) };

const int RookOnSeventh = S(   0,  25);

const int RookMobility[15] = {
    S(-169,-111), S( -79,-125), S( -16, -71), S(  -9, -28),
    S(  -8,  -4), S(  -8,  14), S(  -8,  25), S(  -3,  32),
    S(   1,  36), S(   6,  37), S(  10,  42), S(  20,  49),
    S(  21,  51), S(  28,  47), S(  21,  48),
};
...
const int QueenMobility[28] = {
    S( -61,-265), S(-247,-380), S( -46,-209), S( -37,-195),
    S( -16,-128), S( -26, -65), S( -18, -93), S( -19, -84),
    S( -15, -61), S( -10, -55), S(  -8, -28), S(  -6, -29),
    S(  -5, -15), S(   0, -10), S(   1,  -6), S(  -1,   4),
    S(   5,  17), S(   1,  15), S(  14,  26), S(   0,  25),
    S(   4,  26), S(  22,  30), S(  15,   9), S(  41,  14),
    S(  52,  24), S(  62,   2), S( -30,  -6), S(  31,  10),
};
...
const int KingDefenders[12] = {
    S( -41,  -5), S( -20,   6), S(   0,   1), S(  10,   0),
    S(  25,  -3), S(  37,   2), S(  39,  12), S(  37,-197),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -18,  16), S(   5, -11), S(  18,   2), S(  25,   2), S(  15,   4), S(  35, -10), S( -58, -79), S( -34,   1)},
   {S(   4,   7), S(  16,  -8), S(  15, -10), S(   1, -14), S( -33,  -3), S( -60,  83), S(  76,  87), S( -33,   0)},
   {S(  14,  15), S(   9,  -1), S( -20,   0), S(  -8,  -3), S( -22,  -3), S(   0, -16), S(   1,  52), S( -18,   2)},
   {S(  12,  33), S(  14,   0), S(  -4,  -9), S(  20, -15), S(  16, -36), S( -17, -34), S(-117,  38), S(  -1,   0)},
   {S( -19,  21), S(  -1,   3), S( -32,   1), S( -12,   5), S( -32, -18), S( -50, -22), S(  23, -13), S( -16,   1)},
   {S(  22,   3), S(  17,  -4), S( -19,   0), S(  -2, -21), S(   7, -32), S(  19, -49), S(  71, -42), S( -13,   0)},
   {S(  20,   3), S(   1, -10), S( -31,  -8), S( -19, -14), S( -23, -19), S( -35,   9), S(  13,  50), S( -28,   8)},
   {S( -20,  -3), S(  -1, -10), S(   5,   0), S(   4,   2), S( -11,  14), S(   6,  34), S(-174,  79), S( -18,  15)}},
  {{S(   0,   0), S(   2, -18), S(   7, -14), S( -43,  19), S(   8,  -9), S(   0,  37), S(-167,   3), S( -53,  10)},
   {S(   0,   0), S(  17,  -7), S(   8,  -8), S(   0,  -8), S(  11, -40), S(  42,  91), S(-175,   3), S( -47,   3)},
   {S(   0,   0), S(  24,   0), S(   0,  -7), S(  21, -27), S(  11,  -8), S(-107,  48), S( -83, -80), S( -22,  -1)},
   {S(   0,   0), S(  -7,   8), S( -15,  15), S( -18,   4), S( -23,   2), S(-126,  10), S( -43, -66), S( -27,   1)},
   {S(   0,   0), S(   5,   5), S(   8,  -7), S(  24,  -4), S(   8, -22), S( -53,   8), S(-101, -74), S(  -6,  -2)},
   {S(   0,   0), S(   9,   2), S( -18,  -1), S( -13, -14), S(  33, -37), S( -36,   9), S(  76,  33), S( -32,   0)},
   {S(   0,   0), S(  13,  -1), S(  -2,   0), S( -20,  -4), S( -19, -13), S(  31,   9), S( -58, -54), S( -44,  13)},
   {S(   0,   0), S(   8, -28), S(  10, -16), S( -21,   0), S( -25,  -5), S(  17, -20), S(-192, -85), S( -45,  16)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -34, -29), S( -26,   9), S( -16,  -2), S(  20,  -1), S(  61,  -5), S( 157,  35), S(   0,   0)},
   {S(   0,   0), S(   0,   5), S( -19,  26), S( -12,  37), S(   5,  45), S(  69,  63), S( 199, 132), S(   0,   0)}},
  {{S(   0,   0), S(  -6,  18), S( -12,  10), S(  -8,  29), S(  28,  34), S(  79,  66), S( 236, 151), S(   0,   0)},
   {S(   0,   0), S(  -7,  12), S( -11,  21), S( -18,  56), S(  -9, 112), S(  24, 207), S( 103, 373), S(   0,   0)}},
};


// Definition of evaluation terms related to Threats

const int ThreatPawnAttackedByOne    = S( -19, -16);

const int ThreatMinorAttackedByPawn  = S( -66, -48);

const int ThreatMinorAttackedByMajor = S( -36, -29);

const int ThreatMajorAttackedByMinor = S( -38, -19);
