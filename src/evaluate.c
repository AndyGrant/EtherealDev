...
const int PawnValue   = S(  93, 107);
const int KnightValue = S( 405, 375);
const int BishopValue = S( 416, 386);
const int RookValue   = S( 585, 621);
const int QueenValue  = S(1134,1190);
...
const int PawnPSQT32[32] = {
    ...
    S( -19,   7), S(   1,   2), S( -10,   3), S(  -6,  -2),
    S( -23,   4), S( -13,   4), S( -10,  -5), S(  -4, -12),
    S( -17,  10), S( -13,  10), S(   6,  -9), S(   5, -20),
    S(  -9,  12), S(  -5,   8), S(  -9,  -5), S(  -3, -18),
    S(  -7,  25), S(  -3,  21), S(  -2,  13), S(  22, -12),
    S( -17, -59), S( -63, -31), S(  -3, -45), S(  36, -60),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};

const int KnightPSQT32[32] = {
    S( -42, -27), S(  -6, -28), S( -10, -27), S(   2, -16),
    S(   0, -20), S(   4, -12), S(   4, -26), S(  10, -18),
    S(   9, -24), S(  19, -22), S(  13, -18), S(  22,  -7),
    S(  18,   1), S(  18,   4), S(  27,  11), S(  27,  17),
    S(  21,  12), S(  25,   6), S(  34,  20), S(  27,  30),
    S( -18,  10), S(   1,  10), S(  28,  21), S(  24,  23),
    S(  11, -12), S(  -6,  -2), S(  23, -19), S(  39,  -2),
    S(-168, -18), S( -81,  -1), S(-109,  18), S( -33,  -2),
};

const int BishopPSQT32[32] = {
    S(   4, -25), S(   8, -17), S(  -9,  -9), S(   9, -12),
    S(  26, -32), S(   8, -34), S(  17, -19), S(  11, -12),
    S(  12, -12), S(  24, -15), S(   3, -18), S(  19,  -6),
    S(  12,  -8), S(  17,  -4), S(  14,   2), S(  21,   4),
    S( -11,  10), S(  17,   3), S(   3,  11), S(  10,  14),
    S(   1,   4), S(  -5,  14), S(  -2,   3), S(  13,  10),
    S( -43,  13), S( -50,   2), S( -11,   6), S( -23,  10),
    S( -53,  -8), S( -43,   6), S( -83,  13), S( -88,  25),
};

const int RookPSQT32[32] = {
    S( -12, -27), S( -15, -19), S(  -3, -23), S(   4, -27),
    S( -49, -14), S( -11, -32), S(  -9, -30), S(   2, -34),
    S( -26, -15), S(  -7, -13), S( -16, -16), S(   0, -25),
    S( -16,  -4), S( -11,   4), S(  -9,   0), S(   5,  -6),
    S(  -6,   7), S(   8,   5), S(  16,   1), S(  33,  -3),
    S( -18,  17), S(  19,   4), S(  -2,  17), S(  27,   1),
    S(  -1,   3), S( -14,  11), S(   5,   2), S(  21,   3),
    S(  32,  16), S(  23,  17), S(   4,  24), S(  14,  20),
};

const int QueenPSQT32[32] = {
    S(  23, -58), S(   2, -41), S(  10, -52), S(  16, -39),
    S(  11, -40), S(  22, -53), S(  24, -65), S(  14, -26),
    S(   7, -27), S(  20, -22), S(   3,   0), S(   4,  -3),
    S(   8, -12), S(  10,   7), S(  -5,  12), S( -17,  52),
    S( -13,  12), S( -10,  34), S( -17,  11), S( -33,  58),
    S( -30,  25), S( -24,  15), S( -27,  12), S( -18,  13),
    S(  -8,  23), S( -59,  55), S( -14,   6), S( -46,  44),
    S( -15,  11), S(  12,   3), S(   1,   1), S( -13,  12),
};

const int KingPSQT32[32] = {
    S(  42, -72), S(  37, -47), S( -14,  -9), S( -32, -16),
    S(  28, -24), S(  -6, -15), S( -36,   9), S( -55,  12),
    S(   6, -24), S(  11, -20), S(  14,   1), S( -12,  16),
    S(   6, -30), S(  87, -33), S(  45,   5), S(   0,  23),
    S(   8, -16), S( 100, -31), S(  52,  11), S(   9,  19),
    S(  47, -28), S( 124, -23), S(  98,   2), S(  41,   2),
    S(   5, -54), S(  46, -11), S(  30,   4), S(   8,  -3),
    S(   6,-110), S(  72, -60), S( -22, -19), S( -20, -16),
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -24, -10), S( -12,  11), S( -15,  28),
    S(  -1,  55), S(  34,  61), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -13,  18), S(  -4,  19), S(   3,  41),
    S(  13,  74), S(  29,  54), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S(  -6, -11);

const int PawnStacked[2] = { S(  -4, -17), S(  -7,  -8) };

const int PawnBackwards[2][RANK_NB] = {
   {S(   0,   0), S(   3,  -5), S(   8,  -5), S(   8, -10),
    S(   9, -11), S(   0,   0), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S(  -8, -24), S(  -5, -20), S(   3, -22),
    S(  27, -23), S(   0,   0), S(   0,   0), S(   0,   0)},
};

const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
    S(  -2,  -5), S(  11,   0), S(   2,   1), S(   5,  11),
    S(  12,   0), S(  24,  -3), S(  19,   5), S(  21,  11),
    S(   8,  -1), S(  21,   2), S(   8,   8), S(  14,  13),
    S(   9,   8), S(  19,  12), S(  24,  19), S(  26,  17),
    S(  49,  18), S(  38,  40), S(  58,  47), S(  70,  51),
    S( 111,  -5), S( 203,   2), S( 227,  26), S( 238,  43),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightInSiberia[4] = {
    S(  -7,   0), S(  -8,  -4), S( -17,  -3), S( -21,  -5),
};

const int KnightMobility[9] = {
    S( -73,-105), S( -32, -95), S( -19, -43), S(  -9, -20),
    S(   1, -12), S(   5,   1), S(  12,   5), S(  20,   5),
    S(  31,  -9),
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  20,  59);

const int BishopRammedPawns = S(  -7, -12);
...
const int BishopLongDiagonal = S(  19,  11);

const int BishopMobility[14] = {
    S( -61,-143), S( -30, -89), S( -12, -50), S(  -3, -28),
    S(   6, -19), S(  13,  -8), S(  15,   1), S(  15,   6),
    S(  14,  11), S(  17,  12), S(  17,  12), S(  34,   2),
    S(  36,  11), S(  65, -21),
};

/* Rook Evaluation Terms */

const int RookFile[2] = { S(  11,   3), S(  30,   3) };

const int RookOnSeventh = S(  -6,  27);

const int RookMobility[15] = {
    S(-148,-113), S( -54,-108), S( -19, -57), S( -12, -21),
    S( -12,  -4), S( -12,   9), S( -11,  19), S(  -6,  21),
    S(   1,  24), S(   3,  28), S(   6,  34), S(  11,  36),
    S(  11,  41), S(  24,  30), S(  81,  -9),
};

/* Queen Evaluation Terms */

const int QueenMobility[28] = {
    S( -61,-263), S(-210,-387), S( -56,-200), S( -23,-190),
    S( -10,-137), S(  -8, -78), S(  -3, -46), S(  -3, -17),
    S(   2, -17), S(   2,   3), S(   6,   6), S(   7,  20),
    S(   9,  14), S(  11,  22), S(   8,  26), S(   4,  32),
    S(   5,  31), S(  -4,  34), S(  -2,  31), S(  -7,  25),
    S(   3,  -1), S(  17, -22), S(  20, -52), S(  21, -69),
    S(   5, -82), S(  17,-109), S( -57, -41), S( -33, -65),
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -16,   2), S(  -4,  -1), S(   2,   3), S(   8,   5),
    S(  15,   5), S(  23,   2), S(  24,  -4), S(  12,   0),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  30,  27), S(  18,  20), S(  11,   9), S( -18, -14),
    S( -20, -45), S( -17, -61), S( -16, -68), S( -11, -67),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -10,   1), S(  11, -22), S(  20,  -8), S(  12,   4),
    S(   8,   3), S(  -1,   1), S(  -3, -33), S( -42,  17)},
   {S(  15,  -7), S(  17, -16), S(   1,  -5), S( -15,   3),
    S( -29,  15), S( -71,  67), S(  91,  81), S( -15,   0)},
   {S(  32,  -4), S(  10,  -8), S( -26,   5), S( -11,  -9),
    S( -17,  -4), S( -13,   0), S(   0,  67), S( -10,  -2)},
   {S(   3,   9), S(  21, -12), S(   4, -11), S(  12, -19),
    S(  26, -35), S( -57,   5), S(-136,  52), S(   6,  -7)},
   {S( -15,   9), S(   4,  -3), S( -25,   1), S( -18,   5),
    S( -19,  -7), S( -43,  -4), S(  32, -18), S(  -4,  -1)},
   {S(  42, -16), S(  19, -14), S( -18,   2), S( -10, -18),
    S(   5, -24), S(  15, -19), S(  41, -30), S( -20,   0)},
   {S(  25, -14), S(  -2, -15), S( -22,  -2), S( -21,  -7),
    S( -28,  -7), S( -39,  30), S(   0,  44), S(  -7,   0)},
   {S(  -8, -11), S(   3, -15), S(   6,   2), S(  -3,  10),
    S( -13,  15), S( -12,  34), S(-189,  87), S( -15,  13)}},
  {{S(   0,   0), S( -12, -23), S(   7, -22), S( -38,  17),
    S( -24,   2), S(   3,  42), S(-167,  -7), S( -38,  11)},
   {S(   0,   0), S(  20, -19), S(   5,  -6), S( -20,  -1),
    S(  -1, -15), S(  24,  64), S(-184,  -3), S( -27,   4)},
   {S(   0,   0), S(  22, -11), S(  -1, -11), S(   3, -20),
    S(  16,  -6), S( -86,  47), S( -84, -73), S( -16,  -3)},
   {S(   0,   0), S(  -8,   6), S(  -5,  -3), S( -21,   2),
    S( -27,   3), S( -99,  31), S(   7, -41), S( -16,  -3)},
   {S(   0,   0), S(  12,   1), S(  11,  -6), S(  13, -12),
    S(  14, -27), S( -55,  16), S(-104, -60), S(   2,  -2)},
   {S(   0,   0), S(   0,  -6), S( -22,   0), S( -22,  -9),
    S(  19, -24), S( -37,   3), S(  55,  38), S( -11,  -5)},
   {S(   0,   0), S(  18, -14), S(   9, -13), S( -12,  -7),
    S( -27,   9), S(  -9,  16), S( -56, -49), S( -27,  11)},
   {S(   0,   0), S(   9, -36), S(  17, -28), S( -19,  -6),
    S( -20,  17), S(  -8,  20), S(-228, -55), S( -19,   3)}},
};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S(  -8,  27), S( 105,  -7), S( -21,  22), S( -14,   4),
    S( -13,   1), S(  -8,  -3), S( -16,   4), S( -23,   2)},
   {S(  -7,  47), S(  48,  12), S( -15,  19), S(  -2,   8),
    S(  -3,   4), S(   4,  -3), S(  -3,   3), S( -12,   4)},
   {S(   3,  37), S(   7,  24), S( -20,  17), S(  -9,   5),
    S(   2,   1), S(   5,   0), S(   8,  -4), S(   3,   2)},
   {S(  -5,  25), S(   6,  20), S( -18,   7), S( -13,  -1),
    S( -11,   0), S(   7, -11), S(   3, -10), S( -10,   3)}},
  {{S(   0,   0), S( -17, -17), S( -13,  -2), S(  18, -15),
    S(   7,  -4), S(   4, -17), S(  -1,  -1), S(  18,  29)},
   {S(   0,   0), S( -17, -36), S(   6, -10), S(  37, -10),
    S(   1,  -1), S(  12, -20), S(  -7,  -8), S( -18,   0)},
   {S(   0,   0), S( -29, -52), S( -18,  -8), S(  14, -11),
    S(   3,   0), S(  -7, -11), S( -12, -12), S( -12,   6)},
   {S(   0,   0), S(  -2, -19), S( -13, -16), S( -11,  -3),
    S(  -2,  -6), S(   5, -21), S(  71, -12), S(  14,  21)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -36,   4), S( -46,  25), S( -70,  30),
    S(   0,  12), S(  73,  -8), S( 160,  52), S(   0,   0)},
   {S(   0,   0), S( -30,  12), S( -46,  33), S( -65,  35),
    S(  -5,  32), S(  97,  21), S( 188,  88), S(   0,   0)}},
  {{S(   0,   0), S( -25,  21), S( -46,  30), S( -66,  40),
    S(   3,  38), S(  92,  32), S( 256, 100), S(   0,   0)},
   {S(   0,   0), S( -31,  19), S( -42,  28), S( -61,  45),
    S(   7,  53), S( 100, 101), S( 159, 247), S(   0,   0)}},
};

const int PassedFriendlyDistance[FILE_NB] = {
    S(   0,   0), S(   1,   1), S(   4,  -4), S(   6,  -9),
    S(   4, -12), S( -10,  -9), S( -19,  -2), S(   0,   0),
};

const int PassedEnemyDistance[FILE_NB] = {
    S(   0,   0), S(   3,  -1), S(   4,   0), S(   9,   7),
    S(   1,  18), S(   6,  25), S(  24,  22), S(   0,   0),
};

const int PassedSafePromotionPath = S( -12,  32);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S( -12, -26);
const int ThreatMinorAttackedByPawn  = S( -45, -50);
const int ThreatMinorAttackedByMinor = S( -21, -32);
const int ThreatMinorAttackedByMajor = S( -21, -45);
const int ThreatRookAttackedByLesser = S( -43, -20);
const int ThreatMinorAttackedByKing  = S( -24, -18);
const int ThreatRookAttackedByKing   = S( -23, -21);
const int ThreatQueenAttackedByOne   = S( -33, -42);
const int ThreatOverloadedPieces     = S(  -7, -12);
const int ThreatByPawnPush           = S(  13,  20);
...
const int ClosednessKnightAdjustment[9] = {
    S( -14, -10), S( -12,   4), S( -12,  11), S(  -8,  10),
    S(  -6,  14), S(  -4,   9), S(   0,   4), S( -13,  21),
    S( -12,  10),
};

const int ClosednessRookAdjustment[9] = {
    S(  43, -18), S(  -3,  19), S(  -6,   5), S( -10,  -8),
    S( -15,  -4), S( -17, -16), S( -20, -15), S( -28, -19),
    S( -34, -21),
};
...
const int ComplexityPawnFlanks  = S(   0,  66);
const int ComplexityPawnEndgame = S(   0,  43);
const int ComplexityAdjustment  = S(   0,-126);
