...
const int KingShelter[2][RANK_NB] = {
   {S(  29, -16), S(   0, -17), S( -26,  -1), S( -22,  -7),
    S( -31,  -5), S( -45,  30), S(   3,  44), S(  -8,   0)},
   {S(   0,   0), S(  24, -19), S(  12, -17), S( -10,  -8),
    S( -29,  17), S( -10,  22), S( -56, -51), S( -31,  11)},
};

const int KingStorm[2][RANK_NB] = {
   {S( -24,  57), S(  41,  18), S( -20,  21), S(  -2,   7),
    S(  -1,   3), S(   7,  -6), S(   0,   1), S( -10,   3)},
   {S(   0,   0), S(  -9, -39), S(   5, -11), S(  45, -13),
    S(   4,  -5), S(  19, -28), S(  -1,  -2), S( -19,  -1)},
};
...
int evaluateKings(EvalInfo *ei, Board *board, int colour) {
    ...
    int start = MIN(5, MAX(0, fileOf(kingSq) - 1));
    int end   = MAX(2, MIN(7, fileOf(kingSq) + 1));
    ...
    for (int file = start; file <= end; file++) {
        ...
        ei->pkeval[US] += KingShelter[file == fileOf(kingSq)][ourDist];
        if (TRACE) T.KingShelter[file == fileOf(kingSq)][ourDist][US]++;
        ...
        ei->pkeval[US] += KingStorm[blocked][theirDist];
        if (TRACE) T.KingStorm[blocked][theirDist][US]++;
    }
