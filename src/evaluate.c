...
const int KnightValue = S( 462, 392);
const int BishopValue = S( 470, 416);
const int RookValue   = S( 636, 716);
const int QueenValue  = S(1286,1331);
...
const int PawnConnected32[32] = {
    ...
    S(   7,   0), S(  20,   0), S(  14,   8), S(  17,  22),
    S(   6,   0), S(  19,   4), S(  15,   8), S(  17,  17),
    S(   6,  13), S(  19,  19), S(  18,  24), S(  38,  25),
    S(  24,  54), S(  27,  62), S(  77,  60), S(  58,  77),
    S( 123,  -6), S( 165,  10), S( 208,  25), S( 194,  81),
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0),
};
...
const int KnightOutpost[2] = { S(  18, -35), S(  37,   8) };

const int KnightMobility[9] = {
    S( -88, -98), S( -37, -86), S( -17, -41),
    S(  -5, -11), S(   2, -13), S(   8,   0),
    S(  18,  -1), S(  32,  -1), S(  48, -31),
};
...
const int BishopAttackedByPawn = S( -51, -35);

const int BishopOutpost[2] = { S(  19, -16), S(  53, -10) };

const int BishopMobility[14] = {
    S( -62,-123), S( -48, -65), S( -19, -45), S(  -5, -21),
    S(   4,  -7), S(  16,   0), S(  22,   8), S(  29,   4),
    S(  30,  10), S(  36,   3), S(  42,   4), S(  52, -12),
    S(  43,   0), S(  36, -30),
};
...
const int RookFile[2] = { S(  12,   1), S(  41,  -8) };

const int RookOnSeventh = S(   0,  23);

const int RookMobility[15] = {
    S(-158, -95), S( -73,-121), S( -16, -67), S(  -9, -26),
    S(  -8,  -3), S(  -7,  14), S(  -7,  25), S(  -3,  32),
    S(   0,  36), S(   6,  36), S(   9,  42), S(  19,  48),
    S(  19,  50), S(  25,  47), S(  16,  44),
};


// Definition of evaluation terms related to Queens

const int QueenChecked = S( -36, -32);

const int QueenCheckedByPawn = S( -50, -45);

const int QueenMobility[28] = {
    S( -60,-260), S(-248,-265), S( -40,-192), S( -37,-181),
    S( -17,-120), S( -26, -61), S( -18, -89), S( -18, -86),
    S( -15, -59), S( -10, -53), S(  -8, -27), S(  -5, -29),
    S(  -4, -15), S(  -1, -10), S(   1,  -7), S(   0,   4),
    S(   3,  15), S(   0,  14), S(  11,  25), S(   0,  26),
    S(   5,  28), S(  17,  29), S(  22,  13), S(  36,  16),
    S(  52,  25), S(  50,   1), S(  -9,  -1), S(  24,  12),
};
...
const int KingDefenders[12] = {
    S( -39,  -4), S( -21,   5), S(   0,   1), S(  10,  -1),
    S(  24,  -1), S(  36,   2), S(  39,   8), S(  31,-107),
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6),
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -15,  17), S(   6,  -8), S(  15,   3), S(  20,   4), S(   7,   1), S(  12,  -4), S( -26, -46), S( -33,   0)},
   {S(   2,   7), S(  16,  -5), S(  16,  -8), S(   0, -12), S( -32,  -2), S( -71,  83), S(  46,  77), S( -34,   0)},
   {S(  12,  14), S(   9,   0), S( -18,   0), S(  -9,   0), S( -31,  -2), S(   3, -15), S( -29,  58), S( -16,   1)},
   {S(  12,  27), S(  15,   0), S(  -3,  -8), S(  18, -12), S(  16, -32), S( -26, -32), S( -90,  21), S(  -2,   0)},
   {S( -12,  18), S(   0,   1), S( -28,   1), S( -13,   2), S( -37, -15), S( -43, -24), S(   7,   0), S( -15,   0)},
   {S(  21,   2), S(  17,  -4), S( -19,  -1), S(  -2, -20), S(   4, -32), S(  13, -48), S(  67, -37), S( -14,   0)},
   {S(  20,   1), S(   3,  -9), S( -27,  -8), S( -20, -13), S( -23, -16), S( -41,   0), S( -10,  37), S( -31,   8)},
   {S( -15,  -3), S(   0,  -8), S(   5,   0), S(   1,   3), S( -13,  11), S(   0,  31), S(-158,  71), S( -18,  15)}},
  {{S(   0,   0), S(   0, -16), S(   5, -16), S( -57,  14), S(  11, -13), S( -16,  34), S(-144,  15), S( -57,   9)},
   {S(   0,   0), S(  16,  -5), S(   6,  -5), S(  -1,  -5), S(   6, -28), S(  16,  85), S(-339,  18), S( -46,   2)},
   {S(   0,   0), S(  24,   0), S(   0,  -5), S(  19, -25), S(  14,  -5), S(-102,  46), S(-144, -90), S( -21,  -1)},
   {S(   0,   0), S(  -4,   8), S(  -9,  13), S( -17,   1), S( -27,  -3), S(-107,   1), S(  22, -34), S( -25,   0)},
   {S(   0,   0), S(   5,   4), S(   8,  -7), S(  22,  -5), S(   8, -19), S( -52,  12), S( -85, -94), S(  -5,  -2)},
   {S(   0,   0), S(   9,   1), S( -13,  -2), S( -19, -13), S(  16, -32), S( -25,   5), S(  10,  22), S( -30,   0)},
   {S(   0,   0), S(  13,  -1), S(  -1,   0), S( -19,  -5), S( -21, -15), S(  14,  -2), S( -96, -46), S( -45,  13)},
   {S(   0,   0), S(   8, -27), S(  10, -14), S( -24,   0), S( -28,  -2), S(   4, -22), S(-211, -62), S( -46,  16)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -32, -30), S( -24,   8), S( -13,  -2), S(  22,   0), S(  63,  -5), S( 158,  32), S(   0,   0)},
   {S(   0,   0), S(  -2,   1), S( -14,  23), S( -14,  35), S(   5,  44), S(  72,  61), S( 194, 130), S(   0,   0)}},
  {{S(   0,   0), S(  -7,  12), S( -12,   6), S(  -9,  27), S(  27,  32), S(  83,  63), S( 226, 149), S(   0,   0)},
   {S(   0,   0), S(  -6,   8), S( -12,  17), S( -20,  53), S( -12, 110), S(  27, 203), S( 114, 370), S(   0,   0)}},
};
