...
int getCMHistoryScore(Thread *thread, int height, uint16_t move) {
    ...
    // No counter move scores available at near root nodes
    if (previous == NONE_MOVE)
        return 0;

    // No counter moves for following up NULL move
    if (previous == NULL_MOVE)
        return getFUHistoryScore(thread, height, move);
...
int getFUHistoryScore(Thread *thread, int height, uint16_t move) {
    ...
    // No followup move scores available at near root nodes
    if (following == NONE_MOVE)
        return 0;

    // No followup moves for following up a NULL move
    if (following == NULL_MOVE)
        return getCMHistoryScore(thread, height, move);
