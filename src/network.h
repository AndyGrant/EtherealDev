...
typedef struct PKNetwork {
    ...
    // Our current Network is [224x32, 32x2]. The Network is trained to
    // output a Score in CentiPawns for the MG and EG, and thus Output
    // Neurons need do not need activation functions. The 32x1 operation
    // applys a ReLU, where as the 224x32 layer does not due to the inputs
