...
int search(Thread* thread, PVariation* pv, int alpha, int beta, int depth, int height){
    ...
    if (   !PvNode
        ...
        while ((move = selectNextMove(&movePicker, board)) != NONE_MOVE){
            ...
            // Verify with a very reduced depth before the usual probcut reduction
            value = -search(thread, &lpv, -rBeta, -rBeta+1, depth-8, height+1);
            if (value >= rBeta)
            ...
            // Reduced depth produced a cut off
            if (value >= rBeta)
                return beta;
        }
    }
