...
int search(Thread* thread, SearchStack* ss, PVariation* pv, int alpha, int beta, int depth, int height){
    ...
    if (best >= beta && !moveIsTactical(board, bestMove)){
        ...
        updateCounterMoveHistory(thread->counter, board, (ss-1)->currentMove, bestMove, depth*depth);
        
        for (i = 0; i < quiets - 1; i++){
            updateHistory(thread->history, quietsTried[i], board->turn, -depth*depth);
            updateCounterMoveHistory(thread->counter, board, (ss-1)->currentMove, quietsTried[i], -depth*depth);
        }
    }
