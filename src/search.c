...
int search(Thread* thread, PVariation* pv, int alpha, int beta, int depth, int height){
    ...
    while ((currentMove = selectNextMove(&movePicker, board)) != NONE_MOVE){
        ...
        if ((isQuiet = !moveIsTactical(board, currentMove))){
            ...
            hist = getHistoryScore(thread->history, currentMove, board->turn, board->phash, 128);
        }
    ...
    if (played == 0) return inCheck ? -MATE + height : 0;
        ...
        updateHistory(thread->history, bestMove, board->turn, board->phash, 1, depth*depth);
        for (i = 0; i < quiets - 1; i++)
            updateHistory(thread->history, quietsTried[i], board->turn, board->phash, 0, depth*depth);
    }
