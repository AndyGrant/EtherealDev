...
int search(Thread* thread, PVariation* pv, int alpha, int beta, int depth, int height, int skipBetaPruning){
    ...
    if (   !PvNode
        ...
        return depth <= 4 ? eval - depth * 0.95 * PieceValues[PAWN][EG]
                          : search(thread, pv, alpha, beta, 1, height, 1);
