...
int search(Thread* thread, PVariation* pv, int alpha, int beta, int depth, int height, int cutNode){
    ...
    if (getTranspositionEntry(&Table, board->hash, &ttEntry)){
        ...
        if (!PvNode){
            ...
            if (!thread->skipTTPruning && rAlpha - 64 >= rBeta){
            
                if (ttEntry.depth + 1 == depth)
                    return ttValue;
            
                if (ttEntry.depth + 2 == depth){
                    
                    thread->skipTTPruning = 1;
                    
                    value = search(thread, pv, alpha, beta, depth - 1, height, cutNode);
                    
                    thread->skipTTPruning = 0;
