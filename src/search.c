...
int search(Thread *thread, PVariation *pv, int alpha, int beta, int depth, int height) {
    ...
    while ((move = selectNextMove(&movePicker, board, skipQuiets)) != NONE_MOVE) {
        ...
        if (isQuiet && best > MATED_IN_MAX) {
            ...
            if (   depth <= CounterMovePruningDepth[PvNode]
                && cmhist <= CounterMoveHistoryLimit[PvNode])
                continue;
            ...
            if (   depth <= FollowUpMovePruningDepth[PvNode]
                && fmhist <= FollowUpMoveHistoryLimit[PvNode])
                continue;
        }
