...
int search(Thread* thread, PVariation* pv, int alpha, int beta, int depth, int height){
    ...
    uint16_t move, bestMove = NONE_MOVE, quietsTried[MAX_MOVES];
    ...
    if ((ttHit = getTTEntry(board->hash, &ttMove, &ttEval, &ttValue, &ttDepth, &ttBound))){
        ...
        // comes from a greater depth search than the one we are doing here
    ...
    if ((tbresult = tablebasesProbeWDL(board, depth, height)) != TB_RESULT_FAILED){
        ...
        if (    bound == BOUND_EXACT
            ...
            storeTTEntry(board->hash, NONE_MOVE, VALUE_NONE, value, MAX_PLY - 1, bound);
            return value;
        }
    }
    ...
    if (    PvNode
        ...
        // Probe for the newly found best move
        ttHit = getTTEntry(board->hash, &ttMove, &ttEval, &ttValue, &ttDepth, &ttBound);
    }
    ...
    ttBound = (best > oldAlpha && best < beta) ? BOUND_EXACT
            :  best >= beta ? BOUND_LOWER : BOUND_UPPER;
    storeTTEntry(board->hash, bestMove, eval, valueToTT(best, height), depth, ttBound);
...
int moveIsSingular(Thread* thread, uint16_t ttMove, int ttValue, Undo* undo, int depth, int height){
    ...
    int value = -MATE;
    int rBeta = MAX(valueFromTT(ttValue, height) - 2 * depth, -MATE);
