...
void aspirationWindow(Thread *thread) {
    ...
    if (thread->depth >= WindowDepth) {
        ...
        // If we've found a TB win/loss, kick the correct bound towards +/- Mate
        if (thread->pvs[thread->completed].score < -TBWIN_IN_MAX) alpha = -MATE;
        if (thread->pvs[thread->completed].score >  TBWIN_IN_MAX) beta  =  MATE;
    }
