...
uint16_t getBestMove(Thread* threads, Board* board, Limits* limits, double time, double mtg){
    ...
    if (limits->limitedBySelf){
        info.idealusage = mtg >= 0 ? 0.3 * (time / (mtg + 3)) : 0.3 * (time / 25);
        info.maxusage   = mtg >= 0 ? 4.5 * (time / (mtg + 3)) : 4.5 * (time / 25);
    ...
    for (depth = 1; depth < MAX_DEPTH; depth++){
        ...
        if (limits->limitedBySelf){
            ...
            if (depth >= 4 && info->values[depth - 1] > value + 8)
                info->idealusage = MIN(info->maxusage, info->idealusage * (1.00 + MIN(.50, (info->values[depth-1] - value) / 100.0)));
