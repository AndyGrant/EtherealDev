...
int aspirationWindow(Thread* thread, int depth){
    ...
    if (depth > 4 && abs(values[mainDepth-1]) < MATE / 2){
        ...
        upper = MAX(   12, +1.00 * (values[mainDepth-1] - values[mainDepth-2]));
        upper = MAX(upper, +0.75 * (values[mainDepth-2] - values[mainDepth-3]));
        upper = MAX(upper, +0.50 * (values[mainDepth-3] - values[mainDepth-4]));
        
        // Dynamically compute the lower margin based on previous scores
        lower = MAX(   12, -1.00 * (values[mainDepth-1] - values[mainDepth-2]));
        lower = MAX(lower, -0.75 * (values[mainDepth-2] - values[mainDepth-3]));
        lower = MAX(lower, -0.50 * (values[mainDepth-3] - values[mainDepth-4]));
