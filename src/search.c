...
uint16_t getBestMove(Thread* threads, Board* board, Limits* limits){
    ...
    // Update the Transposition Table generation for the new search
...
int search(Thread* thread, PVariation* pv, int alpha, int beta, int depth, int height){
    ...
    if (getTranspositionEntry(board->hash, &ttEntry)){
    ...
    if ((tbresult = tablebasesProbeWDL(board, depth, height)) != TB_RESULT_FAILED){
        ...
        if (    bound == PVNODE
            ...
            storeTranspositionEntry(MAX_PLY - 1, bound, value, NONE_MOVE, board->hash);
    ...
    storeTranspositionEntry(depth, (best > oldAlpha && best < beta)
