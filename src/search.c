...
int search(Thread *thread, PVariation *pv, int alpha, int beta, int depth) {
    ...
    if (   !PvNode
        ...
        R = 4 + depth / 6 + MIN(3, (ns->eval - beta) / 200) + (ns-1)->tactical;
...
int qsearch(Thread *thread, PVariation *pv, int alpha, int beta) {
    ...
    int ttHit, ttValue = VALUE_NONE, ttEval = VALUE_NONE, ttDepth = 0, ttBound = 0;
    uint16_t move, ttMove = NONE_MOVE, bestMove = NONE_MOVE;
    PVariation lpv;
    ...
    while ((move = select_next(&ns->mp, thread, 1)) != NONE_MOVE) {
        ...
        if (   ns->eval + pessimism > beta
            && abs(ns->eval + pessimism) < MATE / 2) {
            revert(thread, board, move);
            pv->length = 1;
            pv->line[0] = move;
            return beta;
