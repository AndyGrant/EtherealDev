...
int aspirationWindow(Thread* thread, int depth){
    ...
    delta = MAX(   16,  1.00 * abs(values[mainDepth-1] - values[mainDepth-2]));
    delta = MAX(delta,  0.50 * abs(values[mainDepth-2] - values[mainDepth-3]));
    delta = MAX(delta,  0.25 * abs(values[mainDepth-3] - values[mainDepth-4]));
