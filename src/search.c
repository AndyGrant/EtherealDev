...
int search(Thread *thread, PVariation *pv, int alpha, int beta, int depth, int height) {
    ...
    while ((move = selectNextMove(&movePicker, board, skipQuiets)) != NONE_MOVE) {
        ...
        if (isQuiet && best > MATED_IN_MAX) {
            ...
            int condition = improving || movePicker.stage < STAGE_QUIET;

            // Step 11D (~8 elo). Counter Move Pruning. Moves with poor counter
            // move history are pruned at near leaf nodes of the search.
            if (   depth <= CounterMovePruningDepth[condition]
                && cmhist < CounterMoveHistoryLimit[condition])
                continue;
            ...
            if (   depth <= FollowUpMovePruningDepth[condition]
                && fmhist < FollowUpMoveHistoryLimit[condition])
                continue;
        }
