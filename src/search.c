...
int search(Thread* thread, PVariation* pv, int alpha, int beta, int depth, int height){
    ...
    while ((move = selectNextMove(&movePicker, board, skipQuiets)) != NONE_MOVE){
        ...
        // Step 13. Quiet Move Pruning. Prune any quiet move that meets one
        // of the criteria below, except for mated lines and Root node moves
        if (!RootNode && isQuiet && best > MATED_IN_MAX) {

            // Step 13A. Futility Pruning. If our score is far below alpha, and we
            ...
            // Step 13D. Follow Up Move Pruning. Moves with poor follow up
            // move history are pruned at near leaf nodes of the search.
        ...
        // Step 16C. History Extensions. We extend quiet moves with strong
