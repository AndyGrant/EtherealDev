...
int aspirationWindow(Thread* thread, int depth){
    ...
    if (depth > 4 && abs(values[mainDepth-1]) < MATE / 2){
        ...
        upper = MAX(   12, 1.6 * abs(values[mainDepth-1] - values[mainDepth-2]));
        upper = MAX(upper, 1.3 * abs(values[mainDepth-2] - values[mainDepth-3]));
        upper = MAX(upper, 1.0 * abs(values[mainDepth-3] - values[mainDepth-4]));
        
        // Dynamically compute the lower margin based on previous scores
        lower = MAX(   12, 1.6 * abs(values[mainDepth-1] - values[mainDepth-2]));
        lower = MAX(lower, 1.3 * abs(values[mainDepth-2] - values[mainDepth-3]));
        lower = MAX(lower, 1.0 * abs(values[mainDepth-3] - values[mainDepth-4])); 
