...
int search(Thread* thread, PVariation* pv, int alpha, int beta, int depth, int height){
    ...
    while ((move = selectNextMove(&movePicker, board, skipQuiets)) != NONE_MOVE){
        ...
        // Step 13. Early Pruning. Quiet moves from non Root nodes are
        // candidates for Early Pruning, so long as we have already
        // found a move which does not lead to being mated.
