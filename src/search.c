...
int search(Thread* thread, PVariation* pv, int alpha, int beta, int depth, int height){
    ...
    if ((tbresult = tablebasesProbeWDL(board, depth, height)) != TB_RESULT_FAILED){
        ...
        if (    ttBound == BOUND_EXACT
            ...
            storeTTEntry(board->hash, NONE_MOVE, value, MAX_PLY-1, ttBound);
            return value;
        }
    }
    ...
    if (   !PvNode
        ...
        if (value >= beta) return value >= MATE_IN_MAX ? beta : value;
    }
    ...
    if (    PvNode
        ...
        ttHit = getTTEntry(board->hash, &ttMove, &ttValue, &ttDepth, &ttBound);
