...
uint16_t getBestMove(Thread* threads, Board* board, Limits* limits, double time, double inc, double mtg){
    ...
    if (limits->limitedBySelf){
        idealusage = mtg >= 0 ? 0.45 * (time / (mtg + 1)) : 0.5 * (time / 30);
        maxusage   = mtg >= 0 ? 1.90 * (time / (mtg + 1)) : inc + (time / 15);
    ...
    for (depth = 1; depth < MAX_DEPTH; depth++){
        ...
        if (abort == ABORT_NONE){
            ...
            if (thread->limits->limitedBySelf){
                ...
                if (depth >= 4 && thread->info->values[thread->info->depth] > value + 10)
                ...
                if (depth >= 4 && thread->info->bestmoves[thread->info->depth] != thread->pv.line[0])
                    *thread->idealusage = MIN(thread->maxusage, *thread->idealusage * 1.40);
            }
