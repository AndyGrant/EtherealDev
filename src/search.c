...
uint16_t getBestMove(Thread* threads, Board* board, Limits* limits, double time, double mtg, double inc){
    ...
    if (limits->limitedBySelf){
        ...
        if (mtg >= 0){
            info.idealusage =  1.00 * time / (mtg + 1) + inc;
            info.maxalloc   =  4.00 * time / (mtg + 1) + inc;
            info.maxusage   = 10.00 * time / (mtg + 1) + inc;
        }
        
        else {
            info.idealusage =  1.00 * (time + 23 * inc) / 25;
...
void* iterativeDeepening(void* vthread){
    ...
    for (depth = 1; depth < MAX_DEPTH; depth++){
        ...
        if (limits->limitedBySelf && depth >= 4){
            ...
            int pvChange = info->bestmoves[depth-1] != thread->pv.line[0];
            
            int scoreChange = value - info->values[depth-1];
            
            if (pvChange){
                info->idealusage += info->idealusage * 0.25 * MAX(1.00, info->pvStability);
                info->pvStability = (0.50 + (depth - 1) * info->pvStability) / depth;
            }
            
            else {
                info->idealusage -= info->idealusage * 0.05 * MIN(1.00, info->pvStability);
                info->pvStability = (1.10 + (depth - 1) * info->pvStability) / depth;
            }
            
            if (scoreChange <= -10){
                info->idealusage += info->idealusage * 0.10 * MAX(1.00, info->scoreStability);
                info->scoreStability = (0.50 + (depth - 1) * info->scoreStability) / depth;
            }
            
            else if (scoreChange >= 10){
                info->idealusage -= info->idealusage * 0.02 * MIN(1.00, info->scoreStability);
                info->scoreStability = (1.10 + (depth - 1) * info->scoreStability) / depth;
            }
