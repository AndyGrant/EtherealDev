...
int search(Thread* thread, PVariation* pv, int alpha, int beta, int depth, int height){
    ...
    while ((move = selectNextMove(&movePicker, board, skipQuiets)) != NONE_MOVE){
        ...
        // Step 13. Early Pruning. Try pruning this move, or future moves,
        // before playing. This can be done for quiets in non-Root nodes,
        // so long as we have found at least one move to avoid checkmate
        if (!RootNode && isQuiet && best > MATED_IN_MAX) {

            // Step 13A. Futility Pruning. If our score is far below alpha,
            // and we have gotten to moves with poor enough history, then
            // we will skip all other quiets after trying this one
            if (    futilityMargin <= alpha
                &&  depth <= FutilityPruningDepth
                &&  hist < FutilityPruningHistoryLimit[improving])
                skipQuiets = 1;

            // Step 13B. Late Move Pruning / Move Count Pruning. If we have
            // tried many quiets in this position already, and we don't
            // expect anything from this move, we can skip all other quiets
            if (    depth <= LateMovePruningDepth
                &&  quiets > LateMovePruningCounts[improving][depth])
                skipQuiets = 1;

            // Step 13C. Counter Move Pruning. Moves with poor counter
            ...
            // Step 13D. Follw Up Move Pruning. Moves with poor counter
            // move history are pruned at near leaf nodes of the search.
            if (    depth <= FollowUpMovePruningDepth
                &&  fuhist < FollowUpMoveHistoryLimit[improving])
                continue;

            // Step 13E. Static Exchange Evaluation Pruning (Quiets).
            // Prune moves which fail to beat a depth dependent SEE threshold.
            if (    depth <= SEEPruningDepth
                && !staticExchangeEvaluation(board, move, SEEMarginQuiet * depth * depth))
                continue;
        }

        // Step 14. Static Exchange Evaluation Pruning (Captures).
        // Prune moves which fail to beat a depth dependent SEE threshold.
        // The use of movePicker.stage is a speedup, which assumes that
        // good noisy moves have a positive SEE
        if (   !RootNode
            && !isQuiet
            &&  best > MATED_IN_MAX
            &&  depth <= SEEPruningDepth
            &&  movePicker.stage > STAGE_GOOD_NOISY
            && !staticExchangeEvaluation(board, move, SEEMarginNoisy * depth))
            continue;
        ...
        // Step 16B. Check Extensions. We extend captures and good quiets that
