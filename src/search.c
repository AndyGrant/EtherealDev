...
int search(Thread* thread, PVariation* pv, int alpha, int beta, int depth, int height, int isProbCut){
    ...
    if (   !PvNode
        ...
        while ((currentMove = selectNextMove(&movePicker, board)) != NONE_MOVE){
            ...
            if (   -search(thread, &lpv, -rBeta, -rBeta+1,       0, height+1, 1) >= rBeta
                && -search(thread, &lpv, -rBeta, -rBeta+1, depth-4, height+1, 1) >= rBeta){
