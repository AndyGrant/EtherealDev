...
uint16_t getBestMove(Thread* threads, Board* board, Limits* limits, double time, double mtg){
    ...
    if (limits->limitedBySelf){
        idealusage = mtg >= 0 ? 0.5 * (time / (mtg + 3)) : 0.28 * (time / 25);
        maxusage   = mtg >= 0 ? 2.8 * (time / (mtg + 1)) : 5.60 * (time / 25);
        ...
        if (abort == ABORT_NONE){
            ...
            if (thread->limits->limitedBySelf){
                ...
                if (depth >= 4 && info->values[info->depth] > value + 8)
                    *thread->idealusage = MIN(thread->maxusage, *thread->idealusage * (1 + depth / 80.0));
                ...
                if (depth >= 4 && info->bestmoves[info->depth] != thread->pv.line[0])
                    *thread->idealusage = MIN(thread->maxusage, *thread->idealusage * (1 + depth / 25.0));
            }
