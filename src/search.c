...
void* iterativeDeepening(void* vthread){
    ...
    for (depth = 1; depth < MAX_DEPTH; depth++){
        ...
        if (limits->limitedBySelf && depth >= 4){
            ...
            // Increase our time if the score suddenly dropped by eight centipawns
            if (info->values[depth-1] > value + 8)
                info->idealusage *= MAX(info->scoreStability, 1.05);
            
            // Decrease our time if the score suddenly jumped by eight centipawns
            if (info->values[depth-1] < value - 8)
                info->idealusage *= MAX(0.98, MIN(info->pvStability, 1.00));
