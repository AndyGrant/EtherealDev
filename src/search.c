...
int bestTacticalMoveValue(Board* board, EvalInfo* ei){
    ...
    uint64_t promos, targets = ei->attacked[board->turn];
    ...
    promos =   board->pieces[PAWN]
            &  board->colours[board->turn]
            & (board->turn == WHITE ? RANK_7 : RANK_2);
    promos = board->turn == WHITE ? (promos << 8) : (promos >> 8);
    promos = promos & ~(board->colours[WHITE] | board->colours[BLACK]);
