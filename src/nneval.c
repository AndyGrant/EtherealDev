...
int evaluateEndgames(Board *board) {
    ...
    // We only handle RPvRP (we allow a Rook imbalance)
...
void computeEndgameNeurons(EGNetwork *nn, NNCacheEntry *entry, Board *board) {
    ...
    // BAD FOR NOW! WE SHOULD FORCE PAWNS + KINGS TO BE AT THE START OF
    // THE NETWORK INPUTS. FOR CACHING, AND SO THAT ALL NETWORKS, REGARDLESS
    // OF SIZE, CAN USE A SINGLE FUNCTION TO COMPUTE THE PAWN KING PORTION

    const int PawnIDX[COLOUR_NB] = {   0, 176 };
    const int KingIDX[COLOUR_NB] = { 112, 288 };
...
static int evaluateRPvRP(EGNetwork *nn, NNCacheEntry *entry, Board *board) {
    ...
    const int RookIDX[COLOUR_NB] = {  48, 224 };
