...
void initMasks() {
    ...
    // Init a table of attack rays from sq1 along sq2 (aligned on diagonal)
    for (int sq1 = 0; sq1 < SQUARE_NB; sq1++)
        for (int sq2 = 0; sq2 < SQUARE_NB; sq2++)
            if (testBit(bishopAttacks(sq1, 0ull), sq2))
                    AttackRayMasks[sq1][sq2] = (1ull << sq1) | (1ull << sq2)
                                             | (bishopAttacks(sq1, 0ull) & bishopAttacks(sq2, 1ull << sq1));

    // Init a table of attack rays from sq1 along sq2 (aligned on straight)
    for (int sq1 = 0; sq1 < SQUARE_NB; sq1++)
        for (int sq2 = 0; sq2 < SQUARE_NB; sq2++)
            if (testBit(rookAttacks(sq1, 0ull), sq2))
                    AttackRayMasks[sq1][sq2] = (1ull << sq1) | (1ull << sq2)
                                             | (rookAttacks(sq1, 0ull) & rookAttacks(sq2, 1ull << sq1));
...
uint64_t attackRayMasks(int sq1, int sq2) {
    assert(0 <= sq1 && sq1 < SQUARE_NB);
    assert(0 <= sq2 && sq2 < SQUARE_NB);
    return BitsBetweenMasks[sq1][sq2];
}
