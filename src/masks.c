...
uint64_t BitsAndEndpointsBetweenMasks[SQUARE_NB][SQUARE_NB];
    ...
    for (i = 0; i < SQUARE_NB; i++){
        for (j = 0; j < SQUARE_NB; j++){
            ...
            if (bishopAttacks(i, 0ull, 1ull << j)){
                ...
                BitsAndEndpointsBetweenMasks[i][j] = BitsBetweenMasks[i][j] | (1ull << i) | (1ull << j);
                
                LineBySquaresMasks[i][j] = (1ull << i) | (1ull << j) 
                                         | (bishopAttacks(i, 0ull, ~0ull) & bishopAttacks(j, 0ull, ~0ull));
            }
            ...
            if (rookAttacks(i, 0ull, 1ull << j)){
                ...
                LineBySquaresMasks[i][j] = (1ull << i) | (1ull << j) 
                                         | (rookAttacks(i, 0ull, ~0ull) & rookAttacks(j, 0ull, ~0ull));
            }
        }
    }
