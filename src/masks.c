...
void initMasks() {
    ...
    // Init a table of bitmasks for the attack rays from one square to another (aligned on diagonal)
    for (int sq1 = 0; sq1 < SQUARE_NB; sq1++)
        for (int sq2 = 0; sq2 < SQUARE_NB; sq2++)
            if (testBit(bishopAttacks(sq1, 0ull), sq2))
                AttackRayMasks[sq1][sq2] = (1ull << sq2) | (bishopAttacks(sq1, 0ull) & bishopAttacks(sq2, 1ull << sq1));

    // Init a table of bitmasks for the attack rays from ont square to another (aligned on straight)
    for (int sq1 = 0; sq1 < SQUARE_NB; sq1++)
        for (int sq2 = 0; sq2 < SQUARE_NB; sq2++)
            if (testBit(rookAttacks(sq1, 0ull), sq2))
                AttackRayMasks[sq1][sq2] = (1ull << sq2) | (rookAttacks(sq1, 0ull) & rookAttacks(sq2, 1ull << sq1));
