...
uint64_t RanksAboveOrOneBelow[COLOUR_NB][RANK_NB];
...
void initializeMasks(){
    ...
    for (i = 0; i < RANK_NB; i++){
        ...
        for (j = MAX(0, i - 1); j < RANK_NB; j++)
            RanksAboveOrOneBelow[WHITE][i] |= Ranks[j];
        
        for (j = MIN(7, i + 1); j >= 0; j--)
            RanksAboveOrOneBelow[BLACK][i] |= Ranks[j];
    }
