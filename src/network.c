...
int evaluate_nnue(Board *board) {
    ...
    int index1, index2;
    uint64_t pieces = board->pieces[PAWN];
    ...
    for (int i = 0; i < nnue.layers[1].rows; i++)
        out1[i] = fmaxf(0.0f, out1[i]);


    for (int i = 0; i < nnue.layers[1].rows; i++)
        for (int j = 0; j < nnue.layers[1].cols; j++)
            out2[j] += out1[i] * nnue.layers[1].weights[i * nnue.layers[1].cols + j];

    for (int i = 0; i < nnue.layers[1].rows; i++)
        out2[i] = fmaxf(0.0f, out2[i]);


    for (int i = 0; i < nnue.layers[2].rows; i++)
        for (int j = 0; j < nnue.layers[2].cols; j++)
            out3[j] += out2[i] * nnue.layers[2].weights[i * nnue.layers[2].cols + j];

    for (int i = 0; i < nnue.layers[1].rows; i++)
        out3[i] = fmaxf(0.0f, out3[i]);


    for (int i = 0; i < nnue.layers[3].rows; i++)
        for (int j = 0; j < nnue.layers[3].cols; j++)
            out4[j] += out3[i] * nnue.layers[3].weights[i * nnue.layers[3].cols + j];

    return MakeScore((int) out4[MG], (int) out4[EG]);
}
...
void compute_nnue_indices(const Board *board, int sq, int *index1, int *index2) {
    ...
    *index1 = (64 * 2 *  sksq) + (64 * (colour == board->turn)) + srelsq;
    *index2 = (64 * 2 * nsksq) + (64 * (colour != board->turn)) + nsrelsq;
}