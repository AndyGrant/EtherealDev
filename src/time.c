...
void initializeTimeManagment(SearchInfo* info, Limits* limits){
    ...
    if (limits->limitedBySelf){
        ...
        if (limits->mtg >= 0){
            info->idealUsage =  0.70 * limits->time / (limits->mtg + 1) + limits->inc;
            info->maxAlloc   =  4.00 * limits->time / (limits->mtg + 1) + limits->inc;
            info->maxUsage   = 10.00 * limits->time / (limits->mtg + 1) + limits->inc;
        }
