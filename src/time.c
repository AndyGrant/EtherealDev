...
void initializeTimeManagment(SearchInfo* info, Limits* limits){
    ...
    if (limits->limitedBySelf){
        ...
        if (limits->mtg >= 0){
            info->idealUsage =  0.75 * limits->time / (limits->mtg + 3) + limits->inc;
            info->maxAlloc   =  4.00 * limits->time / (limits->mtg + 5) + limits->inc;
            info->maxUsage   = 10.00 * limits->time / (limits->mtg + 7) + limits->inc;
        }
