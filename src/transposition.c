...
void storeTTEntry(uint64_t hash, uint16_t move, int value, int depth, int bound) {
    ...
    assert(0 <= depth < MAX_PLY); // Depth should be within our regular search
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (   replace->depth - ((259 + Table.generation - replace->generation) & 0xFC) * 3
            >= slots[i].depth - ((259 + Table.generation - slots[i].generation) & 0xFC) * 3)
            replace = &slots[i];
    }
