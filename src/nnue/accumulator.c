...
static int mirror_square(int sq) {
    return square(rankOf(sq), 7 - fileOf(sq));
}

static int sq64_to_sq32(int sq) {
    static const int Mirror[] = { 3, 2, 1, 0, 0, 1, 2, 3 };
    return 4 * rankOf(sq) + Mirror[fileOf(sq)];
}
...
static int nnue_index_delta(int piece, int relksq, int colour, int sq) {
    ...
    const int mksq = queen_side_sq(relksq) ? mirror_square(relksq) : relksq;
    const int mpsq = queen_side_sq(relksq) ? mirror_square(relpsq) : relpsq;
