    ...
    for (int epoch = 0; epoch < MAXEPOCHS; epoch++) {
        ...
        for (int batch = 0; batch < NPOSITIONS / BATCHSIZE; batch++) {
            ...
            for (int i = 0; i < NTERMS; i++) {
                adagrad[i][MG] += pow((K / 400.0) * gradient[i][MG] / BATCHSIZE, 2.0);
                adagrad[i][EG] += pow((K / 400.0) * gradient[i][EG] / BATCHSIZE, 2.0);
                params[i][MG] += (K / 400.0) * (gradient[i][MG] / BATCHSIZE) * (rate / sqrt(1e-8 + adagrad[i][MG]));
                params[i][EG] += (K / 400.0) * (gradient[i][EG] / BATCHSIZE) * (rate / sqrt(1e-8 + adagrad[i][EG]));
            }
        }
